---
title: "Windows 8.x 앱 용 c + +에서 비동기 작업 만들기 | Microsoft Docs"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
---
# <a name="creating-asynchronous-operations-in-c-for-windows-8x-apps"></a>Windows 8.x 앱 용 c + +에서 비동기 작업 만들기
이 문서에서는 [!INCLUDE[win8_appname_long](../../build/includes/win8_appname_long_md.md)] 앱에서 작업 클래스를 사용하여 Windows ThreadPool 기반 비동기 작업을 생성하는 경우 주의해야 할 몇 가지 주요 사항을 설명합니다.  
 ...
 비동기 프로그래밍을 통해 사용자 입력에 대한 앱의 응답 성능을 유지할 수 있기 때문에 비동기 프로그래밍의 사용은 [!INCLUDE[win8_appname_long](../../build/includes/win8_appname_long_md.md)] 앱 모델의 핵심 구성 요소입니다. UI 스레드를 차단하지 않고 장기 실행 작업을 시작할 수 있으며 해당 작업의 결과를 나중에 받을 수 있습니다. 또한 작업을 취소하고 작업이 백그라운드에서 실행될 때 진행률 알림을 받을 수도 있습니다. [C++의 비동기 프로그래밍](http://msdn.microsoft.com/library/windows/apps/hh780559.aspx) 문서는 Visual C++에서 [!INCLUDE[win8_appname_long](../../build/includes/win8_appname_long_md.md)] 앱을 만들기 위해 사용할 수 있는 비동기 패턴의 개요를 제공합니다. 해당 문서 모두 사용 하 고 비동기 Windows 런타임 작업의 체인을 만드는 방법에 설명 합니다. 이 섹션에서는 ppltasks.h에서의 형식을 사용 하 여 다른 Windows 런타임 구성 요소에서 사용할 수 있는 비동기 작업을 생성 하는 방법을 설명 하 고 어떻게 비동기 작업을 제어 하는 방법을 실행 됩니다. 또한 [Hilo의 비동기 프로그래밍 패턴 및 팁(C++ 및 XAML을 사용하는 Windows 스토어 앱)](http://msdn.microsoft.com/library/windows/apps/jj160321.aspx) 을 읽고 C++ 및 XAML을 사용하는 [!INCLUDE[win8_appname_long](../../build/includes/win8_appname_long_md.md)] 앱인 Hilo에서 어떻게 작업 클래스를 사용하여 비동기 작업을 구현했는지 살펴보세요.  
  
> [!NOTE]
>  사용할 수는 [병렬 패턴 라이브러리](../../parallel/concrt/parallel-patterns-library-ppl.md) (PPL) 및 [비동기 에이전트 라이브러리](../../parallel/concrt/asynchronous-agents-library.md) 에 [!INCLUDE[win8_appname_long](../../build/includes/win8_appname_long_md.md)] 앱. 그러나 작업 스케줄러 또는 리소스 관리자는 사용할 수 없습니다. 이 문서에서는 데스크톱 앱에는 사용할 수 없고 [!INCLUDE[win8_appname_long](../../build/includes/win8_appname_long_md.md)] 앱에만 사용할 수 있는, PPL에서 제공하는 추가 기능에 대해 설명합니다.  
...
-   [concurrency::task::then](reference/task-class.md#then) 메서드를 사용하여 차례로 실행되는 작업의 체인을 만들 수 있습니다. [!INCLUDE[win8_appname_long](../../build/includes/win8_appname_long_md.md)] 앱에서 작업의 연속에 대한 기본 컨텍스트는 해당 작업이 생성된 방식에 따라 달라집니다. 비동기 동작을 작업 생성자에 전달하거나 비동기 동작을 반환하는 람다 식을 전달하여 작업을 만든 경우 해당 작업의 모든 연속에 대한 기본 컨텍스트는 현재 컨텍스트입니다. 비동기 동작에서 작업을 생성하지 않은 경우에는 임의의 컨텍스트가 작업의 연속에 기본적으로 사용됩니다. 기본 컨텍스트는 [concurrency::task_continuation_context](../../parallel/concrt/reference/task-continuation-context-class.md) 클래스를 사용하여 재정의할 수 있습니다.  
...
##  <a name="create-async"></a> 비동기 작업 만들기  
 ...
 Windows 런타임에서 만드는 데 사용할 수 있는 프로그래밍 인터페이스 [!INCLUDE[win8_appname_long](../../build/includes/win8_appname_long_md.md)] 특수 한 운영 체제 환경 에서만에서 실행 되는 앱입니다. 이러한 앱은 승인된 함수, 데이터 형식 및 장치를 사용하며 [!INCLUDE[win8_appstore_long](../../build/reference/includes/win8_appstore_long_md.md)]에서 배포됩니다. Windows Runtime로 표시 됩니다는 *응용 프로그램 이진 인터페이스* (ABI). ABI는 Windows 런타임 Api를 Visual c + +와 같은 프로그래밍 언어에 사용할 수 있도록 하는 기본 이진 계약입니다.  
...
>  정확 하 게 보고 하는 Windows 런타임에서 작업을 취소 했습니다 것 PPL을 사용 하려면이 내부 예외 형식을 catch 하지 마십시오. 즉, 모든 예외도 catch하지 않아야 합니다(`catch (...)`). 모든 catch 해야 하는 경우 예외를 제외 하는 Windows 런타임에서 취소 작업을 완료할 수 있도록 예외를 다시 throw 합니다.  
 ...
 ![Windows 스토어 Primes 앱](../../parallel/concrt/media/concrt_windows_primes.png "concrt_windows_primes")  
...
>  이 예에서는 실행 컨텍스트를 지정하는 방법과 연속의 체인을 구성하는 방법을 보여 줍니다. 앞에서 설명했듯이 기본적으로 비동기 작업에서 만든 작업은 `task::then`을 호출한 아파트에서 연속을 실행합니다. 따라서 이 예에서는 `task_continuation_context::use_arbitrary` 를 사용하여 UI와 관련이 없는 작업이 백그라운드 스레드에서 수행되도록 지정합니다.  
 ...
 ![Windows 스토어 CommonWords 앱](../../parallel/concrt/media/concrt_windows_common_words.png "concrt_windows_common_words")  
