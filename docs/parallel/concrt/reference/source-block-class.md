...
|이름|설명|  
|----------|-----------------|  
|[source_block 생성자](#ctor)|`source_block` 개체를 생성합니다.|  
...
|이름|설명|  
|----------|-----------------|  
|[accept 메서드](#accept)|이 제공 된 메시지를 수락 `source_block` 개체를 호출자에 게 소유권을 전송 합니다.|  
|[acquire_ref 메서드](#acquire_ref)|이 대 한 참조 횟수를 획득 `source_block` 삭제를 방지 하기 위해 개체입니다.|  
|[consume 메서드](#consume)|이전에 제공한이 메시지를 생성 `source_block` 개체를 호출자에 게 소유권을 전송 하 여 대상에 의해 성공적으로 예약 되어 있습니다.|  
|[link_target 메서드](#link_target)|이 대상 블록을 연결 `source_block` 개체입니다.|  
|[release 메서드](#release)|이전의 성공적인 메시지 예약을 해제합니다.|  
|[release_ref 메서드](#release_ref)|이 대 한 참조 횟수를 해제 `source_block` 개체입니다.|  
|[reserve 메서드](#reserve)|이전에 제공한이 메시지를 예약 `source_block` 개체입니다.|  
|[unlink_target 메서드](#unlink_target)|이 대상 블록을 연결 해제 `source_block` 개체입니다.|  
|[unlink_targets 메서드](#unlink_targets)|이 모든 대상 블록의 연결을 해제 `source_block` 개체입니다. (재정의 [isource:: Unlink_targets](isource-class.md#unlink_targets).)|  
  
### <a name="protected-methods"></a>보호된 메서드  
  
|이름|설명|  
|----------|-----------------|  
|[accept_message 메서드](#accept_message)|파생된 클래스에서 재정의 되 면 소스에서 제공된 된 메시지를 수락 합니다. 메시지 블록의 유효성을 검사 하려면이 메서드를 재정의 하는 `_MsgId` 고 메시지를 반환 합니다.|  
|[async_send 메서드](#async_send)|비동기적으로 메시지를 큐에 대기 하 고 이미 수행 되지 않은 경우 전파 작업을 시작|  
|[consume_message 메서드](#consume_message)|파생된 클래스에서 재정의 되 면 이전에 예약 된 메시지를 생성 합니다.|  
|[enable_batched_processing 메서드](#enable_batched_processing)|이 블록에 대한 일괄 처리를 할 수 있도록 합니다.|  
|[initialize_source 메서드](#initialize_source)|초기화는 `message_propagator` 이 `source_block`합니다.|  
|[link_target_notification 메서드](#link_target_notification)|새 대상에 연결 되어 있는 알리는 콜백입니다 `source_block` 개체입니다.|  
|[process_input_messages 메서드](#process_input_messages)|입력된 메시지를 처리합니다. source_block에서 파생되는 전파자 블록에만 유용합니다.|  
|[propagate_output_messages 메서드](#propagate_output_messages)|메시지를 대상으로 전파합니다.|  
|[propagate_to_any_targets 메서드](#propagate_to_any_targets)|파생된 클래스에서 재정의 되 면 연결 된 대상 중 일부 또는 모두 지정된 된 메시지를 전파 합니다. 이것이 메시지 블록에 대 한 주 전파 루틴입니다.|  
|[release_message 메서드](#release_message)|파생된 클래스에서 재정의 되 면 이전 메시지 예약을 해제 합니다.|  
|[remove_targets 메서드](#remove_targets)|이 소스 블록에 대 한 모든 대상 링크를 제거합니다. 이 소멸자에서 호출 되어야 합니다.|  
|[reserve_message 메서드](#reserve_message)|파생된 클래스에서 재정의 되 면 이전에 제공한이 메시지를 예약 `source_block` 개체입니다.|  
|[resume_propagation 메서드](#resume_propagation)|파생된 클래스에서 재정의 되 면 전파 예약이 해제 된 후 다시 시작 합니다.|  
|[sync_send 메서드](#sync_send)|동기적으로 메시지를 큐에 대기 하 고 이미 수행 되지 않은 경우 전파 작업을 시작 합니다.|  
|[unlink_target_notification 메서드](#unlink_target_notification)|대상에서 연결 되었음을 알리는 콜백입니다 `source_block` 개체입니다.|  
|[wait_for_outstanding_async_sends 메서드](#wait_for_outstanding_async_sends)|모든 비동기 전파가 완료 될 때까지 기다립니다. 이 전파 자가 특정 스핀 대기 모든 비동기 전파가 블록을 삭제 하기 전에 완료 시간이 있는지 확인 하려면 메시지 블록의 소멸자에 사용 됩니다.|  
