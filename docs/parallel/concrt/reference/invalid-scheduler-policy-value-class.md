...
이 클래스는 `SchedulerPolicy` 개체의 정책 키가 해당 키에 유효하지 않은 값으로 설정될 때 throw되는 예외를 설명합니다.  
...
|Name|설명|  
|----------|--------|  
|[invalid\_scheduler\_policy\_value::invalid\_scheduler\_policy\_value 생성자](../Topic/invalid_scheduler_policy_value::invalid_scheduler_policy_value%20Constructor.md)|오버로드됨.  `invalid_scheduler_policy_value` 개체를 생성합니다.|  
