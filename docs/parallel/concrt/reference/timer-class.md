...
|이름|설명|  
|----------|-----------------|  
|[타이머](#ctor)|오버로드됨. 생성 된 `timer` 지정된 된 간격 이후에 지정된 된 메시지를 발생 시키는 메시징 블록입니다.|  
...
|이름|설명|  
|----------|-----------------|  
|[일시 중지](#pause)|중지 된 `timer` 메시징 블록입니다. 반복 되는 경우 `timer` 메시징 블록 것을 다시 시작할 수 이후의 `start()` 호출 합니다. -반복 되지 않는 타이머,이 동일한 것과 효과가 `stop` 호출 합니다.|  
