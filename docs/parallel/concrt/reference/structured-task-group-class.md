...
|이름|설명|  
|----------|-----------------|  
|[structured_task_group 생성자](#ctor)|오버로드됨. 새 `structured_task_group` 개체를 생성합니다.|  
...
|[is_canceling 메서드](#is_canceling)|여부는 작업 그룹은 현재 취소 중에 호출자에 게 알립니다. 이 반드시 하는 `cancel` 메서드가 `structured_task_group` 개체 (반드시 반환 하려면이 메서드 `true`). 경우일 수 하는 `structured_task_group` 개체가 인라인으로 실행 되 고 작업 그룹을 추가로 구성 작업 트리에 취소 되었습니다. 이 경우 런타임에서 취소는이 통해 이동 합니다 런타임까지 확인할 수 `structured_task_group` 개체 `true` 도 반환 됩니다.|  
|[run 메서드](#run)|오버로드됨. 작업을 예약 된 `structured_task_group` 개체입니다. 호출자의 수명을 관리 하는 `task_handle` 에 전달 된 개체는 `_Task_handle` 매개 변수입니다. `_Placement` 매개 변수를 사용하는 버전은 이 매개 변수로 지정된 위치에서 작업이 실행되도록 합니다.|  
|[run_and_wait 메서드](#run_and_wait)|오버로드됨. 작업의 도움으로 호출 컨텍스트에서 인라인 실행을 예약 된 `structured_task_group` 전체 취소 지원에 대 한 개체입니다. 하는 경우는 `task_handle` 개체에 대 한 매개 변수로 전달 됩니다 `run_and_wait`, 호출자의 수명을 관리 하는 것에 대 한 책임이 `task_handle` 개체입니다. 함수는 다음에 사용 될 때까지 대기는 `structured_task_group` 개체 완료 되거나 취소 되었습니다.|  
|[wait 메서드](#wait)|에 사용 될 때까지 대기는 `structured_task_group` 완료 되거나 취소 됩니다.|  
...
### <a name="parameters"></a>매개 변수  
 ...
 `_Func`  
 함수 본문은 작업을 호출 하기 위해 호출할입니다. 람다 또는 서명 사용 하 여 함수 호출 연산자의 버전을 지 원하는 다른 개체 때문일 `void operator()()`합니다.  
