---
title: "concurrent_unordered_map 클래스 | Microsoft 문서"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
|이름|설명|  
|----------|-----------------|  
|[concurrent_unordered_map 생성자](#ctor)|오버로드됨. 동시 순서가 지정 되지 않은 맵을 생성합니다.|  
  
### <a name="public-methods"></a>Public 메서드  
  
|이름|설명|  
|----------|-----------------|  
|[at 메서드](#at)|오버로드됨. 요소를 찾습니다는 `concurrent_unordered_map` 지정 된 키 값을 가진... 이 메서드는 동시성이 보장 합니다.|  
|[hash_function 메서드](#hash_function)|저장된 해시 함수 개체를 가져옵니다.|  
|[insert 메서드](#insert)|오버로드됨. 요소를 추가 하는 `concurrent_unordered_map` 개체입니다.|  
|[key_eq 메서드](#key_eq)|저장 된 같음 비교 함수 개체를 가져옵니다.|  
|[swap 메서드](#swap)|두 개의의 내용을 바꿉니다 `concurrent_unordered_map` 개체입니다. 이 메서드는 동시성이 보장 되지 않습니다.|  
|[unsafe_erase 메서드](#unsafe_erase)|오버로드됨. 요소를 제거는 `concurrent_unordered_map` 지정 된 위치에 있습니다. 이 메서드는 동시성이 보장 되지 않습니다.|  
  
### <a name="public-operators"></a>Public 연산자  
  
|이름|설명|  
|----------|-----------------|  
|[operator] 연산자](#operator_at)|오버로드됨. 지정된 키가 있는 요소를 찾거나 삽입합니다. 이 메서드는 동시성이 보장 합니다.|  
|[operator = 연산자](#operator_eq)|오버로드됨. 다른 할당 `concurrent_unordered_map` 여기에 개체입니다. 이 메서드는 동시성이 보장 되지 않습니다.|  
  
## <a name="remarks"></a>주의  
 에 대 한 자세한 내용은 `concurrent_unordered_map` 클래스를 참조 하십시오 [병렬 컨테이너 및 개체](../../../parallel/concrt/parallel-containers-and-objects.md)합니다.  
...
##  <a name="a-nameata-at"></a><a name="at"></a>에서 
 ...
 요소를 찾습니다는 `concurrent_unordered_map` 지정 된 키 값을 가진... 이 메서드는 동시성이 보장 합니다.  
...
### <a name="return-value"></a>반환 값  
 A [쌍](http://msdn.microsoft.com/en-us/c5a37023-d939-4eb2-ae24-ce8e0cd4505d) 여기서 첫 번째 요소는 시작 부분에 대 한 반복기 하 고 두 번째 요소는 반복기가 범위 끝에 있습니다.  
...
##  <a name="a-nameinserta-insert"></a><a name="insert"></a>삽입 
 ...
 요소를 추가 하는 `concurrent_unordered_map` 개체입니다.  
...
### <a name="remarks"></a>주의  
 첫 번째 멤버 함수 X 요소 시퀀스의 순서 지정이 동일할 키가 있는지 여부를 확인 `value`합니다. 그렇지는 이러한 요소가 X 만들고으로 초기화 하는 경우 `value`합니다. 이 함수는 반복기를 결정 한 다음 `where` X를 지정 합니다. 함수 반환 삽입 발생 한 경우 `std::pair(where, true)`합니다. 그 외의 경우 `std::pair(where, false)`를 반환합니다.  
...
### <a name="remarks"></a>주의  
 ...
 `operator[]`map에 요소를 삽입 하는 데 사용 될 수 있습니다 `m` 를 사용 하 여 `m[key] = DataValue;`여기서 `DataValue` 값인는 `mapped_type` 의 키 값을 사용 하 여 요소의 `key`합니다.  
  
 `operator[]`를 사용하여 요소를 삽입하는 경우 반환된 참조는 삽입이 기존 요소를 변경하는지 또는 새 요소를 생성하는지 여부를 나타내지 않습니다. 멤버 함수는 `find` 및 [삽입](#insert) 지정 된 키가 있는 요소가 삽입 전에 표시 되어 있는지 여부를 확인 하기 위해 사용할 수 있습니다.  
  
##  <a name="a-nameoperatoreqa-operator"></a><a name="operator_eq"></a>연산자 = 

 다른 할당 `concurrent_unordered_map` 여기에 개체입니다. 이 메서드는 동시성이 보장 되지 않습니다.  
...
### <a name="remarks"></a>주의  
 ...
 Throw는 [out_of_range](../../../standard-library/out-of-range-class.md) 버킷 수가 올바르지 않으면 예외 (0 또는 최대 버킷 개수 보다 큰).  
...
##  <a name="a-nameswapa-swap"></a><a name="swap"></a>스왑 
 ...
 두 개의의 내용을 바꿉니다 `concurrent_unordered_map` 개체입니다. 이 메서드는 동시성이 보장 되지 않습니다.  
...
##  <a name="a-nameunsafeerasea-unsafeerase"></a><a name="unsafe_erase"></a>unsafe_erase 
 ...
 요소를 제거는 `concurrent_unordered_map` 지정 된 위치에 있습니다. 이 메서드는 동시성이 보장 되지 않습니다.  
...
### <a name="return-value"></a>반환 값  
 첫 번째 두 멤버 함수, 제거 된 요소에 남아 있는 첫 번째 요소를 지정 하는 반복기를 반환 하거나 `concurrent_unordered_map::end`이러한 요소가 없는 경우 (). 세 번째 멤버 함수를 제거 하는 요소의 수를 반환 합니다.  
  
### <a name="remarks"></a>주의  
 첫 번째 멤버 함수는 `_Where`로 지정된 제어 시퀀스의 요소를 제거합니다. 두 번째 멤버 함수 범위에서 요소 제거 [ `_Begin`, `_End`).  
  
 구분 된 범위의의 요소를 제거 하는 세 번째 멤버 함수 `concurrent_unordered_map::equal_range`(KVal).  
