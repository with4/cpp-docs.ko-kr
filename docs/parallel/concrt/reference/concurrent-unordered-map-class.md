...
#### <a name="parameters"></a>매개 변수  
 ...
 `_Allocator_type`  
 할당 및 순서가 지정 되지 않은 동시 맵에 대 한 메모리 할당 취소에 대 한 세부 정보를 캡슐화 하는 저장 된 할당자 개체를 나타내는 형식입니다. 이 인수는 선택 사항이 며 기본값은 `std::allocator<std::pair<K`, `_Element_type>>`합니다.  
...
|이름|설명|  
|----------|-----------------|  
|[concurrent_unordered_map](#ctor)|오버로드됨. 동시 unorderedmap를 생성합니다.|  
  
### <a name="public-methods"></a>Public 메서드  
  
|이름|설명|  
|----------|-----------------|  
|[at](#at)|오버로드됨. 요소를 찾습니다는 `concurrent_unordered_map` 지정 된 키 값으로... 이 메서드는 동시성 으로부터 안전한.|  
|[hash_function](#hash_function)|저장된 해시 함수 개체를 가져옵니다.|  
|[insert](#insert)|오버로드됨. 요소를 추가 하 고 `concurrent_unordered_map` 개체입니다.|  
|[key_eq](#key_eq)|저장 된 같음 비교 함수 개체를 가져옵니다.|  
|[swap](#swap)|두 개의의 내용을 바꿉니다 `concurrent_unordered_map` 개체입니다. 이 메서드는 동시성 으로부터 안전한 없습니다.|  
|[unsafe_erase](#unsafe_erase)|오버로드됨. 요소 제거는 `concurrent_unordered_map` 지정 된 위치에 있습니다. 이 메서드는 동시성 으로부터 안전한 없습니다.|  
  
### <a name="public-operators"></a>Public 연산자  
  
|이름|설명|  
|----------|-----------------|  
|[operator[]](#operator_at)|오버로드됨. 지정된 키가 있는 요소를 찾거나 삽입합니다. 이 메서드는 동시성 으로부터 안전한.|  
|[operator=](#operator_eq)|오버로드됨. 다른 할당 `concurrent_unordered_map` 여기에 개체입니다. 이 메서드는 동시성 으로부터 안전한 없습니다.|  
...
##  <a name="at"></a> 에서 
 ...
 요소를 찾습니다는 `concurrent_unordered_map` 지정 된 키 값으로... 이 메서드는 동시성 으로부터 안전한.  
...
##  <a name="insert"></a> 삽입 
 ...
 요소를 추가 하 고 `concurrent_unordered_map` 개체입니다.  
...
### <a name="return-value"></a>반환 값  
 찾은 또는 삽입 된 요소의 데이터 값에 대 한 참조입니다.  
  
### <a name="remarks"></a>설명  
 인수 키 값이 없으면 데이터 형식의 기본값과 함께 삽입됩니다.  
  
 `operator[]` map에 요소를 삽입 하는 데 사용 될 `m` 를 사용 하 여 `m[key] = DataValue;`여기서 `DataValue` 의 값인는 `mapped_type` 의 키 값을 사용 하 여 요소의 `key`합니다.  
  
 `operator[]`를 사용하여 요소를 삽입하는 경우 반환된 참조는 삽입이 기존 요소를 변경하는지 또는 새 요소를 생성하는지 여부를 나타내지 않습니다. 멤버 함수 `find` 및 [삽입](#insert) 지정 된 키가 있는 요소가 삽입 하기 전에 이미 있는지 여부를 확인 하기 위해 사용할 수 있습니다.  
  
##  <a name="operator_eq"></a> 연산자 = 

 다른 할당 `concurrent_unordered_map` 여기에 개체입니다. 이 메서드는 동시성 으로부터 안전한 없습니다.  
...
##  <a name="swap"></a> 스왑 
 ...
 두 개의의 내용을 바꿉니다 `concurrent_unordered_map` 개체입니다. 이 메서드는 동시성 으로부터 안전한 없습니다.  
...
##  <a name="unsafe_erase"></a> unsafe_erase 
 ...
 요소 제거는 `concurrent_unordered_map` 지정 된 위치에 있습니다. 이 메서드는 동시성 으로부터 안전한 없습니다.  
...
### <a name="return-value"></a>반환 값  
 제거 된 요소 뒤에 남은 첫 번째 요소를 지정 하는 반복기를 반환 하는 처음 두 멤버 함수 또는 `concurrent_unordered_map::end`이러한 요소가 없을 경우 (). 세 번째 멤버 함수를 제거 하는 요소의 수를 반환 합니다.  
  
### <a name="remarks"></a>설명  
 첫 번째 멤버 함수는 `_Where`로 지정된 제어 시퀀스의 요소를 제거합니다. 두 번째 멤버 함수 범위에서 요소 제거 [ `_Begin`, `_End`).  
  
 구분 된 범위의 요소를 제거 하는 세 번째 멤버 함수 `concurrent_unordered_map::equal_range`(KVal).  
