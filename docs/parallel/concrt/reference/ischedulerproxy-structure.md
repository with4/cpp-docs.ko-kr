...
|[Ischedulerproxy:: Createoversubscriber](#createoversubscriber)|기존 실행 리소스와 관련 된 하드웨어 스레드에 새로운 가상 프로세서 루트를 만듭니다.|  
...
##  <a name="shutdown"></a>Ischedulerproxy:: Shutdown 메서드  
 스케줄러가 종료 되는 리소스 관리자에 게 알립니다. 이렇게 하면 리소스 관리자에서 스케줄러에 부여 된 모든 리소스를 즉시 회수를 합니다.  
...
### <a name="remarks"></a>주의  
 ...
 스케줄러 있으면 모든 가상 프로세서 루트를 비활성화, 활성화 해야 합니다를 사용 하 여 [ivirtualprocessorroot:: Activate](ivirtualprocessorroot-structure.md#activate), 있고 둡니다 에서도 실행 스레드 프록시는 `Dispatch` 호출 먼저 디스패치 하는 이러한 실행 컨텍스트의 메서드 `Shutdown` 스케줄러 프록시에 있습니다.  
