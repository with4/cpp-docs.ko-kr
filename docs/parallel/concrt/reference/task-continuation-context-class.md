...
### <a name="remarks"></a>설명  
 기본 컨텍스트를 지정 하지 않으면 연속 컨텍스트를 호출할 때 사용 된 `then` 메서드. 아래와 Windows 7 용 Windows 응용 프로그램 뿐만 아니라 데스크톱 응용 프로그램 Windows 8 이상, 런타임 작업 연속 실행 됩니다 결정 합니다. 그러나 Windows 런타임 앱 아파트 인식 작업의 연속에 대 한 기본 연속 컨텍스트는 아파트 여기서 `then` 가 호출 됩니다.  
