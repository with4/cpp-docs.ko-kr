...
|이름|설명|  
|----------|-----------------|  
|[get 메서드](#get)|연결 된 비동기 작업이 완료 될 때까지 기다립니다.|  
|[then 메서드](#then)|콜백 함수 개체에이 검사기는 `completion_future` 연결된 된 비동기 작업 실행이 끝날 때 실행 될 개체입니다.|  
|[to_task 메서드](#to_task)|반환 된 `task` 개체는 연결된 된 비동기 작업에 해당 합니다.|  
|[유효한 메서드](#valid)|개체가 비동기 작업에 연결 되어 있는지 여부를 나타내는 부울 값을 가져옵니다.|  
|[wait 메서드](#wait)|연결된 된 비동기 작업 완료 될 때까지 차단 됩니다.|  
|[wait_for 메서드](#wait_for)|연결 된 비동기 작업이 완료 될 때까지 차단 또는 지정 된 시간 `_Rel_time` 경과 합니다.|  
|[wait_until 메서드](#wait_until)|연결된 된 비동기 작업 완료 될 때까지 또는 현재 시간에서 지정한 값을 초과할 때까지 차단 `_Abs_time`합니다.|  
  
### <a name="public-operators"></a>Public 연산자  
  
|이름|설명|  
|----------|-----------------|  
|[operator std:: shared_future\<void > 연산자](#operator_shared_future)|암시적으로 변환 된 `completion_future` 개체는 `std::shared_future` 개체입니다.|  
|[operator = 연산자](#operator_eq)|지정 된 내용을 복사 `completion_future` 을 여기에 개체입니다.|  
...
## <a name="a-namevalida-valid"></a><a name="valid"></a>유효한 
