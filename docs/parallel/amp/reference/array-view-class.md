---
title: "array_view 클래스 | Microsoft 문서"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
#### <a name="parameters"></a>매개 변수  
 `value_type`  
 에 있는 요소의 데이터 형식에서 `array_view` 개체입니다.  
...
|이름|설명|  
|----------|-----------------|  
|[array_view 생성자](#ctor)|`array_view` 클래스의 새 인스턴스를 초기화합니다. 에 대 한 기본 생성자는 `array<T,N>`합니다. 모든 생성자만 CPU에서 실행 되도록 제한 되며 Direct3D 대상에서 실행할 수 없습니다.|  
|[~ array_view 소멸자](#ctor)|소멸은 `array_view` 개체입니다.|  
  
### <a name="public-methods"></a>Public 메서드  
  
|이름|설명|  
|----------|-----------------|  
|[copy_to 메서드](#copy_to)|내용을 복사는 `array_view` 를 호출 하 여 지정한 대상 개체 `copy(*this, dest)`합니다.|  
|[데이터 메서드](#data)|원시 데이터에 대 한 포인터를 반환 된 `array_view`합니다.|  
|[discard_data 메서드](#discard_data)|이 보기는 현재 데이터를 삭제 합니다.|  
|[get_extent 메서드](#get_extent)|Array_view 개체의 범위 개체를 반환합니다.|  
|[get_ref 메서드](#get_ref)|인덱싱된 요소에 대 한 참조를 반환합니다.|  
|[get_source_accelerator_view 메서드](#get_source_accelerator_view)|반환 된 [accelerator_view](accelerator-view-class.md) 여기서의 데이터 소스는 `array_view` 있는 합니다.|  
|[refresh 메서드](#refresh)|알립니다는 `array_view` 외부에서 수정 되었으며 바인딩된 메모리 개체는 `array_view` 인터페이스입니다. 이 메서드를 호출 오래 된 모든 캐시 된 정보를 렌더링합니다.|  
|[reinterpret_as 메서드](#reinterpret_as)|에 있는 모든 요소를 포함 하는&1; 차원 배열을 반환는 `array_view` 개체입니다.|  
|[섹션 메서드](#section)|하위 섹션을 반환 된 `array_view` 지정된 된 origin을 필요에 따라 하에 있는 개체에는 지정 된 범위입니다.|  
|[synchronize 메서드](#synchronize)|모든 수정 내용이 동기화는 `array_view` 해당 원본 데이터에 다시 개체입니다.|  
|[synchronize_async 메서드](#synchronize_async)|모든 수정 내용이 비동기적으로 동기화는 `array_view` 해당 원본 데이터에 다시 개체입니다.|  
|[메서드를 synchronize_to](#synchronize_to)|모든 수정 내용이 동기화는 `array_view` 개체는 지정 된 [accelerator_view](accelerator-view-class.md)합니다.|  
|[synchronize_to_async 메서드](#synchronize_to_async)|모든 수정 내용이 비동기적으로 동기화는 `array_view` 개체는 지정 된 [accelerator_view](accelerator-view-class.md)합니다.|  
|[view_as 메서드](#view_as)|생성 된 `array_view` 이 사용 하 여 다른 차수의 개체 `array_view` 개체의 데이터입니다.|  
  
### <a name="public-operators"></a>Public 연산자  
  
|이름|설명|  
|----------|-----------------|  
|[operator () 연산자](#operator_call)|매개 변수 또는 매개 변수에서 지정한 요소의 값을 반환 합니다.|  
|[operator] 연산자](#operator_at)|매개 변수에 의해 지정 된 요소를 반환 합니다.|  
|[operator = 연산자](#operator_eq)|지정 된 내용을 복사 `array_view` 을 여기에 개체입니다.|  
...
|이름|설명|  
|----------|-----------------|  
|[extent 데이터 멤버](#extent)|`extent` 개체의 모양을 정의하는 `array_view` 개체를 가져옵니다.|  
|[source_accelerator_view 데이터 멤버](#source_accelerator_view)|가져옵니다는 [accelerator_view](accelerator-view-class.md) 여기서의 데이터 소스는 `array_view` 있는|  
|[value_type 데이터 멤버](#value_type)|값 형식으로는 `array_view` 및 바인딩된 배열입니다.|  
  
## <a name="remarks"></a>주의  
 `array_view` 클래스에 포함 된 데이터에 대 한 뷰를 나타냅니다.는 [배열](array-class.md) 개체 또는의 하위 섹션은 `array` 개체입니다.  
...
-   뷰는 시스템 메모리 포인터를 통해 전달 되는 [parallel_for_each](../../../parallel/concrt/reference/concurrency-namespace-functions.md#parallel_for_each) 액셀러레이터를 호출 하 고 액셀러레이터 키에 액세스 합니다.  
  
-   액셀러레이터 키에 있는 배열에 보기 방법으로 전달 되는 `parallel_for_each` 다른 엑셀 러 레이 터를 호출 하 고 있는 액세스할 수 있습니다.  
  
-   액셀러레이터 키에 있는 배열 하는 뷰를 CPU에 액세스 합니다.  
...
##  <a name="a-namecopytoa-copyto"></a><a name="copy_to"></a>copy_to 
 ...
 내용을 복사는 `array_view` 개체를 호출 하 여 지정 된 대상 개체에 `copy(*this, dest)`합니다.  
...
##  <a name="a-namedataa-data"></a><a name="data"></a>데이터 
 ...
 원시 데이터에 대 한 포인터를 반환 된 `array_view`합니다.  
...
##  <a name="a-namerefresha-refresh"></a><a name="refresh"></a>새로 고침 
...
소스 array_view 보다 다른 값 형식을 포함할 수 있는 옵션으로는&1; 차원 array_view 통해 array_view를 재해석 합니다.  
...
### <a name="remarks"></a>주의  
 소스 배열 보다 다른 값 형식이 있을 수를 선형,&1; 차원 배열에는 다차원 배열 보려면 편리 하 게 됩니다. 이를 달성할 수 있는 `array_view` 이 방법을 사용 하 여 합니다.  
  
**경고** Reinterpeting 다른 값 형식을 사용 하 여 array_view 개체는 잠재적으로 위험한 작업입니다. 이 기능은 주의 하 여 사용 해야 합니다.  
...
##  <a name="a-namesectiona-section"></a><a name="section"></a>섹션 
 ...
 하위 섹션을 반환 된 `array_view` 지정된 된 origin을 필요에 따라 하에 있는 개체에는 지정 된 범위입니다.  
...
##  <a name="a-namesynchronizea-synchronize"></a><a name="synchronize"></a>동기화 
...
### <a name="parameters"></a>매개 변수  
 `_Access_type`  
 의도 한 [access_type](concurrency-namespace-enums-amp.md#access_type) 대상에서 [accelerator_view](accelerator-view-class.md)합니다. 이 매개 변수는 기본값은 `access_type_read`합니다.  
...
### <a name="parameters"></a>매개 변수  
 ...
 `value_type`  
 데이터 형식에서 원래 요소의 [배열](array-class.md) 개체와 반환 된 `array_view` 개체입니다.  
