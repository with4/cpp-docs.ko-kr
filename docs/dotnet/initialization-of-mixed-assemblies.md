...
ms.locfileid: "33137893"
---
# <a name="initialization-of-mixed-assemblies"></a>혼합형 어셈블리 초기화

Windows 개발자 항상 있다는 점을 유의 해야 로더 잠금 하는 동안 코드를 실행할 때 `DllMain`합니다. 그러나 C +를 처리할 때 고려해 야 하는 몇 가지 추가 고려 사항 사항이 + clr 혼합 모드 어셈블리입니다.
...
로더 잠금이 여전히 발생할 수 있지만 이제 잠금이 일정하게 발생하며 이를 감지할 수 있습니다. 경우 `DllMain` MSIL 명령이 포함 되어 컴파일러 경고를 생성 [컴파일러 경고 (수준 1) C4747](../error-messages/compiler-warnings/compiler-warning-level-1-c4747.md)합니다. 또한 CRT 또는 CLR에서는 로더 잠금 상황에서 MSIL을 실행하려는 시도를 감지 및 보고합니다. CRT에서 이러한 상황이 감지되면 런타임 진단 C 런타임 오류 R6033이 발생합니다.
...
사용자 제공 버전을 MSIL로 컴파일하는 경우 이러한 이니셜라이저는 로더 잠금 상태에서도 MSIL 명령을 실행하려 합니다. 사용자가 제공한 `malloc` 는 동일한 결과가 발생 합니다. 이 문제를 해결하려면 이러한 오버로드나 사용자 제공 정의를 모두 #pragma `unmanaged` 지시문을 사용하여 네이티브 코드로 구현해야 합니다.
...
1. 디버거 모드를 네이티브 전용 모드로 설정합니다.
   ...
   이 작업을 수행 하려면 엽니다는 **속성** 솔루션의 시작 프로젝트에 대 한 표입니다. 선택 **구성 속성** > **디버깅**합니다. 설정의 **디버거 형식** 를 **네이티브 전용**합니다.

1. 디버거 (F5)을 시작 합니다.
  
1. 경우는 **/clr** 진단이 생성 되, 선택 **을 다시 시도** 선택한 후 **중단**합니다.
  
1. 호출 스택 창을 엽니다. (메뉴 모음에서 **디버그** > **Windows** > **호출 스택**.) 잘못 된 `DllMain` 또는 정적 이니셜라이저는 녹색 화살표와 함께 식별 됩니다. 위반하는 함수를 식별할 수 없으면 다음 단계를 수행하여 이를 찾아야 합니다.

1. 열기는 **직접 실행** 창 (메뉴 모음에서 **디버그** > **Windows** > **직접 실행**.)

1. .Load sos.dll을 입력으로 **직접 실행** SOS 디버깅 서비스를 로드 하는 창입니다.
  
1. 형식! dumpstack는 **직접 실행** 내부의 전체 목록을 가져오기 위해 창을 **/clr** 스택 합니다.

1. _Cordllmain (스택 맨 아래에 가장 가까운) 첫 번째 인스턴스를 찾습니다 (경우 `DllMain` 문제를 일으키는), _VTableBootstrapThunkInitHelperStub 또는 GetTargetForVTableEntry (정적 이니셜라이저는 문제를 일으키는) 하는 경우. 이 호출 바로 아래에 있는 스택 항목이 로더 잠금 상태에서 실행하려 했던 MSIL 구현 함수의 호출입니다.
...
다음 예제에서 코드를 이동 하 여 로더 잠금을 해결 하는 방법을 보여 줍니다 `DllMain` 는 전역 개체의 생성자에 있습니다.

이 샘플은 전역 관리 되는 개체의 생성자에 원래 있던 관리 되는 개체에 포함 `DllMain`합니다. 이 샘플의 두 번째 부분에서는 어셈블리를 참조하고 관리되는 개체의 인스턴스를 만들어 초기화를 수행하는 모듈 생성자를 호출합니다.
