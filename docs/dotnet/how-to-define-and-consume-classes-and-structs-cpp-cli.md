---
title: "방법: 정의 및 클래스 및 구조체 (c + + CLI)를 사용 합니다. | Microsoft Docs"
ms.custom: ""
ms.date: "12/14/2016"
...
이 문서에서는 [!INCLUDE[cppcli](../build/reference/includes/cppcli_md.md)]에서 사용자 정의 참조 형식 및 값 형식을 정의하고 사용하는 방법을 보여 줍니다.  
  
##  <a name="a-namebkmkcontentsa-contents"></a><a name="BKMK_Contents"></a> 내용  
...
##  <a name="a-namebkmkobjectinstantiationa-object-instantiation"></a><a name="BKMK_Object_instantiation"></a> 개체 인스턴스화  
...
##  <a name="a-namebkmkimplicitlyabstractclassesa-implicitly-abstract-classes"></a><a name="BKMK_Implicitly_abstract_classes"></a> 암시적 추상 클래스  
...
##  <a name="a-namebkmktypevisibilitya-type-visibility"></a><a name="BKMK_Type_visibility"></a> 형식 표시 유형  
 어셈블리가 참조된 경우 어셈블리의 형식이 어셈블리 외부에서 표시되거나 표시되지 않도록 CLR(공용 언어 런타임)의 표시 유형을 제어할 수 있습니다.  
  
 `public` 형식이 포함 된 소스 파일에 표시 되는지 나타냅니다는 `#using` 형식을 포함 하는 어셈블리에 대 한 지시문입니다.  `private` 형식이 포함 된 소스 파일에 표시 되지 않음을 나타냅니다는 `#using` 형식을 포함 하는 어셈블리에 대 한 지시문입니다. 그러나 private 형식은 동일한 어셈블리 내에는 표시됩니다. 기본적으로 클래스에 대한 표시 유형은 `private`입니다.  
  
 기본적으로 Visual C++ 2005 이전에는 네이티브 형식에 어셈블리 외부의 public 액세스 가능성이 있었습니다. 사용 하도록 설정 [컴파일러 경고 (수준 1) C4692](../error-messages/compiler-warnings/compiler-warning-level-1-c4692.md) private 네이티브 형식이 사용 되는 위치 하지 올바르게 볼 수 있습니다. 사용 하 여 [make_public](../preprocessor/make-public.md) pragma를 수정할 수 없는 소스 코드 파일에 네이티브 형식에 public 액세스 가능성을 제공 합니다.  
...
##  <a name="a-namebkmkmembervisibilitya-member-visibility"></a><a name="BKMK_Member_visibility"></a> 멤버 표시 유형  
...
|internal|멤버는 어셈블리 내부에서 public 이지만 어셈블리 외부에서는 private 합니다.  `internal`는 상황에 맞는 키워드입니다.  자세한 내용은 참조 [상황에 맞는 키워드](../windows/context-sensitive-keywords-cpp-component-extensions.md)합니다.|  
...
##  <a name="a-namebkmkpublicandprivatenativeclassesa-public-and-private-native-classes"></a><a name="BKMK_Public_and_private_native_classes"></a> Public 및 private 네이티브 클래스  
...
##  <a name="a-namebkmkstaticconstructorsa-static-constructors"></a><a name="BKMK_Static_constructors"></a> 정적 생성자  
...
##  <a name="a-namebkmksemanticsofthethispointera-semantics-of-the-this-pointer"></a><a name="BKMK_Semantics_of_the_this_pointer"></a> 의미 체계의 this 포인터  
...
##  <a name="a-namebkmkhidebysignaturefunctionsa-hide-by-signature-functions"></a><a name="BKMK_Hide_by_signature_functions"></a> 서명으로 숨기기 함수  
 표준 C++에서 기본 클래스의 함수는 파생 클래스 함수에 동일하지 않은 매개 변수의 수나 종류가 있는 경우에도 파생 클래스에서 동일한 이름을 가진 함수에 의해 숨겨져 있습니다. 이 라고 *이름으로 숨기기* 의미 합니다. 참조 형식에서 기본 클래스의 함수는 이름 및 매개 변수의 목록이 모두 동일한 경우 파생 클래스의 함수에 의해서만 숨겨질 수 있습니다. 로 알려져 *서명으로 숨기기* 의미 합니다.  
  
 클래스의 모든 함수가 메타데이터에 `hidebysig`로 표시되면 클래스는 hide-by-signature 클래스라고 간주됩니다. 기본적으로 아래에 생성 된 모든 클래스 **/clr** 가 `hidebysig` 함수입니다. 그러나 클래스를 사용 하 여 컴파일되는 **/clr:oldSyntax** 없는 `hidebysig` 함수는 이름으로 숨기기 함수 대신 합니다. 클래스에 `hidebysig` 함수가 있는 경우 컴파일러는 함수를 직접 기본 클래스의 이름으로 숨기지 않습니다. 하지만 컴파일러에서 상속 체인의 hide-by-name 클래스를 발견하는 경우 hide-by-name 동작이 계속됩니다.  
...
```  
 ...
 사용 하 여 컴파일된 구성 요소를 정의 하는 다음 샘플 **/clr:oldSyntax**합니다. Managed Extensions for C++를 사용하여 정의된 클래스는 hide-by-name 멤버 함수를 가집니다.  
...
```  
 ...
 다음 샘플은 이전 샘플에서 빌드된 구성 요소를 사용합니다. 알림 방법을 숨기기 서명으로 기능을 사용 하 여 컴파일된 형식의 기본 클래스에 적용 되지 않습니다 **/clr:oldSyntax**합니다.  
...
##  <a name="a-namebkmkcopyconstructorsa-copy-constructors"></a><a name="BKMK_Copy_constructors"></a> 복사 생성자  
 C++ 표준에 따르면 복사 생성자는 개체가 동일한 주소에서 생성되고 제거되는 것처럼 개체가 이동하면 호출됩니다.  
  
 그러나, **/clr** 컴파일 및 MSIL 호출 있는 네이티브 함수를 네이티브 클래스에 컴파일된 함수가 하는 데 사용 됩니다-하나 이상의-값으로 하 고 있는 네이티브 클래스에는 복사 생성자 및/또는 소멸자, 및 전달 됩니다 복사 생성자가 호출 되지 개체가 만들어진 위치와 다른 주소에서 소멸 됩니다. 클래스 자체에 대한 포인터가 있거나 코드가 주소로 개체를 추적하는 경우 문제가 발생할 수 있습니다.  
...
##  <a name="a-namebkmkdestructorsandfinalizersa-destructors-and-finalizers"></a><a name="BKMK_Destructors_and_finalizers"></a> 소멸자 및 종료자  
 참조 형식에서 소멸자는 리소스의 명확한 정리를 수행합니다. 종료자는 관리되지 않는 리소스를 정리하고 소멸자에 의해 명확하게 호출되거나 가비지 수집기에 의해 불명확하게 호출될 수 있습니다. 표준 c + +에서 소멸자에 대 한 정보를 참조 하십시오. [소멸자](../cpp/destructors-cpp.md)합니다.  
...
```  
 ...
 관리되는 Visual C++ 클래스의 소멸자와 Managed Extensions for C++의 소멸자는 동작이 다릅니다. 이러한 변경에 대 한 자세한 내용은 참조 [소멸자 의미의 변경 내용](../dotnet/changes-in-destructor-semantics.md)합니다.  
 ...
 Visual c + + 종료자는 동일 하지는 <xref:System.Object.Finalize%2A> 메서드. (CLR 문서는 종료자와 <xref:System.Object.Finalize%2A> 메서드 같은 뜻으로 사용).  <xref:System.Object.Finalize%2A> 메서드 클래스 상속 체인의 각 종료자를 호출 하는 가비지 수집기에 의해 호출 됩니다. Visual C++ 소멸자와 달리 파생 클래스 종료자가 호출되면 컴파일러는 모든 기본 클래스의 종료자를 호출하지 않습니다.  
  
 Visual c + + 컴파일러가 리소스의 명확한 해제를 지원 하므로 구현 하지 마십시오는 <xref:System.IDisposable.Dispose%2A> 또는 <xref:System.Object.Finalize%2A> 메서드. 그러나 이러한 방법에 잘 알고 있다면 여기는에 매핑하는 Visual c + + 종료자 및 소멸자가 종료자를 호출 하 여 <xref:System.IDisposable.Dispose%2A> 패턴:  
...
```  
 ...
 소멸자를 호출 합니다.-를 사용 하 여 <xref:System.GC.SuppressFinalize%2A>-개체의 종료 합니다. 소멸자가 호출되지 않으면 해당 형식의 종료자가 결국 가비지 수집기에 의해 호출됩니다.  
...
-   호출 된 [삭제](../cpp/delete-operator-cpp.md) 연산자에 대 한 핸들 ([개체 연산자 (^)에 대 한 핸들](../windows/handle-to-object-operator-hat-cpp-component-extensions.md)).  
  
-   명시적으로 소멸자를 호출한 경우  
  
 다른 언어로 작성된 클라이언트에 의해 형식이 사용 중인 경우 소멸자가 다음과 같이 호출됩니다.  
  
-   에 대 한 호출에서 <xref:System.IDisposable.Dispose%2A>합니다.  
  
-   형식의 `Dispose(void)`에 대한 호출인 경우  
  
-   형식이 C#의 `using` 문의 범위를 벗어나는 경우  
  
 사용 하 여 (하지 참조 형식에 대 한 스택 의미 체계 사용) 관리 되는 힙에서 참조 형식의 개체를 만드는 경우 [-try-finally](../cpp/try-finally-statement.md) 실행에서 예외가 소멸자를 방지 하지 않도록 확인 하려면 다음 구문을 사용 합니다.  
...
```  
 ...
 형식에 소멸자가 하는 경우 컴파일러가 생성 한 `Dispose` 메서드를 구현 하는 <xref:System.IDisposable>합니다. Visual C++로 작성되고 다른 언어에서 사용된 소멸자가 있는 형식의 경우 해당 형식에서 `IDisposable::Dispose`를 호출하면 해당 형식의 소멸자가 호출됩니다. 형식이 Visual C++ 클라이언트에서 사용되면 `Dispose`를 직접 호출할 수 없습니다. 대신 `delete` 연산자를 사용하여 소멸자를 호출합니다.  
  
 형식에 종료자, 컴파일러가 생성 하는 `Finalize(void)` 메서드를 재정의 하는 <xref:System.Object.Finalize%2A>합니다.  
  
 형식에 종료자 또는 소멸자가 있는 경우 컴파일러는 디자인 패턴에 따라 `Dispose(bool)` 메서드를 생성합니다. (내용은 [삭제 패턴](../Topic/Dispose%20Pattern.md)). Visual C++에서 `Dispose(bool)`를 명시적으로 작성하거나 호출할 수 없습니다.  
 ...
 종료 자가 관리 되는 형식에서 네이티브 포인터를 삭제 하면 있어야는 참조 또는 네이티브 포인터를 통한 중간 수집 되지 않습니다. 사용 하는 대신 관리 되는 형식에서 소멸자를 호출 <xref:System.GC.KeepAlive%2A>합니다.  
