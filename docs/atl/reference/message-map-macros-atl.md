...
|[CHAIN_MSG_MAP](#chain_msg_map)|기본 메시지를 체인의 기본 클래스에 매핑합니다.|  
|[CHAIN_MSG_MAP_DYNAMIC](#chain_msg_map_dynamic)|메시지에 대 한 체인 실행 시 다른 클래스에 매핑합니다.|  
|[CHAIN_MSG_MAP_MEMBER](#chain_msg_map_member)|클래스의 데이터 멤버의 기본 메시지 맵에 체인입니다.|  
|[COMMAND_CODE_HANDLER](#command_code_handler)|지도 **WM_COMMAND** 알림 코드에 따라를 처리기 함수는 메시지입니다.|  
|[COMMAND_HANDLER](#command_handler)|지도 **WM_COMMAND** 알림 코드와 해당 메뉴 항목, 컨트롤 또는 액셀러레이터 키의 식별자에 따라를 처리기 함수는 메시지입니다.|  
|[COMMAND_ID_HANDLER](#command_id_handler)|지도 **WM_COMMAND** 메시지 처리기 함수에 메뉴 항목, 컨트롤 또는 액셀러레이터 키의 식별자를 기반 합니다.|  
|[COMMAND_RANGE_CODE_HANDLER](#command_range_code_handler)|지도 **WM_COMMAND** 알림 코드와 컨트롤 식별자의 연속 된 범위에 따라를 처리기 함수는 메시지입니다.|  
|[COMMAND_RANGE_HANDLER](#command_range_handler)|지도 **WM_COMMAND** 컨트롤 식별자의 연속 된 범위에 따라를 처리기 함수는 메시지입니다.|  
...
|[MESSAGE_RANGE_HANDLER](#message_range_handler)|연속 Windows 메시지를 처리기 함수에 매핑합니다.|  
|[NOTIFY_CODE_HANDLER](#notify_code_handler)|지도 **WM_NOTIFY** 알림 코드에 따라를 처리기 함수는 메시지입니다.|  
|[NOTIFY_HANDLER](#notify_handler)|지도 **WM_NOTIFY** 알림 코드와 컨트롤 식별자에 따라를 처리기 함수는 메시지입니다.|  
|[NOTIFY_ID_HANDLER](#notify_id_handler)|지도 **WM_NOTIFY** 컨트롤 식별자에 따라를 처리기 함수는 메시지입니다.|  
|[NOTIFY_RANGE_CODE_HANDLER](#notify_range_code_handler)|지도 **WM_NOTIFY** 알림 코드와 컨트롤 식별자의 연속 된 범위에 따라를 처리기 함수는 메시지입니다.|  
|[NOTIFY_RANGE_HANDLER](#notify_range_handler)|지도 **WM_NOTIFY** 컨트롤 식별자의 연속 된 범위에 따라를 처리기 함수는 메시지입니다.|  
|[REFLECT_NOTIFICATIONS](#reflect_notifications)|알림 메시지를 다시 보낸 창에 반영 됩니다.|  
|[REFLECTED_COMMAND_CODE_HANDLER](#reflected_command_code_handler)|매핑하는 반영 **WM_COMMAND** 알림 코드에 따라를 처리기 함수는 메시지입니다.|  
|[REFLECTED_COMMAND_HANDLER](#reflected_command_handler)|매핑하는 반영 **WM_COMMAND** 알림 코드와 해당 메뉴 항목, 컨트롤 또는 액셀러레이터 키의 식별자에 따라를 처리기 함수는 메시지입니다.|  
|[REFLECTED_COMMAND_ID_HANDLER](#reflected_command_id_handler)|매핑하는 반영 **WM_COMMAND** 메시지 처리기 함수에 메뉴 항목, 컨트롤 또는 액셀러레이터 키의 식별자를 기반 합니다.|  
|[REFLECTED_COMMAND_RANGE_CODE_HANDLER](#reflected_command_range_code_handler)|매핑하는 반영 **WM_COMMAND** 알림 코드와 컨트롤 식별자의 연속 된 범위에 따라를 처리기 함수는 메시지입니다.|  
|[REFLECTED_COMMAND_RANGE_HANDLER](#reflected_command_range_handler)|매핑하는 반영 **WM_COMMAND** 컨트롤 식별자의 연속 된 범위에 따라를 처리기 함수는 메시지입니다.|  
|[REFLECTED_NOTIFY_CODE_HANDLER](#reflected_notify_code_handler)|매핑하는 반영 **WM_NOTIFY** 알림 코드에 따라를 처리기 함수는 메시지입니다.|  
|[REFLECTED_NOTIFY_HANDLER](#reflected_notify_handler)|매핑하는 반영 **WM_NOTIFY** 알림 코드와 컨트롤 식별자에 따라를 처리기 함수는 메시지입니다.|  
|[REFLECTED_NOTIFY_ID_HANDLER](#reflected_notify_id_handler)|매핑하는 반영 **WM_NOTIFY** 컨트롤 식별자에 따라를 처리기 함수는 메시지입니다.|  
...
### <a name="remarks"></a>설명  
 ATL 각 메시지 맵에 번호로 식별합니다. 기본 메시지 맵 (사용 하 여 선언는 `BEGIN_MSG_MAP` 매크로) 0으로 식별 됩니다. 로 식별 되는 대체 메시지 맵을 `msgMapID`합니다.  
  
 메시지 맵은 창으로 전송 하는 메시지를 처리 하는 데 사용 됩니다. 예를 들어 [CContainedWindow](../../atl/reference/ccontainedwindowt-class.md) 포함 하는 개체에서 메시지 맵의 식별자를 지정할 수 있습니다. [CContainedWindow::WindowProc](ccontainedwindowt-class.md#windowproc) 다음이 메시지 맵을 사용 하 여 포함된 된 창의 메시지를 적절 한 처리기 함수에 또는 다른 메시지 맵에 있습니다. 처리기 함수를 선언 하는 매크로의 목록이 참조 [BEGIN_MSG_MAP](#begin_msg_map)합니다.  
  
 항상 사용 하 여 메시지 map 시작 `BEGIN_MSG_MAP`합니다. 그런 다음 후속 대체 메시지 맵 선언할 수 있습니다.  
  
 [END_MSG_MAP](#end_msg_map) 매크로 메시지 맵에의 끝을 표시 합니다. 인스턴스 중 하나만 항상는 `BEGIN_MSG_MAP` 및 `END_MSG_MAP`합니다.  
...
### <a name="example"></a>예  
 ...
 다음 예제에서는 두 개의 대체 메시지 맵이 보여 줍니다. 기본 메시지 맵을 비어 있습니다.  
...
### <a name="remarks"></a>설명  
 [CWindowImpl::WindowProc](cwindowimpl-class.md#windowproc) 기본 메시지 맵을 사용 하 여 창에 보내는 메시지를 처리 합니다. 메시지 맵 적절 한 처리기 함수 또는 다른 메시지 맵을 메시지를 보냅니다.  

  
 다음 매크로를 처리기 함수는 메시지를 매핑합니다. 이 함수에 정의 되어야 합니다 `theClass`합니다.  
...
|[COMMAND_HANDLER](#command_handler)|지도 **WM_COMMAND** 알림 코드와 해당 메뉴 항목, 컨트롤 또는 액셀러레이터 키의 식별자에 따라를 처리기 함수는 메시지입니다.|  
|[COMMAND_ID_HANDLER](#command_id_handler)|지도 **WM_COMMAND** 메시지 처리기 함수에 메뉴 항목, 컨트롤 또는 액셀러레이터 키의 식별자를 기반 합니다.|  
|[COMMAND_CODE_HANDLER](#command_handler)|지도 **WM_COMMAND** 알림 코드에 따라를 처리기 함수는 메시지입니다.|  
|[COMMAND_RANGE_HANDLER](#command_range_handler)|인접 한 범위를 매핑합니다 **WM_COMMAND** 메뉴 항목, 컨트롤 또는 액셀러레이터 키의 식별자를 기반으로 메시지를 처리기 함수입니다.|  
|[NOTIFY_HANDLER](#notify_handler)|지도 **WM_NOTIFY** 알림 코드와 컨트롤 식별자에 따라를 처리기 함수는 메시지입니다.|  
|[NOTIFY_ID_HANDLER](#notify_id_handler)|지도 **WM_NOTIFY** 컨트롤 식별자에 따라를 처리기 함수는 메시지입니다.|  
|[NOTIFY_CODE_HANDLER](#notify_code_handler)|지도 **WM_NOTIFY** 알림 코드에 따라를 처리기 함수는 메시지입니다.|  
|[NOTIFY_RANGE_HANDLER](#notify_range_handler)|인접 한 범위를 매핑합니다 **WM_NOTIFY** 컨트롤 식별자를 기반으로 메시지를 처리기 함수입니다.|  
  
 다음 매크로 다른 메시지 맵에 대 한 메시지를 전달합니다. 이 프로세스를 "체인" 라고  
...
|[CHAIN_MSG_MAP_ALT](#chain_msg_map_alt)|기본 클래스에는 대체 메시지 맵에 연결 합니다.|  
|[CHAIN_MSG_MAP_ALT_MEMBER](#chain_msg_map_alt_member)|클래스의 데이터 멤버에는 대체 메시지 맵에 연결 합니다.|  
|[CHAIN_MSG_MAP_DYNAMIC](#chain_msg_map_dynamic)|기본 메시지를 체인 실행 시 다른 클래스에 매핑합니다.|  
  
 다음 매크로 부모 창에서 "반영" 메시지를 전달합니다. 예를 들어 컨트롤 일반적으로 알림 메시지를 해당 부모 창에 대 한 처리를 보내지만 부모 창 컨트롤에 다시 메시지를 반영할 수 있습니다.  
...
### <a name="example"></a>예  
 ...
 기본 메시지 맵을 외에도 대체 메시지 맵을 정의할 수 있습니다 [ALT_MSG_MAP](#alt_msg_map)합니다. 항상 사용 하 여 메시지 map 시작 `BEGIN_MSG_MAP`합니다. 그런 다음 후속 대체 메시지 맵 선언할 수 있습니다. 다음 예제에서는 기본 메시지 맵과 각각 하나의 처리기 함수를 포함 하는 하나의 대체 메시지 맵을 보여 줍니다.  
...
-   창 프로시저를 사용 하는 경우 `CMyClass`의 기본 메시지 맵 및 `OnPaint` 메시지는 메시지가 전송 되는 핸들 하지 않습니다 `CMyBaseClass`의 기본 메시지 맵을 처리 합니다.  
...
> [!NOTE]
>  항상 사용 하 여 메시지 map 시작 [BEGIN_MSG_MAP](#begin_msg_map)합니다. 포함 된 후속 대체 메시지 맵을 선언할 수 있습니다 `ALT_MSG_MAP`합니다. [END_MSG_MAP](#end_msg_map) 매크로 메시지 맵에의 끝을 표시 합니다. 모든 메시지 맵을의 인스턴스 중 하나만 있어야 합니다. `BEGIN_MSG_MAP` 및 `END_MSG_MAP`합니다.  
...
-   창 프로시저를 사용 하는 경우 `CMyClass`의 기본 메시지 맵 및 `OnPaint` 메시지는 메시지가 전송 되는 핸들 하지 않습니다 `m_obj`의 기본 메시지 맵을 처리 합니다.  
  
-   창 프로시저에서 첫 번째 대체 메시지 맵을 사용 하는 경우 `CMyClass`, 모든 메시지 이동 되 고 `m_obj`의 기본 메시지 맵.  
  
-   창 프로시저를 사용 하는 경우 `CMyClass`의 두 번째 대체 메시지 매핑 및 `OnChar` 메시지, 메시지의 지정 된 대체 메시지 구조에 전달 되는 핸들 하지 않는 `m_obj`합니다. 클래스 `CMyContainedClass` 로이 메시지 맵 선언 있어야 `ALT_MSG_MAP(1)`합니다.  
...
> [!NOTE]
>  항상 사용 하 여 메시지 map 시작 [BEGIN_MSG_MAP](#begin_msg_map)합니다. 포함 된 후속 대체 메시지 맵을 선언할 수 있습니다 [ALT_MSG_MAP](#alt_msg_map)합니다. [END_MSG_MAP](#end_msg_map) 매크로 메시지 맵에의 끝을 표시 합니다. 모든 메시지 맵을의 인스턴스 중 하나만 있어야 합니다. `BEGIN_MSG_MAP` 및 `END_MSG_MAP`합니다.  
  
 외에 `COMMAND_HANDLER`를 사용할 수 있습니다 [MESSAGE_HANDLER](#message_handler) 에 매핑하는 **WM_COMMAND** 식별자 나 코드에 관계 없이 메시지입니다. 이 경우 `MESSAGE_HANDLER(WM_COMMAND, OnHandlerFunction)` 모든 데이터 웨어하우스가 **WM_COMMAND** 메시지의 메시지를 `OnHandlerFunction`합니다.  
...
### <a name="parameters"></a>매개 변수  
 `idFirst`  
 [in] 연속 된 범위 컨트롤 식별자의 시작을 표시 합니다.  
  
 `idLast`  
 [in] 연속 된 범위 컨트롤 식별자의 끝을 표시 합니다.  
...
### <a name="remarks"></a>설명  
 이 범위를 메뉴 항목, 컨트롤 또는 메시지를 보내는 액셀러레이터 키의 식별자를 기반으로 합니다.  
...
### <a name="remarks"></a>설명  
 항상 사용 하는 [BEGIN_MSG_MAP](#begin_msg_map) 매크로를 메시지 맵의 시작을 표시 합니다. 사용 하 여 [ALT_MSG_MAP](#alt_msg_map) 후속 대체 메시지 맵 선언할 수 있습니다.  
  
 인스턴스 중 하나만 항상는 `BEGIN_MSG_MAP` 및 `END_MSG_MAP`합니다.  
...
>  항상 사용 하 여 메시지 map 시작 [BEGIN_MSG_MAP](#begin_msg_map)합니다. 포함 된 후속 대체 메시지 맵을 선언할 수 있습니다 [ALT_MSG_MAP](#alt_msg_map)합니다. [END_MSG_MAP](#end_msg_map) 매크로 메시지 맵에의 끝을 표시 합니다. 모든 메시지 맵을의 인스턴스 중 하나만 있어야 합니다. `BEGIN_MSG_MAP` 및 `END_MSG_MAP`합니다.  
 ...
 외에 `MESSAGE_HANDLER`를 사용할 수 있습니다 [COMMAND_HANDLER](#command_handler) 및 [NOTIFY_HANDLER](#notify_handler) 매핑할 [WM_COMMAND](http://msdn.microsoft.com/library/windows/desktop/ms647591) 및 [WM_NOTIFY](http://msdn.microsoft.com/library/windows/desktop/bb775583) 메시지를 각각 합니다.  
...
>  항상 사용 하 여 메시지 map 시작 [BEGIN_MSG_MAP](#begin_msg_map)합니다. 포함 된 후속 대체 메시지 맵을 선언할 수 있습니다 [ALT_MSG_MAP](#alt_msg_map)합니다. [END_MSG_MAP](#end_msg_map) 매크로 메시지 맵에의 끝을 표시 합니다. 모든 메시지 맵을의 인스턴스 중 하나만 있어야 합니다. `BEGIN_MSG_MAP` 및 `END_MSG_MAP`합니다.  
 ...
 외에 `NOTIFY_HANDLER`를 사용할 수 있습니다 [MESSAGE_HANDLER](#message_handler) 에 매핑하는 **WM_NOTIFY** 식별자 나 코드에 관계 없이 메시지입니다. 이 경우 `MESSAGE_HANDLER(WM_NOTIFY, OnHandlerFunction)` 모든 데이터 웨어하우스가 **WM_NOTIFY** 메시지의 메시지를 `OnHandlerFunction`합니다.  
