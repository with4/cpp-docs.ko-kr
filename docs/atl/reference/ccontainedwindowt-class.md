---
title: "CContainedWindowT 클래스 | Microsoft 문서"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
> [!NOTE]
> [CContainedWindow](ccontainedwindowt-class.md) 의 특수화 `CContainedWindowT`합니다. 기본 클래스 또는 특성을 변경 하려는 경우 사용 하 여 `CContainedWindowT` 직접.  
...
|이름|설명|  
|----------|-----------------|  
|[CContainedWindowT::CContainedWindowT](#ccontainedwindowt)|생성자입니다. 메시지 맵이 포함 된 창 메시지를 처리를 지정 하려면 데이터 멤버를 초기화 합니다.|  
...
|[CContainedWindowT::SwitchMessageMap](#switchmessagemap)|변경 내용이 포함 된 창 메시지를 처리 하는 메시지 맵에 사용 됩니다.|  
...
|이름|설명|  
|----------|-----------------|  
|[CContainedWindowT::m_dwMsgMapID](#m_dwmsgmapid)|어떤 메시지 맵이 포함 된 창 메시지를 처리를 식별 합니다.|  
...
## <a name="remarks"></a>주의  
 `CContainedWindowT`다른 개체 내에 포함 된 창을 구현 합니다. `CContainedWindowT`창 프로시저는 메시지를 적절 한 처리기에 직접 메시지를 포함 하는 개체에 매핑합니다. 생성할 때는 `CContainedWindowT` 개체는 메시지 맵을 사용할지를 지정 합니다.  
  
 `CContainedWindowT`기존 창 클래스 슈퍼 클 래 싱 하 여 새 창을 만들 수 있습니다. **만들기** 메서드는 먼저 기존 클래스를 기반으로 하지만 사용 하는 창 클래스 등록 `CContainedWindowT::WindowProc`합니다. **만들** 그런 다음이 새 창 클래스를 기반으로 창을 만듭니다. 각 인스턴스에 `CContainedWindowT` 슈퍼 클래스는 다양 한 창 클래스를 수 있습니다.  
...
> [!NOTE]
>  지정 된 모든 작업에 대 한 `CContainedWindowT` 개체, 호출 **만들기** 또는 `SubclassWindow`합니다. 동일한 개체에서 두 메서드를 호출 하지 않아야 합니다.  
  
 사용 하는 경우는 **컨트롤에 따라 추가** 마법사가 자동으로 추가 하는 ATL 프로젝트 마법사에서 옵션을 `CContainedWindowT` 컨트롤을 구현 하는 클래스 데이터 멤버입니다. 다음 예제에서는 포함 된 창은 선언 되는 방법을 보여 줍니다.  
...
### <a name="parameters"></a>매개 변수  
 ...
 `dwMsgMapID`  
 [in] 포함 된 창 메시지를 처리 하는 메시지 맵을 식별 합니다. 기본값인 0으로 선언 된 기본 메시지 맵을 지정 [BEGIN_MSG_MAP](http://msdn.microsoft.com/library/8bbb5af9-18b1-48c6-880e-166f599ee554)합니다. 으로 선언 된 대체 메시지 맵을 사용 하 여 [ALT_MSG_MAP(msgMapID)](http://msdn.microsoft.com/library/2c8871bf-abc0-4d52-bcf7-6b2ab9eb5af8), 전달 `msgMapID`합니다.  
  
### <a name="remarks"></a>주의  
 새 창을 통해 만들려는 경우 [만들기](#create)에 대 한 기존 창 클래스의 이름을 전달 해야는 `lpszClassName` 매개 변수입니다. 예를 들어 참조는 [CContainedWindow](../../atl/reference/ccontainedwindowt-class.md) 개요.  
  
 생성자&3; 가지가 있습니다.  
  
-   세 개의 인수를 사용 하 여 생성자는 일반적으로 호출입니다.  
  
-   두 개의 인수를 사용 하 여 생성자에서 클래스 이름을 사용 하 여 **TBase::GetWndClassName**합니다.  
  
-   인수가 없는 생성자는 인수를 나중에 제공 하려는 경우에 사용 됩니다. 나중에 호출할 때 창 클래스 이름, 메시지 맵 개체 및 메시지 맵 ID를 제공 해야 **만들기**합니다.  
...
### <a name="parameters"></a>매개 변수  
 ...
 `dwStyle`  
 [in] 창 스타일입니다. 기본값은 **WS_CHILD | WS_VISIBLE**합니다. 가능한 값 목록은 참조 하십시오. [CreateWindow](http://msdn.microsoft.com/library/windows/desktop/ms632679) 에 [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]합니다.  
  
 `dwExStyle`  
 [in] 확장된 창 스타일입니다. 기본값은 0, 의미 없는 확장된 스타일입니다. 가능한 값 목록은 참조 하십시오. [CreateWindowEx](http://msdn.microsoft.com/library/windows/desktop/ms632680) 에 [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]합니다.  
  
 `MenuOrID`  
 [in] 자식 창의 창 식별자입니다. 최상위 창에 대 한 메뉴는 창에 대 한 처리 합니다. 기본값은 **0U**합니다.  
  
 `lpCreateParam`  
 [in] 창 만들기 데이터에 대 한 포인터입니다. 자세한 설명은 마지막 매개 변수에 대 한 설명을 참조 [CreateWindowEx](http://msdn.microsoft.com/library/windows/desktop/ms632680)합니다.  
  
### <a name="return-value"></a>반환 값  
 성공 하면 새로 만든된 창;에 대 한 핸들 그렇지 않으면 **NULL**합니다.  
...
### <a name="remarks"></a>주의  
 ...
 기본 메시지 맵을 사용 하 여 선언 [BEGIN_MSG_MAP](http://msdn.microsoft.com/library/8bbb5af9-18b1-48c6-880e-166f599ee554), 항상&0;으로 식별 됩니다. 대체 메시지 맵을 사용 하 여 선언 [ALT_MSG_MAP(msgMapID)](http://msdn.microsoft.com/library/2c8871bf-abc0-4d52-bcf7-6b2ab9eb5af8),으로 식별 되 `msgMapID`합니다.  
...
### <a name="remarks"></a>주의  
 창을 만들 때 [만들기](#create) 이 기존 클래스를 기반으로 하지만 사용 하는 새 창 클래스 등록 [CContainedWindowT::WindowProc](#windowproc)합니다.  
...
### <a name="remarks"></a>주의  
 포함 된 기간은 슈퍼 클래스로 지정 하는 경우 기존 클래스를 수정 하는 창 클래스에 기반 것을 의미 `m_pfnSuperWindowProc` 기존 창 클래스의 창 프로시저를 가리킵니다.  
...
### <a name="remarks"></a>주의  
 해당 클래스에서 파생 되어야 하는이 컨테이너를 [CMessageMap](../../atl/reference/cmessagemap-class.md), 포함 된 창에서 사용 하는 메시지 맵을 선언 합니다.  
...
### <a name="return-value"></a>반환 값  
 성공 하면 atom 고유 하 게 식별 하는 등록 되는 창 클래스 그렇지 않으면&0;입니다.  
...
### <a name="return-value"></a>반환 값  
 **True 이면** 창이 있는 경우 성공적으로 서브 클래스로 지정 된이 고, 그렇지 않으면 **FALSE**합니다.  
...
### <a name="parameters"></a>매개 변수  
 `dwMsgMapID`  
 [in] 메시지 맵 식별자입니다. 으로 선언 된 기본 메시지 맵을 사용 하 여 [BEGIN_MSG_MAP](http://msdn.microsoft.com/library/8bbb5af9-18b1-48c6-880e-166f599ee554),&0;을 전달 합니다. 으로 선언 된 대체 메시지 맵을 사용 하 여 [ALT_MSG_MAP(msgMapID)](http://msdn.microsoft.com/library/2c8871bf-abc0-4d52-bcf7-6b2ab9eb5af8), 전달 `msgMapID`합니다.  
...
### <a name="parameters"></a>매개 변수  
 `bForce`  
 [in] 로 설정 **TRUE** 복원할 원본 창 프로시저를 강제로 경우에이 위한 창 프로시저 `CContainedWindowT` 개체를 현재 활성화 되지 않았습니다. 경우 `bForce` 로 설정 된 **FALSE** 및이 위한 창 프로시저 `CContainedWindowT` 개체 현재 활성 상태 이면 원본 창 프로시저는 복원 되지 것입니다.  
  
### <a name="return-value"></a>반환 값  
 이전에 서브클래싱된 창 핸들입니다. 경우 `bForce` 로 설정 된 **FALSE** 및이 위한 창 프로시저 `CContainedWindowT` 개체 현재 활성 상태인 반환 **NULL**합니다.  
...
### <a name="remarks"></a>주의  
 `WindowProc`로 식별 되는 메시지 맵에 대 한 메시지를 보냅니다 [m_dwMsgMapID](#m_dwmsgmapid)합니다. 필요한 경우 `WindowProc` 호출 [DefWindowProc](#defwindowproc) 추가 메시지 처리를 위해.  
