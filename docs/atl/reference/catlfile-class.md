---
title: "CAtlFile 클래스 | Microsoft 문서"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
이 클래스는 씬 래퍼입니다 Windows 파일 처리 API를 제공 합니다.  
...
|이름|설명|  
|----------|-----------------|  
|[CAtlFile::Create](#create)|이 메서드는 파일 만들기 또는 열기를 호출 합니다.|  
|[CAtlFile::Flush](#flush)|파일에 대 한 버퍼를 지우고 버퍼링 된 모든 데이터가 파일에 쓸 수 있는이 메서드를 호출 합니다.|  
...
|[CAtlFile::LockRange](#lockrange)|다른 프로세스에 액세스 하지 못하도록 파일에 영역을 잠그려고이 메서드를 호출 합니다.|  
|[CAtlFile::Read](#read)|파일 포인터에 의해 지정 된 위치부터 시작 하는 파일에서 데이터를 읽는이 메서드를 호출 합니다.|  
...
|[CAtlFile::Write](#write)|파일 포인터에 의해 지정 된 위치에서 시작 하 여 파일에 데이터를 기록 하려면이 메서드를 호출 합니다.|  
...
## <a name="remarks"></a>주의  
 파일 처리 요구는 비교적 간단 하지만 MFC 종속성을 포함 하지 않고 Windows api를 사용 하는 것 보다 더 많은 추상화가 필요 하는 경우에이 클래스를 사용 합니다.  
...
### <a name="parameters"></a>매개 변수  
 `pOverlapped`  
 Overlapped 구조체입니다. 참조 `lpOverlapped` 에서 [GetOverlappedResult](http://msdn.microsoft.com/library/windows/desktop/ms683209) 에 [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]합니다.  
  
 *dwBytesTransferred*  
 바이트를 전송 합니다. 참조 *lpNumberOfBytesTransferred* 에서 `GetOverlappedResult`합니다.  
  
 `bWait`  
 대기 옵션입니다. See `bWait` in `GetOverlappedResult`.  
...
### <a name="parameters"></a>매개 변수  
 ...
 `pOverlapped`  
 Overlapped 구조체입니다. 참조 `lpOverlapped` 에서 [ReadFile](http://msdn.microsoft.com/library/windows/desktop/aa365467) 에 [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]합니다.  
  
 `pfnCompletionRoutine`  
 완료 루틴입니다. 참조 *lpCompletionRoutine* 에서 [ReadFileEx](http://msdn.microsoft.com/library/windows/desktop/aa365468) 에 [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]합니다.  
...
### <a name="parameters"></a>매개 변수  
 ...
 `pOverlapped`  
 Overlapped 구조체입니다. 참조 `lpOverlapped` 에서 [WriteFile](http://msdn.microsoft.com/library/windows/desktop/aa365747) 에 [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]합니다.  
  
 `pfnCompletionRoutine`  
 완료 루틴입니다. 참조 *lpCompletionRoutine* 에서 [WriteFileEx](http://msdn.microsoft.com/library/windows/desktop/aa365748) 에 [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]합니다.  
