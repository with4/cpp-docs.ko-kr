...
### <a name="remarks"></a>설명  
 설정 `pdwAttributes` 를 NULL의 존재 여부를 확인 하는 방법을 제공는 `CSid` 특성에 액세스 하지 않고 있습니다. Windows 2000에서 잘못 된 결과가 발생할 수 있습니다 액세스 권한을 확인 하려면이 메서드를 사용 하지 해야 note 합니다. 응용 프로그램을 대신 사용 해야는 [CAccessToken::CheckTokenMembership](../../atl/reference/caccesstoken-class.md#checktokenmembership) 메서드.  
