...
**strtoimax** 문자열의 값을 표시를 반환 합니다. *strSource*, 표현을 오버플로가 인해 경우를 제외 하 고-반환 경우 **INTMAX_MAX** 또는 **INTMAX_MIN**, 및 **errno** 로 설정 된 **ERANGE**합니다. 변환을 수행할 수 없으면 이 함수는 0을 반환합니다. **wcstoimax** 유사 하 게에 값을 반환 **strtoimax**합니다.

**INTMAX_MAX** 및 **INTMAX_MIN** stdint.h에 정의 됩니다.
...
**strtoimax** 변환 함수 *strSource* 에 **intmax_t**합니다. 와이드 문자 버전의 **strtoimax** 은 **wcstoimax**; 해당 *strSource* 인수는 와이드 문자 문자열입니다. 그 외의 경우에는 이들 함수가 동일하게 동작합니다. 두 함수는 문자열 읽기 중지 *strSource* 숫자의 일환으로 인식할 수 없습니다 첫 번째 문자에서 합니다. Null 종결 문자 수 또는 것이 보다 크거나 같지 않은 첫 번째 숫자 문자 *기본*합니다.

로캘의 **LC_NUMERIC** 범주 설정에는 내의 기 수 문자 인식 여부 결정 *strSource*; 자세한 내용은 참조 [setlocale, _wsetlocale](setlocale-wsetlocale.md)합니다. 에 없는 함수는 **_l** 접미사 사용은 현재 로캘; **_strtoimax_l** 및 **_wcstoimax_l** 없는 대응 함수와 동일는 **_l** 로캘을 대신 사용 한다는 점을 제외 하 고 접미사가 전달합니다. 자세한 내용은 [Locale](../../c-runtime-library/locale.md)을 참조하세요.
