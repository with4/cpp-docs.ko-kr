...
`n`바이트의 문자열을 대상 문자열에 복사합니다.  이러한 기능의 더 안전한 버전을 사용할 수 있습니다. [\_mbsnbcpy\_s, \_mbsnbcpy\_s\_l](../../c-runtime-library/reference/mbsnbcpy-s-mbsnbcpy-s-l.md)을 참조하세요.  
...
## 설명  
 ...
 `strSource` 또는 `strDest`가 null 포인터인 경우 이 함수는 [매개 변수 유효성 검사](../../c-runtime-library/parameter-validation.md)의 설명대로 잘못된 매개 변수 처리기를 호출합니다.  계속해서 실행하도록 허용된 경우 함수가 `NULL`를 반환하며 `errno`를 `EINVAL`로 설정합니다.  
