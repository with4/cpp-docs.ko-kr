...
파일 포인터를 다시 할당합니다.  이러한 함수의 더 안전한 버전을 사용할 수 있습니다. [freopen\_s, \_wfreopen\_s](../../c-runtime-library/reference/freopen-s-wfreopen-s.md)를 참조하세요.  
...
## 반환 값  
 ...
 이러한 오류 코드 및 기타 오류 코드에 대한 자세한 내용은 [\_doserrno, errno, \_sys\_errlist 및 \_sys\_nerr](../../c-runtime-library/errno-doserrno-sys-errlist-and-sys-nerr.md)을 참조하세요.  
  
## 설명  
 이러한 함수의 더 안전한 버전이 있습니다. [freopen\_s, \_wfreopen\_s](../../c-runtime-library/reference/freopen-s-wfreopen-s.md)를 참조하세요.  
  
 `freopen` 함수는 현재 `stream`과 연결된 파일을 닫고 `stream`을 `path`에서 지정된 파일에 다시 할당합니다. `_wfreopen`은 `_freopen`의 와이드 문자 버전입니다. `_wfreopen`에 대한 `path` 및 `mode`는 와이드 문자 문자열입니다.  그렇지 않으면 `_wfreopen`과 `_freopen`이 동일하게 작동합니다.  
...
|`_tfreopen`|`freopen`|`freopen`|`_wfreopen`|  
 ...
 `freopen`은 보통 미리 열린 `stdin`, `stdout` 및 `stderr` 파일을 사용자가 지정한 파일로 리디렉션하는 데 사용됩니다.  `stream`과 연결된 새 파일은 다음과 같이 파일에 대해 요청된 액세스 형식을 지정하는 문자 문자열인 `mode`로 열립니다.  
