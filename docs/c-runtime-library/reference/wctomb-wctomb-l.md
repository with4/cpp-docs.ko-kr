...
경우 **wctomb** 와이드 문자 변환 멀티 바이트 문자 바이트의 수를 반환 (보다 큰는 **MB_CUR_MAX**) 와이드 문자에서입니다. 경우 *wchar* 와이드 null 문자 (L'\ \0')은 **wctomb** 1을 반환 합니다. 하는 경우 대상 포인터 *mbchar* 이 NULL 이면 **wctomb** 0을 반환 합니다. 현재 로캘 변환이 가져올 수 없으면 **wctomb** -1을 반환 하 고 **errno** 로 설정 된 **EILSEQ**합니다.
