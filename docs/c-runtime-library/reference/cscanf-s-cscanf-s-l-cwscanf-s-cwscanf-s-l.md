...
이러한 함수는 해당 함수 매개 변수의 유효성을 검사합니다. 경우 *형식* 가 null 포인터 이면 이러한 함수에 설명 된 대로 잘못 된 매개 변수 처리기를 호출 [매개 변수 유효성 검사](../../c-runtime-library/parameter-validation.md)합니다. 실행을 계속 허용 된 경우 이러한 함수가 반환 **EOF** 및 **errno** 로 설정 된 **EINVAL**합니다.

## <a name="remarks"></a>설명

**_cscanf_s** 함수에 의해 지정 된 위치로 console에서 직접 데이터를 읽습니다 *인수*합니다. [_getche](getch-getwch.md) 함수는 문자를 읽는 데 사용합니다. 각 선택적 매개 변수에의 형식 지정자에 해당 하는 형식의 변수에 대 한 포인터 여야 합니다. *형식*합니다. 형식 컨트롤 입력의 해석은 필드와 같은 형태와 기능을 *형식* 에 대 한 매개 변수는 [scanf_s](scanf-scanf-l-wscanf-wscanf-l.md) 함수입니다. 동안 **_cscanf_s** 일반적으로 입력된 문자를 에코 것 이렇게 하지 않으면에 마지막으로 호출한 경우 **_ungetch**합니다.
