...
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "_aligned_offset_recalloc_dbg 함수"
  - "aligned_offset_recalloc_dbg 함수"
...
## 설명  
 `_aligned_offset_realloc_dbg`는 [\_aligned\_offset\_recalloc](../../c-runtime-library/reference/aligned-offset-recalloc.md) 함수의 디버그 버전입니다.  [\_DEBUG](../../c-runtime-library/debug.md)를 정의하지 않은 경우 `_aligned_offset_recalloc_dbg`에 대한 각 호출이 \_`aligned_offset_recalloc`에 대한 호출로 줄어듭니다.  \_`aligned_offset_recalloc`와 `_aligned_offset_recalloc_dbg` 둘 다 기본 힙에서 메모리 블록을 다시 할당하지만 `_aligned_offset_recalloc_dbg`는 여러 디버깅 기능을 수용합니다. 이러한 기능에는 메모리 블록의 사용자 부분 한 쪽에서의 버퍼\(어느 쪽이든지 상관없이\)로 누수 테스트, 블록 형식 매개 변수로 특정 할당 형식 추적 및 `filename`\/`linenumber` 정보로 할당 요청의 원점을 확인하는 기능이 있습니다.  
