...
## <a name="return-value"></a>반환 값  
 `_kbhit`은 키가 눌렸으면 0이 아닌 값을 반환하고, 그렇지 않으면 0을 반환합니다.  
  
## <a name="remarks"></a>설명  
 `_kbhit` 함수는 콘솔에서 최근의 키 입력을 확인합니다. 이 함수가 0이 아닌 값을 반환하면 키 입력이 버퍼에서 대기 중인 것입니다. 그러면 프로그램이 `_getch` 또는 `_getche`를 호출하여 키 입력을 가져올 수 있습니다.  
