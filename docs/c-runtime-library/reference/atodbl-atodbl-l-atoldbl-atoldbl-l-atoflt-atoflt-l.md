...
## <a name="return-value"></a>반환 값  
 성공하면 0을 반환합니다. 가능한 오류 코드는 Math.h 헤더 파일에 정의된 `_UNDERFLOW` 또는 `_OVERFLOW`입니다.  
  
## <a name="remarks"></a>설명  
 이러한 함수는 문자열을 부동 소수점 값으로 변환합니다. 이러한 함수와 `atof` 함수 패밀리 간의 차이는, 이러한 함수는 부동 소수점 코드를 생성하지 않으며 하드웨어 예외를 일으키지 않는다는 점입니다. 대신 오류 조건은 오류 코드로 보고됩니다.  
  
 문자열에 부동 소수점 값으로의 유효한 해석이 없으면 `value`는 0으로 설정되고 반환 값은 0이 됩니다.  
  
 `_l` 접미사가 있는 이러한 함수 버전은 현재 스레드 로캘 대신 전달된 로캘 매개 변수를 사용한다는 점만 제외하고, 접미사가 없는 버전과 동일합니다.  
