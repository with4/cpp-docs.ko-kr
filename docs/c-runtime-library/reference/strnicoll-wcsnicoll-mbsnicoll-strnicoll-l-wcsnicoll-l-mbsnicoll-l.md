...
이러한 각 함수 반환 **_NLSCMPERROR**합니다. 사용 하도록 **_NLSCMPERROR**, 두 문자열 중 하나를 포함 합니다. H 또는 MBSTRING 합니다. 8. **_wcsnicoll** 경우 중 하나가 실패할 수 있습니다 *string1* 또는 *string2* 도메인 외부의 데이터 정렬 시퀀스의 와이드 문자 코드를 포함 합니다. 오류가 발생 하면 **_wcsnicoll** 설정할 수 있습니다 **errno** 를 **EINVAL**합니다. 에 대 한 호출에서 오류를 확인 하려면 **_wcsnicoll**설정, **errno** 0으로 확인 후 **errno** 호출한 후 **_wcsnicoll**합니다.

## <a name="remarks"></a>설명

첫 번째 대/소문자 구분 비교를 수행 하는 이러한 각 함수 *count* 에서 문자 *string1* 및 *string2* 코드 페이지에 따라 합니다. 코드 페이지에서 문자 집합 순서와 사전적 문자 순서가 다르며 이러한 차이가 문자열 비교 시 중요한 경우에만 이러한 함수를 사용해야 합니다. 없는 이러한 함수 버전은 **_l** 접미사 현재 로캘 및 코드 페이지를 사용 합니다. 버전에는 **_l** 대신 전달 된 로캘을 사용 하는 점을 제외 하 고 접미사는 동일 합니다. 자세한 내용은 [Locale](../../c-runtime-library/locale.md)을 참조하세요.

이러한 모든 함수는 해당 함수 매개 변수의 유효성을 검사합니다. 어느 경우 *string1* 또는 *string2* null 포인터가 개수 보다 큰 경우 또는 **INT_MAX**에 설명 된 대로 잘못 된 매개 변수 처리기가 호출 [ 매개 변수 유효성 검사](../../c-runtime-library/parameter-validation.md) 합니다. 실행을 계속 허용 된 경우 이러한 함수가 반환 **_NLSCMPERROR** 설정 **errno** 를 **EINVAL**합니다.
