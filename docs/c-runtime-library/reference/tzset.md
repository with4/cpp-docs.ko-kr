...
## 설명  
 ...
 `dzn`  
 3자의 일광 절약 시간 표준 시간대\(예: PDT\)입니다. 일광 절약 시간이 현지에서 적용되지 않는 경우 `dzn`에 대한 값 없이 `TZ` 설정합니다. C 런타임 라이브러리에서는 DST\(일광 절약 시간\) 계산 구현을 위한 미국의 규칙이 사용된다고 가정합니다.  
...
```  
 ...
 `TZ` 환경 변수 값에 따라 `_tzset`을 호출할 때 다음 값이 전역 변수 `_daylight`, `_timezone` 및 `_tzname`에 할당됩니다.  
