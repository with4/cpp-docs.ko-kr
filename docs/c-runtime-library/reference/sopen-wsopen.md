...
공유할 파일을 엽니다. 이러한 함수의 더 안전한 버전을 사용할 수 있는: 참조 [_sopen_s, _wsopen_s](sopen-s-wsopen-s.md)합니다.
...
경우 *filename* 또는 *oflag* 는 **NULL** 포인터 또는 *oflag* 또는 *shflag* 유효한 내에 있지 않습니다 범위 값은 잘못 된 매개 변수 처리기가 호출에 설명 된 대로 [매개 변수 유효성 검사](../../c-runtime-library/parameter-validation.md)합니다. 실행을 계속 허용 된, 하는 경우 이러한 함수가-1을 반환 하 고 설정 **errno** 다음 값 중 하나에 있습니다.

|errno 값|조건|
|-|-|
**EACCES**|지정된 경로가 디렉터리거나 파일이 읽기 전용인데 쓰기 위한 열기 작업을 시도했습니다.
**EEXIST**|**_O_CREAT** 및 **_O_EXCL** 플래그를 지정 하지만 *filename* 이미 있습니다.
**EINVAL**|잘못 된 *oflag* 또는 *shflag* 인수입니다.
...
**_sopen** 함수에서 지정한 파일을 열고 *filename* 정의한 대로 공유 읽기 또는 쓰기를 위해 파일을 준비 *oflag* 및 *shflag* . **_wsopen** 의 와이드 문자 버전이 **_sopen**; *filename* 인수를 **_wsopen** 는 와이드 문자 문자열입니다. **_wsopen** 및 **_sopen** 동일 하 게 작동 합니다.
...
정수 식 *oflag* 에 정의 된 다음 매니페스트 상수 중 하나 이상을 조합 하 여 형식이 \<fcntl.h > 합니다. 상수 두 개 이상의 인수를 구성할 때 *oflag*, 비트 OR 연산자와 결합 됩니다 ( **&#124;** ).
...
경우 **_sopen** 사용 하 여 호출 **_O_WRONLY** | **_O_APPEND** (추가 모드) 및 **_O_WTEXT**, **_O_ U16TEXT**, 또는 **_O_U8TEXT**BOM을 읽은 먼저 읽기 및 쓰기를 위해 파일을 열려고 시도 차례로 쓰기 위해서만 다시 여세요. 읽고 쓰기 위해 파일을 여는데 실패하면 쓰기 위해서만 파일을 열고 유니코드 모드 설정에 기본값을 사용합니다.

인수 *shflag* 는 상수 식에 정의 된 다음 매니페스트 상수 중 하나로 구성 \<share.h > 합니다.
...
*pmode* 인수가 필요한 경우에만 **_O_CREAT** 지정 됩니다. 파일이 없으면 *pmode* 새 파일이 처음 닫힐 때 설정 되는 파일의 권한 설정을 지정 합니다. 그렇지 않으면 *pmode* 는 무시 됩니다. *pmode* 매니페스트 상수 중 하나 또는 모두를 포함 하는 정수 식 **_S_IWRITE** 및 **_S_IREAD**에 정의 되어 있는 \<sys\stat.h > 합니다. 두 상수가 지정된 경우 비트 OR 연산자를 사용하여 결합합니다. 의미 *pmode* 는 다음과 같습니다.
...
**_sopen** 적용 하는 현재 파일 권한 마스크를 *pmode* 사용 권한을 설정 하기 전에. [_umask](umask.md)를 참조하세요.
