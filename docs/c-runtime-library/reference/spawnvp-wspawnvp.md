...
이러한 각 함수는 새 프로세스를 생성 하 고 실행 한 포인터 배열을 명령줄 인수를 사용 하 여 전달 된 **경로** 실행할 파일을 찾을 수 있는 환경 변수입니다.
