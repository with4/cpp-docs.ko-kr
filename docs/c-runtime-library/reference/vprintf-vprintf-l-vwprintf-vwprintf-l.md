...
**vprintf** 및 **vwprintf** 는 출력 오류가 발생 하는 경우 종료 null 문자 또는 음수 값을 포함 하지 않고 작성 된 문자 수를 반환 합니다. 경우 *형식* 가 null 포인터 또는에 설명 된 대로 잘못 된 매개 변수 처리기가 호출 형식 문자열에 잘못 된 서식 문자가 들어 [매개 변수 유효성 검사](../../c-runtime-library/parameter-validation.md)합니다. 함수 실행을 계속 허용 된,-1을 반환 하 고 설정 **errno** 를 **EINVAL**합니다.
...
**vwprintf** 의 와이드 문자 버전이 **vprintf**; 스트림이 ANSI 모드에서 열리는 경우 두 함수는 동일 하 게 작동 합니다. **vprintf** 현재 출력 유니코드 스트림을 지원 하지 않습니다.
