...
또한 텍스트 모드에서 캐리지 리턴-줄 바꿈 조합은 입력 시 단일 줄 바꿈으로 변환 되 고 줄 바꿈 문자는 출력에서 캐리지 리턴-줄 바꿈 조합으로 변환 합니다. 유니코드 스트림 I/O 함수가 텍스트 모드에서 작동할 경우(기본값) 소스 또는 대상 스트림은 멀티바이트 문자 시퀀스로 간주됩니다. 따라서 유니코드 스트림 입력 함수는 와이드 문자를 멀티 바이트 문자를 변환 (것으로 간주에 대 한 호출에서 **mbtowc** 함수). 와이드 문자 멀티 바이트 문자를 유니코드 스트림 출력 함수 같은 이유로 변환 (것으로 간주에 대 한 호출에서 **wctomb** 함수).
