...
**_resetstkoflw** 함수는 스택 오버플로 상태, 심각한 예외 오류와 함께 실패 하는 대신 계속 하려면는 프로그램에서에서 복구 합니다. 경우는 **_resetstkoflw** 페이지는 없으며 가드 이전 예외 다음, 함수는 호출 되지 않습니다. 다음번에 스택 오버플로가 발생하는 경우에는 예외가 전혀 발생하지 않으며 프로세스는 경고 없이 종료됩니다.
...
호출 **_resetstkoflw** 스택 오버플로 예외가 발생 한 후 복원이 수행 될 때마다 가드 페이지를 복원 합니다. 본문 내에서이 함수를 호출할 수는 **__except** 블록 또는 외부 프로그램 **__except** 블록입니다. 그러나 사용할 때 몇 가지 제한 사항이 있습니다. **_resetstkoflw** 에서 호출할 수 없습니다.
...
스택 오버플로 예외 하므로 c + + 예외를 구조적 예외로 생성은 **_resetstkoflw** 일반에서 유용 하지 않습니다 **catch** 스택 오버플로 예외를 catch 하지 않기 때문에 차단 합니다. 그러나 [_set_se_translator](set-se-translator.md)가 C++ 예외를 throw하는 구조적 예외 변환기를 구현하는 데 사용되는 경우(두 번째 예제) 스택 오버플로 예외는 C++ catch 블록에 의해 처리될 수 있는 C++ 예외를 발생시킵니다.
...
다음 예에서는 권장된 사용법은 **_resetstkoflw** 함수입니다.
...
다음 예에서는 권장된 사용을 보여 줍니다. **_resetstkoflw** 구조화 된 예외가 c + + 예외는 변환할 프로그램에서.
