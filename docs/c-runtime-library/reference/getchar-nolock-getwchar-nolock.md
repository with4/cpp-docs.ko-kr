---
title: "_getchar_nolock, _getwchar_nolock | Microsoft 문서"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
## <a name="return-value"></a>반환 값  
 [getchar, getwchar](../../c-runtime-library/reference/getchar-getwchar.md)를 참조하세요.  
  
## <a name="remarks"></a>설명  
 `_getchar_nolock` 및 `_getwchar_nolock`은 다른 스레드의 방해로부터 보호되지 않는다는 점을 제외하고 `getchar` 및 `getwchar`와 동일합니다. 이들은 다른 스레드를 잠그는 오버헤드를 유발하지 않으므로 속도가 더 빠를 수 있습니다. 단일 스레드 응용 프로그램과 같은 스레드로부터 안전한 컨텍스트 또는 이미 스레드 격리를 처리한 호출 범위에서만 이러한 함수를 사용합니다.  
