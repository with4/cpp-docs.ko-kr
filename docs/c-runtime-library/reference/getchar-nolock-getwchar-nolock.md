...
**_getchar_nolock** 및 **_getwchar_nolock** 동일 **getchar** 및 **getwchar** 제외 하 고 다른 방해 로부터 보호 되지 않습니다 스레드입니다. 이들은 다른 스레드를 잠그는 오버헤드를 유발하지 않으므로 속도가 더 빠를 수 있습니다. 단일 스레드 응용 프로그램과 같은 스레드로부터 안전한 컨텍스트 또는 이미 스레드 격리를 처리한 호출 범위에서만 이러한 함수를 사용합니다.
