...
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "_aligned_recalloc_dbg 함수"
  - "aligned_recalloc_dbg 함수"
...
[\_aligned\_malloc](../../c-runtime-library/reference/aligned-malloc.md) 또는 [\_aligned\_offset\_malloc](../../c-runtime-library/reference/aligned-offset-malloc.md)를 사용하여 할당된 메모리 블록의 크기를 변경하고 메모리를 0으로 초기화합니다\(디버그 버전에만 해당\).  
...
## 설명  
 `_aligned_recalloc_dbg`는 [\_aligned\_recalloc](../../c-runtime-library/reference/aligned-recalloc.md) 함수의 디버그 버전입니다.  [\_DEBUG](../../c-runtime-library/debug.md)를 정의하지 않은 경우 `_aligned_recalloc_dbg`에 대한 각 호출이 `_aligned_recalloc`에 대한 호출로 줄어듭니다.  `_aligned_recalloc`와 `_aligned_recalloc_dbg` 둘 다 기본 힙에서 메모리 블록을 다시 할당하지만 `_aligned_recalloc_dbg`는 여러 디버깅 기능을 수용합니다. 이러한 기능에는 메모리 블록의 사용자 부분 한 쪽에서의 버퍼\(어느 쪽이든지 상관없이\)로 누수 테스트, 블록 형식 매개 변수로 특정 할당 형식 추적 및 `filename`\/`linenumber` 정보로 할당 요청의 원점을 확인하는 기능이 있습니다.  
 ...
 메모리 할당에 실패하면 `_aligned_recalloc_dbg`는 `errno`를 `ENOMEM`으로 설정하고 필요한 메모리 양\(앞에서 언급한 오버헤드 포함\)이 `_HEAP_MAXREQ`를 초과하면 `EINVAL`이 반환됩니다.  이러한 오류 코드 및 기타 오류 코드에 대한 자세한 내용은 [errno, \_doserrno, \_sys\_errlist 및 \_sys\_nerr](../../c-runtime-library/errno-doserrno-sys-errlist-and-sys-nerr.md)을 참조하세요.  
  
 또한 `_aligned_recalloc_dbg`는 매개 변수의 유효성을 검사합니다.  `alignment`가 2의 거듭제곱이 아니면 이 함수는 [매개 변수 유효성 검사](../../c-runtime-library/parameter-validation.md)에서 설명된 대로 잘못된 매개 변수 처리기를 호출합니다.  계속해서 실행하도록 허용한 경우 이 함수는 `NULL`을 반환하고 `errno`를 `EINVAL`로 설정합니다.  
