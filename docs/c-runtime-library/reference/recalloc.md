...
## <a name="remarks"></a>설명  
 ...
 메모리 할당이 실패하거나 요청된 메모리의 양이 `_HEAP_MAXREQ`를 초과하는 경우 `_recalloc`는 `errno`를 `ENOMEM`으로 설정합니다. 이 오류 코드 및 기타 오류 코드에 대한 내용은 [errno, _doserrno, _sys_errlist 및 _sys_nerr](../../c-runtime-library/errno-doserrno-sys-errlist-and-sys-nerr.md)를 참조하세요.  
