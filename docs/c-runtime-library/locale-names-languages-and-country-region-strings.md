...
```  
 ...
 로캘 이름 형식(예: 영어(미국)의 경우 `en-US` 또는 보스니아어(키릴 자모, 보스니아 헤르체고비나)의 경우 `bs-Cyrl-BA` )을 사용하는 것이 좋습니다. 로캘 이름 집합은 [Locale Names(로캘 이름)](http://msdn.microsoft.com/library/windows/desktop/dd373814.aspx)에 설명되어 있습니다. Windows 운영 체제 버전에서 지원되는 로캘 이름 목록을 보려면 **National Language Support (NLS) API Reference(국가별 언어 지원(NLS) API 참조)** 의 [문화권 이름](http://msdn.microsoft.com/goglobal/bb896001.aspx)열을 참조하세요. 이 리소스는 로캘 이름의 지원되는 언어, 스크립트 및 영역 부분을 나열합니다. 비-기본 정렬 순서를 사용하는 지원되는 로캘 이름에 대한 정보는 **Sort Order Identifiers(정렬 순서 식별자)** 에서 [로캘 이름](http://msdn.microsoft.com/library/windows/desktop/dd374060.aspx)열을 참조하세요.  
