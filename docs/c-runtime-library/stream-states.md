...
스트림의 유효한 상태 및 상태 변화가 다음 그림에 표시됩니다.  
 ...
 ![스트림](../c-runtime-library/media/stream.png "stream")  
  
 각 원은 고정 상태를 나타냅니다.  각 줄은 스트림에서 동작하는 함수 호출의 결과로 일어날 수 있는 전환을 나타냅니다.  5개 그룹의 함수가 상태 변화를 일으킬 수 있습니다.  
  
 처음 세 개 그룹의 함수에는 \<stdio.h\>에 선언되어 있습니다.  
...
-   스트림의 마지막 작업이 읽기이며, 읽기 작업이 파일 끝 지표를 설정하지 않는다면, 쓰기 함수를 호출할 수 없습니다.  
 ...
 마지막으로, 상태 다이어그램은 따라갈 수 있는 유효한 함수 호출 숫자가 결코 줄어들지 않는 위치 작업을 보여줍니다.  
