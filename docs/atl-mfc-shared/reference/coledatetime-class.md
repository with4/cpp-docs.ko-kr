 ...
 *systimeSrc*  
 ...
 `filetimeSrc`  
 A `FILETIME` 날짜/시간 값으로 변환 하 고 새 복사 구조 `COleDateTime` 개체입니다. `FILETIME` utc (협정 세계시)를 사용 하므로 구조에 현지 시간을 전달 하는 경우 결과 잘못 됩니다. 참조 [파일 시간이](http://msdn.microsoft.com/library/windows/desktop/ms724290) 에 [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)] 자세한 정보에 대 한 합니다.  
...
- `COleDateTime(``filetimeSrc` **)** 생성 한 `COleDateTime` 에서 개체는 `FILETIME` 값입니다. 입니다. `FILETIME` utc (협정 세계시)를 사용 하므로 구조에 현지 시간을 전달 하는 경우 결과 잘못 됩니다. 참조 [파일 시간이](http://msdn.microsoft.com/library/windows/desktop/ms724290) 에 [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)] 자세한 정보에 대 한 합니다.  
...
- **연산자 = (** *varSrc* **)** 경우 변환 하는 중는 [VARIANT](http://msdn.microsoft.com/en-us/e305240e-9e11-4006-98cc-26f4932d2118) 값 (또는 [COleVariant](../../mfc/reference/colevariant-class.md) 개체)를 날짜/시간 ( `VT_DATE`)은 성공적으로 변환 된 값이 복사 됩니다 `COleDateTime` 개체 및 서버 인스턴스의 상태를 잘못 설정 됩니다. 이 개체의 값은 0 (30 1899 년 12 월 자정)로 설정 변환이 실패 하는 경우 및 해당 상태를 잘못 되었습니다.  
