---
title: "설치 하 고 ODBC 시작 | Microsoft Docs"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
ms.technology: cpp-windows
ms.tgt_pltfrm: 
ms.topic: article
dev_langs: C++
helpviewer_keywords:
- installing ODBC
- ODBC, installing
ms.assetid: 6b473481-1d68-468f-89f6-82b0fd7716fd
caps.latest.revision: "7"
author: mikeblome
ms.author: mblome
manager: ghogen
ms.workload:
- cplusplus
- data-storage
ms.openlocfilehash: d74854ce4d4e7e0fa2636fe04b01908582754f38
ms.sourcegitcommit: 8fa8fdf0fbb4f57950f1e8f4f9b81b4d39ec7d7a
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 12/21/2017
---
# <a name="installing-and-getting-started-with-odbc"></a>ODBC 설치 및 시작
다음 항목은 개방형 데이터베이스 연결 (ODBC) 작업을 시작하는 데 필요한 구성 요소를 설치하는 데 도움이됩니다.  
  
## <a name="in-this-section"></a>섹션 내용  
  
-   [Visual c + +에 제공 되는 ODBC 드라이버 설치](../../data/odbc/odbc-administrator.md)  
  
-   [고객에 게 ODBC 구성 요소 재배포](../../data/odbc/odbc-basics.md)  
  
## <a name="see-also"></a>참고 항목  
 [ODBC(Open Database Connectivity)](../../data/odbc/open-database-connectivity-odbc.md)