...
2.  **솔루션 탐색기**에서 프로젝트의 바로 가기 메뉴를 열고 **속성**을 선택합니다. 에 **속성** 대화 상자의 **구성 속성** > **C/c + +** 페이지에서 설정 **Windows 런타임 확장을사용** 를 **예 (/ZW)**합니다.  
 ...
 UWP 지원 속성을로 설정 되도록 라이브러리 프로젝트의 속성 설정을 업데이트 해야 할 수는 UWP 앱 솔루션에 c + + 정적 라이브러리 프로젝트를 추가 하는 경우 **예**합니다. 이 설정이 없으면 코드가 빌드되고 연결 되지만 오류가 Microsoft 저장소에 대 한 응용 프로그램을 확인 하려고 할 때 발생 합니다. 정적 lib는 해당 lib를 사용하는 프로젝트와 동일한 컴파일러 설정을 사용하여 컴파일해야 합니다.  
