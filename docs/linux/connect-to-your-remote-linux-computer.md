...
ms.locfileid: "33329324"
---
# <a name="connect-to-your-remote-linux-computer"></a>원격 Linux 컴퓨터에 연결

빌드할 때 Linux 코드는 원격 Linux 컴퓨터에 복사되어 Visual Studio에서 선택한 설정에 따라 해당 시스템에서 컴파일됩니다.  이 원격 연결을 설정하려면 다음을 수행합니다.

1. 처음으로 프로젝트를 빌드하거나 수동으로 새 항목을 만듭니다. 만드는 방법은 **도구 > 옵션**을 선택한 다음, **크로스 플랫폼 > 연결 관리자** 노드를 열고 **추가** 단추를 클릭하면 됩니다.

   ![연결 관리자](media/settings_connectionmanager.png)

   두 경우 모두에, **Connect to Remote System**(원격 시스템에 연결) 창이 표시됩니다.
...
1. 다음 정보를 입력합니다.
   ...
   | **암호**(Password)            | 입력한 사용자 이름의 암호
   | **개인 키 파일**    | SSH 연결을 위해 생성된 개인 키
   | **암호**(Passphrase)          | 위에서 선택한 개인 키와 함께 사용되는 암호
