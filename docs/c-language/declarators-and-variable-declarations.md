...
|[Unions](../c-language/union-declarations.md)|같은 저장소 공간을 차지하는 서로 다른 형식의 여러 값으로 구성된 변수|  
...
> [!NOTE]
>  `declarator`를 참조하는 구문은 [C 언어 구문 요약](../c-language/c-language-syntax-summary.md) 또는 [선언 개요](../c-language/overview-of-declarations.md)에서 `declaration`에 대한 구문을 참조하십시오.  
  
 선언자가 수정되지 않은 식별자로 구성되면 선언되는 항목에 기본 형식이 있습니다.  식별자 왼쪽에 별표\(**\***\)가 나타나는 경우 형식은 포인터 형식으로 수정됩니다.  식별자 뒤에 대괄호\(**\[ \]**\)가 올 경우 형식은 배열 형식으로 수정됩니다.  식별자 뒤에 괄호가 올 경우 형식은 함수 형식으로 수정됩니다.  선언 내에서 우선 순위를 해석하는 방법에 대한 자세한 내용은 [더 복잡한 선언자 해석](../c-language/interpreting-more-complex-declarators.md)을 참조하십시오.  
