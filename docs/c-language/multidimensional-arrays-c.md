...
3.  6개 요소 배열의 각 요소는 `int` 값이므로 마지막 첨자인 `3`이 `prop[2][1]`에 추가되기 전에 `int`의 크기로 곱해집니다.  결과 포인터는 6개 요소 배열의 네 번째 요소의 주소를 지정합니다.  
...
```  
 ...
 마찬가지로 두 번째 문 `ipp = prop[2];`의 `prop[2]` 식의 결과는 2차원 배열의 주소를 지정하는 포인터 값입니다.  
