...
## 설명  
 `__svm_stgi` 함수는 해당 하는 `STGI` 컴퓨터 명령.  글로벌 인터럽트 플래그 마이크로프로세서 무시, 연기, 또는 I\/O 완료, 하드웨어 온도 경고, 디버그 예외와 같은 이벤트 때문에 인터럽트를 처리 하는지 여부를 결정 합니다.  
...
|내장|아키텍처|  
|--------|----------|  
|`__svm_stgi`|x 86[!INCLUDE[vcprx64](../assembler/inline/includes/vcprx64_md.md)]|  
