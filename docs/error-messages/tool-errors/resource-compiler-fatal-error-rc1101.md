---
title: 리소스 컴파일러 심각한 오류 RC1101 | Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-diagnostics
ms.topic: error-reference
f1_keywords:
- RC1101
dev_langs:
- C++
helpviewer_keywords:
- RC1101
ms.assetid: ff273384-b819-464a-8c0e-d5951e4a28b4
author: corob-msft
ms.author: corob
ms.workload:
- cplusplus
ms.openlocfilehash: ef8024ebb2058e9bce2a3b911ead23ca1c131a13
ms.sourcegitcommit: 76b7653ae443a2b8eb1186b789f8503609d6453e
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 05/04/2018
ms.locfileid: "33320829"
---
# <a name="resource-compiler-fatal-error-rc1101"></a>리소스 컴파일러 심각한 오류 RC1101
지정 된 없는 리소스 이진 파일 이름  
  
 출력 이름 바꾸기 (/ fo) 옵션 뒤에 파일 이름이 없습니다.  
  
 /Fo 옵션에 대해 다음 구문을 사용 합니다.  
  
 **RC /fo**   
 ***outfilename infilename 합니다.*  rc**