---
title: "컴파일러 경고 (수준 1) C4305 | Microsoft Docs"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
ms.technology: cpp-tools
ms.tgt_pltfrm: 
ms.topic: error-reference
f1_keywords: C4305
dev_langs: C++
helpviewer_keywords: C4305
ms.assetid: 56eecc21-2092-4e36-8017-0a8ca0ea0466
caps.latest.revision: "6"
author: corob-msft
ms.author: corob
manager: ghogen
ms.workload: cplusplus
ms.openlocfilehash: df1626d188d1b8e00674d6af6ce604998d317b78
ms.sourcegitcommit: 8fa8fdf0fbb4f57950f1e8f4f9b81b4d39ec7d7a
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 12/21/2017
---
# <a name="compiler-warning-level-1-c4305"></a>컴파일러 경고(수준 1) C4305
'identifier': 'type1'에서 'type2' (으) 잘림  
  
 식별자 정보 손실이 발생 하는 더 작은 형식으로 변환 됩니다.  
  
 ATL 연결 지점 마법사를 사용 하는 경우 기술 자료 문서를 참조 하세요.  
  
-   Q250847: ATL 연결 지점 마법사에서 생성 한 코드 C4305 경고를 제공 하는 인수를 사용 하 여 이벤트에 대 한