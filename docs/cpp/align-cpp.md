...
없이 `__declspec(align(#))`, 컴파일러는 대상 프로세서와 32 비트 프로세서에서 4 바이트 경계 최대 데이터 크기에 따라 자연 경계 및 64 비트 프로세서에서 8 바이트 경계에 데이터를 일반적으로 맞춥니다. 클래스 또는 구조체의 자연 맞춤 및 현재 압축 설정을 최소한의 클래스 또는 구조체의 데이터 정렬 (에서 #pragma **pack** 또는 **/Zp** 컴파일러 옵션).
...
메모리가 힙에 할당된 경우 호출되는 할당 함수에 따라 맞춤이 결정됩니다.  예를 들어, 사용 하는 경우 **malloc**, 결과 피연산자 크기에 따라 달라 집니다. 하는 경우 *arg* > = 8 이면 반환 되는 메모리는 8 바이트에 맞춰집니다. 하는 경우 *arg* < 8, 반환 되는 메모리의 맞춤은 2의 첫 번째 거듭제곱이 미만 *arg*합니다. 예를 들어 malloc(7)를 사용하는 경우의 맞춤은 4바이트입니다.
...
합니다 **/Zp** 컴파일러 옵션 및 **팩** pragma는 구조체 및 공용 구조체 멤버에 대 한 데이터를 압축 하는 중입니다. 이 예제에서는 어떻게 **/Zp** 및 `__declspec(align(#))` 함께 사용:
...
다음 표에서 다양 한 각 멤버의 오프셋을 나열 **/Zp** (또는 #pragma **팩**) 두 개의 상호 작용 하는 방법을 보여 줍니다.
