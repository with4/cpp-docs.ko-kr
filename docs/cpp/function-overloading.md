...
## <a name="argument-type-differences"></a>인수 형식 차이  
 ...
 동일한 이유로 함수에 의해 수정 되는 형식의 인수 **const** 또는 `volatile` 오버 로드에 대 한 기본 형식과 다르게 취급 되지 않습니다.  
  
 그러나 메커니즘 오버 로드 함수를 구별할 수로 정규화 된 참조 **const** 및 `volatile` 및 기본 형식에 대 한 참조입니다. 이를 사용하면 다음과 같은 코드를 작성할 수 있습니다.  
...
```  
 ...
 에 대 한 포인터 **const** 및 `volatile` 개체는 오버 로드에 대 한도 기본 형식 포인터와에서 다르게 간주 됩니다.  
...
-   중간 변환을 제거하여 단축할 수 있는 변환 시퀀스는 고려하지 않습니다.  
 ...
 변환 결과 시퀀스가 있을 경우 이를 가장 일치하는 시퀀스라고 합니다. 형식의 개체를 변환 하는 방법은 여러 가지가 `int` 입력 `unsigned long` 표준 변환을 통한 (에 설명 된 [표준 변환](../cpp/standard-conversions.md)):  
...
기본 변환을 보여 주는 그래프  
 ...
 ![다중 &#45; 기본 설정된 변환을 보여 주는 상속](../cpp/media/vc391t2.gif "vc391T2")  
...
```  
 ...
 클래스에 대 한 사용 가능한 사용자 정의 변환 `UDC` 형식에서는 `int` 유형과 **긴**합니다. 따라서 컴파일러가 일치하는 개체 형식을 위한 변환을 고려합니다(`UDC`). `int`로의 변환이 존재하며 선택되었습니다.  
...
```  
 ...
 위의 예제에서는 사용자 정의 변환 **연산자 long**, 변환 하기 위해 호출 된 `udc` 입력 **긴**합니다. 형식으로 사용자 정의 변환이 없는 경우 **긴** 되었을 정의 변환 것가 앞에 다음과 같이: 형식 `UDC` 형식으로 변환 된 `int` 사용자 정의 변환을 사용 하 여 합니다. 다음 형식에서의 표준 전환은 `int` 입력 **긴** 선언에 있는 인수와 일치 하도록 적용 되었을 것입니다.  
...
-   형식에서 변환할 `int` 입력 **긴**; 다음 형식으로 변환 `UDC2` (2 단계 변환).  
...
## <a name="argument-matching-and-the-this-pointer"></a>인수 일치 및 this 포인터  
 ...
 이 비정적 멤버 함수들은 함수에 포함된 `this` 포인터가 함수를 요청하고 있는 개체 형식에 일치되도록 하거나 오버로드된 연산자를 위해 첫 번째 인수가 연산자가 적용되는 개체에 일치되도록 합니다. (오버 로드 된 연산자에 대 한 자세한 내용은 참조 [오버 로드 된 연산자](../cpp/operator-overloading.md).)  
  
 오버로드된 함수의 다른 인수와 달리, 임시 개체를 생성하지 않으며, `this` 포인터 인수와 일치시키기 위해 변환을 시도하지 않습니다.  
  
 때는 `->` 멤버 선택 연산자는 클래스의 멤버 함수를 액세스 하는 데 `class_name`, `this` 포인터 인수 형식이 `class_name * const`합니다. 구성원으로 선언 된 경우 `const` 또는 `volatile`, 유형은 `const class_name * const` 및 `volatile class_name * const`각각.  
...
-   반환 형식을 기준으로 동일한 형식의 인수 목록을 가진 함수를 오버 로드하면 오류가 발생합니다.  
 ...
 오버 로드할 수 있으며 **new 연산자** 만에 따라의 반환 형식-구체적으로, 지정 된 메모리 모델 한정자를 기반으로 합니다.  
