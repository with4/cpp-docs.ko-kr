...
-   [명시적 생성자](#explicit_constructors)  
  
-   [기본 생성자](#default_constructors)  
  
-   [복사 및 이동 생성자](#copy_and_move_constructors)  
  
-   [명시적으로 기본 설정 및 삭제 된 생성자](#explicitly_defaulted_and_deleted_constructors)  
  
-   [파생된 클래스의 생성자](#constructors_in_derived_classes)  
  
-   [생성자의 가상 함수](#virtual_functions_in_constructors)  
  
-   [생성자 및 복합 클래스](#constructors_in_composite_classes)  
  
-   [위임 생성자](#delegating_constructors)  
  
-   [상속 생성자 (C + + 11)](#inheriting_constructors)  
  
-   [생성자를 선언하기 위한 규칙](#rules_for_declaring_constructors)  
  
-   [생성자를 명시적으로 호출](#explicitly_invoking_constructors)  
...
##  <a name="member_lists"></a>멤버 목록  
 멤버 이니셜라이저 목록을 사용하여 생성자 인수에서 클래스 멤버를 초기화합니다. 이 방법은 사용 하 여 *직접 초기화*를 생성자 본문 내에서 할당 연산자를 사용 하 여 보다 더 효율적인 합니다.  
 ...
 *기본 생성자* 가 매개 변수가 없으며, 약간 다른 규칙을 따릅니다.  
  
 기본 생성자가 중 하나는 *특수 멤버 함수*클래스에 선언 된 생성자는 없을 경우 컴파일러는 기본 생성자를 제공 합니다.:  
...
##  <a name="copy_and_move_constructors"></a>복사 및 이동 생성자  
 A *복사 생성자* 는 동일한 개체에 대 한 참조를 입력 하 고 복사본을 사용 하면 입력으로 사용 하는 특수 멤버 함수입니다. 자세한 내용은 참조 [복사 생성자 및 복사 할당 연산자 (c + +)](../cpp/copy-constructors-and-copy-assignment-operators-cpp.md)합니다. A *이동 생성자* 원래 데이터를 복사 하지 않고 기존 개체의 데이터 소유권을 새 변수에 이동 하는 특수 멤버 함수 이기도 합니다. 자세한 내용은 참조 [이동 생성자 및 이동 할당 연산자 (c + +)](../cpp/move-constructors-and-move-assignment-operators-cpp.md)합니다.  
  
##  <a name="explicitly_defaulted_and_deleted_constructors"></a>명시적으로 기본 설정 및 삭제 된 생성자  
 명시적으로 수 *기본* 복사 생성자, 기본 생성자, 이동 생성자, 복사 할당 연산자, 대입 연산자 및 소멸자를 이동 합니다. 명시적으로 수 *삭제* 모든 특수 멤버 함수입니다. 자세한 내용은 참조 [명시적으로 기본 설정 및 삭제 된 함수](../cpp/explicitly-defaulted-and-deleted-functions.md)합니다.  
  
##  <a name="constructors_in_derived_classes"></a>파생된 클래스의 생성자  
...
### <a name="constructors-for-classes-that-have-multiple-inheritance"></a>다중 상속을 포함하는 클래스에 대한 생성자  
...
##  <a name="delegating_constructors"></a>위임 생성자  
 A *위임 생성자* 초기화 작업의 일부를 수행 하는 같은 클래스의 다른 생성자를 호출 합니다. 다음 예제에서는 파생 클래스에 세 개의 생성자가 있는데 두 번째 생성자는 첫 번째 생성자에 위임되고, 세 번째 생성자는 두 번째 생성자에 위임됩니다.  
...
```  
 ...
 자세한 내용은 참조 [균일 초기화 및 생성자 위임](../cpp/uniform-initialization-and-delegating-constructors.md)합니다.  
...
##  <a name="rules_for_declaring_constructors"></a>생성자 선언에 대 한 규칙  
 생성자의 이름은 해당 클래스의 이름과 같습니다. 오버로드된 함수의 규칙에 따라 임의의 수의 생성자를 선언할 수 있습니다.  
  
 `argument-declaration-list`가 비어 있을 수 있습니다.  
  
 C++에서는 다음 표에 설명되어 있는 두 가지 특별한 종류의 생성자인 기본 및 복사 생성자를 정의합니다.  
  
### <a name="default-and-copy-constructors"></a>기본 및 복사 생성자  
...
|복사 생성자|동일한 클래스 형식에 대한 참조의 단일 인수를 수락할 수 있습니다.|클래스 형식의 개체를 복사합니다.|  
 ...
 Stdcall 호출 규칙은 정적 멤버 함수에 사용 되 고 전역 함수를 사용 하 여 선언 된 **__stdcall** 키워드를 가변 인수 목록은 사용 하지 않습니다. 사용 하는 경우는 **__stdcall** 키워드는 생성자와 같은 비정적 멤버 함수에 컴파일러 thiscall 호출 규칙을 사용 합니다. "  
  
 기본 클래스의 생성자는 파생 클래스에서 상속되지 않습니다. 파생 클래스 형식의 개체를 만들 때 해당 개체는 기본 클래스 구성 요소로 시작하여 생성된 후 파생 클래스 구성 요소로 이동합니다. 컴파일러는 완전 한 개체의 해당 부분에서 초기화 될 때 각 기본 클래스의 생성자를 사용 합니다 (가상 파생의 경우 제외 합니다.  
...
```  
 ...
 앞의 두 예제에서와 같이 생성자를 명시적으로 호출하여 생성된 개체는 명명되지 않고 자신을 생성한 식의 수명을 갖습니다. 에 더 자세히 설명 되어 있음 [임시 개체](../cpp/temporary-objects.md)합니다.  
