...
`make_exception_ptr` 함수에서 반환 된 `exception_ptr` 로 지정 된 예외를 참조 하는 개체는 *e* 매개 변수입니다.
...
- **/CLR** 컴파일러 옵션 및 `catch` 문은 c + + 예외를 전송할 수 있습니다. **/CLR** 컴파일러 옵션의 사양을 의미는 **/EHa** 옵션입니다. 컴파일러는 관리되는 예외의 전송을 지원하지 않습니다. 즉, 관리 되는 예외에서 파생 되는 [System.Exception 클래스](../standard-library/exception-class.md), 이미 하는 개체의 공용 언어 런타임의 기능을 사용 하 여 스레드 간에 이동할 수 있습니다.
...
그 이름에도 불구하고 `exception_ptr` 개체 자체는 포인터가 아닙니다. 포인터 의미 체계를 준수 하지 않으며 포인터 멤버 액세스 함께 사용할 수 없습니다 것 (`->`) 또는 간접 참조 (`*`) 연산자. `exception_ptr` 개체에는 공용 데이터 멤버 또는 멤버 함수가 없습니다.
