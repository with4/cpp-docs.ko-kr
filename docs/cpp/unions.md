...
#### 매개 변수  
 ...
 `member-list`  
 공용 구조체에 포함할 수 있는 멤버입니다.  설명 부분을 참조하십시오.  
...
## 공용 구조체 사용  
 이전 예제에서 공용 구조체에 액세스하는 코드는 데이터를 보유하고 있는 멤버를 알고 있어야 합니다.  이 문제의 가장 일반적인 솔루션은 현재 공용 구조체에 저장되는 데이터의 형식을 나타내는 추가 열거형 멤버와 함께 구조체의 용 구조체를 묶는 것입니다.  이를 *구분된 공용 구조체*라고 하며 다음 예제에서는 기본 패턴을 보여 줍니다.  
...
```  
 ...
 익명 공용 구조체에는 [공용 구조체 멤버 데이터](../misc/union-member-data.md)에 나와 있는 제한 이외에 다음 제한이 추가로 적용됩니다.  
  
-   파일 또는 네임스페이스 범위에 선언된 경우 **정적**으로도 선언되어야 합니다.  
