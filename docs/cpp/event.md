...
|—|—|데이터 멤버|  
 ...
 사용 하 여 [__hook](../cpp/hook.md) 처리기 메서드를 이벤트 메서드와 연결할 이벤트 수신기에서 합니다. `__event` 키워드를 사용하여 이벤트를 만든 후에는 이벤트가 호출될 때 이후에 해당 이벤트에 후크된 모든 이벤트 처리기가 호출됩니다.  
...
## <a name="com-events"></a>COM 이벤트  
 COM 이벤트는 인터페이스입니다. 이벤트 소스 인터페이스에 있는 메서드의 매개 변수 여야 **에** 매개 변수 (그러나 엄격 하 게 적용 되지) 때문에 프로그램 **아웃** 매개 변수가 멀티 캐스팅 시 유용 하지 않습니다. 사용 하는 경우 수준 1 경고가 발행 됩니다는 **아웃** 매개 변수입니다.  
  
 반환 형식은 일반적으로 `HRESULT` 또는 `void`이지만 `enum`을 비롯한 모든 정수 계열 형식일 수 있습니다. 이벤트가 정수 계열 반환 형식을 사용하고 이벤트 처리기가 0이 아닌 값을 반환하는 경우는 오류 조건이며 이 경우 발생하는 이벤트가 다른 대리자에 대한 호출을 중단합니다. 컴파일러는 자동으로 이벤트 소스 인터페이스를 표시 참고는 [소스](../windows/source-cpp.md) 생성된 된 IDL에서 합니다.  
  
 [__interface](../cpp/interface.md) 키워드는 항상 후 있어야 `__event` COM 이벤트 원본에 대 한 합니다.  
...
## <a name="managed-events"></a>관리되는 이벤트  
 에 대 한 내용은 새 구문에서 이벤트를 코딩 하십시오 [이벤트](../windows/event-cpp-component-extensions.md)합니다.  
  
 관리되는 이벤트는 데이터 멤버 또는 메서드입니다. 을 사용 하는 이벤트는 대리자의 반환 형식을 준수 해야는 [공용 언어 사양](/dotnet/standard/language-independence-and-language-independent-components)합니다. 이벤트 처리기의 반환 형식은 대리자의 반환 형식과 일치해야 합니다. 대리자에 대 한 자세한 내용은 참조 하십시오. [대리자 및 이벤트](../dotnet/delegates-and-events.md)합니다. 관리되는 이벤트가 데이터 멤버인 경우 그 형식은 대리자에 대한 포인터여야 합니다.  
