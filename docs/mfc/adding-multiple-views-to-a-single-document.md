...
-   이름에서 `CWinApp`-파생 된 개체는 `CMyWinApp`, 및 `CMyWinApp` 선언 되 고에 정의 된 *MYWINAPP 합니다. H* 및 *MYWINAPP 합니다. CPP*합니다.  
  
-   `CNewView` 새 이름인 `CView`-파생 개체를 및 `CNewView` 선언 되 고에 정의 된 *NEWVIEW 합니다. H* 및 *NEWVIEW 합니다. CPP*합니다.  
...
##  <a name="vcconmodifyexistingapplicationa1"></a> 기존 응용 프로그램 클래스를 수정 합니다.  
 ...
 선언에 다음 코드를 추가 `CMyWinApp` 에 *MYWINAPP 합니다. H*:  
...
##  <a name="vcconnewviewclassa2"></a> 만들고 새 뷰 클래스를 수정 합니다.  
 ...
 수정 *NEWVIEW 합니다. H* 에서 액세스 지정자를 변경 하 여 **보호** 를 **공용** 생성자 및 소멸자에 대 한 합니다. 이 클래스를를 만들고 동적으로 소멸 및 표시 되기 전에 뷰 모양을 수정할 수 있습니다.  
...
##  <a name="vcconswitchingfunctiona4"></a> 전환 함수 구현  
 ...
 응용 프로그램 클래스에 대 한 구현 파일의 끝 (*MYWINAPP 합니다. CPP*), 다음과 같은 메서드 정의 추가 합니다.  
