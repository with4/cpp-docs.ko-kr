...
##  <a name="_core_using_the_declare_serial_macro"></a> DECLARE_SERIAL 매크로 사용 하 여  
 DECLARE_SERIAL 매크로 다음과 같이 serialization을 지 원하는 클래스의 선언에 필요 합니다.  
...
> [!NOTE]
>  DECLARE_SERIAL 및 IMPLEMENT_SERIAL 매크로 사용 하는 클래스에 인수가 없는 생성자를 정의 하지 않으면 IMPLEMENT_SERIAL 매크로 사용 되는 줄에는 "사용할 수 있는 기본 생성자 없음" 컴파일러 경고를 받아볼 수 있습니다.  
...
##  <a name="_core_using_the_implement_serial_macro_in_the_implementation_file"></a> IMPLEMENT_SERIAL 매크로 사용 하 여 구현 파일  
 ...
 원하는 경우 프로그램 `Serialize` 멤버 함수를 여러 버전을 읽을 수-응용 프로그램의 서로 다른 버전으로 작성 된 파일, 즉-값을 사용할 수 있습니다 *VERSIONABLE_SCHEMA* 는 IMPLEMENT_SERIAL 인수로 매크로입니다. 사용 정보 및 예제에 대 한 참조는 `GetObjectSchema` 클래스의 멤버 함수 `CArchive`합니다.  
  
 다음 예제에서는 IMPLEMENT_SERIAL 클래스에서 사용 하는 방법을 보여 줍니다. `CPerson`, 즉에서 파생 된 `CObject`:  
