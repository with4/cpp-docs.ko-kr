...
-   현재 위치 활성화가 가능한 하나 이상의 보기를 지원해야 합니다. 즉, 뷰 인터페이스를 지원 해야 `IOleDocumentView` 인터페이스 뿐만 아니라 `IOleInPlaceObject` 및 `IOleInPlaceActiveObject` (컨테이너의를 사용 하 여 `IOleInPlaceSite` 및 `IOleInPlaceFrame` 인터페이스).  
