...
4. 디자인 뷰에서 해당 속성을 표시하려면 응용 프로그램 단추를 클릭합니다. 속성 값을 다음과 같이 변경: **이미지** 를 `IDB_RIBBON_MAIN`, **프롬프트** 를 `File`, **키** 를 `f`, **큰이미지** 를 `IDB_RIBBON_FILELARGE`, 및 **작은 이미지** 를 `IDB_RIBBON_FILESMALL`합니다.

5. 다음과 같이 수정하면 사용자가 응용 프로그램 단추를 클릭할 때 나타나는 메뉴가 만들어집니다. 줄임표를 클릭 (**...** ) 옆에 **주 항목** 열려는 **항목 편집기**합니다.

   1. 클릭 **추가** 단추를 추가 합니다. 변경 **캡션** 를 `&New`, **ID** 를 `ID_FILE_NEW`, **이미지** 를 `0`, **큰 이미지** 에`0`.

   2. 클릭 **추가** 두 번째 단추를 추가 합니다. 변경 **캡션** 를 `&Save`, **ID** 를 `ID_FILE_SAVE`, **이미지** 를 `2`, 및 **큰 이미지** 에`2`.

   3. 클릭 **추가** 세 번째 단추를 추가 합니다. 변경 **캡션** 를 `Save &As`, **ID** 를 `ID_FILE_SAVE_AS`, **이미지** 를 `3`, 및 **큰 이미지** 에`3`.

   4. 클릭 **추가** 네 번째 단추를 추가 합니다. 변경 **캡션** 를 `&Print`, **ID** 를 `ID_FILE_PRINT`, **이미지** 를 `4`, 및 **큰 이미지** 에`4`.

   5. 변경의 **항목** 형식을 **구분 기호** 클릭 하 고 **추가**합니다.

   6. 변경 된 **항목** 형식을 **단추**합니다. 클릭 **추가** 다섯 번째 단추를 추가 합니다. 변경 **캡션** 를 `&Close`, **ID** 를 `ID_FILE_CLOSE`, **이미지** 를 `5`, 및 **큰 이미지** 에`5`.

6. 다음과 같이 수정하면 이전 단계에서 만든 인쇄 단추 아래에 하위 메뉴가 만들어집니다.

   1. 클릭는 **인쇄** 단추, 변경는 **항목** 형식을 **레이블**, 클릭 하 고 **삽입**합니다. 변경 **캡션** 를 `Preview and print the document`합니다.

   2. 클릭는 **인쇄** 단추, 변경의 **항목** 형식을 **단추**, 클릭 하 고 **삽입**합니다. 변경 **캡션** 를 `&Print`, **ID** 를 `ID_FILE_PRINT`, **이미지** 를 `4`, 및 **큰 이미지** 에`4`.

   3. 클릭는 **인쇄** 단추를 선택한 다음 클릭 **삽입** 단추를 추가 합니다. 변경 **캡션** 를 `&Quick Print`, **ID** 를 `ID_FILE_PRINT_DIRECT`, **이미지** 를 `7`, 및 **큰 이미지** 에`7`.

   4. 클릭는 **인쇄** 단추를 선택한 다음 클릭 **삽입** 다른 단추를 추가 합니다. 변경 **캡션** 를 `Print Pre&view`, **ID** 를 `ID_FILE_PRINT_PREVIEW`, **이미지** 를 `6`, 및 **큰 이미지** 에`6`.
...
7. 다음과 같이 수정하면 응용 프로그램 단추 메뉴의 아래쪽에 나타나는 종료 단추가 만들어집니다.
   ...
   2. 클릭 **추가** 단추를 추가 합니다. 변경 **캡션** 를 `E&xit`, **ID** 를 `ID_APP_EXIT`, **이미지** 를 `8`합니다.
...
1. Scribble 프로그램에서는 하나의 범주만 필요로 합니다. 디자인 뷰에서 클릭 **범주** 해당 속성을 표시 합니다. 속성 값을 다음과 같이 변경: **캡션** 를 `&Home`, **큰 이미지** 를 `IDB_RIBBON_HOMELARGE`, **작은 이미지** 를 `IDB_RIBBON_HOMESMALL`합니다.

2. 각 리본 범주는 명명된 패널로 구성되어 있습니다. 각 패널에는 관련 작업을 수행하는 컨트롤 집합이 포함되어 있습니다. 이 범주에는 한 패널이 있습니다. 클릭 **패널**를 바꾼 다음 **캡션** 를 `Edit` 및 **이미지 인덱스:** 를 `0`합니다.

3. 에 **편집** 패널에서 문서의 콘텐츠 지우기를 담당 하는 단추를 추가 합니다. 이 단추에 대한 메시지 ID는 IDR_SCRIBBTYPE 메뉴 리소스에서 이미 정의되었습니다. 지정 `Clear All` 단추 텍스트 및 단추를 데코레이팅하는 비트맵 인덱스입니다. 열기는 **도구 상자**, 한 다음 끌어는 **단추** 에 **편집** 패널입니다. 단추를 클릭 한 다음 변경 **캡션** 를 `Clear All`, **ID** 를 `ID_EDIT_CLEAR_ALL`, **이미지 인덱스:** 를 `0`, **Large Image Index**  를 `0`합니다.
