...
helpviewer_keywords: 
  ...
  - "OnIdle 메서드"
  - "PeekMessage 메서드"
  - "PeekMessage 메서드, 메시지 루프 이외"
  - "처리"
  - "처리, 배경"
  - "처리, 유휴 루프 중"
  - "스레딩[MFC], 다중 스레딩 대체 방법"
...
> [!NOTE]
>  **Run**, `OnIdle`, and certain other member functions are now members of class `CWinThread` rather than of class `CWinApp`.  `CWinApp`는 `CWinThread`에서 파생됩니다.  
