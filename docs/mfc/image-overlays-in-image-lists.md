...
모든 이미지 목록 ([CImageList](../mfc/reference/cimagelist-class.md)) 오버레이 마스크도 사용할 이미지의 목록을 포함 합니다. "오버레이 마스크"에 다른 이미지 위에 투명 하 게 그려지는 하는 이미지입니다. 모든 이미지 오버레이 마스크도 사용할 수 있습니다. 이미지 목록 당 최대 4 개의 오버레이 마스크를 지정할 수 있습니다.  
 ...
 한 번 호출을 사용 하 여 이미지 오버레이 마스크 끌면 `Draw`합니다. 매개 변수를 그릴 이미지의 인덱스 및 오버레이 마스크의 인덱스에 포함 됩니다. 사용 해야 합니다는 [INDEXTOOVERLAYMASK](http://msdn.microsoft.com/library/windows/desktop/bb761408) 매크로 오버레이 마스크의 인덱스를 지정 합니다. 호출할 때에 오버레이 이미지를 지정할 수는 [DrawIndirect](../mfc/reference/cimagelist-class.md#drawindirect) 멤버 함수입니다.  
