---
title: "CMDIFrameWnd 클래스 | Microsoft 문서"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
|[CMDIFrameWnd::MDIGetActive](#mdigetactive)|자식 최대화 여부를 나타내는 플래그와 함께 현재 활성 상태인 MDI 자식 창을 검색 합니다.|  
|[CMDIFrameWnd::MDIIconArrange](#mdiiconarrange)|모든 아이콘 표시 된 자식 창을 정렬합니다.|  
...
|[CMDIFrameWnd::MDIRestore](#mdirestore)|MDI 자식 창 크기를 최대화 또는 최소화에서 복원합니다.|  
...
## <a name="remarks"></a>주의  
 응용 프로그램에 대 한 유용한 하는 MDI 프레임 창을 만들기 위해에서 클래스를 파생 `CMDIFrameWnd`합니다. 응용 프로그램에 데이터를 저장 하 고 파생된 클래스에 멤버 변수를 추가 합니다. 파생 클래스에서 메시지 처리기 멤버 함수 및 메시지 맵을 구현하여 메시지가 창에 전달될 때 수행되는 작업을 지정합니다.  
  
 호출 하 여 MDI 프레임 창은 생성할 수는 [만들기](../../mfc/reference/cframewnd-class.md#create) 또는 [LoadFrame](../../mfc/reference/cframewnd-class.md#loadframe) 의 멤버 함수 `CFrameWnd`합니다.  
 ...
 `LoadFrame`보다 적은 인수가 필요 **만들기**, 대신 프레임의 캡션, 아이콘, 액셀러레이터 키 테이블 및 메뉴를 포함 하 여 리소스에서 대부분의 기본 값을 검색 합니다. 에 액세스할 수 `LoadFrame`, 이러한 모든 리소스는 동일한 리소스 ID를 갖고 있어야 (예를 들어 **IDR_MAINFRAME**).  
  
 하지만 **MDIFrameWnd** 에서 파생 된 `CFrameWnd`, 프레임 창 클래스에서 파생 된 `CMDIFrameWnd` 으로 선언 될 필요가 `DECLARE_DYNCREATE`합니다.  
  
 `CMDIFrameWnd` 대부분의 기본 구현에서의 상속 `CFrameWnd`합니다. 이러한 기능의 자세한 목록은 참조는 [CFrameWnd](../../mfc/reference/cframewnd-class.md) 클래스 설명 합니다. `CMDIFrameWnd` 클래스에 다음과 같은 추가 기능:  
  
-   MDI 프레임 창에 관리 하는 **MDICLIENT** 창, 컨트롤 막대와 함께 합니다. MDI 클라이언트 MDI 자식 프레임 창의 직계 부모입니다. **WS_HSCROLL** 및 **WS_VSCROLL** 에 지정 된 창 스타일은 `CMDIFrameWnd` MDI 클라이언트 창에 적용 하는 대신 사용자 (에서 같이 Windows 프로그램 관리자, 예를 들어) 서 MDI 클라이언트 영역을 스크롤할 수 있도록 주 프레임 창입니다.  
  
-   MDI 프레임 창은 활성 MDI 자식 창이 없는 경우 메뉴 모음으로 사용 되는 기본 메뉴를 소유 합니다. 활성 MDI 자식 이면 MDI 프레임 창의 메뉴 표시줄은 MDI 자식 창 메뉴로 자동 교체 됩니다.  
  
-   MDI 프레임 창에 있는 경우 현재 MDI 자식 창와 함께에서 작동 합니다. 예를 들어, 명령 메시지 전에 MDI 프레임 창은 현재 활성 상태인 MDI 자식에 위임 됩니다.  
...
-   MDI 프레임 창 구현 역시 **ID_WINDOW_NEW**는 새로운 프레임 및 현재 문서에서 보기를 만듭니다. 응용 프로그램은 MDI 창 처리를 사용자 지정 하려면 이러한 기본 명령 구현을 재정의할 수 있습니다.  
...
### <a name="remarks"></a>주의  
 재정의 하는 경우이 멤버 함수를 호출 해야는 `OnCreate` 멤버 함수를 직접.  
...
### <a name="remarks"></a>주의  
 이 함수를 사용 하 여 windows MDI 프레임 창의 자식을 만들 수 있습니다.  
  
### <a name="example"></a>예제  
 [!code-cpp[NVC_MFCWindowing #&15;](../../mfc/reference/codesnippet/cpp/cmdiframewnd-class_3.cpp)]  
  
 이 예제는 Q201045, 기술 자료 문서에서 발췌 한 것 "방법: 비-문서/뷰 MDI 응용 프로그램에 여러 개의 창 유형을 추가 합니다." 기술 자료 문서 또는 MSDN 라이브러리의 Visual Studio 설명서에서 사용할 수 있는 [http://support.microsoft.com](http://support.microsoft.com/)합니다.  
...
##  <a name="a-namegetwindowmenupopupa--cmdiframewndgetwindowmenupopup"></a><a name="getwindowmenupopup"></a>CMDIFrameWnd::GetWindowMenuPopup  
 이를 "창" (MDI 창 관리에 대 한 메뉴 항목 팝업 메뉴) 라는 현재 팝업 메뉴에 대 한 핸들을 가져오는 함수를 호출 합니다.  
...
### <a name="remarks"></a>주의  
 기본 구현은 같은 표준 창 메뉴 명령이 포함 된 팝업 메뉴를 찾고 **ID_WINDOW_NEW** 및 **ID_WINDOW_TILE_HORZ**합니다.  
...
### <a name="remarks"></a>주의  
 이 멤버 함수는 [WM_MDIACTIVATE](../../mfc/reference/cwnd-class.md#onmdiactivate) 활성화 되 고 자식 창 및 해제 하 고 자식 창 모두에 대 한 메시지입니다.  
  
 사용자는 마우스나 키보드를 사용 하 여 MDI 자식 창에 포커스를 변경 하는 경우 전송 되는 동일한 메시지입니다.  
  
> [!NOTE]
>  MDI 자식 창에는 MDI 프레임 창을 독립적으로 활성화 됩니다. 자식 창에 마지막으로 활성화 된 전송 되는 프레임 활성화 되 면는 [WM_NCACTIVATE](../../mfc/reference/cwnd-class.md#onncactivate) 메시지를 활성 창 프레임 및 캡션 표시줄 만들었지만 그릴 다른 수신 되지 않으면 `WM_MDIACTIVATE` 메시지입니다.  
...
### <a name="parameters"></a>매개 변수  
 `nType`  
 Cascade 플래그를 지정합니다. 다음 플래그에만 지정할 수 있습니다: `MDITILE_SKIPDISABLED`, 되에 계단식으로 표시에서 사용할 수 없는 MDI 자식 창을 수입니다.  
  
### <a name="remarks"></a>주의  
 첫 번째 버전의 `MDICascade`, 매개 변수 없이 비활성화 된 것을 비롯 한 모든 MDI 자식 창을 계단식으로 나타납니다. 지정 하는 경우 필요에 따라 두 번째 버전 사용 하지 않도록 설정 하는 하위 MDI 자식 창을 하지 않는 `MDITILE_SKIPDISABLED` 에 대 한는 `nType` 매개 변수입니다.  
  
### <a name="example"></a>예제  
 [!code-cpp[NVC_MFCWindowing #&17;](../../mfc/reference/codesnippet/cpp/cmdiframewnd-class_5.cpp)]  
  
##  <a name="a-namemdigetactivea--cmdiframewndmdigetactive"></a><a name="mdigetactive"></a>CMDIFrameWnd::MDIGetActive  
 현재 활성 MDI 자식 창에서 자식 창을 최대화 되었는지 여부를 나타내는 플래그와 함께 검색 합니다.  
...
### <a name="parameters"></a>매개 변수  
 *pbMaximized*  
 에 대 한 포인터는 **BOOL** 값을 반환 합니다. 로 설정 **TRUE** 반환이 창이 최대화 하 고, 그렇지 않으면 않으면 **FALSE**합니다.  
...
### <a name="remarks"></a>주의  
 자식 창을 최대화 하는 경우 Windows 클라이언트 창 채우기의 클라이언트 영역을 찾을 수 있도록 크기가 조정 됩니다. Windows는 사용자를 복원 하거나 자식 창을 닫을 수 있게 프레임의 메뉴 모음에서 자식 창의 컨트롤 메뉴를 배치 합니다. 또한 자식 창의 제목을 프레임 창 제목에 추가합니다.  
  
 현재 활성 상태인 MDI 자식 창을 최대화 되 면 다른 MDI 자식 창이 활성화 되 면 Windows는 현재 활성 자식 복원 하 고 새로 활성화 된 자식 창을 최대화 됩니다.  
...
### <a name="remarks"></a>주의  
 현재 활성 상태인 MDI 자식 창을 최대화 되 면 멤버 함수는 현재 활성 자식 복원 하 고 새로 활성화 된 자식 최대화 됩니다.  
...
##  <a name="a-namemdirestorea--cmdiframewndmdirestore"></a><a name="mdirestore"></a>CMDIFrameWnd::MDIRestore  
 MDI 자식 창 크기를 최대화 또는 최소화에서 복원합니다.  
...
### <a name="parameters"></a>매개 변수  
 *pFrameMenu*  
 새로운 프레임 창 메뉴의 메뉴를 지정합니다. 경우 **NULL**, 메뉴는 변경 되지 않습니다.  
  
 `pWindowMenu`  
 새 창 팝업 메뉴의 메뉴를 지정합니다. 경우 **NULL**, 메뉴는 변경 되지 않습니다.  
  
### <a name="return-value"></a>반환 값  
 이 메시지에 의해 대체 프레임 창 메뉴에 대 한 포인터입니다. 해당 포인터는 임시적이며, 나중에 사용하려고 저장하면 안됩니다.  
  
### <a name="remarks"></a>주의  
 호출한 후 `MDISetMenu`를 호출 하는 응용 프로그램은 [DrawMenuBar](../../mfc/reference/cwnd-class.md#drawmenubar) 의 멤버 함수 `CWnd` 메뉴 표시줄을 업데이트 하 합니다.  
  
 이 호출은 대체 창 팝업 메뉴를 MDI 자식 창 메뉴 항목의 이전 창 메뉴에서 제거 되며이 새 창 팝업 메뉴에 추가 됩니다.  
  
 MDI 자식 창을 최대화 하는 경우이 호출은 바꾸면 MDI 프레임 창 메뉴 컨트롤 메뉴 및 복원 컨트롤 이전 프레임 창 메뉴에서 제거 되며 새로 만들기 메뉴에 추가 됩니다.  
...
### <a name="parameters"></a>매개 변수  
 `nType`  
 바둑판식 배열 플래그를 지정합니다. 이 매개 변수는 다음 플래그 중 하나가 될 수 있습니다.  
  
- `MDITILE_HORIZONTAL`타일 MDI 자식 창을 해당 창 위에 다른 나타나도록 합니다.  
  
- `MDITILE_SKIPDISABLED`비활성화 된 MDI 자식 창을 바둑판식으로 배열 되 고에서 방지 합니다.  
  
- `MDITILE_VERTICAL`타일 MDI 자식 창 옆에 있는 다른 하나의 해당 창이 나타납니다.  
  
### <a name="remarks"></a>주의  
 첫 번째 버전의 `MDITile`, 매개 변수 없이 Windows 버전 3.1 이상에서 세로로 창을 바둑판식으로 배열입니다. 두 번째 버전 창을 바둑판식으로 배열 세로 또는 가로로 값에 따라는 `nType` 매개 변수입니다.  
