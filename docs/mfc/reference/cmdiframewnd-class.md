...
-   MDI 프레임 창은 관리는 **MDICLIENT** 창, 컨트롤 막대와 함께에서 위치를 변경 합니다. MDI 클라이언트 MDI 자식 프레임 창의 직계 부모입니다. **WS_HSCROLL** 및 **WS_VSCROLL** 에 지정 된 창 스타일은 `CMDIFrameWnd` MDI 클라이언트 창을 적용할 주 프레임 창 (에서처럼 Windows 프로그램 관리자, 예를 들어) 사용해 서 MDI 클라이언트 영역을 스크롤하여 볼 수 있도록 하는 대신 합니다.  
...
> [!NOTE]
>  MDI 자식 창은 MDI 프레임 창을 독립적으로 활성화 됩니다. 프레임 활성화 되 면 마지막으로 활성화 된 자식 창을 보내집니다는 [WM_NCACTIVATE](../../mfc/reference/cwnd-class.md#onncactivate) 를 활성 창 프레임 및 캡션 표시줄 하지만 그릴 메시지에서 다른 수신 하지 `WM_MDIACTIVATE` 메시지.  
