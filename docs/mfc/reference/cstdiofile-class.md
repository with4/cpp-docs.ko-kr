---
title: "CStdioFile 클래스 | Microsoft 문서"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
런타임 함수에서 연 것과 C 런타임 스트림 파일을 나타내는 [fopen](../../c-runtime-library/reference/fopen-wfopen.md)합니다.  
...
|이름|설명|  
|----------|-----------------|  
|[CStdioFile::Open](#open)|오버로드됨. 열기는 기본 용도로 설계 `CStdioFile` 생성자 (재정의 [CFile::Open](../../mfc/reference/cfile-class.md#open)).|  
|[CStdioFile::ReadString](#readstring)|한 줄 텍스트를 읽습니다.|  
|[CStdioFile::Seek](#seek)|현재 파일 포인터를 위치합니다.|  
...
## <a name="remarks"></a>주의  
 스트림 파일에는 버퍼 되 고 텍스트 모드 (기본값) 또는 이진 모드에서 열 수 있습니다.  
  
 텍스트 모드 캐리지 리턴-줄 바꿈 쌍에 대 한 특수 한 처리를 제공합니다. 줄 바꿈 문자를 작성 하는 경우 문자 (0x0A) 텍스트 모드를 `CStdioFile` 개체, 바이트 쌍 (0x0D, 0x0A) 파일에 전송 됩니다. 읽을 때, 바이트 쌍 (0x0D, 0x0A) 0x0A 싱글바이트도 변환 됩니다.  
...
### <a name="parameters"></a>매개 변수  
 ...
 `lpszFileName`  
 원하는 파일의 경로를 나타내는 문자열을 지정 합니다. 상대 또는 절대 경로일 수 있습니다.  
  
 `nOpenFlags`  
 파일 만들기, 파일 공유 및 파일 액세스 모드에 대 한 옵션을 지정 합니다. 비트 OR를 사용 하 여 여러 옵션을 지정할 수 있습니다 ( `|`) 연산자.  
  
 한 파일 액세스 모드 옵션은 필요 합니다. 다른 모드는 선택적입니다. 참조 [CFile::CFile](../../mfc/reference/cfile-class.md#cfile) 모드 옵션 목록과 다른 플래그입니다. Mfc 3.0 이상 버전에서 공유 플래그 허용 됩니다.  
...
### <a name="remarks"></a>주의  
 ...
 단일 매개 변수는 열려 있는 파일 스트림을 첨부는 `CStdioFile` 개체입니다. 미리 정의 된 입/출력 파일 포인터를 포함 하는 포인터 값을 허용 `stdin`, `stdout`, 또는 `stderr`합니다.  
  
 두 매개 변수 생성자를 만듭니다를 `CStdioFile` 개체 지정된 된 경로와 해당 파일을 엽니다.  
  
 전달 하는 경우 `NULL` 중 하나에 대 한 `pOpenStream` 또는 `lpszFileName`, 생성자에서 throw 한 `CInvalidArgException*`합니다.  
...
### <a name="parameters"></a>매개 변수  
 ...
 `nOpenFlags`  
 공유 및 액세스 모드입니다. 파일을 열 때 수행할 동작을 지정 합니다. 비트 OR (|) 연산자를 사용 하 여 옵션을 결합할 수 있습니다. 에 대 한 액세스 권한 및 공유 하나 옵션은 필요 합니다. modeCreate 및 modeNoInherit 모드는 선택적입니다.  
...
### <a name="parameters"></a>매개 변수  
 ...
 `nMax`  
 Null 종결 문자를 제외를 읽으려면 문자의 최대 수를 지정 합니다.  
...
### <a name="remarks"></a>주의  
 첫 번째 줄 바꿈 문자로 읽기 중지 됩니다. 보다 적은 경우, if, `nMax`–&1; 문자를 읽은, 줄 바꿈 문자는 버퍼에 저장 됩니다. Null 문자 ('\0')는 두 경우 모두 추가 됩니다.  
  
 [CFile::Read](../../mfc/reference/cfile-class.md#read) 캐리지 리턴-줄 바꿈 쌍에 텍스트 모드 입력 하지만 종료 되지 않는 제공 됩니다.  
  
> [!NOTE]
>  `CString` 이 버전의 제거는 `'\n'` 있으면;는 `LPTSTR` 버전 하지 않습니다.  
...
- `CFile::begin`: 파일 포인터를 이동 합니다. `lOff` 파일의 시작 부분에서 바이트를 전달 합니다.  
  
- `CFile::current`: 파일 포인터를 이동 합니다. `lOff` 파일의 현재 위치에서 바이트입니다.  
  
- `CFile::end`: 파일 포인터를 이동 합니다. `lOff` 파일의 끝에서 바이트입니다. `lOff` 해야 수 음수는 기존 검색 파일; 파일의 끝을 지나서 검색 값이 양수입니다.  
  
### <a name="return-value"></a>반환 값  
 요청된 된 위치 하는 경우 `Seek` 파일의 처음부터 새 바이트 오프셋을 반환 합니다. 그렇지 않으면 반환 값은 정의 되지 않습니다 및 `CFileException` 개체를 throw 합니다.  
  
### <a name="remarks"></a>주의  
 `Seek` 함수를 사용 파일의 내용에 대 한 임의 액세스 포인터를 이동 하 여 지정된 된 시간 또는 절대입니다. 검색 하는 동안 데이터가 실제로 읽힙니다. 요청된 된 위치를 파일의 크기 보다 큰 경우 해당 위치에 확장 되도록 파일 길이 및 예외가 throw 됩니다.  
  
 파일을 열 때 파일 포인터는 오프셋 0, 파일의 시작 부분에 배치 됩니다.  
  
 이 구현 `Seek` 런타임 라이브러리 (CRT) 함수를 기반으로 하는 `fseek`합니다. 용도에 몇 가지 제한이 `Seek` 스트림에 텍스트 모드로 열립니다. 자세한 내용은 참조 [fseek, _fseeki64](../../c-runtime-library/reference/fseek-fseeki64.md)합니다.  
  
### <a name="example"></a>예제  
 다음 예제를 사용 하는 방법을 보여 줍니다 `Seek` 의 시작 부분에서 1000 바이트 포인터를 이동 하는 `cfile` 파일입니다. `Seek` 이후에 호출 해야 하므로 데이터를 읽지 않습니다 [CStdioFile::ReadString](#readstring) 데이터를 읽을 수 있습니다.  
...
### <a name="remarks"></a>주의  
 ...
 null로 끝나는 파일에 사용 되지 않은 데이터를 작성 하려는 경우 `CStdioFile::Write` 또는 [CFile::Write](../../mfc/reference/cfile-class.md#write)합니다.  
