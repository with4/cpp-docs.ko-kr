---
title: "CDrawingManager 클래스 | Microsoft 문서"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
`CDrawingManager` 복합 그리기 알고리즘을 구현 하는 클래스입니다.  
...
|[CDrawingManager::DrawRotated](#drawrotated)|소스 사각형 내부에 지정 하 여 90도 + /-DC 콘텐츠 회전|  
|[CDrawingManager::DrawEllipse](#drawellipse)|제공 된 테두리 및 채우기 색으로 타원을 그립니다.|  
...
|[CDrawingManager::DrawRect](#drawrect)|제공 된 테두리 및 채우기 색으로 사각형을 그립니다.|  
|[CDrawingManager::DrawShadow](#drawshadow)|사각형 영역에 대해 그림자를 그립니다.|  
|[CDrawingManager::Fill4ColorsGradient](#fill4colorsgradient)|두 가지 색 그라데이션으로 사각형 영역을 채웁니다.|  
...
|[CDrawingManager::GrayRect](#grayrect)|지정 된 회색 색으로 사각형을 채웁니다.|  
...
|[CDrawingManager::PixelAlpha](#pixelalpha)|반투명 픽셀에 대 한 최종 색을 결정 하는 도우미 메서드.|  
...
|[CDrawingManager::SetAlphaPixel](#setalphapixel)|비트맵에서 부분적으로 투명 한 픽셀 색 지정 하는 도우미 메서드.|  
|[CDrawingManager::SetPixel](#setpixel)|단일 픽셀 비트맵에 지정 된 색을 변경 하는 도우미 메서드.|  
|[CDrawingManager::SmartMixColors](#smartmixcolors)|가중치가 적용 된 비율에 따라 두 가지 색을 결합 합니다.|  
  
## <a name="remarks"></a>주의  
 `CDrawingManager` 클래스 그림자, 색 그라데이션 및 강조 표시 된 사각형 그리기에 대 한 함수를 제공 합니다. 알파 혼합도 수행합니다. 직접 응용 프로그램의 UI를 변경 하려면이 클래스를 사용할 수 있습니다.  
...
|[out] `pBits`|데이터 포인터를 받는 DIB의 위치에 대 한 포인터 비트 값입니다.|  
|`bitmap`|원래 비트맵에 대 한 핸들|  
|`clrTransparent`|원래 비트맵의 투명 한 색을 지정 하는 RGB 값입니다.|  
  
### <a name="return-value"></a>반환 값  
 이 메서드는 성공 하는 경우 새로 만든된 DIB 비트맵에 대 한 핸들 그렇지 않으면 `NULL`합니다.  
...
### <a name="parameters"></a>매개 변수  
 ...
 [in] `nAngle`  
 초기 그라데이션 그리기 각도 지정 하는 매개 변수입니다. 이 값은 0과 360 사이 여야 합니다.  
...
|-|-|  
|매개 변수|설명|  
|[in] `x1`|줄이 시작 되는 x 좌표입니다.|  
|[in] `y1`|줄이 시작 되는 y 좌표입니다.|  
...
### <a name="parameters"></a>매개 변수  
 ...
 [in] `pBmpSaveBottom`  
 그림자의 아래 부분에 대 한 이미지를 포함 하는 비트맵에 대 한 포인터입니다.  
  
 [in] `pBmpSaveRight`  
 그림자를 그릴 사각형의 오른쪽에 대 한 이미지를 포함 하는 비트맵에 대 한 포인터입니다.  
  
 [in] `clrBase`  
 그림자의 색입니다.  
  
 [in] `bRightShadow`  
 그림자를 그리는 방법을 나타내는 부울 매개 변수입니다. 경우 `bRightShadow` 는 `TRUE`, `DrawShadow` 사각형의 오른쪽에 그림자를 그립니다.  
  
### <a name="return-value"></a>반환 값  
 성공하면 0이 아니고, 그렇지 않으면 0입니다.  
  
### <a name="remarks"></a>주의  
 매개 변수를 사용 하 여 아래쪽 및 오른쪽 그림자에 대 한 두 개의 유효한 비트맵을 제공할 수 있습니다 `pBmpSaveBottom` 및 `pBmpSaveRight`합니다. 이러한 경우 [CBitmap](../../mfc/reference/cbitmap-class.md) 개체는 연결 된 GDI 개체를 `DrawShadow` 어두운으로 이러한 비트맵을 사용 합니다. 하는 경우는 `CBitmap` 매개 변수에 사용 하 여 연결 된 GDI 개체 없는 `DrawShadow` 매개 변수를 비트맵에 연결 하 고 그림자를 그립니다. 에 대 한 호출 이후 `DrawShadow`, 그리기 작업 속도를 높이려면 이러한 비트맵을 제공할 수 있습니다. 에 대 한 자세한 내용은 `CBitmap` 클래스와 GDI 개체를 참조 하십시오 [그래픽 개체](../../mfc/graphic-objects.md)합니다.  
  
 이러한 매개 변수 중 하나에 해당 하는 경우 `NULL`, `DrawShadow` 는 자동으로 그림자를 그립니다.  
  
 설정한 경우 `bRightShadow` 를 `FALSE`, 아래 및 사각형 영역 왼쪽 그림자 그려집니다.  
  
### <a name="example"></a>예제  
 다음 예제에 사용 하는 방법을 보여 줍니다는 `DrawShadow` 의 메서드는 `CDrawingManager` 클래스입니다. 이 코드 조각은의 일부인는 [Prop 시트 데모 샘플](../../visual-cpp-samples.md)합니다.  
...
### <a name="parameters"></a>매개 변수  
 ...
 [in] `colorFinish1`  
 에 대 한 첫 번째 색 그라데이션의 마지막 색입니다.  
  
 [in] `colorStart2`  
 두 번째 색 그라데이션에 대 한 초기 색입니다.  
  
 [in] `colorFinish2`  
 에 대 한 두 번째 색 그라데이션의 마지막 색입니다.  
  
 [in] `bHorz`  
 나타내는 부울 매개 변수 여부 `Fill4ColorsGradient` 수평 또는 수직 그라데이션 색입니다. `TRUE`가로 그라데이션을 나타냅니다.  
  
 [in] `nPercentage`  
 0에서 100 까지의 정수입니다. 이 값을 채우는 첫 번째 색 그라데이션을 사용 하 여 사각형의 비율을 나타냅니다.  
  
### <a name="remarks"></a>주의  
 서로 위에 있는 또는 값에 따라, 다음 중 하나는 두 색 그라데이션으로 사각형 가득 차면 `bHorz`합니다. 각 색 그라데이션을 메서드로 독립적으로 계산 됩니다 [CDrawingManager::FillGradient](#fillgradient)합니다.  
...
### <a name="parameters"></a>매개 변수  
 ...
 [in] `bHorz`  
 지정 하는 부울 매개 변수 여부 `FillGradient` 수평 또는 수직 그라데이션 그려야 합니다.  
  
 [in] `nStartFlatPercentage`  
 사각형의 백분율입니다 `FillGradient` 채웁니다 `colorStart` 그라데이션의 시작 하기 전에 합니다.  
  
 [in] `nEndFlatPercentage`  
 사각형의 백분율입니다 `FillGradient` 채웁니다 `colorFinish` 그라데이션 완료 된 후입니다.  
  
### <a name="example"></a>예제  
 다음 예제에 사용 하는 방법을 보여 줍니다는 `FillGradient` 의 메서드는 `CDrawingManager` 클래스입니다. 이 코드 조각은의 일부인는 [MS Office 2007 데모 샘플](../../visual-cpp-samples.md)합니다.  
...
### <a name="parameters"></a>매개 변수  
 ...
 [in] `nAngle`  
 0과 360 사이의 정수입니다. 이 매개 변수는 색 그라데이션의 방향을 지정합니다.  
  
### <a name="remarks"></a>주의  
 사용 하 여 `nAngle` 색 그라데이션의 방향을 지정할 수 있습니다. 색 그라데이션의 방향을 지정 하면 지정할 있습니다 색 그라데이션의 시작 되는 위치입니다. 값이 0에 대 한 `nAngle` 그라데이션 사각형의 위쪽에서 시작을 나타냅니다. 로 `nAngle` 증가 하면 시작 되는 위치에 기울기 각도에 따라 시계 반대 방향으로 이동 합니다.  
  
### <a name="example"></a>예제  
 다음 예제에 사용 하는 방법을 보여 줍니다는 `FillGradient2` 의 메서드는 `CDrawingManager` 클래스입니다. 이 코드 조각은의 일부인는 [새 컨트롤 샘플](../../visual-cpp-samples.md)합니다.  
...
### <a name="parameters"></a>매개 변수  
 ...
 [in] `clrDisabled`  
 디 포화 상태에 대 한 경우이 메서드를 사용 하는 색 `nPercentage` -1로 설정 합니다.  
  
### <a name="return-value"></a>반환 값  
 `TRUE`메서드가 성공 하면 그렇지 않으면 `FALSE`합니다.  
  
### <a name="remarks"></a>주의  
 매개 변수에 대해 `nPercentage`, 값이 낮을수록 어두운 색을 나타냅니다.  
...
### <a name="remarks"></a>주의  
 경우 `nPercentage` 0에서 99 사이의 `HighlightRect` 알파 혼합 알고리즘을 사용 합니다. 알파 혼합 하는 방법에 대 한 자세한 내용은 참조 [알파 혼합 선 및 채우기](http://msdn.microsoft.com/library/5440f48c-3ac9-44c3-b170-c1c110bdbab8)합니다. 경우 `nPercentage` -1 이면이 메서드는 기본 강조 표시 수준을 사용 합니다. 경우 `nPercentage` 은 100,이 메서드는 아무 작업도 수행 하 고 반환 `TRUE`합니다.  
...
### <a name="parameters"></a>매개 변수  
 ...
 [in] `L`  
 0과 1 사이의 숫자 색의 광도 나타냅니다.  
  
 [in] `S`  
 0과 1 사이의 숫자 색의 채도 나타냅니다.  
...
|-|-|  
|매개 변수|설명|  
|[in] `H`|색에 대 한 색상을 나타내는 0과 360 사이의 숫자입니다.|  
|[in] `S`|0과 1 사이의 숫자 색의 채도 나타냅니다.|  
...
### <a name="remarks"></a>주의  
 이 메서드는 도우미 메서드는는 `CDrawingManager` HSV 또는 HSL 표현에 있는 색의 개별 빨간색, 녹색 및 파랑 구성 요소를 계산 하는 데 사용 하 여 합니다. 이 메서드는은 프로그래머가 직접 호출할 수는 설계 되지 않았습니다. 입력된 매개 변수는 변환 알고리즘에 의존 하는 값입니다.  
...
### <a name="parameters"></a>매개 변수  
 ...
 [in] `percent`  
 투명도의 백분율을 나타내는 0에서 100 사이의 수입니다. 값이 100 초기 색상 완전히 투명 하 게 임을 나타냅니다.  
...
### <a name="remarks"></a>주의  
 반투명 비트맵 색을 칠하기 위한 도우미 클래스 이며 프로그래머가 직접 호출 하도록 설계 되지 않았습니다.  
...
|매개 변수|설명|  
|[in] `rgb`|RGB 값에는 색입니다.|  
|[out] `H`|Double 메서드는 색에 대 한 색상을 저장 하는 위치에 대 한 포인터입니다.|  
|[out] `S`|Double 메서드 색의 채도 저장 하는 위치에 대 한 포인터입니다.|  
|[out] `L`|Double 메서드는 색에 대 한 밝기를 저장 하는 위치에 대 한 포인터입니다.|  
...
### <a name="parameters"></a>매개 변수  
 ...
 [out] `H`  
 이 메서드는 색에 대 한 결과 색상을 저장 하는 double 데이터에 대 한 포인터입니다.  
...
### <a name="parameters"></a>매개 변수  
 [in] `pBits`  
 비트맵에 대 한 비트 값에 대 한 포인터입니다.  
  
 [in] `rect`  
 응용 프로그램에서 사각형 영역입니다. 그리기 관리자 및이 영역의 오른쪽 아래에 있는 그림자를 그립니다.  
...
|-|-|  
|매개 변수|설명|  
|[in] `color1`|첫 번째 색을 혼합입니다.|  
|[in] `color2`|혼합 하는 두 번째 색입니다.|  
|[in] `dblLumRatio`|새 색의 광도 대 한 비율입니다. `SmartMixColors`최종 색을 결정 하기 전에이 비율에 따라 혼합된 색의 광도 곱합니다.|  
...
### <a name="remarks"></a>주의  
 어느 경우이 메서드는 오류와 함께 실패 `k1` 또는 `k2` 가&0; 보다 작은 합니다. 메서드가 반환 하는 경우에 0으로 설정 되어 이러한 매개 변수를 모두, `RGB(0, 0, 0)`합니다.  
...
### <a name="parameters"></a>매개 변수  
 ...
 `bClockWise`  
 `TRUE`나타냅니다 +&90;도 회전 합니다. `FALSE`&90;도 회전을 나타냅니다.  
