---
title: "CUIntArray 클래스 | Microsoft 문서"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
## <a name="members"></a>멤버  
 멤버 함수는 `CUIntArray` 클래스의 멤버 함수와 유사 [CObArray](../../mfc/reference/cobarray-class.md)합니다. 이처럼 두 함수가 비슷하므로 `CObArray` 참조 설명서에서 멤버 함수 관련 사항을 확인할 수 있습니다. 표시 될 때마다는 `CObject` 포인터를 함수 매개 변수 또는 반환 값으로 대체 한 **UINT**합니다.  
...
## <a name="remarks"></a>주의  
 부호 없는 정수 또는 **UINT**, 단어 및&2; 배 워드를에서 다른의 실제 크기는 **UINT** 환경을 운영 하는 대상에 따라 변경할 수 있습니다. A **UINT** 는 더블 워드와 같은 크기입니다.  
  
 `CUIntArray`통합 된 [IMPLEMENT_DYNAMIC](run-time-object-model-services.md#implement_dynamic) 런타임 형식 액세스 및 덤프를 지원 하기 위해 매크로 [CDumpContext](../../mfc/reference/cdumpcontext-class.md) 개체입니다. 부호 없는 정수를 개별 요소의 덤프가 필요한 경우 덤프 컨텍스트의 수준을 1 이상으로 설정 해야 합니다. 부호 없는 정수 배열 serialize 할 수 없습니다.  
...
>  배열을 사용하기 전에 `SetSize`를 사용하여 배열 크기를 설정하고 배열에 대해 메모리를 할당합니다. `SetSize`를 사용하지 않는 경우 배열에 요소를 추가하면 배열이 자주 다시 할당되고 복사됩니다. 이처럼 다시 할당 및 복사가 자주 수행되면 효율성이 떨어지며 메모리가 조각화될 수 있습니다.  
 ...
 사용 하 여 대 한 자세한 내용은 `CUIntArray`, 문서를 참조 하십시오 [컬렉션](../../mfc/collections.md)합니다.  
