...
## <a name="remarks"></a>설명  
 ...
 `CMultiLock` 개체는 스레드에 많은 수의 때 가장 유용 `CEvent` 개체에 응답할 수 있습니다. 모든 포함 된 배열을 만듭니다는 `CEvent` 포인터 및 호출 `Lock`합니다. 이렇게 하면 스레드를 신호를 받는 이벤트 중 하나가 될 때까지 대기 합니다.  
...
##  <a name="lock"></a>  CMultiLock::Lock  
 이 함수를 하나 이상에 제공 되는 동기화 개체에 의해 제어 되는 리소스에 대 한 액세스 권한을 얻으려고 호출는 `CMultiLock` 생성자입니다.  
