...
|이름|설명|  
|----------|-----------------|  
|[CMFCShellListCtrl::DisplayFolder](#displayfolder)|제공 된 폴더에 포함 된 항목 목록을 표시 합니다.|  
|[CMFCShellListCtrl::DisplayParentFolder](#displayparentfolder)|현재 표시 된 폴더의 부모가 있는 폴더에 포함 된 항목 목록을 표시 합니다.|  
...
|[CMFCShellListCtrl::GetCurrentItemIdList](#getcurrentitemidlist)|현재 목록 컨트롤 항목 PIDL를 반환합니다.|  
...
|[CMFCShellListCtrl::OnCompareItems](#oncompareitems)|두 항목을 비교할 때 프레임 워크에서이 메서드를 호출 합니다. (재정의 [CMFCListCtrl::OnCompareItems](../../mfc/reference/cmfclistctrl-class.md#oncompareitems).)|  
|[CMFCShellListCtrl::OnFormatFileDate](#onformatfiledate)|목록 컨트롤에서 표시 되는 파일 날짜를 검색 하는 프레임 워크를 호출 합니다.|  
...
## <a name="remarks"></a>설명  
 `CMFCShellListCtrl` 의 기능을 확장 하는 클래스는 [CMFCListCtrl 클래스](../../mfc/reference/cmfclistctrl-class.md) Windows 셸 항목을 나열 하 여 프로그램을 사용 하 여 합니다. 사용 되는 표시 형식을 탐색기 창에 대 한 목록 보기의 비슷합니다.  
...
### <a name="return-value"></a>반환 값  
 에 대 한 포인터는 [IShellFolder 인터페이스](http://msdn.microsoft.com/library/windows/desktop/bb775075) 선택한 개체에 대 한 합니다.  
...
### <a name="return-value"></a>반환 값  
 A [SHCONTF](http://msdn.microsoft.com/library/windows/desktop/bb762539) 에 나열 된 항목의 유형을 포함 하는 값은 `CMFCShellListCtrl`합니다.  
...
### <a name="return-value"></a>반환 값  
 A `CString` 항목과 연결 된 텍스트가 들어 있는입니다.  
