...
### <a name="remarks"></a>설명  
 ...
 다음 목록에 설정할 수 있는 플래그를 설명 *nFlags*:  
...
- **MF_OWNERDRAW** 항목 소유자 그리기 항목 임을 지정 합니다. 메뉴는 처음으로 표시 되 면 메뉴를 소유 하는 창이 메뉴 항목의 너비와 높이 검색 하는 WM_MEASUREITEM 메시지를 표시 합니다. WM_DRAWITEM 메시지 소유자는 메뉴 항목의 시각적 모양을 업데이트 해야 될 때마다 전송 됩니다. 이 옵션은 최상위 메뉴 항목에 대 한 유효 하지 않습니다.  
 ...
 *nCheck*  
 메뉴 항목을 확인 하 고 메뉴에서 항목의 위치를 결정 하는 방법을 지정 합니다. *nCheck* 매개 변수 조합 수 **MF_CHECKED** 또는 **MF_UNCHECKED** 와 **MF_BYPOSITION** 또는 **MF_ BYCOMMAND** 플래그입니다. 비트 OR 연산자를 사용 하 여 이러한 플래그를 결합할 수 있습니다. 다음과 같은 의미가 있습니다.  
 ...
 *nEnable*  
 수행할 동작을 지정 합니다. 조합 수 **MF_DISABLED**, **MF_ENABLED**, 또는 **MF_GRAYED**와 **MF_BYCOMMAND** 또는 **MF_BYPOSITION** . 비트 OR 연산자를 사용 하 여 이러한 값을 결합할 수 있습니다. 이러한 값에는 다음과 같은 의미가 있습니다.  
 ...
 *nMaxCount*  
 복사할 레이블의 최대 길이 문자 단위로 지정 합니다. 레이블이에 지정 된 최대 길이 보다 길면 *nMaxCount*, 나머지 문자는 잘립니다.  
 ...
 *lpszNewItem*  
 새 메뉴 항목의 내용을 지정합니다. *nFlags* 를 해석 하는 데 사용 될 *lpszNewItem* 다음과 같은 방법으로:  
