...
### <a name="remarks"></a>설명  
 경우 `bActivate` 은 **TRUE**, 하는 경우 컨트롤이 활성화 되 고; **FALSE**, 비활성화 됩니다.  
...
### <a name="parameters"></a>매개 변수  
 ...
 `bLarger`  
 경우 **TRUE**, `lprc` 텍스트 표시 사각형을 지정 하는 데 사용 됩니다 하 고 해당 창 사각형을 받습니다. 경우 **FALSE**, `lprc` 창 사각형을 지정 하는 데 사용 되 고 해당 텍스트 표시 사각형을 받습니다.  
...
### <a name="remarks"></a>설명  
 생성 한 `CToolTipCtrl` 두 단계를 수행에서 합니다. 먼저,을 만드는 생성자를 호출는 `CToolTipCtrl` 개체를 호출 **만들기** 도구 설명 컨트롤을 만들고에 연결 하는 `CToolTipCtrl` 개체입니다.  
...
|**TTS_NOPREFIX**|이 스타일에서 앰퍼샌드 (&)는 문자열에서 문자를 제거 합니다. 시스템을 방지 합니다. 도구 설명 컨트롤에 없는 경우는 **TTS_NOPREFIX** 스타일 시스템 앰퍼샌드 문자를 응용 프로그램 및 도구 설명 컨트롤의 텍스트를 메뉴 항목으로 동일한 문자열을 사용 하도록 자동으로 제거 합니다.|  
...
- `TTDT_INITIAL`도구 설명 창이 표시 될 때까지 포인터 도구의 경계 사각형 내에서 고정 되어 있어야 하는 시간의 길이 검색 합니다.  
  
- `TTDT_RESHOW`다음 도구 설명 창이 포인터 하나의 도구에서 다른 위치로 이동 하는 대로 표시 하는 데 걸리는 시간의 길이 검색 합니다.  
...
|**아래쪽**|아래쪽 테두리의 픽셀 단위로 팁 텍스트를 아래쪽 사이의 거리입니다.|  
