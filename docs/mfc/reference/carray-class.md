 ...
 *형식*  
 배열에 저장 된 개체의 형식을 지정 하는 템플릿 매개 변수입니다. *형식* 에서 반환 하는 매개 변수 `CArray`합니다.  
  
 *ARG* *_* *유형*  
 배열에 저장 된 개체에 액세스 하는 데 사용 되는 인수 형식을 지정 하는 템플릿 매개 변수 에 대 한 참조 종종 *형식*합니다. *ARG_TYPE* 에 전달 되는 매개 변수는 `CArray`합니다.  
 ...
 **복사** 메모리를 해제 하지는 않습니다; 하지만 필요한 경우 `Copy` 배열에 복사 된 요소를 수용 하기 위해 추가 메모리를 할당할 수 있습니다.  
...
### <a name="remarks"></a>설명  
 ...
 조건 `GetUpperBound( )` =-1 배열에 요소가 포함 되어 있음을 나타냅니다.  
...
### <a name="remarks"></a>설명  
 *pNewData* 는 항상 모든 포함할 수 있는 크기는 *pData* 요소입니다.  
