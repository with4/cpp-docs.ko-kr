...
### <a name="return-value"></a>반환 값  
 A `CSize` 확인란 중간 크기를 포함 하는 개체입니다.  
...
### <a name="return-value"></a>반환 값  
 반환 된 `CSize` 확인란의 일반 크기를 포함 하는 개체입니다.  
...
### <a name="return-value"></a>반환 값  
 반환 `TRUE` 은 확인란과 관련 된 도구 설명 이미지가 없는 경우 또는 `FALSE` 그렇지 않은 경우.  
...
### <a name="remarks"></a>설명  
 기본적으로이 메서드를 설정 및 항상 확인란에 대 한 내게 필요한 옵션 데이터 반환 `TRUE`합니다. 내게 필요한 옵션 데이터를 설정하고 성공 또는 실패를 나타내는 값을 반환하려면 이 메서드를 재정의합니다.  
