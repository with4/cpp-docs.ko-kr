...
|[CWnd::get_accHelp](#get_acchelp)|개체를 검색 프레임 워크에서 호출 **도움말** 속성 문자열입니다.|  
...
|[CWnd::GetNextDlgTabItem](#getnextdlgtabitem)|검색 된 첫 번째 컨트롤의 [WS_TABSTOP](window-styles.md) 스타일을 지정된 된 컨트롤을 따릅니다 (또는 앞).|  
...
|[CWnd::OpenClipboard](#openclipboard)|클립보드를 엽니다. 다른 응용 프로그램까지 Windows 클립보드를 수정할 수 없습니다 [CloseClipboard](http://msdn.microsoft.com/library/windows/desktop/ms649035) 함수를 호출 합니다.|  
...
|[CWnd::PreSubclassWindow](#presubclasswindow)|다른 필요한 서브클래싱 발생할 수 있습니다. [SubclassWindow](#subclasswindow) 호출 됩니다.|  
...
|[CWnd::SetTimer](#settimer)|설치를 보내는 시스템 타이머는 [WM_TIMER](#ontimer) 트리거될 때 메시지입니다.|  
...
|[CWnd::UnsubclassWindow](#unsubclasswindow)|창에서 분리 된 `CWnd` 개체|  
...
|[CWnd::GetCurrentMessage](#getcurrentmessage)|이 창에서 현재 처리 중인 메시지에 대한 포인터를 반환합니다. 만 호출 해야 때는 `On` *메시지* 메시지 처리기 멤버 함수입니다.|  
...
|[CWnd::OnCharToItem](#onchartoitem)|자식 목록 상자에 의해 호출 된 [LBS_WANTKEYBOARDINPUT](list-box-styles.md) 스타일에 대 한 응답에는 [WM_CHAR](#onchar) 메시지.|  
...
|[CWnd::OnDestroyClipboard](#ondestroyclipboard)|Windows에 대 한 호출을 통해는 클립보드를 비울 때 호출 [EmptyClipboard](http://msdn.microsoft.com/library/windows/desktop/ms649037) 함수입니다.|  
...
|[CWnd::OnMouseHover](#onmousehover)|이전 호출에 지정 된 기간 동안 창의 클라이언트 영역 위로 커서를 이동할 때 호출 [TrackMouseEvent](http://msdn.microsoft.com/library/windows/desktop/ms646265)합니다.|  
|[CWnd::OnMouseHWheel](#onmousehwheel)|현재 창이 DWM(바탕 화면 창 관리자)에 의해 구성되고 해당 창이 최대화되면 호출됩니다.|  
|[CWnd::OnMouseLeave](#onmouseleave)|커서에 대 한 이전 호출에 지정 된 창의 클라이언트 영역을 벗어나면 호출 [TrackMouseEvent](http://msdn.microsoft.com/library/windows/desktop/ms646265)합니다.|  
...
|[CWnd::OnNcCreate](#onnccreate)|이전에 호출 [OnCreate](#oncreate) 비클라이언트 영역을 만들 때.|  
...
|[CWnd::OnNcMouseHover](#onncmousehover)|이전 호출에 지정 된 기간 동안 창의 비클라이언트 영역 위로 커서를 이동할 때 호출 [TrackMouseEvent](http://msdn.microsoft.com/library/windows/desktop/ms646265)합니다.|  
|[CWnd::OnNcMouseLeave](#onncmouseleave)|프레임 워크는 커서에 대 한 이전 호출에 지정 된 창의 비클라이언트 영역을 벗어날 때이 멤버 함수를 호출 [TrackMouseEvent](http://msdn.microsoft.com/library/windows/desktop/ms646265)합니다.|  
...
|[CWnd::OnStyleChanged](#onstylechanged)|나타내는 [SetWindowLong](http://msdn.microsoft.com/library/windows/desktop/ms633591) Windows 함수는 창 스타일 중 하나 이상을 변경 되었습니다.|  
|[CWnd::OnStyleChanging](#onstylechanging)|나타내는 [SetWindowLong](http://msdn.microsoft.com/library/windows/desktop/ms633591) Windows 함수는 창 스타일 중 하나 이상을 변경 되려고 합니다.|  
...
|[CWnd::OnTimer](#ontimer)|각 간격에 지정 된 후에 호출 [SetTimer](#settimer)합니다.|  
...
|[CWnd::OnVKeyToItem](#onvkeytoitem)|가 소유 하는 목록 상자에 의해 호출 `CWnd` 에 대 한 응답을 [WM_KEYDOWN](#onkeydown) 메시지입니다.|  
...
|[CWnd::OnWindowPosChanged](#onwindowposchanged)|크기, 위치, 또는 Z 순서에 대 한 호출의 결과로 변경 될 때 호출 [SetWindowPos](#setwindowpos) 또는 다른 창 관리 함수입니다.|  
...
|[CWnd::PostNcDestroy](#postncdestroy)|이 가상 함수를 호출 하 고 기본 [OnNcDestroy](#onncdestroy) 창이 소멸 된 후에 작동 합니다.|  
...
|[CWnd::operator! =](#operator_neq)|같은지 여부를 확인 하는 경우 창이 하지 핸들은 창으로 [m_hWnd](#m_hwnd)합니다.|  
...
## <a name="remarks"></a>주의  
 `CWnd` 개체는 Windows 창과 다르지만 둘이 밀접하게 연결되어 있습니다. `CWnd` 개체는 `CWnd` 생성자 및 소멸자로 만들거나 제거합니다. Windows 창 반면에 Windows에 의해 만들어진 내부 데이터 구조는 **만들기** 멤버 함수를 제거 하 고는 `CWnd` 가상 소멸자입니다. [DestroyWindow](#destroywindow) 함수 개체를 삭제 하지 않고 Windows 창을 소멸 시킵니다.  
  
 `CWnd` 클래스 및 숨기기 메시지 맵 메커니즘은 **WndProc** 함수입니다. 들어오는 Windows 알림 메시지를 적절 한 메시지 맵을 통해 자동으로 라우팅됩니다 **에***메시지* `CWnd` 멤버 함수입니다. 재정의 **에***메시지* 파생된 클래스에서 멤버의 특정 메시지를 처리 하는 멤버 함수입니다.  
 ...
 자식 창은 두 단계로 만듭니다. 먼저 생성자를 호출 `CWnd` 생성 하는 `CWnd` 개체를 만든 다음 호출에서 [만들기](#create) 자식 창을 만들고에 연결 하는 멤버 함수는 `CWnd` 개체입니다.  
  
 사용자가 자식 창을 종료하면 `CWnd` 개체를 제거하거나 `DestroyWindow` 멤버 함수를 호출하여 창을 제거하고 해당 데이터 구조를 제거합니다.  
  
 Microsoft Foundation Class 라이브러리 내에서 추가 클래스를 `CWnd`에서 파생시켜 특정 창 형식을 제공합니다. 이러한 클래스를 포함 하 여 많은 [CFrameWnd](../../mfc/reference/cframewnd-class.md), [CMDIFrameWnd](../../mfc/reference/cmdiframewnd-class.md), [CMDIChildWnd](../../mfc/reference/cmdichildwnd-class.md), [CView](../../mfc/reference/cview-class.md), 및 [CDialog](../../mfc/reference/cdialog-class.md), 추가 파생을 위한 설계 되었습니다. 컨트롤 클래스에서 파생 된 `CWnd`, 예: [CButton](../../mfc/reference/cbutton-class.md)직접 사용할 수 있습니다, 또는 클래스의 추가 파생에 사용할 수 있습니다.  
  
 사용 하 여 대 한 자세한 내용은 `CWnd`, 참조 [프레임 창](../../mfc/frame-windows.md) 및 [창 개체](../../mfc/window-objects.md)합니다.  
...
### <a name="parameters"></a>매개 변수  
 `varChild`  
 기본 동작을 호출할 수의 개체 또는 개체의 자식 요소 중 하나 인지를 지정 합니다. 이 매개 변수는 CHILDID_SELF (수행 하려면 개체의 기본 동작) 또는 자식 ID (개체의 자식 요소 중 하나의 기본 작업 수행) 일 수 있습니다.  
  
### <a name="return-value"></a>반환 값  
 성공, 실패 시 COM 오류 코드는 S_OK를 반환 합니다. 참조 **반환 값** 에서 [의 상호](http://msdn.microsoft.com/library/windows/desktop/dd318470) 에 [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]합니다.  
...
### <a name="parameters"></a>매개 변수  
 ...
 `pvarChild`  
 으로 지정한 지점에 있는 개체를 식별 하는 정보를 받는 `xLeft` 및 `yTop`합니다. 참조 *pvarID* 에서 [IAccessible::accHitTest](http://msdn.microsoft.com/library/windows/desktop/dd318471) 에 [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]합니다.  
  
### <a name="return-value"></a>반환 값  
 성공, 실패 시 COM 오류 코드는 S_OK를 반환 합니다. 참조 **반환 값** 에서 **IAccessible::accHitTest** 에 [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]합니다.  
  
### <a name="remarks"></a>주의  
 이 함수는 MFC의 일부가 [Active Accessibility](http://msdn.microsoft.com/library/windows/desktop/dd373592) 지원 합니다.  
  
 이 함수를 재정의 하면 `CWnd`-nonwindowed 사용자 인터페이스 요소 (외의 기타 창 없는 ActiveX 컨트롤을 MFC 처리)을 사용 하는 경우 클래스를 파생 합니다.  
 ...
 *pcyHeight*  
 ...
 `varChild`  
 검색할 위치 개체 또는 개체의 자식 요소 중 하나 인지를 지정 합니다. 이 매개 변수는 CHILDID_SELF (에 대 한 정보가 개체) 또는 자식 ID (개체의 자식 요소에 대 한 정보를 가져올) 일 수 있습니다.  
  
### <a name="return-value"></a>반환 값  
 성공, 실패 시 COM 오류 코드는 S_OK를 반환 합니다. 참조 **반환 값** 에서 **IAccessible::accLocation** 에 [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]합니다.  
...
### <a name="parameters"></a>매개 변수  
 `navDir`  
 이동할 방향을 지정 합니다. 참조 `navDir` 에서 [IAccessible::accNavigate](http://msdn.microsoft.com/library/windows/desktop/dd318473) 에 [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]합니다.  
  
 `varStart`  
 시작 개체를 지정합니다. 참조 `varStart` 에서 **IAccessible::accNavigate** 에 [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]합니다.  
  
 *pvarEndUpAt*  
 대상 사용자 인터페이스 개체에 대 한 정보를 받습니다. 참조 *pvarEnd* 에서 **IAccessible::accNavigate** 에 [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]합니다.  
  
### <a name="return-value"></a>반환 값  
 성공, 실패 시 COM 오류 코드는 S_OK를 반환 합니다. 참조 **반환 값** 에서 **IAccessible::accNavigate** 에 [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]합니다.  
...
### <a name="parameters"></a>매개 변수  
 `flagsSelect`  
 현재 선택 영역이 나 포커스를 변경 하는 방법을 지정 합니다. 참조 `flagsSelect` 에서 [IAccessible::accSelect](http://msdn.microsoft.com/library/windows/desktop/dd318474) 에 [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]합니다.  
  
 `varChild`  
 선택할 개체를 지정 합니다. 이 매개 변수는 CHILDID_SELF (개체를 선택 하 여 자체) 또는 (중 하나를 선택 된 개체의 자식) 자식 ID 일 수 있습니다.  
  
### <a name="return-value"></a>반환 값  
 성공, 실패 시 COM 오류 코드는 S_OK를 반환 합니다. 참조 **반환 값** 에서 **IAccessible::accSelect** 에 [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]합니다.  
...
### <a name="parameters"></a>매개 변수  
 *dwTime*  
 밀리초 단위로 애니메이션을 재생 하는 데 걸리는 시간을 지정 합니다. 일반적으로 애니메이션 재생 200 밀리초를 사용 합니다.  
  
 `dwFlags`  
 애니메이션의 종류를 지정합니다. 가능한 값의 전체 목록은 참조 하십시오. [AnimateWindow](http://msdn.microsoft.com/library/windows/desktop/ms632669)합니다.  
...
### <a name="return-value"></a>반환 값  
 함수가 성공 하면 아이콘의 한 행의 높이 그렇지 않으면 0입니다.  
  
### <a name="remarks"></a>주의  
 이 멤버 함수는 또한 전체 화면에 설명 하는 데스크톱 창에서 아이콘을 정렬 합니다. [GetDesktopWindow](#getdesktopwindow) 멤버 함수는 데스크톱 창 개체에 대 한 포인터를 검색 합니다.  
...
### <a name="example"></a>예제  
 이 예제에서는 MDI 클라이언트 창에 매핑할 Attach 및 Detach를 사용 하는 방법을 보여 줍니다.  
...
### <a name="return-value"></a>반환 값  
 장치 컨텍스트를 식별 `CWnd`합니다. 포인터가 일시적일 수 고의 범위를 벗어납니다 저장 해야 [EndPaint](#endpaint)합니다.  
  
### <a name="remarks"></a>주의  
 그리기 구조는 완전히 업데이트 영역 및 백그라운드 삭제 여부를 지정 하는 플래그를 포함 하는 가장 작은 사각형을가 하는 RECT 데이터 구조를 포함 합니다.  
  
 업데이트 영역에 의해 설정 됩니다는 [Invalidate](#invalidate), [InvalidateRect](#invalidaterect), 또는 [InvalidateRgn](#invalidatergn) 멤버 함수 및 크기, 이동, 생성, 스크롤, 또는 클라이언트 영역에 영향을 주는 다른 작업을 수행 하는 후 시스템에서. 업데이트 영역 지우기, 표시 되는 경우 `BeginPaint` 보냅니다는 [WM_ONERASEBKGND](#onerasebkgnd) 메시지입니다.  
  
 호출 하지 마십시오는 `BeginPaint` 대 한 응답으로 제외 하 고 멤버 함수는 [WM_PAINT](#onpaint) 메시지입니다. 각 호출에는 `BeginPaint` 멤버 함수에 대 한 일치 하는 호출 해야 합니다.는 [EndPaint](#endpaint) 멤버 함수입니다. 캐럿을 그릴 영역에 있는 경우는 `BeginPaint` 멤버 함수를 지우거 방지 하기 위해 캐럿을 자동으로 숨깁니다.  
...
### <a name="parameters"></a>매개 변수  
 `dwDispID`  
 데이터 소스 컨트롤에 바인딩될 수 있는 데이터 바인딩된 컨트롤에 속성의 경우 DISPID를 지정 합니다.  
 ...
 `pDSCWnd`  
 속성은 바인딩된 데이터 소스 호스트 제어를 하는 창을 가리킵니다. 호출 `GetDlgItem` 이 포인터를 검색 하는 DC 호스트 창의 리소스 id입니다.  
...
### <a name="parameters"></a>매개 변수  
 [in, out] `lpClientRect`  
 사각형 구조에 대 한 포인터입니다. 이 구조는 입력에 대해 클라이언트 사각형을 포함 합니다. 메서드가 완료 되 면이 구조는 지정된 된 클라이언트 사각형을 포함할 수 있는 창 사각형을 포함 합니다.  
  
 [in] `nAdjustType`  
 사용 하 여 `CWnd::adjustBorder` 없이 창 좌표를 계산 하는 `WS_EX_CLIENTEDGE` 스타일 지정 합니다; 그렇지 않으면 사용 `CWnd::adjustOutside`합니다.  
  
### <a name="remarks"></a>주의  
 계산 된 창 사각형의 크기는 메뉴 모음에 대 한 공간을 포함 하지 않습니다.  
  
 자세한 사용 제한에 대 한 참조 [AdjustWindowRectEx](http://msdn.microsoft.com/library/windows/desktop/ms632667)합니다.  
...
### <a name="parameters"></a>매개 변수  
 *bKeys*  
 **True 이면** 키를 누른 기본적으로 상태 표시줄 텍스트를 설정 하는 경우 도구 설명을 취소 하려면 그렇지 않으면 **FALSE**합니다.  
  
### <a name="remarks"></a>주의  
  
> [!NOTE]
>  이 멤버 함수를 사용 하 여 해도 코드에서 관리 하는 도구 설명에는 영향이 없습니다. 관리 하는 도구 설명 컨트롤에만 영향을 [CWnd::EnableToolTips](#enabletooltips)합니다.  
...
### <a name="parameters"></a>매개 변수  
 `pAlternateOwner`  
 대체 되는 것이 창에 대 한 포인터를 가운데 맞춤 (부모 창 보다 기타).  
  
### <a name="remarks"></a>주의  
 일반적으로에서 호출 [CDialog::OnInitDialog](../../mfc/reference/cdialog-class.md#oninitdialog) 응용 프로그램의 주 창 기준으로 대화 상자 가운데에 있습니다. 기본적으로 함수에는 해당 부모 창 및 소유자를 기준으로 팝업 창을 기준으로 자식 창을 가운데로 맞춥니다. 팝업 창을 소유 하지 않은 화면을 기준으로 가운데 표시 됩니다. 소유자 또는 부모 되지 않는 특정 창에 상대적인 창의 가운데에 `pAlternateOwner` 유효한 창으로 매개 변수를 설정할 수 있습니다. 반환 된 값을 전달 하는 화면을 기준으로 가운데 맞춤 되도록 [CWnd::GetDesktopWindow](#getdesktopwindow) 으로 `pAlternateOwner`합니다.  
...
### <a name="parameters"></a>매개 변수  
 `hWndNext`  
 다음에 나오는 창을 식별 `CWnd` 클립보드 뷰어 체인에 있습니다.  
...
### <a name="parameters"></a>매개 변수  
 ...
 `nCheck`  
 수행할 동작을 지정 합니다. 경우 `nCheck` 이 값은 0에서 `CheckDlgButton` 단추 옆에 확인 표시를 삽입 하는 멤버 함수; 0 인 경우에 확인 표시가 제거 됩니다. 3 단계 단추에 대 한 경우 `nCheck` 2 인 단추 상태를 확인할 수 없습니다.  
  
### <a name="remarks"></a>주의  
 `CheckDlgButton` 보냅니다 함수는 [BM_SETCHECK](http://msdn.microsoft.com/library/windows/desktop/bb775989) 메시지 지정한 단추입니다.  
...
### <a name="remarks"></a>주의  
 `CheckRadioButton` 보냅니다 함수는 [BM_SETCHECK](http://msdn.microsoft.com/library/windows/desktop/bb775989) 지정된 라디오 단추에 대 한 메시지입니다.  
 ...
 *nflags*  
 표시 하지 않으려면 될 자식 창을 지정 합니다. 이 매개 변수는 다음 값의 조합 될 수 있습니다.  
  
|값|의미|  
|-----------|-------------|  
|**CWP_ALL**|모든 자식 창을 건너뛰지 마십시오|  
|**CWP_SKIPINVISIBLE**|보이지 않는 자식 창을 건너뛰기|  
|**CWP_SKIPDISABLED**|비활성화 된 자식 창을 건너뛰기|  
|**CWP_SKIPTRANSPARENT**|투명 한 자식 창을 건너뛰기|  
  
### <a name="return-value"></a>반환 값  
 지점을 포함 하는 자식 창을 식별 합니다. **NULL** 하 여 주어진된 지점 클라이언트 영역 외부에 있는 경우. 지점 클라이언트 영역 내 이지만 모든 자식 창 내에 포함 되어 있지 않은 경우 `CWnd` 반환 됩니다.  
  
 이 멤버 함수는 지정 된 지점을 포함 하는 숨김 또는 비활성화 자식 창에 반환 됩니다.  
  
 창이 여러 개 지정된 된 지점에 포함할 수 있습니다. 그러나이 함수는만 반환는 `CWnd`* 지점을 포함 하는 첫 번째 창의 발생 합니다.  
  
 `CWnd`* 임시 수 있으며 나중에 사용할 저장 해야 하는 반환 됩니다.  
...
```  
가상 BOOL (LPCTSTR lpszClassName, 만들기  
...
```  
가상 HRESULT CreateAccessibleProxy (WPARAM wParam,  
    LPARAM lParam  
    LRESULT* pResult);
```  
...
```  
가상 BOOL CreateEx (DWORD dwExStyle,  
...
```  
가상 void DoDataExchange (CDataExchange * pDX);
```  
...
```  
DragAcceptFiles void (BOOL bAccept = TRUE);
```  
...
```  
EnableD2DSupport void (BOOL bEnable = TRUE,  
...
```  
정적 CWnd * 파스칼 FindWindow (LPCTSTR lpszClassName,  
    LPCTSTR lpszWindowName);
```  
...
```  
정적 CWnd * 파스칼 FromHandle(HWND hWnd);
```  
...
```  
정적 CWnd * 파스칼 FromHandlePermanent(HWND hWnd);
```  
...
```  
가상 HRESULT get_accChild (VARIANT varChild,  
...
```  
가상 HRESULT get_accChildCount (long * pcountChildren);
```  
...
```  
가상 HRESULT get_accDefaultAction (VARIANT varChild,  
...
```  
가상 HRESULT get_accDescription (VARIANT varChild,  
    BSTR* pszDescription);
```  
...
```  
가상 HRESULT get_accFocus (VARIANT * pvarChild);
```  
...
```  
가상 HRESULT get_accHelp (VARIANT varChild,  
    BSTR* pszHelp);
```  
...
```  
가상 HRESULT get_accHelpTopic (BSTR* pszHelpFile, VARIANT varChild 긴* pidTopic);
```  
...
```  
가상 HRESULT get_accKeyboardShortcut (VARIANT varChild,  
...
```  
가상 HRESULT get_accName (VARIANT varChild,  
    BSTR* pszName);
```  
...
```  
가상 HRESULT get_accParent (IDispatch * * ppdispParent);
```  
...
```  
가상 HRESULT get_accRole (VARIANT varChild,  
    VARIANT* pvarRole);
```  
...
```  
가상 HRESULT get_accSelection (VARIANT * pvarChildren);
```  
...
```  
가상 HRESULT get_accState (VARIANT varChild,  
    VARIANT* pvarState);
```  
...
```  
가상 HRESULT get_accValue (VARIANT varChild,  
    BSTR* pszValue);
```  
...
```  
int (int nIDFirstButton, GetCheckedRadioButton  
    int nIDLastButton);
```  
...
```  
정적 CWnd * 파스칼 GetClipboardViewer();
```  
...
```  
정적 const MSG * 파스칼 GetCurrentMessage();
```  
...
```  
정적 CWnd * 파스칼 GetForegroundWindow();
```  
...
```  
BOOL GetLayeredWindowAttributes (COLORREF* pcrKey, 바이트* pbAlpha,  
...
```  
BOOL GetMenuBarInfo (긴 idObject,  
    긴 idItem  
...
```  
정적 CWnd * 파스칼 GetOpenClipboardWindow();
```  
...
```  
정적 CWnd* GetSafeOwner (CWnd* pParent = NULL,  
    HWND* pWndTop = NULL);
```  
...
```  
가상 CScrollBar * GetScrollBarCtrl(int nBar) const;  
```  
...
```  
BOOL GetScrollBarInfo (긴 idObject,  
...
```  
GetWindowText (LPTSTR lpszStringBuf, int  
    int nMaxCount) const;  
...
```  
가상 void HtmlHelp (DWORD_PTR dwData,  
    UINT nCmd = 0x000F);
```  
...
```  
Invalidate void (BOOL bErase = TRUE);
```  
...
void AFX_CDECL InvokeHelper (DISPID dwDispID,  
    WORD wFlags  
    VARTYPE vtRet  
    void* pvRet, const 바이트* pbParamInfo,  
 ...);
```  
...
```  
int (LPCTSTR lpszText, MessageBox  
...
MoveWindow void (LPCRECT lpRect, BOOL bRepaint = TRUE);
```  
...
```  
가상 BOOL OnAmbientProperty (COleControlSite* pSite, DISPID dispid VARIANT* pvar);
```  
...
```  
afx_msg void OnChangeUIState (UINT n 작업,  
    UINT nUIElement);
```  
...
```  
가상 BOOL OnChildNotify (UINT 메시지,  
    WPARAM wParam  
    LPARAM lParam  
    LRESULT* pResult);
```  
...
```  
가상 BOOL OnCommand (WPARAM wParam,  
    LPARAM lParam);
```  
...
```  
HBRUSH OnCtlColor afx_msg (CDC* pDC CWnd* pWnd,  
    UINT nCtlColor);
```  
...
```  
가상 void OnDrawIconicThumbnailOrLivePreview (CDC / dc  
    CRect rect  
...
```  
afx_msg void OnInputDeviceChange (부호 없는 short uFlag);
```  
...
```  
afx_msg 주는 OnMenuChar (UINT nChar,  
    UINT nFlags  
    CMenu* pMenu);
```  
...
```  
afx_msg 주는 OnNcHitTest(CPoint point);
```  
...
```  
void OnNcXButtonDblClk (짧은 nHitTest,   
    UINT nButton   
    CPoint 지점)입니다.
```  
...
```  
afx_msg void OnNcXButtonDown (짧은 nHitTest,   
    UINT nButton   
    CPoint 지점)입니다.
```  
...
```  
afx_msg void OnNcXButtonUp (짧은 nHitTest,   
    UINT nButton   
    CPoint 지점)입니다.
```  
...
```  
가상 BOOL OnNotify (WPARAM wParam,  
    LPARAM lParam  
    LRESULT* pResult);
```  
...
```  
afx_msg void OnParentNotify (UINT 메시지,  
    LPARAM lParam);
```  
...
```  
afx_msg 주는 OnRegisteredMouseWheel (WPARAM wParam,  
    LPARAM lParam);
```  
...
```  
가상 INT_PTR OnToolHitTest (CPoint 지점  
    TOOLINFO* pTI) const;  
```  
...
```  
가상 BOOL OnTouchInput (CPoint pt,  
    int nInputNumber  
    int nInputsCount  
    PTOUCHINPUT pInput);
```  
...
```  
가상 BOOL OnTouchInputs (UINT nInputsCount,  
    PTOUCHINPUT pInputs);
```  
...
```  
afx_msg void OnUpdateUIState (UINT n 작업,  
    UINT nUIElement);
```  
...
```  
가상 BOOL PreCreateWindow(CREATESTRUCT& cs);
```  
...
```  
가상 BOOL PreTranslateMessage(MSG* pMsg);
```  
...
BOOL RedrawWindow (LPCRECT lpRectUpdate = NULL,  
    ...
    UINT 플래그 RDW_INVALIDATE = | RDW_UPDATENOW | RDW_ERASE);
```  
...
```  
BOOL ReflectChildNotify (UINT 메시지,  
    WPARAM wParam  
    LPARAM lParam  
    LRESULT* pResult);
```  
...
```  
정적 BOOL 파스칼 ReflectLastMsg (HWND hWndChild,  
...
```  
RepositionBars void (UINT nIDFirst, UINT nIDLast UINT nIDLeftOver, UINT nFlag reposDefault, LPRECT lpRectParam = = NULL, LPCRECT lpRectClient = NULL, BOOL bStretch = TRUE);  
```  
...
### <a name="remarks"></a>주의  
 `ScreenToClient` 멤버 함수에 지정 된 화면 좌표를 대체 `lpPoint` 또는 `lpRect` 클라이언트 좌표를 사용 합니다. 왼쪽 위 모퉁이 기준으로 새 좌표는는 `CWnd` 클라이언트 영역입니다.  
...
### <a name="parameters"></a>매개 변수  
 ...
 `yAmount`  
 세로 스크롤 장치 단위로 크기를 지정합니다. 이 매개 변수는 출력 창에 음수 값 이어야 합니다.  
  
 `lpRect`  
 가리키는 [CRect](../../atl-mfc-shared/reference/crect-class.md) 개체 또는 [RECT 구조체](../../mfc/reference/rect-structure1.md) 클라이언트 영역을 스크롤할 수의 부분을 지정 하는 합니다. 경우 `lpRect` 는 **NULL**, 전체 클라이언트 영역이 스크롤됩니다. 커서 사각형이 스크롤 사각형 교차 하는 경우 캐럿 위치가 변경 합니다.  
  
 `lpClipRect`  
 가리키는 `CRect` 개체 또는 `RECT` 스크롤해야 클리핑 사각형을 지정 하는 구조입니다. 이 사각형 안에 비트만 스크롤됩니다. Bits이이 사각형 외부에 있는 경우에 영향을 받지 않는 `lpRect` 사각형입니다. 경우 `lpClipRect` 는 **NULL**, 없습니다 클리핑 스크롤 사각형에서 수행 됩니다.  
  
### <a name="remarks"></a>주의  
 캐럿이 있는 경우는 `CWnd` 스크롤 되 고 `ScrollWindow` 스크롤 완료 된 후 캐럿을 다음 복원 하 고 자동으로 않게 방지 하기 위해 캐럿을 숨깁니다. 캐럿 위치에 따라 조정 됩니다.  
  
 영역으로 처리는 `ScrollWindow` 멤버 함수 다시 표시 하지는 않지만 현재도 통합 됩니다 `CWnd` 개체의 업데이트 영역입니다. 응용 프로그램을 최종적으로 받습니다는 [WM_PAINT](http://msdn.microsoft.com/library/windows/desktop/dd145213) 영역을 다시 그려야 할을 알리는 메시지입니다. 호출 이루어집니다 스크롤 동시에 적용 되지 않은 영역을 다시 그릴 수는 [UpdateWindow](#updatewindow) 멤버 함수가 호출한 후 바로 `ScrollWindow`합니다.  
  
 경우 `lpRect` 는 **NULL**,으로 지정 된 크기에서 오프셋 하는 모든 자식 창은 창에서의 위치 `xAmount` 및 `yAmount`, 및 잘못 된 (만져) 영역에는 `CWnd` 또한 오프셋 됩니다. `ScrollWindow`더 빠른 경우는 `lpRect` 는 **NULL**합니다.  
 ...
 *dy*  
 세로 스크롤 장치 단위로 크기를 지정합니다. 이 매개 변수 위로 스크롤하려면 음수 값을 가져야 합니다.  
  
 `lpRectScroll`  
 가리키는 [RECT 구조체](../../mfc/reference/rect-structure1.md) 클라이언트 영역을 스크롤할 수의 부분을 지정 하는 합니다. 이 매개 변수가 **NULL**, 전체 클라이언트 영역이 스크롤됩니다.  
  
 `lpRectClip`  
 가리키는 `RECT` 스크롤해야 클리핑 사각형을 지정 하는 구조입니다. 이 구조에서 가리키는 사각형에 우선 `lpRectScroll`합니다. 이 사각형 안에 비트만 스크롤됩니다. Bits이이 사각형 외부에 있는 경우에 영향을 받지 않는 `lpRectScroll` 사각형입니다. 이 매개 변수가 **NULL**, 없습니다 클리핑 스크롤 사각형에서 수행 됩니다.  
  
 *prgnUpdate*  
 스크롤 무효화 된 영역을 포함 하도록 수정 되는 영역을 식별 합니다. 이 매개 변수 수 **NULL**합니다.  
  
 `lpRectUpdate`  
 가리키는 `RECT` 구조를 무효화 하지 스크롤 하는 사각형의 경계를 받게 됩니다. 이 매개 변수 수 **NULL**합니다.  
  
 `flags`  
 다음 값 중 하나일 수 있습니다.  
  
- **SW_ERASE** 함께 지정 하면 **SW_INVALIDATE**를 전송 하 여 새로 무효화 된 영역을 지웁니다는 [WM_ERASEBKGND](http://msdn.microsoft.com/library/windows/desktop/ms648055) 창에 메시지입니다.  
  
- **SW_INVALIDATE** 로 식별 되는 영역을 무효화 *prgnUpdate* 스크롤한 후입니다.  
  
- **SW_SCROLLCHILDREN** 가리키는 사각형 교차 하는 모든 자식 창을 스크롤하여 `lpRectScroll` 에 지정 된 픽셀 수 여 `dx` 및 *dy*합니다. Windows에서 보냅니다는 [WM_MOVE](http://msdn.microsoft.com/library/windows/desktop/ms632631) 교차 하는 모든 자식 창에 메시지 `lpRectScroll`이동 하지 않는 경우에 합니다. 자식 창을 스크롤할 때 커서 사각형이 교차 스크롤 사각형 캐럿의 위치가 바뀌었습니다.  
  
### <a name="return-value"></a>반환 값  
 반환 값은 **SIMPLEREGION** (사각형 무효화 된 영역), **COMPLEXREGION** (무효화 된 비 직사각형 영역, 사각형 겹치는), 또는 **NULLREGION** (무효화 된 영역 없음)를 함수에 성공 하면 그렇지 않은 경우 반환 값은 **오류**합니다.  
  
### <a name="remarks"></a>주의  
 이 함수는는 [ScrollWindow](http://msdn.microsoft.com/library/windows/desktop/bb787591) 함수에 몇 가지 추가 기능입니다.  
  
 경우 [SW_INVALIDATE](http://msdn.microsoft.com/library/windows/desktop/bb787593) 및 [SW_ERASE](http://msdn.microsoft.com/library/windows/desktop/bb787593) 지정 되지 않은 경우는 `ScrollWindowEx` 멤버 함수에서 멀리 스크롤 하는 영역을 무효화 하지 않습니다. 이러한 플래그 중 하나가 설정 되 면 `ScrollWindowEx` 이 영역을 무효화 합니다. 영역 응용 프로그램이 호출 될 때까지 업데이트 되지 않습니다는 [UpdateWindow](http://msdn.microsoft.com/library/windows/desktop/dd145167) 멤버 함수를 호출은 [RedrawWindow](http://msdn.microsoft.com/library/windows/desktop/dd162911) 멤버 함수 (지정 [RDW_UPDATENOW](http://msdn.microsoft.com/library/windows/desktop/dd162911) 또는 [RDW_ERASENOW](http://msdn.microsoft.com/library/windows/desktop/dd162911)), 검색 또는 [WM_PAINT](http://msdn.microsoft.com/library/windows/desktop/dd145213) 응용 프로그램 큐에서 메시지입니다.  
 ...
 하는 경우는 [SW_SCROLLCHILDREN](http://msdn.microsoft.com/library/windows/desktop/bb787593) 플래그가 지정 되어, Windows 업데이트 되지 것입니다 제대로 화면 자식 창의 일부 스크롤됩니다. 소스 사각형 범위를 벗어난 스크롤된 자식 창의 부분 지워지지 및 활성 및 비활성 제대로 새 목적지에 합니다. 사용 하는 [DeferWindowPos](http://msdn.microsoft.com/library/windows/desktop/ms632681) Windows 함수 내에 완전히 벗어난다 이동 자식 창에는 `lpRectScroll` 사각형입니다. 커서 위치가 변경 되는 **SW_SCROLLCHILDREN** 플래그가 설정 되어 있고 캐럿 사각형이 스크롤 사각형을 교차 합니다.  
...
### <a name="return-value"></a>반환 값  
 자식 창에 부모;에 전송 되는 메시지를 처리 하는 0이 아닌 그렇지 않으면 0입니다.  
  
### <a name="remarks"></a>주의  
 `SendChildNotifyLastMsg`메시지가 반영 되는 경우 원본에 현재 메시지를 보냅니다.  
...
### <a name="parameters"></a>매개 변수  
 `nID`  
 메시지를 받을 수 있는 대화 상자 컨트롤의 식별자를 지정 합니다.  
...
### <a name="return-value"></a>반환 값  
 컨트롤을 찾지 못한 경우 컨트롤의 창 프로시저 또는 0에서 반환 된 값을 지정 합니다.  
  
### <a name="remarks"></a>주의  
 `SendDlgItemMessage` 멤버 함수는 메시지가 처리 될 때까지 반환 하지 않습니다.  
  
 사용 하 여 `SendDlgItemMessage` 획득을 위한 절차와 같습니다는 `CWnd`* 지정 된 컨트롤을 호출 하는 [SendMessage](#sendmessage) 멤버 함수입니다.  
...
### <a name="return-value"></a>반환 값  
 메시지 처리;의 결과 보낸 메시지에 해당 값에 따라 달라 집니다.  
  
### <a name="remarks"></a>주의  
 **SendMessage** 멤버 함수는 창 프로시저를 직접 호출 하 고 해당 창 프로시저에서 메시지 처리 될 때까지 반환 되지 않습니다. 대조적으로 이것이 [PostMessage](#postmessage) 멤버 함수를 창의 메시지 큐에 메시지를 배치 하 고 즉시 반환 합니다.  
...
### <a name="parameters"></a>매개 변수  
 ...
 `bOnlyPerm`  
 임시 windows에서 메시지를 받을지 여부를 지정 합니다. 경우 **TRUE**, 경우 임시 windows에서 메시지를 받을 수 **FALSE**, 영구 windows 메시지를 수신 합니다. 임시 windows에 대 한 자세한 내용은 참조 [기술 참고 3](../../mfc/tn003-mapping-of-windows-handles-to-objects.md)합니다.  
  
### <a name="remarks"></a>주의  
 경우 `bDeep` 는 **FALSE**, 창 바로 아래 자식에 전송 됩니다; 그렇지 않은 경우 모든 하위 창을 메시지가 전송 됩니다.  
  
 경우 `bDeep` 및 `bOnlyPerm` 는 **TRUE**, 임시 windows 아래 계속 검색 합니다. 이 경우에 영구 windows 검색 하는 동안 발생 한 메시지를 받습니다. 경우 `bDeep` 는 **FALSE**, 창 바로 아래 자식에만 전송 됩니다.  
...
### <a name="return-value"></a>반환 값  
 ...
 포인터가 반환 되 면 임시 수 있으며 나중에 사용할 저장 해야 합니다.  
...
### <a name="return-value"></a>반환 값  
 이전에 모든 마우스 입력을 수신 하는 창 개체에 대 한 포인터입니다. `NULL` 경우 이러한 창이 없습니다. 포인터가 반환 되 면 임시 수 있으며 나중에 사용할 저장 해야 합니다.  
...
### <a name="parameters"></a>매개 변수  
 `point`  
 새 x 및 y를 지정 합니다 (클라이언트 좌표로) 캐럿의 좌표입니다.  
  
### <a name="remarks"></a>주의  
 `SetCaretPos` 멤버 함수는 현재 작업의 창에서 소유 하는 경우에 캐럿 이동 합니다. `SetCaretPos`캐럿 숨겨지는지 여부 캐럿을 이동 합니다.  
  
 캐럿에는 공유 리소스입니다. 캐럿을 소유 하지 않은 경우 창이 캐럿을 이동 하지 않아야 합니다.  
...
### <a name="return-value"></a>반환 값  
 성공 하는 경우 클립보드 뷰어 체인의 다음 창에 대 한 핸들입니다. 응용 프로그램 (저장할 수 있습니다 멤버 변수로)이이 핸들을 저장 해야 클립보드 뷰어 체인 메시지에 응답할 때 사용 합니다.  
  
### <a name="remarks"></a>주의  
 클립보드 뷰어 체인의 일부인 창에 응답 해야 [WM_DRAWCLIPBOARD](#ondrawclipboard), [WM_CHANGECBCHAIN](#onchangecbchain), 및 [WM_DESTROY](#ondestroy) 체인의 다음 창에 메시지를 전달 하 고 메시지입니다.  
  
 이 멤버 함수는 `WM_DRAWCLIPBOARD` 창에 메시지입니다. 응용 프로그램 전달 해서는 안 되므로 클립보드 뷰어 체인의 다음 창에 대 한 핸들 아직 반환 되지 않은 `WM_DRAWCLIPBOARD` 호출 하는 동안 수신한 메시지 `SetClipboardViewer`합니다.  
...
### <a name="parameters"></a>매개 변수  
 `nID`  
 변경할 수 있는 컨트롤의 정수 ID를 지정 합니다.  
  
 `nValue`  
 항목 텍스트를 생성 하는 데 사용 하는 정수 값을 지정 합니다.  
  
 `bSigned`  
 정수 값은 서명 되거나 서명 되지 않은 있는지 여부를 지정 합니다. 이 매개 변수가 **TRUE**, `nValue` 서명 됩니다. 이 매개 변수가 **TRUE** 및 `nValue` 가 0 보다 작거나, 빼기 기호는 문자열에서 첫 번째 숫자 앞에 배치 됩니다. 이 매개 변수가 **FALSE**, `nValue` 서명 되지 않았습니다.  
  
### <a name="remarks"></a>주의  
 `SetDlgItemInt`보냅니다는 [WM_SETTEXT](http://msdn.microsoft.com/library/windows/desktop/ms632644) 메시지를 지정 된 컨트롤입니다.  
...
### <a name="parameters"></a>매개 변수  
 `nID`  
 설정할 텍스트가 포함 된 컨트롤을 식별 합니다.  
  
 `lpszString`  
 가리키는 [CString](../../atl-mfc-shared/reference/cstringt-class.md) 개체 또는 컨트롤에 복사 되도록 텍스트가 포함 된 null로 끝나는 문자열입니다.  
  
### <a name="remarks"></a>주의  
 `SetDlgItemText`보냅니다는 [WM_SETTEXT](http://msdn.microsoft.com/library/windows/desktop/ms632644) 메시지를 지정 된 컨트롤입니다.  
...
### <a name="remarks"></a>주의  
 키보드 입력 창에 전송 되 고 사용자에 대 한 다양 한 시각 신호 변경 됩니다. 전경 현재 사용자가 작업 창이입니다. 전경 창 최상위 창 (프레임 창 또는 대화 상자 상자)에 적용 됩니다.  
...
### <a name="return-value"></a>반환 값  
 이전에 입력된 포커스가 있던 창 개체에 대 한 포인터입니다. **NULL** 경우 이러한 창이 없습니다. 포인터가 반환 되 면 임시 수 있으며 저장 해야 합니다.  
  
### <a name="remarks"></a>주의  
 입력된 포커스가이 창에 대 한 모든 후속 키보드 입력을 지시합니다. 이전에 입력된 포커스가 있던 모든 창 것 손실 됩니다.  
  
 `SetFocus` 멤버 함수는 [WM_KILLFOCUS](http://msdn.microsoft.com/library/windows/desktop/ms646282) 입력된 포커스를 잃을 창에 메시지와 [WM_SETFOCUS](http://msdn.microsoft.com/library/windows/desktop/ms646283) 입력 포커스가 있는 창에 메시지를 합니다. 또한 창 또는 해당 부모를 활성화합니다.  
...
### <a name="parameters"></a>매개 변수  
 ...
 `bRedraw`  
 `TRUE`후 즉시 다시 그리게 합니다 창에 대 한 처리는 `WM_SETFONT` 메시지; 그렇지 않으면 `FALSE`합니다.  
...
### <a name="parameters"></a>매개 변수  
 `crKey`  
 에 대 한 포인터는 **COLORREF** 계층화 된 창을 작성할 때 사용 되는 투명 색상 키를 지정 하는 값입니다. 이 색의 창으로 그린 모든 픽셀에 투명 하 게 됩니다. 생성 하는 **COLORREF**, RGB 매크로 사용 합니다.  
  
 `bAlpha`  
 계층화 된 창의 불투명도 설명 하는 데는 알파 값입니다. 자세한 내용은 참조는 **SourceConstantAlpha** 의 멤버는 [BLENDFUNCTION](http://msdn.microsoft.com/library/windows/desktop/dd183393) 구조입니다. 때 `bAlpha` 은 0, 창은 완전히 투명 합니다. 때 `bAlpha` 는 255 자, 기간은 불투명 합니다.  
  
 `dwFlags`  
 수행할 동작을 지정 합니다. 이 매개 변수는 다음 값 중 하나 이상을 수 있습니다. 가능한 값 목록은 참조 하십시오. [SetLayeredWindowAttributes](http://msdn.microsoft.com/library/windows/desktop/ms633540)합니다.  
...
### <a name="remarks"></a>주의  
 ...
 `SetMenu`이전 메뉴를 삭제 하지 않습니다. 응용 프로그램에서 호출 해야는 [CMenu::DestroyMenu](../../mfc/reference/cmenu-class.md#destroymenu) 멤버 함수를이 작업을 수행 합니다.  
...
### <a name="parameters"></a>매개 변수  
 *pOwnerWnd*  
 Window 개체의 새 소유자를 식별합니다. 이 매개 변수가 **NULL**, 창 개체에 소유자가 없는 합니다.  
  
### <a name="remarks"></a>주의  
 그런 다음이 소유자는 현재 창 개체에서 명령 메시지를 받을 수 있습니다. 기본적으로 현재 창의 부모에 해당 소유자입니다.  
  
 종종 창 계층 구조와 관련이 없는 창 개체 간의 연결을 설정 하는 것이 유용 합니다. 예를 들어 [CToolBar](../../mfc/reference/ctoolbar-class.md) 대신 부모에 해당 소유자에 게 알림을 전송 합니다. 따라서 도구 모음을 (예: 내부 프레임 창) 다른 창에 알림을 보내는 동안 하나의 창 (예: OLE 컨테이너 응용 프로그램 창)의 자식 될 수 있습니다. 또한 서버 창이 비활성화 되거나 내부 중 활성화 하는 경우 편집, 프레임 창을 소유 하는 모든 창 숨기 거 나 표시 합니다. 이 소유권을 호출 하 여 명시적으로 설정 되어 `SetOwner`합니다.  
...
### <a name="return-value"></a>반환 값  
 성공 하면 이전 부모 창 개체에 대 한 포인터입니다. 포인터가 반환 되 면 임시 수 있으며 나중에 사용할 저장 해야 합니다.  
...
> [!NOTE]
>  에 대해서만 호출 해야 하는이 함수는 `CWnd` OLE 컨트롤을 나타내는 개체입니다.  
...
### <a name="parameters"></a>매개 변수  
 `bRedraw`  
 다시 그리기 플래그의 상태를 지정합니다. 이 매개 변수가 **TRUE**, 하는 경우 다시 그리기 플래그가 설정 되 고; **FALSE**, 플래그의 선택을 취소 합니다.  
  
### <a name="remarks"></a>주의  
 이 멤버 함수를 설정 하거나 다시 그리기 플래그를 지웁니다. 다시 그리기 플래그가 해제 되는 동안 각 변경 후 업데이트 되지 않습니다 내용과 다시 그리기 플래그가 설정 되지 않으면 다시 되지 않습니다. 예를 들어 목록 상자에 여러 항목을 추가 해야 하는 응용 프로그램 수 다시 그리기 플래그를 지우도록, 항목을 추가 하 고 다시 그리기 플래그를 설정 합니다. 마지막으로, 응용 프로그램이 호출할 수는 [Invalidate](#invalidate) 또는 [InvalidateRect](#invalidaterect) 멤버 함수를 목록 상자를 다시 발생 합니다.  
...
### <a name="parameters"></a>매개 변수  
 `nBar`  
 스크롤 막대 컨트롤 또는 창에서 비클라이언트 영역의 일부 인지 여부를 지정 합니다. 비클라이언트 영역의 일부 이며 이면 nBar 가로, 세로 스크롤 막대를 배치 되는 여부 또는 둘 다도 나타냅니다. 다음 중 하나 여야 합니다.  
  
- **SB_CTL** 스크롤 막대 컨트롤에 대 한 매개 변수를 포함 합니다. `m_hWnd` 데이터 멤버는 스크롤 막대 컨트롤의 핸들 이어야 합니다.  
  
- **SB_HORZ** 창의 가로 스크롤 막대를 임을 지정 합니다.  
  
- **SB_VERT** 창에 세로 스크롤 막대가 임을 지정 합니다.  
...
### <a name="remarks"></a>주의  
 ...
 MFC Windows 메시지 처리기를 나타내는 스크롤 막대 위치 [CWnd::OnHScroll](#onhscroll) 및 [CWnd::OnVScroll](#onvscroll), 16 비트만 위치 데이터를 제공 합니다. [GetScrollInfo](#getscrollinfo) 및 `SetScrollInfo` 32 비트의 스크롤 막대의 위치 데이터를 제공 합니다. 따라서 응용 프로그램이 호출할 수 `GetScrollInfo` 중 하나를 처리 하는 동안 `CWnd::OnHScroll` 또는 `CWnd::OnVScroll` 32 비트 스크롤 막대의 위치 데이터를 가져옵니다.  
...
- **SB_VERT** 창, 세로 스크롤 막대의 스크롤 상자 위치를 설정 합니다.  
  
 `nPos`  
 새 스크롤 상자 위치를 지정합니다. 스크롤 사이 여야 합니다.  
  
 `bRedraw`  
 새 스크롤 상자 위치를 반영 하도록 스크롤 막대를 그릴지 여부를 지정 합니다. 이 매개 변수가 **TRUE**, 스크롤 막대가 다시 그려집니다; 경우 **FALSE**, 스크롤 막대 하지 다시 그려집니다.  
...
- **SB_VERT** 창의 세로 스크롤 막대의 범위를 설정 합니다.  
 ...
 `bRedraw`  
 변경 내용을 반영 스크롤 막대가 다시 그려져 야 하는지 여부를 지정 합니다. 경우 `bRedraw` 는 **TRUE**, 스크롤 막대를 다시 그릴; 경우 **FALSE**, 스크롤 막대를 다시 그려지지 않습니다.  
  
### <a name="remarks"></a>주의  
 표준 스크롤 막대를 표시 하거나 숨기려면 데도 수 있습니다.  
  
 응용 프로그램을 스크롤 막대 알림 메시지를 처리 하는 동안 스크롤 막대를 숨기려면이 함수를 호출 해야 합니다.  
  
 경우에 대 한 호출 `SetScrollRange` 에 대 한 호출 바로 뒤에 [SetScrollPos](#setscrollpos) 멤버 함수는 `bRedraw` 매개 변수는 `SetScrollPos` 멤버 함수에는 스크롤 막대를 두 번 그려지지 않도록 하려면 0 이어야 합니다.  
...
### <a name="parameters"></a>매개 변수  
 `nIDEvent`  
 0이 아닌 타이머 식별자를 지정합니다. 이 동일한 값이 반환한 타이머 식별자가 고유, `SetTimer`합니다. 그렇지 않으면 `SetTimer` 새로운 고유 값을 결정 하 고는 반환 합니다. (여기에 NULL이 콜백 함수를 포함) 창 타이머 값 현재 창에 연관 된 다른 windows 타이머에 대해서만 고유 해야 합니다. 콜백 타이머 값은 모든 프로세스의 모든 타이머에 대해 고유 해야 합니다. 따라서 콜백 타이머를 만들 때 더 많습니다 반환 되는 값 지정 하는 값에서 다를 수 있습니다.  
  
 `nElapse`  
 제한 시간 값 또는 기간을 밀리초 단위로 지정합니다.  
  
 `lpfnTimer`  
 응용 프로그램에서 제공 하는의 주소를 지정 `TimerProc` 를 처리 하는 콜백 함수는 [WM_TIMER](http://msdn.microsoft.com/library/windows/desktop/ms644902) 메시지입니다. 이 매개 변수가 `NULL`, `WM_TIMER` 메시지가 응용 프로그램의 메시지 큐에 배치 되 고 처리는 `CWnd` 개체입니다.  
  
### <a name="return-value"></a>반환 값  
 함수가 성공 하면 새 타이머의 타이머 식별자입니다. 이 값 이거나를 통해 전달 된 값과 같은 아닐는 `nIDEvent` 매개 변수입니다. 응용 프로그램의 반환 값을 전달할지 항상는 [KillTimer](#killtimer) 멤버 함수는 타이머를 중지 해야 합니다. 성공 하면 0이 아니고 그렇지 않으면 0입니다.  
  
### <a name="remarks"></a>주의  
 간격 값을 지정 하 고 간격이 경과 될 때마다 시스템이 게시는 `WM_TIMER` 설치 응용 프로그램의 설치 메시지 큐에 메시지를 응용 프로그램 정의 메시지 전달 또는 `TimerProc` 콜백 함수입니다.  
  
 `lpfnTimer` 콜백 함수 이름을 지정 해야 `TimerProc`, 하지만 선언 되어야 정적으로 정의 된 다음과 같습니다.  
...
### <a name="example"></a>예제  
 이 예에서는 `CWnd::SetTimer`, `CWnd::OnTimer`, 및 `CWnd::KillTimer` 처리 하도록 `WM_TIMER` 메시지입니다. 첫 번째 타이머가 보낼 수 있도록 설정 되어는 `WM_TIMER` 메시지를 주 프레임 창에 2 초 마다 `OnStartTimer`합니다. `OnTimer` 이벤트 처리기 핸들 `WM_TIMER` 주 프레임 창에 대 한 메시지입니다. 이 메서드를 사용 하면 PC 스피커 2 초 마다 경고음을 합니다. 두 번째 타이머 메시지를 보냅니다 콜백 함수에 3.75 초 마다. `OnStopTimer`호출 하 여 두 타이머를 중지 됩니다 `CWnd::KillTimer` 각 타이머 id  
...
### <a name="remarks"></a>주의  
 자식 창 도움말 컨텍스트 식별자가 없으면 해당 부모 창의 식별자를 상속 합니다. 마찬가지로, 소유 된 창을 도움말 컨텍스트 식별자가 없으면 해당 소유자 창의 식별자를 상속 합니다. 이 상속 도움말 컨텍스트 식별자의 대화 상자에 대 한 하나의 식별자와 모든 해당 컨트롤을 설정할 수 있습니다.  
...
### <a name="parameters"></a>매개 변수  
 `lpwndpl`  
 가리키는 [WINDOWPLACEMENT](../../mfc/reference/windowplacement-structure.md) 새로운 쇼 상태와 위치를 지정 하는 구조입니다.  
...
### <a name="parameters"></a>매개 변수  
 `pWndInsertAfter`  
 식별 하는 `CWnd` 앞에 오는 개체 (보다 더 높을)이 `CWnd` Z 순서에서 개체입니다. 이 매개 변수는에 대 한 일 수는 `CWnd` 또는 **포인터** 다음 값 중 하나에 있습니다.  
  
- **wndBottom** 창을 Z 순서 맨 아래에 배치 합니다. 이 경우 `CWnd` 는 맨 위에 있는 창, 창 맨 위에 있는 상태를 잃을; 창의 다른 모든 창 맨 아래에 배치 합니다.  
  
- **wndTop** Z 순서 맨 위에 있는 창을 배치 합니다.  
  
- **wndTopMost** 모든 최상위 비 windows 창을 배치 합니다. 비활성화 된 경우에 창 맨 위에 있는 위치를 유지 관리 합니다.  
  
- **wndNoTopMost** 창 맨 위에 있는 아닌 모든 창의 맨 위에 위치를 변경 (즉, 모든 최상위 창 뒤). 이 플래그는 창 맨 위 창 이미 있으면 효과가 없습니다.  
  
 이 매개 변수를 사용 하는 방법에 대 한 규칙의 경우이 항목의 "주의" 섹션을 참조 하십시오.  
...
- `SWP_NOACTIVATE`창을 활성화 하지 않습니다. 창 활성화 및 맨 위 또는 맨 위 그룹의 맨 위로 이동이 플래그가 설정 되지 않은 경우 (의 설정에 따라는 `pWndInsertAfter` 매개 변수).  
...
- **SWP_NOREDRAW** 변경 내용을 다시 그려지지 않습니다. 이 플래그를 설정 하는 경우 모든 종류의 없습니다 다시 발생 합니다. 클라이언트 영역, 비클라이언트 영역 (제목 및 스크롤 막대가 포함) 및 부모 창 이동된 창의 결과로 처리 되지 않는 부분이이 적용 됩니다. 이 플래그를 설정 하는 경우 응용 프로그램 무효화 하거나 다시 그려야 하는 부모 창을 확인 하 고 창을의 모든 부분을 다시 그릴 명시적으로 해야 합니다.  
  
- **SWP_NOREPOSITION** 동일 **SWP_NOOWNERZORDER**합니다.  
  
- **SWP_NOSENDCHANGING** 창 받지 못하게에서 방지는 `WM_WINDOWPOSCHANGING` 메시지입니다.  
  
- `SWP_NOSIZE`현재 크기를 유지 (무시는 `cx` 및 `cy` 매개 변수).  
  
- `SWP_NOZORDER`현재 순서를 유지 하면서 (무시 `pWndInsertAfter`).  
...
### <a name="remarks"></a>주의  
 Z 순서;에 따라 화면에서 windows 순서 Z 순서 맨 위에 있는 창 순서 대로 다른 모든 창의 맨 위에 나타납니다.  
  
 자식 창에 대 한 모든 좌표는 클라이언트 좌표 (기준으로 상위 창의 클라이언트 영역의 왼쪽 위 모퉁이).  
  
 창을 Z 순서의 맨 위로 이동 될 수 있습니다 설정 하거나는 `pWndInsertAfter` 매개 변수를 **wndTopMost < /** 원활 하 고 `SWP_NOZORDER` 플래그는 하지 설정 하거나 설정 하는 창의 Z 순서 맨 위에 있는 기존 창 위에 있도록 합니다. 맨 위 창 맨 위에 있는 수행 되 면 소유 된 창 맨 위에 있는 이루어집니다. 해당 소유자가 변경 되지 않습니다.  
  
 맨 위 창 아래쪽에 위치가 변경 되 면 맨 위를 더 이상 ( **wndBottom / /**) 맨 위 창 이후인의 Z 순서입니다. 맨 위 창 맨 위 수행 되 면 맨 위에 있는 비 windows도 변경 됩니다 모든 해당 소유자 및 소유 된 창.  
  
 모두 `SWP_NOACTIVATE` 나 `SWP_NOZORDER` 때 지정 됩니다 (즉, 창이 활성화 되 고 지정된 된 Z 순서에 동시에 되기 요청 하는 응용 프로그램)에 지정 된 값 `pWndInsertAfter` 다음과 같은 경우에만 사용 됩니다.  
  
-   모두 **wndTopMost / /** 나 **wndNoTopMost / /** 에 지정 된는 `pWndInsertAfter` 매개 변수입니다.  
  
-   이 창이 활성 창이 아닙니다.  
  
 응용 프로그램에 표시 하지 않고 또한 것의 Z 순서 맨 위에 비활성 창을 활성화할 수 없습니다. 응용 프로그램 제한 없이 활성화 된 창의 Z 순서를 변경할 수 있습니다.  
  
 맨 위 창 맨 위에 있는 창에서 소유할 수 있지만 반대의 경우는 가능 하지 않습니다. 맨 위 창 소유한 모든 창 (예를 들어 대화 상자)에서는 소유한 모든 windows 소유자 위에 유지 되도록 맨 위 창에 수행 됩니다.  
  
 Windows 버전 3.1 이상에 windows Z 순서 맨 위로 이동 하 고 수를 설정 하 여 있습니다 잠긴 자신의 **WS_EX_TOPMOST** 스타일입니다. 최상위 창 맨 위에 있는 위치로 비활성화 하는 경우에 유지 관리 합니다. 예를 들어 WinHelp 항상 위 명령을 선택 하면, 맨 위에 있는 도움말 창 고 만든 후 응용 프로그램에 반환 하는 경우 계속 표시 됩니다.  
  
 맨 위에 있는 창을 만들기 위해 호출 `SetWindowPos` 와 `pWndInsertAfter` 매개 변수를 **wndTopMost / /**, 설정 또는 **WS_EX_TOPMOST** 창을 만들 때 스타일입니다.  
  
 Z 순서와 모든 windows를 포함 하는 경우는 **WS_EX_TOPMOST** 스타일을 사용 하 여 이동 창은 **wndTopMost / /** 값이 모든 최상위 창이 모든 맨 위 창의 맨에 배치 됩니다. 응용 프로그램 없이 비활성 창을 활성화 하는 경우는 **WS_EX_TOPMOST** bit는 창이 이동 하는 모든 맨 위 창 위에 하지만 모든 맨 위 창 아래입니다.  
  
 경우 `SetWindowPos` 될 때 호출 되는 `pWndInsertAfter` 매개 변수는 **wndBottom < /** 및 `CWnd` 는 최상위 창 창 맨 위에 있는 상태를 잃을 ( **WS_EX_TOPMOST** 선택이 취소 되어), 창을 Z 순서 맨 아래에 배치 하 고 합니다.  
...
### <a name="parameters"></a>매개 변수  
 ...
 `bRedraw`  
 경우 **TRUE**, 운영 체제를 창 영역을 설정한 후 다시 그립니다; 그렇지 않으면 그렇지 않은 경우. 일반적으로 설정 `bRedraw` 를 **TRUE** 창이 표시 되는 경우. 경우로 설정 **TRUE**, 시스템 보냅니다는 `WM_WINDOWPOSCHANGING` 및 `WM_WINDOWPOSCHANGED` 메시지를 창입니다.  
  
### <a name="return-value"></a>반환 값  
 함수가 성공 하면 반환 값은&0;이 아닌 값입니다. 함수가 실패하면 반환 값은&0;입니다.  
...
### <a name="remarks"></a>주의  
 ...
 `ShowCaret` 멤버 함수 현재 모양이 및 하지 숨겨진 두 번 이상 연속적으로 하는 경우에 캐럿을 표시 합니다. 이 창에서 캐럿을 소유 하지 않은 경우에 캐럿 표시 되지 않습니다.  
  
 캐럿을 숨기기 누적 됩니다. 하는 경우는 [HideCaret](#hidecaret) 멤버 함수를 호출한&5; 회 연속으로 `ShowCaret` 캐럿을 표시 하려면&5; 회 호출 되어야 합니다.  
  
 캐럿에는 공유 리소스입니다. 창 입력된 포커스를가지고 것 또는 활성화 된 경우에 표시 되는 캐럿을 표시 되어야 합니다.  
...
### <a name="parameters"></a>매개 변수  
 `bShow`  
 팝업 창을 표시 하거나 숨길 수 있는지 여부를 지정 합니다. 이 매개 변수가 **TRUE**, 모든 숨겨진된 팝업 창이 표시 됩니다. 이 매개 변수가 **FALSE**, 표시 된 모든 팝업 창을 숨겨집니다.  
...
### <a name="parameters"></a>매개 변수  
 `nBar`  
 스크롤 막대 컨트롤 또는 창에서 비클라이언트 영역의 일부 인지 여부를 지정 합니다. 비클라이언트 영역의 일부인 경우 `nBar` 도 가로, 세로 스크롤 막대를 배치 되는 여부 또는 둘 모두를 나타냅니다. 다음 중 하나 여야 합니다.  
...
- **SB_VERT** 창에 세로 스크롤 막대가 임을 지정 합니다.  
 ...
 `bShow`  
 Windows 표시 스크롤 막대를 숨기 하는지 여부를 지정 합니다. 이 매개 변수가 **TRUE**, 스크롤 막대가 표시 되는지, 스크롤 막대 그러지 않으면 숨겨집니다.  
...
### <a name="return-value"></a>반환 값  
 이전에 표시 되는 창 되었으면 0이 아닌 인 경우 0은 `CWnd` 이전에 숨겨져 있습니다.  
...
### <a name="remarks"></a>주의  
 Windows 메시지를 통해 라우팅됩니다 컨트롤을 동적으로 서브 클래스로 지정 된 경우는 `CWnd`의 지도 메시지 및 메시지 처리기에서 호출 된 `CWnd`먼저 클래스의 합니다. 메시지의 기본 클래스에 전달 되는 컨트롤의 기본 메시지 처리기로 전달 됩니다.  
  
 이 멤버 함수를 Windows 컨트롤을 연결 된 `CWnd` 개체를 컨트롤의 대체 **WndProc** 및 **AfxWndProc** 함수입니다. 함수가 저장 이전 **WndProc** 에서 반환 되는 위치에는 `GetSuperWndProcAddr` 멤버 함수입니다.  
...
### <a name="remarks"></a>주의  
 Windows 메시지를 통해 라우팅됩니다 창을 동적으로 서브 클래스로 지정 된 경우는 `CWnd`의 지도 메시지 및 메시지 처리기 호출 하는 `CWnd`먼저 클래스의 합니다. 기본 클래스에 전달 되는 메시지 창에는 기본 메시지 처리기에 전달 됩니다.  
  
 이 멤버 함수를 Windows 컨트롤을 연결 된 `CWnd` 개체를 대체 창의 **WndProc** 및 **AfxWndProc** 함수. 이전에 대 한 포인터를 저장 하는 함수 **WndProc** 에 `CWnd` 개체입니다.  
  
> [!NOTE]
>  창 해야 이미에 연결 되어 있지는 MFC 개체가이 함수가 호출 될 때입니다.  
...
### <a name="parameters"></a>매개 변수  
 `bSaveAndValidate`  
 대화 상자 초기화 되 고 있는지 여부를 나타내는 플래그입니다 ( **FALSE**) 또는 데이터를 검색 하는 ( **TRUE**).  
  
### <a name="return-value"></a>반환 값  
 작업에 성공 하면 0이 아닌 그렇지 않으면 0입니다. 경우 *bSaveAndValidat*e는 **TRUE**, 다음 반환 값이&0;이 아닌 값 이면 데이터의 유효성이 성공적으로 검사 됩니다.  
...
### <a name="parameters"></a>매개 변수  
 `pTarget`  
 응용 프로그램의 주 프레임 창 하 고 업데이트 메시지 라우팅에 사용 됩니다 *합니다.*  
  
 `bDisableIfNoHndler`  
 업데이트 처리기가 있는 컨트롤 함으로 자동으로 표시할지 여부를 나타내는 플래그입니다.  
  
### <a name="remarks"></a>주의  
 자식 컨트롤에 처리기가 없는 경우 및 `bDisableIfNoHndler` 는 **TRUE**, 자식 컨트롤을 사용할 수 없습니다.  
...
### <a name="parameters"></a>매개 변수  
 `pDCDst`  
 화면에 대 한 장치 컨텍스트 포인터입니다. 창의 내용이 업데이트 되 면 팔레트 색 일치에 사용 됩니다. 경우 `pDCDst` 는 **NULL**, 기본 색상표 사용 됩니다.  
 ...
 *psize*  
 에 대 한 포인터는 **크기** 계층화 된 창의 새 크기를 지정 하는 구조입니다. 창의 크기를 변경 되지 않는 경우 *psize* 수 **NULL**합니다.  
  
 If `pDCSrc` is **NULL**, *psize* must be **NULL**.  
  
 `pDCSrc`  
 계층화 된 창을 정의 하는 화면에 대 한 DC에 대 한 포인터입니다. 모양 및 창의 시각적 내용을 변경 하지 않는 경우 `pDCSrc` 수 **NULL**합니다.  
 ...
 *pblend*  
 에 대 한 포인터는 [BLENDFUNCTION](http://msdn.microsoft.com/library/windows/desktop/dd183393) 계층화 된 창을 작성할 때 사용 되는 투명도 값을 지정 하는 구조입니다.  
  
 `dwFlags`  
 수행할 동작을 지정 합니다. 이 매개 변수는 다음 값 중 하나 이상을 수 있습니다. 가능한 값 목록은 참조 하십시오. [UpdateLayeredWindow](http://msdn.microsoft.com/library/windows/desktop/ms633556)합니다.  
...
### <a name="remarks"></a>주의  
 `UpdateWindow` 멤버 함수는 `WM_PAINT` 메시지를 직접 응용 프로그램 큐를 무시 합니다. 업데이트 영역 비어 있는 경우 `WM_PAINT` 전송 되지 않습니다.  
...
### <a name="parameters"></a>매개 변수  
 `lpRect`  
 가리키는 [CRect](../../atl-mfc-shared/reference/crect-class.md) 개체 또는 [RECT 구조체](../../mfc/reference/rect-structure1.md) 사각형의 업데이트 영역에서 제거 될 클라이언트 좌표가 포함 된 합니다. 경우 `lpRect` 는 **NULL**, 창의 전체 유효성을 검사 합니다.  
  
### <a name="remarks"></a>주의  
 [BeginPaint](#beginpaint) 멤버 함수는 전체 클라이언트 영역에 자동으로 유효성을 검사 합니다. 모두는 `ValidateRect` 또는 [ValidateRgn](#validatergn) 의 업데이트 영역 일부 하기 전에 유효성을 검사 해야 하는 경우 멤버 함수를 호출 해야 [WM_PAINT](http://msdn.microsoft.com/library/windows/desktop/dd145213) 다음 생성 됩니다.  
...
### <a name="remarks"></a>주의  
 특정된 지역의 만들어야 이전에 지역 함수로 합니다. 지역 좌표는 클라이언트 좌표로 것으로 간주 됩니다.  
...
### <a name="return-value"></a>반환 값  
 정책은는 창 개체에 대 한 포인터입니다. **NULL** 창이 지정된 된 지점에 있는 경우. 포인터가 반환 되 면 임시 수 있으며 나중에 사용할 저장 해야 합니다.  
...
### <a name="parameters"></a>매개 변수  
 ...
 `nCmd`  
 요청한 도움말의 형식을 지정합니다. 가능한 값 목록과 미치는 영향에 대 한는 `dwData` 매개 변수를 참조는 [WinHelp](http://msdn.microsoft.com/library/windows/desktop/bb762267) Windows 함수는 [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
