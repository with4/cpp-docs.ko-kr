---
title: "CMFCVisualManagerWindows7 클래스 | Microsoft 문서"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
|`CMFCVisualManagerWindows7::DrawNcBtn`|프레임에 비클라이언트 영역에서 단추를 그립니다. 프레임 워크 사용 하 여가이 메서드를 그릴 최소화, 최대화을 종료 하 고 창 프레임의 오른쪽 위 모퉁이의 단추를 복원 합니다. 프로그램은 비 Aero 테마를 사용 하는 경우이 메서드가 호출 되지 됩니다.|  
...
|`CMFCVisualManagerWindows7::GetRibbonBar`|검색은 [CMFCRibbonBar 클래스](../../mfc/reference/cmfcribbonbar-class.md) 사용자 인터페이스와 연결 합니다.|  
...
|`CMFCVisualManagerWindows7::IsRibbonPresent`|확인 여부는 `CMFCRibbonBar` 이 존재 하며 표시 합니다.|  
...
|[CMFCVisualManagerWindows7::OnFillMenuImageRect](#onfillmenuimagerect)|메뉴 항목 이미지 영역을 채우도록 때이 메서드를 호출 하는 프레임 워크입니다.|  
...
|`CMFCVisualManagerWindows7::SetStyle`|설정의 색 구성표는 `CMFCVisualManagerWindows7` GUI입니다.|  
  
## <a name="remarks"></a>주의  
 사용 된 `CMFCVisualManagerWindows7` 기본 모방 하기 위해 응용 프로그램의 모양을 변경 하는 클래스 [!INCLUDE[win7](../../build/includes/win7_md.md)] 응용 프로그램입니다. 이 클래스는 버전의 Windows에서 응용 프로그램이 실행 중인 경우 올바르지 않을 이전의 [!INCLUDE[win7](../../build/includes/win7_md.md)]합니다. 이 시나리오에서는 응용 프로그램에 정의 된 기본 비주얼 관리자를 사용 [CMFCVisualManager](../../mfc/reference/cmfcvisualmanager-class.md)합니다.  
  
 CMFCVisualManagerWindows7 모두에서 여러 메서드를 상속 된 [CMFCVisualManagerWindows 클래스](../../mfc/reference/cmfcvisualmanagerwindows-class.md) 및 `CMFCVisualManager` 클래스입니다. 이전 섹션에 나열 된 메서드는 메서드를 처음 사용 하 여 `CMFCVisualManagerWindows7` 클래스입니다.  
...
### <a name="parameters"></a>매개 변수  
 ...
 반환 `TRUE` 리본 패널 있으면 `pEdit` 강조 표시 됩니다.  
...
##  <a name="a-nameonfillmenuimagerecta--cmfcvisualmanagerwindows7onfillmenuimagerect"></a><a name="onfillmenuimagerect"></a>CMFCVisualManagerWindows7::OnFillMenuImageRect  
 메뉴 항목 이미지 영역을 채우도록 때이 메서드를 호출 하는 프레임 워크입니다.  
...
### <a name="parameters"></a>매개 변수  
 [in] `pDC`  
 메뉴 단추의 장치 컨텍스트에 대 한 포인터입니다.  
  
 [in] `pButton`  
 에 대 한 포인터는 `CMFCToolBarButton`합니다. 프레임 워크에는이 단추에 대 한 배경을 채웁니다.  
