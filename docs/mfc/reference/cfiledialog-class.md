---
title: "CFileDialog 클래스 | Microsoft 문서"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
|[CFileDialog::AddPlace](#addplace)|오버로드됨. 목록에 폴더를 열고 항목을 저장 하거나 사용자에 대해 사용할 수 있는 배치를 추가 합니다.|  
...
|[CFileDialog::ApplyOFNToShellDialog](#applyofntoshelldialog)|상태를 업데이트는 `CFileDialog` 매개 변수 및에 저장 된 플래그와 일치 하는 `m_ofn` 멤버 변수입니다.|  
|[CFileDialog::DoModal](#domodal)|대화 상자를 표시 하 고 선택할 수 있습니다.|  
|[CFileDialog::EnableOpenDropDown](#enableopendropdown)|드롭다운 목록에서 사용 하도록 설정 된 **열려** 또는 **저장** 대화 상자에서 단추입니다.|  
|[CFileDialog::EndVisualGroup](#endvisualgroup)|요소를 시각적 그룹 대화 상자에서 추가 중지합니다.|  
|[CFileDialog::GetCheckButtonState](#getcheckbuttonstate)|대화 상자에서 확인 단추 (확인란)의 현재 상태를 가져옵니다.|  
|[CFileDialog::GetControlItemState](#getcontrolitemstate)|대화 상자에 컨테이너 컨트롤에 있는 항목의 현재 상태를 가져옵니다.|  
|[CFileDialog::GetControlState](#getcontrolstate)|지정된 된 컨트롤의 상태를 사용 하도록 설정 하 고 현재 표시 여부를 가져옵니다.|  
...
|[CFileDialog::GetFolderPath](#getfolderpath)|탐색기 스타일에 대 한 현재 열려 있는 폴더 또는 디렉터리의 경로 검색 **열고** 또는 **이름으로 저장** 일반 대화 상자입니다.|  
|[CFileDialog::GetIFileDialogCustomize](#getifiledialogcustomize)|사용자 지정에 대 한 내부 COM 개체를 검색 `CFileDialog` 개체입니다.|  
|[CFileDialog::GetIFileOpenDialog](#getifileopendialog)|에 대 한 내부 COM 개체를 검색 하는 `CFileDialog` 으로 사용 되는 한 **열려** 파일 대화 상자입니다.|  
|[CFileDialog::GetIFileSaveDialog](#getifilesavedialog)|에 대 한 내부 COM 개체를 검색 하는 `CFileDialog` 으로 사용 되는 한 **저장** 파일 대화 상자입니다.|  
...
|[CFileDialog::GetSelectedControlItem](#getselectedcontrolitem)|대화 상자에 지정 된 컨테이너 컨트롤에서 특정 항목을 가져옵니다.|  
|[CFileDialog::GetStartPosition](#getstartposition)|파일 이름 목록의 첫 번째 요소의 위치를 반환합니다.|  
|[CFileDialog::HideControl](#hidecontrol)|탐색기 스타일에서 지정된 된 컨트롤을 숨깁니다 **열려** 또는 **이름으로 저장** 일반 대화 상자입니다.|  
|[CFileDialog::IsPickFoldersMode](#ispickfoldersmode)|있는지 여부를 확인 폴더 선택 모드에서 현재 대화 상자입니다.|  
...
|[CFileDialog::SetControlItemState](#setcontrolitemstate)|대화 상자에는 컨테이너 컨트롤에서 항목의 현재 상태를 설정 합니다.|  
|[CFileDialog::SetControlItemText](#setcontrolitemtext)|컨트롤 항목의 텍스트를 설정합니다. 예를 들어, 라디오 단추 또는 메뉴에 항목을 함께 제공 되는 텍스트입니다.|  
|[CFileDialog::SetControlLabel](#setcontrollabel)|단추 텍스트는 편집 상자 레이블 등의 컨트롤에 연결 된 텍스트를 설정 합니다.|  
|[CFileDialog::SetControlState](#setcontrolstate)|현재 표시 여부를 설정 하 고 지정된 된 컨트롤의 상태를 사용 하도록 설정 합니다.|  
|[CFileDialog::SetControlText](#setcontroltext)|탐색기 스타일에 지정된 된 컨트롤에 대 한 텍스트 설정 **열려** 또는 **이름으로 저장** 일반 대화 상자입니다.|  
|[CFileDialog::SetDefExt](#setdefext)|탐색기 스타일에 대 한 기본 파일 이름 확장명 설정 **열려** 또는 **이름으로 저장** 일반 대화 상자입니다.|  
...
|[CFileDialog::SetSelectedControlItem](#setselectedcontrolitem)|옵션 단추 그룹 또는 대화 상자에 콤보 상자에서 특정 항목의 선택된 된 상태를 설정 합니다.|  
|[CFileDialog::SetTemplate](#settemplate)|설정에 대 한 대화 상자 템플릿에 `CFileDialog` 개체입니다.|  
|[CFileDialog::StartVisualGroup](#startvisualgroup)|대화 상자에서 시각적 그룹을 선언합니다. 이 그룹에 이러한 요소를 추가 하는 이후에 "add" 메서드를 호출 합니다.|  
...
## <a name="remarks"></a>주의  
 일반 파일 대화 상자를 사용 하면 파일 선택 대화 상자, 예를 들어를 구현할 수 **파일 열기** 및 **이름으로 저장**, Windows 표준와 일치 하는 방식에서입니다.  
  
 사용할 수 있습니다 `CFileDialog` 생성자를 제공 하는 있는 그대로 또는에서 고유한 대화 상자 클래스를 파생 시킬 수 있습니다 `CFileDialog` 요구 사항에 맞게 생성자 작성 하 고 있습니다. 두 경우 모두 이러한 대화 상자는 처럼 동작 표준 MFC 대화 상자에서 파생 되므로 [CCommonDialog 클래스](../../mfc/reference/ccommondialog-class.md)합니다. `CFileDialog`COMMDLG에 의존합니다. Windows에 포함 된 DLL 파일입니다.  
  
 모양 및 기능을 모두는 `CFileDialog` 와 [!INCLUDE[wiprlhext](../../c-runtime-library/reference/includes/wiprlhext_md.md)] 이전 버전의 Windows에서 다릅니다. 기본 `CFileDialog` 자동으로 새 사용 [!INCLUDE[wiprlhext](../../c-runtime-library/reference/includes/wiprlhext_md.md)] 컴파일되면 해당 프로그램은 경우에 코드 변경 및 실행 하지 않고 스타일 [!INCLUDE[wiprlhext](../../c-runtime-library/reference/includes/wiprlhext_md.md)]합니다. 사용 된 `bVistaStyle` 이 자동 업데이트를 수동으로 재정의 하는 생성자의 매개 변수입니다. 자동 업데이트 하는 예외에는 사용자 지정된 대화 상자입니다. 또한 새 스타일으로 변환 되지 않습니다. 생성자에 대 한 자세한 내용은 참조 [CFileDialog::CFileDialog](#cfiledialog)합니다.  
  
> [!NOTE]
>  컨트롤 ID 시스템에 다른 [!INCLUDE[wiprlhext](../../c-runtime-library/reference/includes/wiprlhext_md.md)] 에서 이전 버전의 Windows 사용 하는 경우는 `CFileDialog`합니다. 에 대 한 모든 참조를 업데이트 해야 `CFileDialog` 컨트롤 코드 전에 이전 버전의 Windows에서 프로젝트를 이식할 수 있습니다.  
  
 일부 `CFileDialog` 방법에서 지원 되지 않는 [!INCLUDE[wiprlhext](../../c-runtime-library/reference/includes/wiprlhext_md.md)]합니다. 메서드는 지원 여부에 대 한 정보에 대 한 개별 메서드 항목을 확인 합니다. 또한 다음 상속된 되는 함수 따라 지원 되지 않습니다 [!INCLUDE[wiprlhext](../../c-runtime-library/reference/includes/wiprlhext_md.md)]:  
  
- [CDialog::OnInitDialog](../../mfc/reference/cdialog-class.md#oninitdialog)  
  
- [CDialog::OnSetFont](../../mfc/reference/cdialog-class.md#onsetfont)  
  
 에 대 한 windows 메시지는 `CFileDialog` 클래스를 사용 하는 운영 체제에 따라 달라 집니다. 예를 들어 Windows XP에서는 [CDialog::OnCancel](../../mfc/reference/cdialog-class.md#oncancel) 및 [CDialog::OnOK](../../mfc/reference/cdialog-class.md#onok) 에 대 한는 `CFileDialog` 클래스입니다. 그러나 [!INCLUDE[wiprlhext](../../c-runtime-library/reference/includes/wiprlhext_md.md)] 지원지 않습니다. 생성 되는 다양 한 메시지는 수신 된 순서에 대 한 자세한 내용은 참조 [CFileDialog 샘플: 로깅 이벤트 순서](../../visual-cpp-samples.md)합니다.  
  
 사용 하는 `CFileDialog` 개체를 사용 하 여 개체를 먼저 만듭니다는 `CFileDialog` 생성자입니다. 대화 상자를 생성 한 후이 설정 하거나 모든 값을 수정할 수 있습니다는 [CFileDialog::m_ofn](#m_ofn) 구조에서 값 또는 대화 상자 컨트롤의 상태를 초기화 합니다. `m_ofn` 형식의 구조는 `OPENFILENAME`합니다. 자세한 내용은 참조는 [OPENFILENAME](http://msdn.microsoft.com/library/windows/desktop/ms646839) 구조에서 [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]합니다.  
  
 대화 상자 컨트롤을 초기화 한 후 호출 된 [CFileDialog::DoModal](#domodal) 대화 상자를 표시 하려면 상자를 경로 파일 이름을 입력할 수 있습니다. `DoModal`사용자 (IDOK) 확인 또는 취소 (IDCANCEL) 단추를 클릭 했는지 여부를 반환 합니다. 경우 `DoModal` IDOK 반환 중 하나를 사용할 수는 `CFileDialog` 사용자에 의해에 정보를 검색 하는 공용 멤버 함수를 저장 합니다.  
  
> [!NOTE]
>  아래에서 [!INCLUDE[wiprlhext](../../c-runtime-library/reference/includes/wiprlhext_md.md)]를 여러 번 호출 [IFileDialog::SetFileTypes](http://msdn.microsoft.com/library/windows/desktop/bb775980) 하면 오류가 발생 합니다. 두 번째 호출 `SetFileTypes` 의 모든 인스턴스에 대해는 `CFileDialog` 돌아옵니다 `E_UNEXPECTED` 에서 [!INCLUDE[wiprlhext](../../c-runtime-library/reference/includes/wiprlhext_md.md)]. 일부 `CFileDialog` 메서드 호출 함수 `SetFileTypes`합니다. 예를 들어 두 호출이 `CFileDialog::DoModal` 의 동일한 인스턴스에 대 한 한 `CFileDialog` 생성 [ASSERT](http://msdn.microsoft.com/library/1e70902d-d58c-4e7b-9f69-2aeb6cbe476c)합니다.  
  
 `CFileDialog`공유 위반, 파일 이름 유효성 검사 및 목록 상자 변경 알림을 사용자 지정 처리 작업을 수행할 수 있는 몇 가지 보호 된 멤버를 포함 합니다. 이러한 보호 된 멤버는 콜백 함수가 대부분의 응용 프로그램 기본 처리를 자동으로 수행 되기 때문에 사용할 필요가 없습니다. 이러한 함수에 대 한 메시지 맵 항목 표준 가상 함수를 있기 때문에 필요 하지 않습니다.  
 ...
 여러 파일을 선택 하는 사용자를 수 있게 하려면 설정의 `OFN_ALLOWMULTISELECT` 플래그를 호출 하기 전에 `DoModal`합니다. 여러 파일 이름의 반환된 목록에 맞게 고유한 파일 이름을 버퍼를 제공 해야 합니다. 대체 하 여이 작업을 수행 `m_ofn.lpstrFile` 버퍼에 대 한 포인터와 할당 된 경우, 생성 한 후의 `CFileDialog`를 호출 하기 전에 `DoModal`합니다.  
  
 또한 설정 해야 `m_ofn.nMaxFile` 가리키는 버퍼의 문자 수를 사용 하 여 `m_ofn.lpstrFile`합니다. 선택 하려면 파일의 최대 수를 설정 하는 경우 `n`, 필요한 버퍼 크기는 `n * (_MAX_PATH + 1) + 1`합니다. 버퍼에 반환 되는 첫 번째 항목은 선택한는 파일이 있는 폴더의 경로를 합니다. 에 대 한 [!INCLUDE[wiprlhext](../../c-runtime-library/reference/includes/wiprlhext_md.md)]-스타일 대화 상자, 디렉터리 및 파일 이름 문자열은 null로 종료 되는 문자로 null 후 마지막 파일 이름입니다. 이 형식의 공백을 포함 하는 긴 파일 이름을 반환 하려면 탐색기 스타일 대화 상자를 사용 합니다. 이전 스타일 대화 상자에 대 한 디렉터리와 파일 이름 문자열은 공백으로 및 함수는 공백을 사용 하 여 파일 이름에 대 한 짧은 파일 이름을 사용 합니다.  
  
 다음 예제에는 버퍼를 검색 하 여 여러 파일 이름 목록을 사용 하는 방법을 보여 줍니다.  
  
 [!code-cpp[NVC_MFCFiles&23;](../../atl-mfc-shared/reference/codesnippet/cpp/cfiledialog-class_1.cpp)]  
  
 여러 파일 이름을 선택 하 여 사용자에 대 한 응답으로 버퍼 크기를 변경 하려면에서 새 클래스를 파생 시켜야 합니다 `CFileDialog` 재정의 [CFileDialog::OnFileNameChange](#onfilenamechange) 메서드.  
  
 새 클래스를 파생 하는 경우 `CFileDialog`, 모든 메시지를 처리 한 메시지 맵을 사용할 수 있습니다. 기본 메시지 처리를 확장 하려면에서 클래스를 파생 `CFileDialog`, 새 클래스에 메시지 맵에 추가 하 고 새 메시지에 대 한 멤버 함수를 제공 합니다. 사용자 지정 대화 상자에 후크 함수를 제공할 필요가 없습니다.  
  
 대화 상자를 사용자 지정 하려면에서 클래스를 파생 `CFileDialog`한 사용자 지정 대화 상자 템플릿을 제공 하 고 확장 된 컨트롤에서 알림 메시지를 처리 한 메시지 맵을 추가 합니다. 기본 클래스에는 처리 되지 않은 메시지를 전달 합니다. 후크 함수를 사용자 지정할 필요가 없습니다.  
  
 사용 하는 경우는 [!INCLUDE[wiprlhext](../../c-runtime-library/reference/includes/wiprlhext_md.md)] 의 스타일은 `CFileDialog`, 메시지 맵 및 대화 상자 템플릿을 사용할 수 없습니다. 대신, 이와 유사한 기능에 대 한 COM 인터페이스를 사용 해야 합니다.  
...
### <a name="parameters"></a>매개 변수  
 ...
 `bChecked`  
 확인 단추의 현재 상태를 나타내는 부울 값입니다. `TRUE`선택 된 경우 `FALSE` 그렇지 않은 경우  
...
### <a name="remarks"></a>주의  
 이전 Windows 버전에서 [!INCLUDE[wiprlhext](../../c-runtime-library/reference/includes/wiprlhext_md.md)], 멤버 [OPENFILENAME](https://msdn.microsoft.com/library/ms911906.aspx) 데이터 구조의 상태와 지속적으로 동기화 된는 `CFileDialog`합니다. 변경 내용을 [m_ofn](#m_ofn) 멤버 변수 대화 상자 상태의에 즉시 반영 되었습니다. 또한 대화 상자의 상태 변경 내용을 즉시 업데이트 하는 `m_ofn` 멤버 변수입니다.  
  
 [!INCLUDE[wiprlhext](../../c-runtime-library/reference/includes/wiprlhext_md.md)]의 값은 `m_ofn` 멤버 변수 및 상태에는 `CFileDialog` 동기화 되도록 보장 되지 않습니다. 상태를 강제로이 함수는 `CFileDialog` 일치 하도록 업데이트 하는 `m_ofn` 구조입니다. 동안 자동으로이 함수를 호출 하는 Windows [CFileDialog::DoModal](#domodal)합니다.  
...
### <a name="parameters"></a>매개 변수  
 [in] `bOpenFileDialog`  
 어떤 유형의 대화 상자를 지정 하는 매개 변수입니다. 로 설정 `TRUE` 생성 하는 **파일 열기** 대화 상자입니다. 로 설정 `FALSE` 생성 하는 **다른 이름으로 저장** 대화 상자입니다.  
  
 [in] `lpszDefExt`  
 기본 파일 이름 확장명입니다. 확장에서 지정한 파일 이름 상자에 사용자 (사용자의 컴퓨터에 연결 하는 하나) 알려진된 확장명이 포함 되어 있지 않으면, `lpszDefExt` 파일 이름에 자동으로 추가 됩니다. 이 매개 변수가 `NULL`, 확장명이 추가 됩니다.  
  
 [in] `lpszFileName`  
 파일 이름 상자에 표시 되는 초기 파일 이름입니다. 경우 `NULL`, 초기 파일 이름은 표시 되지 않습니다.  
  
 [in] `dwFlags`  
 대화 상자를 사용자 지정 하는 데 사용할 수 있는 하나 이상의 플래그의 조합입니다. 이러한 플래그에 대 한 참조는 [OPENFILENAME](http://msdn.microsoft.com/library/windows/desktop/ms646839) 구조에서 [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]합니다. 수정 하는 경우는 `m_ofn.Flags` 구조 멤버, 기본 동작을 그대로 유지 하려면 변경에서 비트 OR 연산자를 사용 합니다.  
  
 [in] `lpszFilter`  
 일련의 필터를 지정 하는 문자열 쌍 파일에 적용할 수 있습니다. 파일 필터를 지정 하면 필터 조건과 일치 하는 파일에만 파일 목록에 표시 됩니다. 파일 필터를 사용 하는 방법에 대 한 자세한 내용은 설명 섹션을 참조 합니다.  
  
 [in] `pParentWnd`  
 파일 대화 상자의 부모 또는 소유자 창에 대 한 포인터입니다.  
  
 [in] `dwSize`  
 크기는 `OPENFILENAME` 구조입니다. 이 값은 운영 체제 버전에 따라 달라 집니다. MFC 대화 상자를 만들려면 적절 한 종류를 결정 하기 위해이 매개 변수를 사용 (예를 들어, 새로운 [!INCLUDE[Win2kFamily](../../c-runtime-library/includes/win2kfamily_md.md)] NT4 대화 상자 대신 대화 상자). MFC 코드를 사용 하 여 해당 대화 상자 크기를 결정 하는 0으로의 기본 크기는 프로그램이 실행 되는 운영 체제 버전에 기반 합니다.  
  
 [in] `bVistaStyle`  
 **참고** 이 매개 변수는 Visual Studio 2008에서 사용할 수 있는 이상에서 및을 실행 하는 경우에 사용할 새 스타일 대화 [!INCLUDE[wiprlhext](../../c-runtime-library/reference/includes/wiprlhext_md.md)] 이상.  
  
 파일 대화 상자의 스타일을 지정 하는 매개 변수입니다. 로 설정 `TRUE` 새 Vista 스타일 파일 대화 상자를 사용 하도록 합니다. 그렇지 않으면 대화 상자의 이전 스타일 사용 됩니다. Vista에서 실행에 대 한 자세한 내용은 설명 섹션을 참조 합니다.  
  
### <a name="remarks"></a>주의  
 어느 한 **파일 열기** 또는 **다른 이름으로 저장** 값에 따라 대화 상자를 생성 하는 `bOpenFileDialog`합니다.  
  
 사용 하 여 기본 확장 지정 `lpszDefExt` 있기 때문에 대부분의 경우 예측 가능한 어떤 확장 사용자의 컴퓨터에서 파일 연결에는 예상 된 동작을 얻을 수 없습니다. 고유한 클래스를 파생 시킬 수 더 많이 제어할 기본 확장명을 추가 해야 합니다 `CFileDialog`, 재정의 `CFileDialog::OnFileNameOK` 메서드를 고유한 확장 처리를 수행 하도록 합니다.  
  
 사용자가 여러 파일을 선택할 수 있도록 설정 된 `OFN_ALLOWMULTISELECT` 플래그를 호출 하기 전에 [DoModal](#domodal)합니다. 고유한 파일 이름을 버퍼 목록을 저장 하 고 반환 된 파일 이름 여러 개를 제공 해야 합니다. 대체 하 여이 작업을 수행 `m_ofn.lpstrFile` 버퍼에 대 한 포인터와 할당 된 경우, 생성 한 후의 [CFileDialog](../../mfc/reference/cfiledialog-class.md)를 호출 하기 전에 `DoModal`합니다. 또한 설정 해야 `m_ofn.nMaxFile` 가 가리키는 버퍼의 문자 수가 `m_ofn.lpstrFile`합니다. 선택 하려면 파일의 최대 수를 설정 하는 경우 `n`, 필요한 버퍼 크기는 `n`*(_MAX_PATH + 1) + 1입니다. 예:  
  
 [!code-cpp[NVC_MFCFiles&23;](../../atl-mfc-shared/reference/codesnippet/cpp/cfiledialog-class_1.cpp)]  
  
 마우스 또는 키보드를 사용 하 여 크기 조정 된 탐색기 스타일 대화 상자에 사용자를 사용 하도록 설정 하려면 설정의 `OFN_ENABLESIZING` 플래그입니다. 이 플래그를 설정 하는 것은 후크 프로시저 또는 사용자 지정 서식 파일을 제공 하는 경우에 필요 합니다. 탐색기 스타일 대화 상자; 에서만 작동 하는 플래그 이전 스타일 대화 상자 크기를 조정할 수 없습니다.  
  
 `lpszFilter` 파일 목록에 표시 되는 파일 있어야 하는 파일 이름의 형식을 확인할 매개 변수를 사용 합니다. 문자열 쌍의 첫 번째 문자열 필터;에 대해 설명 두 번째 문자열을 사용 하 여 파일 이름 확장명을 나타냅니다. 구분 기호로 세미콜론 (';' 문자)를 사용 하 여 여러 확장을 지정할 수 있습니다. 두 개의로 끝나는 문자열 ' |' 문자 다음에 `NULL` 문자입니다. 사용할 수도 있습니다는 [CString](../../atl-mfc-shared/using-cstring.md) 이 매개 변수에 대 한 개체입니다.  
  
 예를 들어 [!INCLUDE[ofprexcel](../../mfc/reference/includes/ofprexcel_md.md)] 특히 확장.xlc (차트) 또는 (워크시트).xls 파일을 열 수 있습니다. Excel에 대 한 필터는 다음과 같이 작성할 수 있습니다.  
  
 [!code-cpp[NVC_MFCFiles #&24;](../../atl-mfc-shared/reference/codesnippet/cpp/cfiledialog-class_2.cpp)]  
  
 그러나이 문자열을 직접 사용 하려는 경우 업데이트는 `OPENFILENAME` 구조를 문자열 대신 세로 막대에는 ' \0', null 문자로 구분 해야 ('| ').  
  
 `bVistaStyle` 매개 변수는 실행 하는 경우에 적용할 수 [!INCLUDE[wiprlhext](../../c-runtime-library/reference/includes/wiprlhext_md.md)]합니다. 이전 버전의 Windows에서이 매개 변수는 무시 됩니다. 경우 `bVistaStyle` 로 설정 된 `TRUE`사용 하 여 프로그램을 컴파일할 때 [!INCLUDE[vs_orcas_long](../../atl/reference/includes/vs_orcas_long_md.md)] 이상 버전에서는 새로운 Vista 스타일 **파일 대화 상자** 사용 됩니다. 그렇지 않으면 이전 MFC 스타일 **파일 대화 상자** 사용 됩니다.  
...
### <a name="remarks"></a>주의  
 ...
 예를 들어 사용자가 여러 파일을 선택 하도록 허용 하려는 경우 설정의 `OFN_ALLOWMULTISELECT` 플래그를 호출 하기 전에 `DoModal`이 항목의 코드 예제에서와 같이 합니다.  
  
 이 대화 상자의 확인 또는 취소 단추 또는 선택 되어 있는 닫기 옵션 대화 상자에서 사용자가 클릭 메뉴를 제어 하는 경우 컨트롤은 응용 프로그램에 반환 됩니다. 호출할 수 있습니다 설정이 나 정보를 검색 하는 다른 멤버 함수는 사용자 입력 대화 상자에 있습니다.  
  
 `DoModal`클래스에서 재정의 되는 가상 함수 `CDialog`합니다.  
...
### <a name="parameters"></a>매개 변수  
 ...
 `bChecked`  
 확인란의 상태입니다. `TRUE`확인 된; 나타냅니다. `FALSE` 선택 되지 않은 나타냅니다.  
...
### <a name="parameters"></a>매개 변수  
 ...
 `dwState`  
 컨트롤의 현재 상태를 나타내는 CDCONTROLSTATE 열거형에서 더 많은 값 중 하나를 수신 하는 변수 참조입니다.  
...
### <a name="parameters"></a>매개 변수  
 ...
 `dwState`  
 컨트롤의 현재 상태를 나타내는 CDCONTROLSTATE 열거형에서 하나 이상의 값을 수신 하는 변수 참조입니다.  
...
### <a name="remarks"></a>주의  
 파일의 이름에는 접두사와 확장명을 모두 포함 됩니다. 예를 들어 `GetFileName` "텍스트를 반환 합니다입니다. DAT C:\FILES\TEXT.DAT 파일에 대 한 "입니다.  
...
### <a name="remarks"></a>주의  
 파일의 제목에만 해당 접두사를 경로 또는 확장명 없이 포함 됩니다. 예를 들어 `GetFileTitle` C:\FILES\TEXT.DAT 파일에 대 한 "TEXT"를 반환 합니다.  
  
 경우 `m_ofn.Flags` 에 `OFN_ALLOWMULTISELECT` 플래그가 설정,이 문자열에 포함 되는 null로 끝나는 문자열을 선택 하는 파일 그룹의 디렉터리 경로 되 고 첫 번째 문자열의 시퀀스가 포함 된 사용자가 선택한 모든 파일의 이름입니다. 이러한 이유로 사용은 [GetStartPosition](#getstartposition) 및 [GetNextPathName](#getnextpathname) 목록에서 다음 파일 이름을 검색 하는 멤버 함수입니다.  
...
### <a name="remarks"></a>주의  
 대화 상자와 생성 되어 있어야는 **OFN_EXPLORER** 스타일; 메서드는 어설션으로 없으면 실패 합니다.  
...
### <a name="return-value"></a>반환 값  
 에 대 한 내부 COM 개체에 대 한 포인터는 `CFileDialog`합니다. 이 포인터를 적절 하 게 해제 하 여 작업은.  
  
### <a name="remarks"></a>주의  
 아래에이 함수를 사용 하 여 [!INCLUDE[wiprlhext](../../c-runtime-library/reference/includes/wiprlhext_md.md)] 가 있는 개체와 `bVistaStyle` 로 설정 `true`합니다. 이 함수를 사용 하는 경우 때 `bVistaStyle` 는 `false`를 반환 합니다 `NULL` 릴리스 모드가 고 디버그 모드에서 어설션 throw 합니다.  
  
 에 대 한 자세한 내용은 `IFileDialogCustomize` 인터페이스를 참조 [있게 해 주는 IFileDialogCustomize](http://msdn.microsoft.com/library/windows/desktop/bb775912)합니다.  
  
### <a name="example"></a>예제  
 이 예제에서는 내부 COM 개체를 검색 합니다. 이 코드 예제를 실행 하려면 컴파일해야 아래 [!INCLUDE[wiprlhext](../../c-runtime-library/reference/includes/wiprlhext_md.md)]합니다.  
...
### <a name="remarks"></a>주의  
 아래에이 함수를 사용 하 여 [!INCLUDE[wiprlhext](../../c-runtime-library/reference/includes/wiprlhext_md.md)] 가 있는 개체와 `bVistaStyle` 로 설정 `true`합니다. 이 함수는 반환 `NULL` 경우는 `CFileDialog` 없는 **열려** 대화 상자 또는 `bVistaStyle` 로 설정 되어 `false`합니다. 최종이 경우에만 반환 `NULL` -릴리스 모드에서 디버그 모드에서 예외를 throw 한 어설션 합니다.  
  
 에 대 한 자세한 내용은 `IFileOpenDialog` 인터페이스를 참조 [IFileOpenDialog](http://msdn.microsoft.com/library/windows/desktop/bb775834)합니다.  
  
### <a name="example"></a>예제  
 이 예제에서는 내부 COM 개체를 검색 합니다. 이 코드를 실행 하려면 컴파일해야 아래 [!INCLUDE[wiprlhext](../../c-runtime-library/reference/includes/wiprlhext_md.md)]합니다.  
...
### <a name="remarks"></a>주의  
 아래에이 함수를 사용 하 여 [!INCLUDE[wiprlhext](../../c-runtime-library/reference/includes/wiprlhext_md.md)] 가 있는 개체와 `bVistaStyle` 로 설정 `true`합니다. 이 함수는 반환 `NULL` 경우는 `CFileDialog` 없는 **저장** 대화 상자 또는 `bVistaStyle` 로 설정 된 `false`합니다. 최종이 경우에만 반환 `NULL` -릴리스 모드에서 디버그 모드에서 예외를 throw 한 어설션 합니다.  
...
### <a name="remarks"></a>주의  
 파일의 경로 파일의 제목을 전체 디렉터리 경로 포함 됩니다. 예를 들어 `GetNextPathName` "C:\FILES\TEXT를 반환 합니다. DAT C:\FILES\TEXT.DAT 파일에 대 한 "입니다. 사용할 수 있습니다 `GetNextPathName` 정방향 반복 루프를 호출 하 여 초기 위치를 설정 하는 경우에 `GetStartPosition`합니다.  
...
### <a name="remarks"></a>주의  
 파일의 경로 파일의 제목을 전체 디렉터리 경로 포함 됩니다. 예를 들어 `GetPathName` "C:\FILES\TEXT를 반환 합니다. DAT C:\FILES\TEXT.DAT 파일에 대 한 "입니다.  
  
 경우 `m_ofn.Flags` 에 `OFN_ALLOWMULTISELECT` 플래그가 설정,이 문자열 시퀀스를 포함 합니다. null teminated의 파일 그룹 선택의 디렉터리 경로 되 고 첫 번째 문자열이 포함 된 문자열을 다음 사용자가 선택한 모든 파일의 이름입니다. 이러한 이유로 사용은 [GetStartPosition](#getstartposition) 및 [GetNextPathName](#getnextpathname) 목록에서 다음 파일 이름을 검색 하는 멤버 함수입니다.  
...
### <a name="remarks"></a>주의  
 설정 하 여 읽기 전용 확인란을 숨길 수는 `OFN_HIDEREADONLY` 에 스타일을 `CFileDialog` 생성자입니다.  
...
### <a name="return-value"></a>반환 값  
 `TRUE`폴더 선택 모드에서이 대화 상자 그렇지 않으면 `FALSE`합니다.  
...
### <a name="remarks"></a>주의  
 이 구조를 사용 하 여 컨트롤의 모양을 초기화할는 **파일 열기** 또는 **다른 이름으로 저장** 대화 상자를 표시 하기 전에 하지만 그를 생성 한 후의 [DoModal](#domodal) 메서드. 예를 들어, 설정할 수는 `lpstrTitle` 소속 `m_ofn` 캡션을 하려면 대화 상자를 합니다.  
  
 와 [!INCLUDE[wiprlhext](../../c-runtime-library/reference/includes/wiprlhext_md.md)] 의 스타일 [CFileDialog](../../mfc/reference/cfiledialog-class.md), `m_ofn` 항상 일치 하는 대화 상자의 상태를 보장 하지 않습니다. 이전 버전의 Windows에서 대화 상자와 동기화 합니다. 참조 [CFileDialog::ApplyOFNToShellDialog](#applyofntoshelldialog) 및 [CFileDialog::UpdateOFNFromShellDialog](#updateofnfromshelldialog) 동기화에 대 한 자세한 내용은 `m_ofn` 구조 및 `CFileDialog` 상태 [!INCLUDE[wiprlhext](../../c-runtime-library/reference/includes/wiprlhext_md.md)]합니다.  
  
 [!INCLUDE[wiprlhext](../../c-runtime-library/reference/includes/wiprlhext_md.md)]파일 대화 상자에서 스타일 특정 멤버와의 플래그를 지원 하지 않는 `CFileDialog`합니다. 결과적으로,이 효과가 없습니다.  
  
 다음은에서 지원 되지 않는 멤버 목록을 [!INCLUDE[wiprlhext](../../c-runtime-library/reference/includes/wiprlhext_md.md)]:  
...
- `lpTemplateName`  
 ...
 다음 플래그는 지원 되지 않으며 따라서 아무런 효과가 없으며 사용 하는 경우는 [!INCLUDE[wiprlhext](../../c-runtime-library/reference/includes/wiprlhext_md.md)] 의 스타일 `CFileDialog`:  
...
-   OFN_LONGNAMES-항상 효과적으로에서[!INCLUDE[wiprlhext](../../c-runtime-library/reference/includes/wiprlhext_md.md)]  
  
-   OFN_NOLONGNAMES-에서 항상 효과적으로 해제[!INCLUDE[wiprlhext](../../c-runtime-library/reference/includes/wiprlhext_md.md)]  
  
-   OFN_NONETWORKBUTTON-항상 효과적으로에서[!INCLUDE[wiprlhext](../../c-runtime-library/reference/includes/wiprlhext_md.md)]  
...
### <a name="remarks"></a>주의  
 시스템 보냅니다는 `CDN_SELCHANGE` 사용자가 파일 목록에 새 파일이 나 폴더를 선택 하는 경우 메시지는 **열려** 또는 **이름으로 저장** 대화 상자입니다. 이 메시지에 대 한 응답으로 작업을 수행 하려는 경우이 메서드를 재정의 합니다.  
...
### <a name="return-value"></a>반환 값  
 파일 이름이 올바른 파일 이름을; 경우 1 그렇지 않으면 0입니다.  
  
### <a name="remarks"></a>주의  
 이 함수를 사용 하면 어떤 응용 프로그램 관련 이유로 파일 이름을 거부할 수 있습니다. 일반적으로 프레임 워크 파일의 기본 유효성 검사를 제공 하 고 잘못 된 파일 이름을 입력 하면 메시지 상자를 표시 하기 때문에이 함수를 사용할 필요가 없습니다.  
...
### <a name="remarks"></a>주의  
 시스템에서 컨트롤 정렬 끝나면 시스템은이 알림 메시지를 보냅니다는 **열려** 또는 **이름으로 저장** 자식 대화 상자의 컨트롤에 대 한 공간을 만들기 위해 대화 상자입니다.  
  
 시스템 대화 상자 OFN_EXPLORER 스타일으로 만들어진 경우에이 보냅니다. 알림에 대 한 자세한 내용은 참조 [CDN_INITDONE](http://msdn.microsoft.com/library/windows/desktop/ms646863)합니다. OFN_EXPLORER 스타일에 대 한 내용은 참조는 [OPENFILENAME](http://msdn.microsoft.com/library/windows/desktop/ms646839) 구조 및 [열기 및 저장으로 대화 상자](http://msdn.microsoft.com/library/windows/desktop/ms646960)합니다.  
...
### <a name="parameters"></a>매개 변수  
 *nIDBox*  
 목록 상자 또는 콤보 상자 선택을 발생 한의 ID입니다.  
...
- **CD_LBSELSUB** 지정 `iCurSel` 더 이상에서 다중 선택 목록 상자에서 선택 합니다.  
  
- **CD_LBSELADD** 지정 `iCurSel` 에서 다중 선택 목록 상자에서 선택 합니다.  
  
- **CD_LBSELNOITEMS** 에서 다중 선택 목록 상자에 있는 선택 하지 않도록 지정 합니다.  
...
- **OFN_SHARENOWARN** 추가 조치를 수행 해야 합니다.  
  
- **OFN_SHAREWARN** 사용자에 게이 오류에 대 한 표준 경고 메시지가 있습니다.  
  
### <a name="remarks"></a>주의  
 일반적으로 프레임 워크에서는 공유 위반을 확인 하는 기본 제공 하 고 공유 위반이 발생 한 경우 메시지 상자를 표시 하기 때문에이 함수를 사용할 필요가 없습니다.  
...
### <a name="remarks"></a>주의  
 알림 메시지는 사용자가 열기에서 파일 형식의 목록에서 새 파일 형식 또는 다른 이름으로 저장 대화 상자를 선택할 때 전송 됩니다.  
...
### <a name="parameters"></a>매개 변수  
 ...
 `bChecked`  
 확인란의 상태입니다. `TRUE`확인 된; 나타냅니다. `FALSE` 선택 하지 않은 상태를 나타냅니다.  
...
### <a name="parameters"></a>매개 변수  
 ...
 `dwState`  
 하나 이상의 열거형의 값 CDCONTROLSTATE 컨트롤의 새 상태를 나타내는 합니다.  
...
### <a name="parameters"></a>매개 변수  
 ...
 `dwState`  
 하나 이상의 열거형의 값 CDCONTROLSTATE 하는 컨트롤의 현재 상태를 나타냅니다.  
...
### <a name="parameters"></a>매개 변수  
 ...
 [in] `lpsz`  
 컨트롤에 대해 설정 하는 텍스트를 포함 하는 문자열에 대 한 포인터입니다.  
  
### <a name="remarks"></a>주의  
 이 함수의 두 버전 모두 유니코드를 사용 하는 응용 프로그램에 적합 합니다. 그러나 LPCSTR 유형 사용 하 여 버전만는 사용 하는 응용 프로그램에 대 한 유효 [!INCLUDE[vcpransi](../../atl-mfc-shared/reference/includes/vcpransi_md.md)]합니다.  
...
### <a name="parameters"></a>매개 변수  
 `lpszPropList`  
 ";"으로 구분되는 미리 정의된 속성의 목록입니다. 플래그 목록은 대 한 참조는 `Flags` 섹션 [OPENFILENAME](http://msdn.microsoft.com/en-us/8cecfd45-f7c1-4f8d-81a0-4e7fecc3b104)합니다.  
...
### <a name="parameters"></a>매개 변수  
 [in] `nWin3ID`  
 비-탐색기에 대 한 템플릿 리소스의 ID 번호를 포함 `CFileDialog` 개체입니다. 이 템플릿은 Windows NT 3.51 또는 OFN_EXPLORER 스타일 나타나지 않는 경우에 사용 됩니다.  
 ...
 [in] `lpWin3ID`  
 비-탐색기에 대 한 템플릿 리소스의 이름을 포함 `CFileDialog` 개체입니다. 이 템플릿은 Windows NT 3.51 또는 OFN_EXPLORER 스타일 나타나지 않는 경우에 사용 됩니다.  
...
### <a name="remarks"></a>주의  
 지정 된 서식 파일 중 하나에 사용 됩니다. 어떤 템플릿을 사용 하 여 OFN_EXPLORER 스타일 및 응용 프로그램에서 실행 되는 운영 체제의 유무에 따라 결정 됩니다. 비 탐색기와 템플릿 탐색기 스타일을 지정 하 여 쉽습니다 지원, Windows NT 3.51 [!INCLUDE[WinNt4Family](../../mfc/reference/includes/winnt4family_md.md)] 및 이상 버전 및 Windows 95 및 이상 버전.  
...
### <a name="remarks"></a>주의  
 이전 Windows 버전에서 [!INCLUDE[wiprlhext](../../c-runtime-library/reference/includes/wiprlhext_md.md)], 멤버 [OPENFILENAME](https://msdn.microsoft.com/library/ms911906.aspx) 데이터 구조의 상태와 지속적으로 동기화 된는 `CFileDialog`합니다. 변경 내용을 [m_ofn](#m_ofn) 멤버 변수는 대화 상자의 상태에 직접적인 영향을 줍니다. 또한 대화의 상태 변경 내용을 m_ofn 멤버 변수를 즉시 업데이트 합니다.  
...
### <a name="example"></a>예제  
 이 예제에서는 업데이트는 `CFileDialog` 표시 하기 전에 합니다. 업데이트 하기 전에 `m_ofn` 멤버 변수 대화 상자의 현재 상태를 동기화 할 필요 합니다.  
