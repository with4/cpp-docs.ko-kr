...
> [!NOTE]
>  사용 해야 합니다는 `IMPLEMENT_SERIAL` 배열 serialize 하려는 경우 파생 된 클래스의 구현에는 매크로입니다.  
...
### <a name="example"></a>예제  
 ...
 [!code-cpp[NVC_MFCCollections 83 번](../../mfc/codesnippet/cpp/cobarray-class_9.cpp)]  
...
### <a name="remarks"></a>주의  
 첫 번째 버전 `InsertAt` 배열에서 지정된 된 인덱스에 요소 하나 (또는 요소가 여러 개)를 삽입 합니다. 프로세스에서 이동 (인덱스 증가)에서이 인덱스에서 기존 요소 위에 있는 모든 요소를 전환 합니다.  
