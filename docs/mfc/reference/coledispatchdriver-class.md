...
##  <a name="invokehelper"></a>  Coledispatchdriver:: Invokehelper  
 개체 메서드 또는 속성으로 지정 된 호출 *dwDispID*, 의해 지정 된 컨텍스트의 *wFlags*합니다.  
 ...
 *wFlags*  
 **IDispatch::Invoke**호출의 컨텍스트를 설명하는 플래그입니다. . 목록이 가능한 값에 대 한 참조는 *wFlags* 매개 변수에서 [idispatch:: Invoke](http://msdn.microsoft.com/library/windows/desktop/ms221479\(v=vs.85\).aspx) Windows sdk에서입니다.  
  
 *vtRet*  
 반환 값 형식을 지정합니다. 가능한 값은 설명 섹션을 참조하세요.  
  
 *pvRet*  
 속성 값이나 반환 값을 받을 변수의 주소입니다. 지정한 형식과 일치 해야 *vtRet*합니다.  
  
 *pbParamInfo*  
 뒤에 매개 변수 형식을 지정 하는 바이트 null로 끝나는 문자열에 대 한 포인터 *pbParamInfo*합니다.  
  
 *...*  
 에 지정 된 형식의 매개 변수 목록은 *pbParamInfo*합니다.  
  
### <a name="remarks"></a>설명  
 *pbParamInfo* 매개 변수를 메서드 또는 속성에 전달 된 매개 변수의 유형을 지정 합니다. 인수의 변수 목록은 구문 선언에서 **...** 로 표시됩니다.  
  
 에 가능한 값은 *vtRet* 인수에서 가져옵니다는 `VARENUM` 열거형입니다. 다음과 같은 값을 사용할 수 있습니다.  
 ...
 *pbParamInfo* 인수는 공백으로 구분 된 목록이 **VTS_** 상수입니다. 공백(쉼표가 아님)으로 구분된 이러한 값 중 하나 이상이 함수의 매개 변수 목록을 지정합니다. 가능한 값 나열 되는 [EVENT_CUSTOM](event-maps.md#event_custom) 매크로입니다.  
...
##  <a name="setproperty"></a>  COleDispatchDriver::SetProperty  
 로 지정 된 OLE 개체 속성을 설정 *dwDispID*합니다.  
