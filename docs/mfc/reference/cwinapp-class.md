...
### <a name="parameters"></a>매개 변수  
 *bEndSession*  
 Windows 세션을 종료 여부를 지정 합니다. 세션을 종료 합니다; TRUE 이면 그렇지 않으면 FALSE입니다.  
...
### <a name="parameters"></a>매개 변수  
 *lpszAppName*  
 Windows에서 사용 하는 응용 프로그램 이름이 포함 된 null로 끝나는 문자열입니다. 이 인수에 제공 되지 않으면 하거나 NULL 인 경우 `CWinApp` AFX_IDS_APP_TITLE 리소스 문자열 또는 실행 파일의 파일 이름을 사용 합니다.  
...
### <a name="parameters"></a>매개 변수  
 *bEnable*  
 Windows 7 작업 표시줄의 상호 작용 설정할지 (TRUE) 또는 해제 (FALSE) 여부를 지정 합니다.  
  
### <a name="return-value"></a>반환 값  
 작업 표시줄의 상호 작용 사용 또는 사용 하지 않도록 설정 수 있으면 TRUE를 반환 합니다.  
  
### <a name="remarks"></a>설명  
 주 창 만들기 전에이 메서드를 호출 해야, 그렇지 않으면 어설션를 FALSE를 반환 합니다.  
  
##  <a name="exitinstance"></a>  CWinApp::ExitInstance  
 내에서 프레임 워크에서 호출 된 `Run` 멤버 함수를 응용 프로그램의이 인스턴스를 종료 합니다.  
...
### <a name="remarks"></a>설명  
 호출 하지 마십시오이 멤버 함수 어디서 내 하지만 `Run` 멤버 함수입니다.  
...
### <a name="remarks"></a>설명  
 ...
 이 메서드를 Windows Vista 이전 운영 체제에서 NULL을 반환 합니다. Windows Vista 이전 운영 체제에서 다시 시작 관리자 지원 되지 않습니다.  
...
### <a name="remarks"></a>설명  
 에 대 한 호출에서 반환 되는 위치 값을 사용 하 여 [GetNextDocTemplate](#getnextdoctemplate) 첫 번째 가져오려는 [CDocTemplate](../../mfc/reference/cdoctemplate-class.md) 개체입니다.  
...
##  <a name="getnextdoctemplate"></a>  CWinApp::GetNextDocTemplate  
 로 식별 되는 문서 템플릿을 가져옵니다 *pos*, 다음 설정 *pos* 위치 값입니다.  
...
### <a name="parameters"></a>매개 변수  
 *pos*  
 에 대 한 이전 호출에서 반환 되는 위치 값에 대 한 참조 `GetNextDocTemplate` 또는 [GetFirstDocTemplatePosition](#getfirstdoctemplateposition)합니다. 값이이 호출에서 다음 위치로 업데이트 됩니다.  
...
### <a name="remarks"></a>설명  
 ...
 위치 값이 유효한 지 확인 해야 합니다. 유효 하지 않으면 Microsoft Foundation Class 라이브러리의 디버그 버전 어설션 합니다.  
  
 검색 된 문서 서식 파일은 마지막 하는 경우를 사용할 수의 새 값이 다음 *pos* NULL로 설정 됩니다.  
...
### <a name="return-value"></a>반환 값  
 함수가 성공하면 문자열의 정수 값은 지정된 항목 다음에 오게 됩니다. 반환 값의 값은 고 *nDefault* 함수 항목을 찾지 못하면 매개 변수입니다. 지정한 항목에 해당하는 값이 정수가 아닌 경우 반환 값은 0입니다.  
 ...
 *lpszEntry*  
 해당 문자열을 검색할 항목이 포함 된 null로 끝나는 문자열을 가리킵니다. 이 값에 NULL이 아니어야 합니다.  
  
 *lpszDefault*  
 초기화 파일의 항목을 찾을 수 없는 경우 제공된 된 항목에 대 한 기본 문자열 값을 가리킵니다.  
  
### <a name="return-value"></a>반환 값  
 반환 값은 응용 프로그램의에서 문자열입니다. INI 파일 또는 *lpszDefault* 문자열을 찾을 수 없는 경우. 프레임 워크에서 지원 되는 최대 문자열 길이 _MAX_PATH입니다. 경우 *lpszDefault* 가 null 인 경우 반환 값은 빈 문자열입니다.  
...
### <a name="return-value"></a>반환 값  
 TRUE를 반환 합니다 `EnableTaskbarInteraction` 가 호출 된 운영 체제는 Windows 7 이상 및.  
...
### <a name="return-value"></a>반환 값  
 성공 하면 커서에 대 한 핸들 그렇지 않으면 NULL입니다.  
...
### <a name="return-value"></a>반환 값  
 성공 하면 아이콘에 대 한 핸들 그렇지 않으면 NULL입니다.  
...
> [!NOTE]
>  이 멤버 함수는 Win32 API 함수를 호출 [LoadIcon](http://msdn.microsoft.com/library/windows/desktop/ms648072), 필요에 따라 크기가 SM_CXICON 및 SM_CYICON 시스템 메트릭 값을 준수 하는 아이콘만 로드할 수입니다.  
  
##  <a name="loadoemcursor"></a>  CWinApp::LoadOEMCursor  
 창을 로드 하 여 지정 된 커서 리소스 미리 정의 된 *nIDCursor*합니다.  
...
### <a name="parameters"></a>매개 변수  
 *nIDCursor*  
 **OCR_** 매니페스트 미리 정의 된 Windows 커서를 지정 하는 상수 식별자입니다. 있어야 `#define OEMRESOURCE` 전에 `#include \<afxwin.h>` 에 액세스 하는 **OCR_** WINDOWS의 상수입니다. 8.  
...
### <a name="parameters"></a>매개 변수  
 *nIDIcon*  
 **OIC_** 매니페스트 미리 정의 된 Windows 아이콘을 지정 하는 상수 식별자입니다. 있어야 `#define OEMRESOURCE` 전에 `#include \<afxwin.h>` 액세스로는 **OIC_** WINDOWS의 상수입니다. 8.  
...
- Windows는 시간이 많이 걸리는 작업을 수행할 때 사용 되는 IDC_WAIT 모래 시계 커서  
  
- 선택 영역에 대 한 IDC_CROSS 십자 모양 커서  
  
- 위로 IDC_UPARROW 화살표  
  
- 사용 되지 않는 IDC_SIZE / 지원 되지 않는; IDC_SIZEALL 사용  
  
- IDC_SIZEALL A 4 방향 화살표입니다. 창 크기를 조정 하는 데 커서입니다.  
  
- 사용 되지 않는 IDC_ICON / 지원 되지 않는 합니다. IDC_ARROW를 사용 합니다.  
  
- 상단 왼쪽 및 오른쪽 아래에서 끝나는와 IDC_SIZENWSE 양방향 화살표  
  
- 위 오른쪽 및 왼쪽 아래에서 끝나는와 IDC_SIZENESW 양방향 화살표  
...
##  <a name="m_bhelpmode"></a>  CWinApp::m_bHelpMode  
 응용 프로그램 (일반적으로 사용 하 여 호출 SHIFT + F1); 도움말 컨텍스트 모드 이면 TRUE입니다. 그렇지 않으면 FALSE입니다.  
...
### <a name="remarks"></a>설명  
 도움말 컨텍스트 모드 커서가 물음표 되며 사용자 화면에 대 한 이동할 수 있습니다. 도움말 모드에 있을 때 특수 한 처리를 구현 하려는 경우이 플래그를 검사 합니다. `m_bHelpMode` BOOL 형식의 공용 변수가입니다.  
...
##  <a name="m_ehelptype"></a>  CWinApp::m_eHelpType  
 이 데이터 멤버의 형식은 AFX_HELP_TYPE 내에 정의 된 열거 형식은 `CWinApp` 클래스입니다.  
...
### <a name="remarks"></a>설명  
 `m_hInstance` 데이터 멤버는 Windows에서 실행 되는 응용 프로그램의 현재 인스턴스에 대 한 핸들입니다. 전역 함수에서 반환 되는이 [AfxGetInstanceHandle](application-information-and-management.md#afxgetinstancehandle)합니다. `m_hInstance` HINSTANCE 형식의 공용 변수가입니다.  
...
### <a name="remarks"></a>설명  
 응용 프로그램에 대 한 명령줄을 지정 하는 null로 끝나는 문자열을 가리킵니다. 사용 하 여 `m_lpCmdLine` 사용자가 응용 프로그램이 시작 될 때 입력 한 모든 명령줄 인수에 액세스할 수 있습니다. `m_lpCmdLine` LPTSTR 형식의 공용 변수가입니다.  
...
### <a name="remarks"></a>설명  
 이 데이터 멤버가 NULL 인 응용 프로그램에 내부 활성화 되지 않습니다.  
...
### <a name="remarks"></a>설명  
 응용 프로그램 이름에 전달 된 매개 변수에서 가져올 수 있습니다는 [CWinApp](#cwinapp) 생성자를 지정 하지 않으면 AFX_IDS_APP_TITLE ID와 리소스 문자열 또는 합니다. 리소스에서 응용 프로그램 이름이 없으면 프로그램의에서 제공 됩니다. EXE 파일 이름입니다.  
...
>  기본 구현은 `OnIdle` 업데이트 명령 메뉴 항목 및 도구 모음 단추와 같은 사용자 인터페이스 개체 및 내부 데이터 구조 정리 작업을 수행 합니다. 따라서 재정의 하는 경우 `OnIdle`를 호출 해야 `CWinApp::OnIdle` 와 `lCount` 재정의 된 버전에 있는 합니다. 모든 기본 클래스 유휴 프로세스를 먼저 호출 (즉, 기본 클래스까지 `OnIdle` 0을 반환 합니다). 기본 클래스 처리가 완료 되기 전에 작업을 수행 해야 할 경우 적절 한 선택 하는 기본 클래스 구현을 검토 *lCount* 작업을 수행 하는 중입니다.  
 ...
 원하지 않는 경우 `OnIdle` 메시지 큐에서 메시지를 가져올 때마다 호출을 재정의할 수 있습니다는 [CWinThreadIsIdleMessage](../../mfc/reference/cwinthread-class.md#isidlemessage)합니다. 응용 프로그램에 매우 짧은 타이머를 설정 하는 경우 또는 시스템 다음 WM_SYSTIMER 메시지를 보내는 경우 `OnIdle` 반복적으로 호출 되 고 성능이 저하 될 합니다.  
...
### <a name="parameters"></a>매개 변수  
 ...
 [in] *bAddToMRU*  
 TRUE 이면 가장 최근에 사용한 파일; 중 하나는 문서는 FALSE 이면 문서는 가장 최근에 사용한 파일 중 하나가 아닙니다.  
  
### <a name="return-value"></a>반환 값  
 에 대 한 포인터는 `CDocument` 성공 하면 그렇지 않으면 NULL입니다.  
...
### <a name="return-value"></a>반환 값  
 Windows에 반환 되어야 하는 값입니다. 일반적으로 이것이 0 L windows 메시지, 1l (TRUE) 명령 메시지입니다.  
...
### <a name="parameters"></a>매개 변수  
 [in] *bCompat*  
 TRUE는 인쇄 및 인쇄를, 사용자가 프린터 개체에는 파일을 끌어 또는 셸의에서 직접 파일을 인쇄 하는 셸 명령에 대 한 등록 항목을 추가 합니다. 또한 DefaultIcon 키를 추가합니다. 기본적으로이 매개 변수는 FALSE 이전 버전과 호환성에 대 한 합니다.  
...
|-|-|  
|매개 변수|설명|  
|[in] *bRegisterRecoveryCallback*|TRUE 이면 응용 프로그램의이 인스턴스 복구 콜백 함수; 사용 하 고 FALSE 이면 되지 않은 것을 나타냅니다. 응용 프로그램이 예기치 않게 종료 될 때 프레임 워크에서 복구 콜백 함수를 호출 합니다. 자세한 내용은 참조 [CWinApp::ApplicationRecoveryCallback](#applicationrecoverycallback)합니다.|  
...
### <a name="return-value"></a>반환 값  
 TRUE 이면 다시 시작 관리자가 이전에 열려 있는 파일입니다. FALSE 이면 다시 시작 관리자는 그렇지 않습니다.  
...
### <a name="return-value"></a>반환 값  
 데이터 복구 처리기 이전에 열린 문서; 열리면 TRUE 데이터 복구 처리기에서 오류가 발생 하거나 이전에 열린 문서가 없는 경우 FALSE입니다.  
...
### <a name="remarks"></a>설명  
 ...
 이 메서드가 FALSE를 반환 하는 경우는 [CDataRecoveryHandler](../../mfc/reference/cdatarecoveryhandler-class.md) 열려 있는 문서 되었는지 확인 합니다. 열려 있는 문서 인 경우 응용 프로그램이 일반적으로 시작 합니다.  
...
### <a name="return-value"></a>반환 값  
 TRUE 이면 자동 저장 된 파일을 복원 하는 다시 시작 관리자 FALSE 이면 다시 시작 관리자는 그렇지 않습니다.  
...
### <a name="remarks"></a>설명  
 `Run` 획득 하 고 응용 프로그램 WM_QUIT 메시지를 받을 때까지 Windows 메시지를 디스패치합니다. 응용 프로그램의 메시지 큐에 현재 없는 메시지를 포함 하는 경우 `Run` 호출 [OnIdle](#onidle) 유휴 시간 처리를 수행 합니다. 들어오는 메시지는로 이동는 [PreTranslateMessage](#pretranslatemessage) 멤버 함수에 대 한 특수 한 처리 및 다음 Windows 함수 `TranslateMessage` 표준 키보드의 번역;에 대 한 마지막으로 `DispatchMessage` Windows 함수를 호출 합니다.  
  
 `Run` 대부분 재정의 하지만 특별 한 동작을 제공 하려면이 재정의할 수 있습니다.  
...
##  <a name="saveallmodified"></a>  CWinApp::SaveAllModified  
 WM_QUERYENDSESSION 메시지를 통해 또는 응용 프로그램의 주 프레임 창이 닫을 수 있게 되 면 모든 문서를 저장 하기 위해 프레임 워크에서 호출 됩니다.  
...
### <a name="remarks"></a>설명  
 두 *hDevMode* 및 *hDevNames* 가 NULL 이면 `SelectPrinter` 현재 기본 프린터를 사용 합니다.  
...
### <a name="return-value"></a>반환 값  
 TRUE 이면 응용 프로그램을 복구 하는 다시 시작 관리자 FALSE 이면 다시 시작 관리자는 그렇지 않습니다.  
...
### <a name="return-value"></a>반환 값  
 TRUE 이면 다시 시작 관리자에서 열려 있는 문서; FALSE 이면 다시 시작 관리자는 그렇지 않습니다.  
...
### <a name="return-value"></a>반환 값  
 TRUE 이면 응용 프로그램을 다시 시작 되; 열린 문서를 다시 시작 관리자 자동으로 저장 FALSE 이면 다시 시작 관리자는 그렇지 않습니다.  
...
### <a name="return-value"></a>반환 값  
 TRUE 이면 응용 프로그램이 다시 시작 관리자; 지원 FALSE 이면 응용 프로그램은 그렇지 않습니다.  
 ...
 *lpszEntry*  
 값이 쓰여질 항목이 포함 된 null로 끝나는 문자열을 가리킵니다. 지정된 된 섹션에는 항목이 없으면 만들어집니다. 이 매개 변수가 NULL 인 섹션에 지정 된 *lpszSection* 삭제 됩니다.  
  
 *lpszValue*  
 쓸 문자열을 가리킵니다. 지정 된 항목은이 매개 변수가 NULL 인 경우는 *lpszEntry* 매개 변수는 삭제 합니다.  
