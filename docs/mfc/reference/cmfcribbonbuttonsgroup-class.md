---
title: "CMFCRibbonButtonsGroup 클래스 | Microsoft 문서"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
`CMFCRibbonButtonsGroup` 클래스는 리본 단추 집합을 그룹으로 구성할 수 있습니다. 그룹의 모든 단추는 가로로 서로 직접 인접해 있으며 테두리로 둘러싸여 있습니다.  
...
|이름|설명|  
|----------|-----------------|  
|[CMFCRibbonButtonsGroup::AddButton](#addbutton)|그룹에는 단추를 추가합니다.|  
...
|[CMFCRibbonButtonsGroup::GetImageSize](#getimagesize)|리본 그룹의 일반 이미지의 이미지 크기를 반환 (재정의 [CMFCRibbonBaseElement::GetImageSize](../../mfc/reference/cmfcribbonbaseelement-class.md#getimagesize).)|  
|[CMFCRibbonButtonsGroup::GetRegularSize](#getregularsize)|리본 요소의 일반 크기를 반환 합니다 (재정의 [CMFCRibbonBaseElement::GetRegularSize](../../mfc/reference/cmfcribbonbaseelement-class.md#getregularsize).)|  
|[CMFCRibbonButtonsGroup::HasImages](#hasimages)|보고서 여부는 `CMFCRibbonButtonsGroup` 도구 모음 이미지를 포함 하는 개체입니다.|  
|[CMFCRibbonButtonsGroup::OnDrawImage](#ondrawimage)|보통, 강조 표시 된 또는 사용 안 함 단추 인지에 따라 지정 된 단추에 대 한 적절 한 이미지를 그립니다.|  
...
## <a name="remarks"></a>주의  
 그룹에서 파생 된 [CMFCBaseRibbonElement](../../mfc/reference/cmfcribbonbaseelement-class.md) 단일 엔터티로 조작할 수 있습니다. 모든 패널 또는 팝업 메뉴에서 그룹을 배치할 수 있습니다.  
  
## <a name="example"></a>예제  
 다음 예제에서는 `CMFCRibbonButtonsGroup` 클래스에서 다양한 메서드를 사용하는 방법을 보여 줍니다. 예제를 생성 하는 방법을 보여 줍니다는 `CMFCRibbonButtonsGroup` 개체 리본 단추 그룹에 이미지를 지정 하 고 리본 메뉴 단추 그룹에 단추를 추가 합니다. 이 코드 조각은의 일부인는 [그리기 클라이언트 샘플](../../visual-cpp-samples.md)합니다.  
...
### <a name="parameters"></a>매개 변수  
 [in] `pButton`  
 새로 만든에 추가 하는 단추 지정 `CMFCRibbonButtonsGroup` 개체입니다.  
...
### <a name="return-value"></a>반환 값  
 하나라도 있는 경우 또는 도구 모음 이미지의 원본 이미지 크기를 반환 합니다. `CSize` 그렇지 않으면&0;입니다.  
...
### <a name="return-value"></a>반환 값  
 경우 TRUE를 반환 합니다. 보호 된 `CMFCToolBarImages` 멤버 `m_Images` 하지 모든 이미지 또는 false를 포함 합니다.  
...
### <a name="remarks"></a>주의  
 호출 `SetImages` 전에 단추 그룹에 추가 합니다. 그룹에 추가할 수 있는 단추 수와 같거나 큰 이미지 수가 이어야 합니다.  
...
### <a name="parameters"></a>매개 변수  
 [in] `pCategory`  
 설정 하는 부모 범주에 대 한 포인터 (리본 컨트롤에 탭된 그룹의 범주 라고 함).  
