---
title: "CPaneContainerManager 클래스 | Microsoft 문서"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
`CPaneContainerManager` 클래스는 저장 및 현재 도킹 레이아웃의 표시를 관리 합니다.  
...
|[CPaneContainerManager::Serialize](#serialize)|이 개체를 보관 저장소에서 읽어오거나 보관 저장소에 씁니다. (재정의 [CObject::Serialize](../../mfc/reference/cobject-class.md#serialize).)|  
...
### <a name="remarks"></a>주의  
 프레임 워크의 인스턴스를 자동으로 만들고 `CPaneContainerManager` 개체 및 하거나 포함 시킵니다에 [CPaneDivider 클래스](../../mfc/reference/cpanedivider-class.md) 개체 또는 [CMultiPaneFrameWnd 클래스](../../mfc/reference/cmultipaneframewnd-class.md) 개체입니다.  
  
 `CPaneContainerManager` 클래스에서 작성 되는 이진 트리의 루트에 대 한 포인터를 저장 [CPaneContainer](../../mfc/reference/cpanecontainer-class.md) 개체입니다.  
  
## <a name="example"></a>예제  
 다음 예제에서는에 대 한 참조는 `CPaneContainerManager` 개체입니다. 이 코드 조각은의 일부인는 [창 크기 설정 샘플](../../visual-cpp-samples.md)합니다.  
