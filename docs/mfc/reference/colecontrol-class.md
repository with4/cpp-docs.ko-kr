...
- **OLEDC_OFFSCREEN** 개체 하지 않고자 한다면 다음 화면으로 복사 해야 하는 화면에서 벗어난 비트맵으로 렌더링 하는 컨테이너에 알립니다. 개체는 그리기 작업을 수행 하려고 하는 많은 깜빡임을 생성할 때이 플래그를 사용 해야 합니다. 컨테이너는 여부이 요청을 처리할 수 있습니다. 그러나이 플래그를 설정 하지 않으면 컨테이너 해야 다시 전달할는 화면 DC입니다. 이렇게 하면 선택 영역을 보여 주는 같은 직접 화면 연산을 수행 하는 개체 (통해는 **XOR** 작업).  
...
### <a name="remarks"></a>설명  
 일반적으로 이를 위해서는 컨트롤의 창이 놓기 대상으로 등록되어야 합니다. 하지만 컨트롤에는 고유의 창이, 없으므로 컨테이너가 사용할 고유 창을 놓기 대상으로 합니다. 컨트롤의 구현을 제공 해야 단순히는 `IDropTarget` 인터페이스를 컨테이너 호출을 위임할 수 적절 한 시간입니다. 예를 들어:  
...
### <a name="remarks"></a>설명  
 ...
 이 함수에 대 한 처리기 함수로 일반적으로 사용 하는 `ON_OLEVERB` 메시지 맵 항목입니다. 이렇게 하면 컨트롤의 "개체" 메뉴에서 사용할 수 있는 "편집" 동사가 있습니다. 예를 들어:  
