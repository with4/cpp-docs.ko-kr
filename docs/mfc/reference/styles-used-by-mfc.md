...
|스타일|설명|  
|-----------|-----------------|  
|`CBS_AUTOHSCROLL`|줄의 끝에 문자를 입력 하는 경우 오른쪽에 편집 컨트롤의에서 텍스트를 자동으로 스크롤됩니다. 이 스타일을 설정하지 않으면 사각형 경계에 포함되는 텍스트만 허용됩니다.|  
|`CBS_DISABLENOSCROLL`|목록 상자는 비활성화 된 세로 스크롤 막대의 목록 상자에 스크롤할 충분 한 항목이 없습니다를 표시 합니다. 이 스타일을 사용하지 않으면 목록 상자에 충분한 항목이 포함되지 않은 경우 스크롤 막대가 숨겨집니다.|  
|`CBS_DROPDOWN`|유사한 `CBS_SIMPLE`제외 하 고 사용자가 편집 컨트롤 옆에 있는 아이콘을 선택 하지 않으면 목록 상자 표시 되지 않습니다.|  
|`CBS_DROPDOWNLIST`|유사한 `CBS_DROPDOWN`제외 하 고 편집 컨트롤이 목록 상자에서 현재 선택 영역을 표시 하는 정적 텍스트 항목으로 대체 됩니다.|  
|`CBS_HASSTRINGS`|소유자 그리기 콤보 상자의 문자열로 구성 된 항목을 포함 합니다. 콤보 상자는 응용 프로그램에서 `GetText` 멤버 함수를 사용하여 특정 항목에 대한 텍스트를 검색할 수 있도록 문자열에 대한 포인터 및 메모리를 유지합니다.|  
|`CBS_LOWERCASE`|선택 필드와 목록에 있는 모든 텍스트를 소문자로 변환 합니다.|  
|`CBS_NOINTEGRALHEIGHT`|콤보 상자의 크기가 콤보 상자를 만들 때 응용 프로그램에서 지정 된 크기와 정확히 일치 임을 지정 합니다. 일반적으로 Windows에서는 콤보 상자에 특정 항목이 표시되지 않도록 콤보 상자의 크기를 지정합니다.|  
|`CBS_OEMCONVERT`|콤보 상자 편집 컨트롤에 입력 한 텍스트 ANSI 문자 집합에서 OEM 문자 집합을 다음 다시 ANSI로 변환 됩니다. 따라서 응용 프로그램이 콤보 상자의 ANSI 문자열을 OEM 문자로 변환하기 위해 `AnsiToOem` Windows 함수를 호출할 때 문자가 적절히 변환됩니다. 이 스타일으로 만들어진 콤보 상자에만 적용 하는 파일 이름을 포함 하는 콤보 상자에 가장 유용는 `CBS_SIMPLE` 또는 `CBS_DROPDOWN` 스타일입니다.|  
|`CBS_OWNERDRAWFIXED`|목록 상자의 소유자가 내용을; 목록 상자 항목은 높이가 모두 같습니다.|  
|`CBS_OWNERDRAWVARIABLE`|목록 상자의 소유자가 내용을; 목록 상자의 항목은 높이가 가변적입니다.|  
...
|스타일|설명|  
|-----------|-----------------|  
|`ES_AUTOHSCROLL`|줄의 끝에 문자를 입력 하는 경우 10 자 씩 텍스트 오른쪽에 자동으로 스크롤됩니다. 사용자가 ENTER 키를 누르면 컨트롤 0 위치로 복원 하는 모든 텍스트를 스크롤합니다.|  
|`ES_AUTOVSCROLL`|마지막 줄에서 ENTER를 누를 때 텍스트를 한 페이지 위로 자동으로 스크롤됩니다.|  
...
|`ES_MULTILINE`|여러 줄 편집 컨트롤을 지정합니다. (기본값은 한 줄.) 경우는 `ES_AUTOVSCROLL` 스타일을 지정 하면 편집 컨트롤 최대한 많은 줄을 표시 하 고 ENTER 키를 누를 때 세로로 스크롤합니다. 경우 `ES_AUTOVSCROLL` 가 제공 되지 편집 컨트롤 많은 줄 원으로 표시 가능한 경고음이 줄이 더 이상 표시 될 수 있습니다 때 ENTER를 누르면 됩니다. 경우는 `ES_AUTOHSCROLL` 스타일을 지정 하면 여러 줄 편집 컨트롤 자동으로 컨트롤을 가로로 스크롤합니다 캐럿 컨트롤의 오른쪽 가장자리를 지나갈 때. 새 줄을 시작 하려면 ENTER를 눌러야 합니다. 경우 `ES_AUTOHSCROLL` 은 컨트롤 바꿈하는지 필요한 경우에 다음 줄의 시작 부분에 제공 되지 않으면 ENTER를 누를 경우에 새 줄을 시작할 합니다. 단어 잘림 방지의 위치는 창 크기에 따라 결정 됩니다. 창 크기가 변경 되 면 단어 잘림 방지 위치 변경 내용 및 텍스트 다시 표시 됩니다. 여러 줄 편집 컨트롤에 스크롤 막대가 있습니다. 스크롤 막대가 있는 편집 컨트롤 자체 스크롤 막대 메시지를 처리합니다. 스크롤 막대 스크롤 위에 설명 된 대로 사용 하지 않고 컨트롤을 편집 하 고 부모 창에서 전송 된 모든 스크롤 메시지를 처리 합니다.|  
...
|스타일|설명|  
|-----------|-----------------|  
|`LBS_DISABLENOSCROLL`|목록 상자는 비활성화 된 세로 스크롤 막대의 목록 상자에 스크롤할 충분 한 항목이 없습니다를 표시 합니다. 이 스타일을 사용하지 않으면 목록 상자에 충분한 항목이 포함되지 않은 경우 스크롤 막대가 숨겨집니다.|  
...
|`LBS_MULTIPLESEL`|문자열 선택을 클릭 하거나 문자열을 두 번 클릭할 때마다 전환 됩니다. 임의 개수의 문자열을 선택할 수 있습니다.|  
|`LBS_NODATA`|데이터가 없는 목록 상자를 지정합니다. 목록 상자에 있는 항목 수는 1 천을 초과 했을 때이 스타일을 지정 합니다. 데이터가 없는 목록 상자 있어야는 `LBS_OWNERDRAWFIXED` 스타일을 가져서는 안 되지만 `LBS_SORT` 또는 `LBS_HASSTRINGS` 스타일입니다. <br />< b r / > 데이터가 없는 목록 상자 제외 하 고 항목에 대 한 string 또는 bitmap 데이터가 없는 포함 소유자가 그린 목록 상자와 유사 합니다. 명령에 추가 하려면 삽입 또는 항목을 삭제할 데이터; 특정 항목을 항상 무시 요청 문자열을 찾으려면 목록 상자 내에서 항상 실패 합니다. 시스템은 보냅니다는 `WM_DRAWITEM` 항목 그려야 할 때 소유자 창에 메시지입니다. itemID 멤버는 `DRAWITEMSTRUCT` 구조와 함께 전달는 `WM_DRAWITEM` 메시지를 그릴 수 있도록 항목의 줄 번호를 지정 합니다. 데이터가 없는 목록 상자를 전송 하지 않습니다는 `WM_DELETEITEM` 메시지입니다.|  
|`LBS_NOINTEGRALHEIGHT`|목록 상자 크기가 목록 상자를 만들 때 응용 프로그램에서 지정 된 크기와 정확히 일치 합니다. 일반적으로 Windows 목록 상자의 크기를 목록 상자 부분 항목을 표시 하지 않도록 합니다.|  
|`LBS_NOREDRAW`|변경 된 경우에 목록 상자 디스플레이가 업데이트 되지 않습니다. 이 스타일 전송 하 여 언제 든 지 변경할 수 있습니다는 `WM_SETREDRAW` 메시지입니다.|  
|`LBS_NOSEL`|목록 상자 항목을 볼 수 있지만 선택 하지 않은 포함 되도록 지정 합니다.|  
|`LBS_NOTIFY`|부모 창에서 사용자가 클릭 하거나 문자열을 두 번 클릭할 때마다 입력된 메시지를 받습니다.|  
|`LBS_OWNERDRAWFIXED`|목록 상자의 소유자가 내용을; 목록 상자의 항목 높이 같게 만들기 됩니다.|  
|`LBS_OWNERDRAWVARIABLE`|목록 상자의 소유자가 내용을; 목록 상자의 항목은 높이가 가변적입니다.|  
|`LBS_SORT`|목록 상자에서 문자열은 사전순으로 정렬 됩니다.|  
|`LBS_STANDARD`|목록 상자에는 문자열을 사전순으로 정렬 하 고를 클릭 하거나 문자열을 두 번 클릭할 때마다 부모 창 입력된 메시지를 받습니다. 목록 상자에는 모든 면에 테두리가 포함 되어 있습니다.|  
...
|`SS_NOPREFIX`|이 스타일을 지정 하지 않으면 Windows 모든 앰퍼샌드 (&) 액셀러레이터 접두사 문자 수를 컨트롤의 텍스트의 문자를 해석 합니다. 이 경우에 앰퍼샌드 제거 되 고 문자열의 다음 문자에 밑줄이 표시 됩니다. 텍스트를 포함 하는 정적 컨트롤에이 기능이 사용 하지 않으려는 경우 `SS_NOPREFIX` 추가 될 수 있습니다. 이 정적 컨트롤 스타일 정의 된 정적 컨트롤 포함 될 수 있습니다. 결합할 수 `SS_NOPREFIX` 비트 OR 연산자를 사용 하 여 다른 스타일을 사용 합니다. 앰퍼샌드를 포함할 수 있는 다른 문자열이 나 파일 이름을 해야 대화 상자에서 정적 컨트롤에 표시할 때 가장 자주 사용 됩니다.|  
|`SS_NOTIFY`|부모 창 보냅니다 `STN_CLICKED`, `STN_DBLCLK`, `STN_DISABLE`, 및 `STN_ENABLE` 알림 메시지를 클릭 하거나 컨트롤을 두 번 클릭할 때입니다.|  
|`SS_OWNERDRAW`|컨트롤 그리기를 담당 정적 컨트롤의 소유자 임을 지정 합니다. 소유자 창에 수신 된 `WM_DRAWITEM` 컨트롤을 그려야 할 때마다 메시지입니다.|  
|`SS_REALSIZEIMAGE`|정적 아이콘 또는 비트맵 컨트롤이 않습니다 (즉, 정적 컨트롤은 `SS_ICON` 또는 `SS_BITMAP` 스타일)에서 로드 되거나 그릴 때 크기가 변경 되지 합니다. 아이콘 또는 비트맵 대상 영역 보다 큰 경우 이미지가 잘립니다.|  
|`SS_RIGHT`|간단한 사각형을 지정 하 고 플러시 오른쪽의 사각형에 지정된 된 텍스트를 표시 합니다. 텍스트 표시 되기 전에 서식이 지정 됩니다. 줄의 끝을 지나서 에게도 확장 하는 단어는 자동으로 다음 플러시 오른쪽 줄의 시작 부분에 래핑됩니다.|  
|`SS_RIGHTJUST`|지정 된 정적 컨트롤의 오른쪽 아래 모서리는 `SS_BITMAP` 또는 `SS_ICON` 컨트롤 크기를 조정할 때 고정을 유지 하기 위한 스타일은 합니다. 위쪽 및 왼쪽 면 새 비트맵 또는 아이콘을 수용 하기 위해 조정 됩니다.|  
|`SS_SIMPLE`|간단한 사각형을 지정 하 고 사각형에서 왼쪽 플러시 텍스트 한 줄을 표시 합니다. 텍스트 줄을 축소 하거나 어떤 방식으로 변경 수 수 없습니다. (컨트롤의 부모 창 또는 대화 상자를 처리 해야는 `WM_CTLCOLOR` 메시지입니다.)|  
...
|`WS_CAPTION`|제목 표시줄에 있는 창을 만듭니다 (의미는 `WS_BORDER` 스타일). 함께 사용할 수 없습니다는 `WS_DLGFRAME` 스타일입니다.|  
|`WS_CHILD`|자식 창을 만듭니다. 함께 사용할 수 없습니다는 `WS_POPUP` 스타일입니다.|  
|`WS_CHILDWINDOW`|와 동일는 `WS_CHILD` 스타일입니다.|  
|`WS_CLIPCHILDREN`|부모 창 내에서 그릴 때 자식 창으로 사용 하는 영역을 제외 합니다. 부모 창을 만들 때 사용 합니다.|  
|`WS_CLIPSIBLINGS`|서로 상대적인 클립 자식 창 즉, 특정 자식 창 그리기 메시지 수신의 `WS_CLIPSIBLINGS` 스타일 자르는 다른 모든 중첩된 자식 창을 업데이트할 자식 창의 영역 제거 합니다. (경우 `WS_CLIPSIBLINGS` 제공 되지 않습니다 되며 자식 기간이 겹칩니다, 자식 창의 클라이언트 영역 내에서 그릴 때, 인접 한 자식 창의 클라이언트 영역 내에 그릴 수 있습니다.) 와 함께 사용할는 `WS_CHILD` 만 스타일입니다.|  
|`WS_DISABLED`|처음부터 사용할 창을 만듭니다.|  
|`WS_DLGFRAME`|이중선 있지만 제목 없음 창을 만듭니다.|  
|`WS_GROUP`|이동할 수 한 컨트롤에서 다음 화살표 키를 가진 컨트롤 그룹의 첫 번째 컨트롤을 지정 합니다. 정의 된 모든 컨트롤은 `WS_GROUP` 스타일 `FALSE` 후 첫 번째 컨트롤 같은 그룹에 속해야 합니다. 다음 컨트롤은 `WS_GROUP` 스타일 다음 그룹 (즉, 한 그룹 끝 다음 시작 하는 위치)을 시작 합니다.|  
|`WS_HSCROLL`|가로 스크롤 막대를 보유 하는 창을 만듭니다.|  
|`WS_ICONIC`|가 최소화 처음 창을 만듭니다. 와 동일는 `WS_MINIMIZE` 스타일입니다.|  
|`WS_MAXIMIZE`|최대 크기의 창을 만듭니다.|  
|`WS_MAXIMIZEBOX`|최대화 단추가 포함 된 창을 만듭니다.|  
|`WS_MINIMIZE`|가 최소화 처음 창을 만듭니다. 와 함께 사용할는 `WS_OVERLAPPED` 만 스타일입니다.|  
|`WS_MINIMIZEBOX`|최소화 단추가 포함 된 창을 만듭니다.|  
|`WS_OVERLAPPED`|Overlapped 창을 만듭니다. 캡션 및 테두리에 일반적으로 겹치는 창에 있습니다.|  
|`WS_OVERLAPPEDWINDOW`|와 겹쳐 창을 만듭니다는 `WS_OVERLAPPED`, `WS_CAPTION`, `WS_SYSMENU`, `WS_THICKFRAME`, `WS_MINIMIZEBOX`, 및 `WS_MAXIMIZEBOX` 스타일입니다.|  
|`WS_POPUP`|팝업 창을 만듭니다. 함께 사용할 수 없습니다는 `WS_CHILD` 스타일입니다.|  
|`WS_POPUPWINDOW`|포함 된 팝업 창을 만듭니다는 `WS_BORDER`, `WS_POPUP`, 및 `WS_SYSMENU` 스타일입니다. `WS_CAPTION` 스타일와 결합 되어야는 `WS_POPUPWINDOW` 스타일 컨트롤 메뉴 표시 되도록 합니다.|  
|`WS_SIZEBOX`|크기 조정 테두리가 있는 창을 만듭니다. 와 동일는 `WS_THICKFRAME` 스타일입니다.|  
