...
|이름|설명|  
|----------|-----------------|  
|[CDatabase::BeginTrans](#begintrans)|"트랜잭션" 시작-일련의 호출을 되돌릴 수는 `AddNew`, **편집**, **삭제**, 및 **업데이트** 클래스의 멤버 함수 `CRecordset` -에 연결 된 데이터 원본입니다. 데이터 원본에 대 한 트랜잭션을 지원 해야 **BeginTrans** 을 적용 합니다.|  
...
### <a name="remarks"></a>설명  
 트랜잭션이 구성 된 일련의 호출을는 `AddNew`, **편집**, **삭제**, 및 **업데이트** 의 멤버 함수는 `CRecordset` 으로 시작 하는 개체는 에 대 한 호출에서 [BeginTrans](#begintrans) 멤버 함수입니다. **CommitTrans** 트랜잭션을 커밋합니다. 기본적으로 업데이트는 커밋될 즉시; 호출 **BeginTrans** 될 때까지 지연 될 업데이트의 기여를 사용 하면 **CommitTrans** 호출 됩니다.  
...
### <a name="remarks"></a>설명  
 모든 `CRecordset` `AddNew`, **편집**, **삭제**, 및 **업데이트** 마지막 이후 실행 호출 [BeginTrans](#begintrans) 를 전달 하는 호출 하는 시간에 존재 했던 상태로 합니다.  
