...
### <a name="remarks"></a>설명  
 `CArchive` 소멸자는 정상적으로 호출 `Close`, 모든 데이터를 저장 하지 않은 플러시합니다를 연결 된 `CFile` 개체입니다. 이 예외가 발생할 수 있습니다.  
  
 이러한 예외를 catch 하는 경우는 것이 좋습니다 사용할 `Abort`destructing 되도록는 `CArchive` 개체 추가 예외가 발생 하지 않습니다. 예외 처리 때 `CArchive::Abort` 때문에 오류에 예외를 throw 하지 것입니다 달리 [CArchive::Close](#close), `Abort` 오류를 무시 합니다.  
...
- **CArchive::bNoFlushOnDelete** 보관 파일에서 자동으로 호출 되지 않도록 `Flush` 보관 소멸자가 호출 되는 경우. 명시적으로 호출 하는 일을 담당 하는 경우이 플래그를 설정 하면 `Close` 소멸자가 호출 되기 전에 합니다. 이렇게 하지 않으면 데이터가 손상 됩니다.  
...
### <a name="remarks"></a>설명  
 ...
 멤버 함수 `Close` 모든 데이터가 파일에 보관 파일에서 전송 되어 있는지와 보관 사용할 수 없습니다. 파일에서 전송 하기 위해서는 저장 매체를 완료 하려면 먼저 사용 해야 [CFile::Close](../../mfc/reference/cfile-class.md#close) 를 삭제 합니다는 `CFile` 개체입니다.  
...
### <a name="remarks"></a>설명  
 ...
 A `CObject`-파생된 클래스가 사용할 수 있습니다 **VERSIONABLE_SCHEMA** 결합 (비트를 사용 하 여 **또는**) (IMPLEMENT_SERIAL 매크로)의 스키마 버전 자체와 개체를 만들려면 "블록인," 즉, 한 개체의 `Serialize` 멤버 함수는 여러 버전을 읽을 수 있습니다. 기본 프레임 워크 기능 (없이 **VERSIONABLE_SCHEMA**) 버전이 일치 하지 않을 때 예외를 throw 하는 것입니다.  
...
### <a name="remarks"></a>설명  
 ...
 IMPLEMENT_SERIAL 매크로 클래스 구현에서 사용 되는 경우에 대 한 삽입 연산자 오버 로드 된 `CObject` 호출 하 여 보호 된 `WriteObject`합니다. 이 함수를 호출 하 여 `Serialize` 클래스의 함수입니다.  
  
 [CStringT](../../atl-mfc-shared/reference/cstringt-class.md) 삽입 연산자 (<<) 진단 덤핑 및 보관 파일에 저장을 지원 합니다.  
  
### <a name="example"></a>예  
 사용을 보여 주는이 예제는 `CArchive` 삽입 연산자 <<와 **int** 및 **긴** 형식입니다.  
...
### <a name="remarks"></a>설명  
 ...
 IMPLEMENT_SERIAL 매크로 클래스 구현에서 사용 되는 경우에 대 한 추출 연산자 오버 로드 된 `CObject` 호출 하 여 보호 된 `ReadObject` 함수 (0이 아닌 런타임 클래스 포인터로). 이 함수를 호출 하 여 `Serialize` 클래스의 함수입니다.  
...
### <a name="remarks"></a>설명  
 ...
 사용할 수는 `Read` 내에서 멤버 함수에 `Serialize` 사용자 개체에 포함 된 일반 구조를 읽기 위한 함수입니다.  
...
### <a name="remarks"></a>설명  
 ...
 경우 *pSchema* 은 **NULL**를 호출 하 여 저장 된 클래스의 스키마를 검색할 수 있습니다 [CArchive::GetObjectSchema](#getobjectschema)고, 그렇지 않으면, **\** * * pSchema* 이전 저장 된 런타임 클래스의 스키마에 포함 됩니다.  
...
### <a name="remarks"></a>설명  
 ...
 0이 아닌 제공 하는 경우 *pClass* 으로 얻을 수 있는 매개 변수는 [RUNTIME_CLASS](../../mfc/reference/run-time-object-model-services.md#runtime_class) 매크로, 다음 함수는 보관 된 개체의 런타임 클래스를 확인 합니다. 이 클래스의 구현에 IMPLEMENT_SERIAL 매크로 사용을 가정 합니다.  
...
### <a name="remarks"></a>설명  
 멤버 함수를의 버전에는 *최대* 매개 변수 버퍼는 보류 된 제한 *최대* -1 문자입니다. 읽기는 캐리지 리턴-줄 바꿈 쌍으로 중지 됩니다. 후행 줄 바꿈 문자는 항상 제거 됩니다. Null 문자 ('\0')는 두 경우 모두 추가 됩니다.  
...
### <a name="remarks"></a>설명  
 ...
 사용할 수 있습니다는 `Write` 내에서 멤버 함수에 `Serialize` 일반 구조를 작성 하는 함수 개체에 포함 되어 있습니다.  
...
### <a name="remarks"></a>설명  
 ...
 사용 해야 합니다는 `IMPLEMENT_SERIAL` 보관을 사용 하도록 설정 하는 매크로입니다. `WriteObject` 보관 파일에는 ASCII 클래스 이름을 기록합니다. 이 클래스 이름은 로드 프로세스 동안 나중에 유효성이 검사 됩니다. 특별 한 인코딩 체계에는 불필요 한 클래스의 여러 개체에 대 한 클래스 이름 중복이 되지 않습니다. 이 스키마 또한 하나 이상의 포인터의 대상이 되는 개체의 중복 저장소를 방지 합니다.  
...
### <a name="remarks"></a>설명  
 ...
 `Write` 사용할 수 있지만 null 문자를 종료 하는 대신 파일에 요청된 된 바이트 수가 기록 합니다.  
