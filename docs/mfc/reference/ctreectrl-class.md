...
### <a name="remarks"></a>설명  
 트리 컨트롤 다른 창의 자식 창으로 만들려는 경우 사용 하 여는 `Create` 멤버 함수입니다. 사용 하 여 트리 컨트롤을 만드는 경우 `Create`를 전달 해야 WS_VISIBLE, 다른 트리 보기 스타일에 추가 합니다.  
  
 생성 한 `CTreeCtrl` 두 단계를 수행에서 합니다. 첫 번째 호출 생성자 호출 `Create`, 트리 뷰 컨트롤을 만들고에 연결 하는 `CTreeCtrl` 개체입니다.  
...
### <a name="return-value"></a>반환 값  
 끌기 비트맵 추가 된, 성공 하는 경우 이미지 목록에 대 한 포인터 그렇지 않으면 NULL입니다.  
...
### <a name="parameters"></a>매개 변수  
 *hItem*  
 삭제할 트리 항목의 핸들입니다. 경우 *hitem* TVI_ROOT 값을 가진 트리 뷰 컨트롤에서 모든 항목이 삭제 됩니다.  
...
### <a name="return-value"></a>반환 값  
 성공 하면에 대 한 포인터는 `CEdit` NULL이 고, 그렇지 않으면 해당 항목의 텍스트를 편집 하는 데 사용 되는 개체입니다.  
...
|매개 변수|설명|  
|---------------|-----------------|  
|[in] *fCancelWithoutSave*|편집 작업 또는 작업을 결정 하기 전에 트리 보기 항목의 변경 내용을 저장 하는 FALSE를 결정 하기 전에 트리 보기 항목의 변경 내용을 취소 하려면 TRUE입니다.|  
...
### <a name="return-value"></a>반환 값  
 시스템에 지정된 된 항목이 표시 되는지 확인 하려면 트리 뷰 컨트롤의 항목 스크롤할 경우 TRUE를 반환 합니다. 그렇지 않으면 반환 값은 FALSE입니다.  
...
- TVE_COLLAPSERESET 목록 축소 하 고 자식 항목을 제거 합니다. TVIS_EXPANDEDONCE 상태 플래그 다시 설정 됩니다. 이 플래그는 TVE_COLLAPSE 플래그로 사용 되어야 합니다.  
...
### <a name="return-value"></a>반환 값  
 COLORREF 값 컨트롤에 대 한 현재 창의 배경색을 나타내는입니다. 이 값이-1 이면 컨트롤은 시스템 창 색을 사용 합니다. 이 경우 사용할 수 있습니다 `::GetSysColor(COLOR_WINDOW)` 컨트롤을 사용 하는 현재 시스템 색상을 얻으려고 합니다.  
...
### <a name="parameters"></a>매개 변수  
 *hItem*  
 상태 정보를 받을 수 있는 방법에 대 한 HTREEITEM 합니다.  
...
### <a name="return-value"></a>반환 값  
 성공 하면 자식 항목의 핸들 그렇지 않으면 NULL입니다.  
...
### <a name="return-value"></a>반환 값  
 성공 하면 삭제 된 항목의 핸들 그렇지 않으면 NULL입니다.  
...
### <a name="return-value"></a>반환 값  
 성공 하면 항목 텍스트를 편집 하는 데 편집 컨트롤에 대 한 포인터 그렇지 않으면 NULL입니다.  
...
### <a name="return-value"></a>반환 값  
 첫 번째 표시 되는 항목; 핸들 그렇지 않으면 NULL입니다.  
...
- TVSIL_STATE 트리 보기에에서 있는 항목을 사용자 정의 상태에 대 한 이미지를 포함 하는 상태 이미지 목록을 검색 합니다.  
  
### <a name="return-value"></a>반환 값  
 성공 하면 컨트롤의 이미지 목록에 대 한 포인터 그렇지 않으면 NULL입니다.  
...
|---------------|-----------------|  
|[in] *hItem*|트리 뷰 컨트롤 항목에 대 한 핸들입니다.|  
|[in] *nPart*|파트에 대 한 식별자입니다. TVGIPR_BUTTON로 설정 합니다.|  
...
### <a name="remarks"></a>설명  
 ...
 이 메서드는 Windows SDK에 설명 된 TVM_GETITEMPARTRECT 메시지를 보냅니다. 자세한 내용은 참조는 [TreeView_GetItemPartRect](http://msdn.microsoft.com/library/windows/desktop/bb773847) 매크로입니다.  
 ...
 *nStateMask*  
 검색할 하나 이상의 상태를 나타내는 마스크입니다. 가능한 값에 대 한 자세한 내용은 *nStateMask*의 설명을 참조는 `state` 및 `stateMask` 의 멤버는 [TVITEM](http://msdn.microsoft.com/library/windows/desktop/bb773456) Windows SDK에는 구조입니다.  
  
### <a name="return-value"></a>반환 값  
 NStateMask로 지정 된 값의 비트 OR를 보유 하는 UINT 합니다. 가능한 값에 대 한 자세한 내용은 참조 [CTreeCtrl::GetItem](#getitem)합니다. 특정 상태에 대 한 값을 찾으려면 다음 예제에 나와 있는 것 처럼 상태 값 및 반환 값의 비트 AND 연산을 수행 합니다.  
...
### <a name="return-value"></a>반환 값  
 메서드가 성공 하면 마지막 노드를 확장 되지 않은 항목에 대 한 핸들 그렇지 않으면 NULL입니다.  
  
### <a name="remarks"></a>설명  
 이 메서드는 전송 된 [TVM_GETNEXTITEM](http://msdn.microsoft.com/library/windows/desktop/bb773622) 메시지는 Windows SDK에 설명 되어 있습니다. 자세한 내용은 참조에서 TVGN_LASTVISIBLE 플래그는 *플래그* 해당 메시지의 매개 변수입니다.  
...
- TVGN_CARET 현재 선택 된 항목을 검색합니다.  
  
- 지정 된 항목의 첫 번째 자식 항목을 검색 하는 TVGN_CHILD는 *hItem* 매개 변수입니다.  
...
### <a name="return-value"></a>반환 값  
 성공 하면 다음 항목의 핸들 그렇지 않으면 NULL입니다.  
  
### <a name="remarks"></a>설명  
 이 함수는 검색 되는 항목 트리의 루트 노드인 경우 NULL을 반환 합니다. 예를 들어이 메시지를 사용 하 여 트리 뷰의 루트 노드의 첫 번째 수준 자식에서 TVGN_PARENT 플래그로 경우 메시지는 NULL을 반환 합니다.  
...
### <a name="return-value"></a>반환 값  
 다음 표시 되는 항목; 핸들 그렇지 않으면 NULL입니다.  
...
### <a name="return-value"></a>반환 값  
 부모 항목; 핸들 그렇지 않으면 NULL입니다.  
  
### <a name="remarks"></a>설명  
 이 함수는 지정된 된 항목의 부모는 트리의 루트 노드인 경우 NULL을 반환 합니다.  
...
### <a name="return-value"></a>반환 값  
 이전 표시 되는 항목; 핸들 그렇지 않으면 NULL입니다.  
...
### <a name="return-value"></a>반환 값  
 현재 텍스트 색을 나타내는 COLORREF 값입니다. 이 값이-1 이면 컨트롤 텍스트 색의 시스템 색을 사용 됩니다.  
...
### <a name="return-value"></a>반환 값  
 에 대 한 포인터는 [CToolTipCtrl](../../mfc/reference/ctooltipctrl-class.md) 트리 컨트롤에서 사용할 개체입니다. 경우는 [만들기](#create) TVS_NOTOOLTIPS 스타일을 사용 하는 멤버 함수, 도구 설명 없음 사용 및 NULL이 반환 됩니다.  
 ...
 *pFlags*  
 적중 횟수 테스트 결과 대 한 정보를 수신 하는 정수에 대 한 포인터입니다. 아래 나열 된 값 또는 하나일 수 있습니다는 `flags` 주의 섹션에는 멤버입니다.  
...
### <a name="return-value"></a>반환 값  
 시점을 차지 하는 항목이 없는 경우 지정 된 위치 또는 NULL을 차지 하는 트리 뷰 항목의 핸들입니다.  
  
### <a name="remarks"></a>설명  
 이 함수를 호출할 때는 *pt* 매개 변수를 테스트 한 점의 좌표를 지정 합니다. 함수 시점을 차지 하는 항목이 없는 경우 지정 된 위치 또는 NULL에 있는 항목의 핸들을 반환 합니다. 또한는 *pFlags* 매개 변수에 지정 된 지점의 위치를 나타내는 값을 포함 합니다. 가능한 값은 다음과 같습니다.  
...
### <a name="return-value"></a>반환 값  
 성공 하면 새 항목의 핸들 그렇지 않으면 NULL입니다.  
...
- 지정된 된 항목이 표시 되는 첫 번째 항목은 있도록 세로로 트리 보기 TVGN_FIRSTVISIBLE 스크롤.  
  
### <a name="return-value"></a>반환 값  
 성공하면 0이 아니고, 그렇지 않으면 0입니다.  
  
### <a name="remarks"></a>설명  
 경우 *nCode* TVGN_CARET, 값이 포함 된 경우 부모 창 TVN_SELCHANGING 및 TVN_SELCHANGED 알림 메시지를 수신 합니다. 또한 지정된 된 항목은 축소 된 부모 항목의 자식 이면 부모의 자식 항목 목록이 지정된 된 항목을 표시 하기 위해 확장 됩니다. 이 경우 부모 창 TVN_ITEMEXPANDING 및 TVN_ITEMEXPANDED 알림 메시지를 받습니다.  
...
### <a name="remarks"></a>설명  
 경우 *hItem* 가 NULL 이면이 함수는 없는 항목을 선택 합니다.  
...
### <a name="remarks"></a>설명  
 자동 스크롤 매개 변수는 현재 표시 되지 않는 항목 보기로 스크롤할 하는 데 사용 됩니다. 트리 뷰 컨트롤에는 확장 스타일에 설명 된 TVS_EX_AUTOHSCROLL 있어야 [트리 뷰 컨트롤에 대 한 확장 스타일](http://msdn.microsoft.com/library/windows/desktop/bb759981)합니다.  
...
### <a name="parameters"></a>매개 변수  
 *clr*  
 새 배경 색을 포함 하는 COLORREF 값입니다. 이 값이-1 이면 배경색에 대 한 시스템 색을 사용 하 여 컨트롤이 되돌아갑니다.  
...
### <a name="remarks"></a>설명  
 트리 컨트롤 항목이 선택 되는 경우 ( *fCheck* TRUE로 설정), 인접 한 확인 표시가 있는 항목이 나타납니다.  
...
### <a name="example"></a>예  
 다음 코드 예제에서는 현재 트리 뷰 컨트롤에 TVS_EX_AUTOHSCROLL 확장 스타일을 추가합니다. 코드 예제에서는 표시 되지 않는의 이전 섹션에는 United States에 대 한 루트 국가/지역 노드, 펜실베니아 및 워싱턴의 상태에 대 한 하위 노드가 및 도시 상태에 대 한 트리 항목으로 구성 된 트리 뷰를 만들었습니다. 에서는 의도적으로 트리 뷰 컨트롤 좁은 하 게 포커스가 있는 트리 항목을 표시 하도록 자동으로 스크롤하여 해야 합니다.  
...
### <a name="parameters"></a>매개 변수  
 *pImageList*  
 할당할 이미지 목록에 대 한 포인터입니다. 경우 *pImageList* 가 null 인 경우 모든 이미지 트리 뷰 컨트롤에서 제거 됩니다.  
...
- TVSIL_STATE 트리 보기에에서 있는 항목을 사용자 정의 상태에 대 한 이미지를 포함 하는 상태 이미지 목록을 설정 합니다.  
  
### <a name="return-value"></a>반환 값  
 있는 경우 이전 이미지 목록에 대 한 포인터 그렇지 않으면 NULL입니다.  
...
### <a name="parameters"></a>매개 변수  
 *hItem*  
 항목을 삽입 표시가 배치 되도록 지정 하는 HTREEITEM 합니다. 이 인수가 NULL 이면 삽입 표시가 제거 됩니다.  
  
 *fAfter*  
 지정된 된 항목 전후 삽입 표시가 배치 하는 경우 지정 하는 부울 값입니다. 이 인수를 0이 아닌 경우 삽입 표시가 항목 뒤 배치 됩니다. 이 인수는 0 인 경우 삽입 표시가 항목 앞 배치 됩니다.  
 ...
 *nMask*  
 설정 하는 특성을 지정 하는 정수입니다. 참조는 `mask` 의 멤버는 `TVITEM` 구조입니다.  
 ...
 *nState*  
 항목의 상태에 대 한 값을 지정합니다. 참조는 `State` 의 멤버는 `TVITEM` 구조입니다.  
...
### <a name="remarks"></a>설명  
 에 `TVITEM` 구조는 `hItem` 는 항목을 식별 하는 멤버 및 `mask` 멤버는 특성을 설정 하려면을 지정 합니다.  
  
 경우는 `mask` 멤버 또는 *nMask* TVIF_TEXT 값을 지정 하는 매개 변수는 `pszText` 멤버 또는 *lpszItem* 는 null로 끝나는 문자열의 주소 및 `cchTextMax` 멤버는 무시 됩니다. 경우 `mask` (또는 *nMask*) TVIF_STATE 값을 지정 된 `stateMask` 멤버 또는 *nStateMask* 매개 변수 항목을 변경 하려면 상태를 지정 및 `state` 멤버 또는 *nState* 매개 변수는 해당 상태에 대 한 값을 포함 합니다.  
...
### <a name="parameters"></a>매개 변수  
 *clr*  
 새 텍스트 색을 포함 하는 COLORREF 값입니다. 이 인수는-1을 하는 경우 컨트롤의 텍스트 색에 대 한 시스템 색을 사용 하 여 되돌아갑니다.  
  
### <a name="return-value"></a>반환 값  
 이전 텍스트 색을 나타내는 COLORREF 값입니다. 이 값이-1 이면 컨트롤의 시스템 색을 사용 하 여 텍스트 색에 대 한 했습니다.  
...
### <a name="return-value"></a>반환 값  
 에 대 한 포인터는 [CToolTipCtrl](../../mfc/reference/ctooltipctrl-class.md) 도구 설명에 사용 되 던 컨트롤 또는 NULL 도구 설명 없음 이전에 사용 되는 경우를 포함 하는 개체입니다.  
  
### <a name="remarks"></a>설명  
 만들 때 도구 설명을 사용 하려면 TVS_NOTOOLTIPS 스타일을 나타내는 `CTreeCtrl` 개체입니다.  
...
### <a name="parameters"></a>매개 변수  
 *hItem*  
 정렬 된 자식 항목을 부모 항목의 핸들입니다. 경우 *hItem* 가 null 인 경우 정렬 트리의 루트에서 진행 됩니다.  
