---
title: "CMFCOutlookBarPane 클래스 | Microsoft 문서"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
|[CMFCOutlookBarPane::CanBeAttached](#canbeattached)|창에서 다른 창이 나 프레임 창에 도킹할 수 있는 여부를 결정 합니다. (재정의 [CBasePane::CanBeAttached](../../mfc/reference/cbasepane-class.md#canbeattached).)|  
...
|[CMFCOutlookBarPane::ClearAll](#clearall)|Outlook 표시줄 창에 있는 이미지에서 사용 하는 리소스를 해제 합니다.|  
...
|`CMFCOutlookBarPane::Dock`|Outlook 표시줄 창을 도킹 하려면 프레임 워크에 의해 호출 됩니다. (`CPane::Dock`를 재정의합니다.)|  
|[CMFCOutlookBarPane::EnablePageScrollMode](#enablepagescrollmode)|이동할지 여부 Outlook 표시줄 창에 있는 스크롤 화살표 단추 목록 페이지에서 또는 단추를 클릭 합니다.|  
|[CMFCOutlookBarPane::GetRegularColor](#getregularcolor)|Outlook 표시줄 창 (선택 되지 않은)는 일반 텍스트 색을 반환합니다.|  
|`CMFCOutlookBarPane::GetThisClass`|에 대 한 포인터를 가져오는 데 프레임 워크에 의해는 [CRuntimeClass](../../mfc/reference/cruntimeclass-structure.md) 이 클래스 형식으로 연결 된 개체입니다.|  
...
|[CMFCOutlookBarPane::IsDrawShadedHighlight](#isdrawshadedhighlight)|단추가 강조 표시 하 고 배경 이미지에 표시 되 면 단추 테두리 회색으로 표시 되는지 확인 합니다.|  
|`CMFCOutlookBarPane::OnBeforeFloat`|Float로에 대 한이 되 면 프레임 워크에 의해 호출 됩니다. (재정의 [CPane::OnBeforeFloat](../../mfc/reference/cpane-class.md#onbeforefloat).)|  
...
|[CMFCOutlookBarPane::SetBackColor](#setbackcolor)|배경색을 설정합니다.|  
...
|[CMFCOutlookBarPane::SetTextColor](#settextcolor)|Outlook 표시줄 창에 정규 및 강조 표시 된 텍스트의 색을 설정 합니다.|  
...
## <a name="remarks"></a>주의  
 ...
 Outlook 표시줄의 예를 들어 OutlookDemo 샘플 프로젝트를 참조 하십시오.  
  
## <a name="example"></a>예제  
 다음 예제에서는의 다양 한 메서드를 사용 하는 방법을 `CMFCOutlookBarPane` 클래스입니다. 가 Outlook 표시줄 창을 만들기, 페이지 스크롤 모드를 사용 하도록 설정, 도킹을 사용 하도록 설정 및 Outlook 표시줄의 배경색을 설정 하는 방법을 보여 줍니다. 이 코드 조각은의 일부인는 [Outlook 다중 보기 샘플](../../visual-cpp-samples.md)합니다.  
...
### <a name="parameters"></a>매개 변수  
 ...
 [in] `iInsertAt`  
 Outlook 표시줄의 페이지를 단추를 삽입할&0;부터 시작 인덱스를 지정 합니다.  
...
### <a name="return-value"></a>반환 값  
 `TRUE`단추 추가 되었으면; 그렇지 않으면 `FALSE`합니다.  
  
### <a name="remarks"></a>주의  
 이 메서드를 사용 하 여 Outlook 표시줄의 페이지에 새 단추를 삽입 합니다. 단추의 이미지 또는 디스크 파일에서 응용 프로그램 리소스에서 로드할 수 있습니다.  
  
 페이지 ID에서 지정한 경우 `uiPageID` -1 이면 단추는 첫 번째 페이지에 삽입 됩니다.  
  
 지정 된 인덱스의 경우 `iInsertAt` -1 이면 단추는 페이지의 끝에 추가 됩니다.  
...
##  <a name="clearall"></a>CMFCOutlookBarPane::ClearAll  
 Outlook 표시줄 창에 있는 이미지에서 사용 하는 리소스를 해제 합니다.  
...
### <a name="remarks"></a>주의  
 이 메서드를 직접 호출 [CMFCToolBarImages::Clear](../../mfc/reference/cmfctoolbarimages-class.md#clear), Outlook 표시줄 창에 사용 되는 이미지에서 이라고 합니다.  
...
### <a name="parameters"></a>매개 변수  
 ...
 [in] `dwStyle`  
 창 스타일입니다.  창 스타일의 목록에 대 한 참조 [창 스타일](../../mfc/reference/window-styles.md)합니다.  
  
 [in] `uiID`  
 컨트롤 id입니다. 사용할 수 있도록 고유 해야 컨트롤의 상태를 저장 합니다.  
  
 [in] `dwControlBarStyle`  
 Outlook 표시줄에서 분리 될 때의 Outlook 표시줄 창 컨트롤의 동작을 정의 하는 특별 한 스타일을 지정 합니다.  
...
### <a name="return-value"></a>반환 값  
 반환 `TRUE` 바로 가기 메뉴를 하면 표시 되 고, 그렇지 않으면 `FALSE`합니다.  
  
### <a name="remarks"></a>주의  
 사용자 지정 모드에는 프레임 워크가 표시 되는 프레임 워크 표준 바로 가기 메뉴를 수정 하려면이 메서드를 재정의 합니다.  
  
 기본 구현에서는 사용자 지정 모드를 확인 ( [CMFCToolBar::IsCustomizeMode](../../mfc/reference/cmfctoolbar-class.md#iscustomizemode))로 설정 된 경우 및 `TRUE`를 제외한 모든 바로 가기 메뉴 항목을 사용 하지 않도록 설정 **삭제**합니다. 그런 다음 입력된 매개 변수를 바로 전달 `CMFCToolBar::EnableContextMenuItems`합니다.  
  
> [!NOTE]
> *상황에 맞는 메뉴* 바로 가기 메뉴에 대 한 동의어.  
  
##  <a name="enablepagescrollmode"></a>CMFCOutlookBarPane::EnablePageScrollMode  
 Outlook 표시줄 창에 있는 스크롤 화살표를 페이지 별로 또는 단추에서 단추의 목록을 앞으로 이동 하는지 여부를 지정 합니다.  
...
### <a name="remarks"></a>주의  
 사용 하 여 [CMFCOutlookBarPane::SetTextColor](#settextcolor) Outlook 표시줄의 현재 (일반 및 선택한) 텍스트 색을 설정 합니다. 호출 하 여 기본 텍스트 색을 가져올 수는 [GetSysColor](http://msdn.microsoft.com/library/windows/desktop/ms724371) 작동는 `COLOR_WINDOW` 인덱스입니다.  
...
### <a name="return-value"></a>반환 값  
 `TRUE`배경 이미지를 표시 하 고, 없는 경우 그렇지 않으면 `FALSE`합니다.  
  
### <a name="remarks"></a>주의  
 호출 하 여 배경 이미지를 추가할 수 있습니다 [CMFCOutlookBarPane::SetBackImage](#setbackimage) 함수입니다.  
  
 배경을 사용 하 여 지정 된 색으로 그려집니다 배경 이미지가 없습니다 이면 [CMFCOutlookBarPane::SetBackColor](#setbackcolor)합니다.  
  
##  <a name="isdrawshadedhighlight"></a>CMFCOutlookBarPane::IsDrawShadedHighlight  
 단추가 강조 표시 하 고 배경 이미지에 표시 되 면 단추 테두리 회색으로 표시 되는지 확인 합니다.  
...
### <a name="return-value"></a>반환 값  
 `TRUE`단추의 테두리; 회색으로 표시 된 경우 그렇지 않으면 `FALSE`합니다.  
...
### <a name="return-value"></a>반환 값  
 `TRUE`단추는 성공적으로 제거 되었으면; `FALSE` 경우 지정 된 명령 ID가 올바르지 않습니다.  
...
### <a name="remarks"></a>주의  
 Outlook 표시줄에 대 한 현재 배경색을 설정 하려면이 함수를 호출 합니다. 배경색과 배경 이미지가 없는 경우에 사용 됩니다.  
...
### <a name="remarks"></a>주의  
 Outlook을 설정 하려면이 메서드를 호출 표시줄의 배경 이미지입니다. 배경 이미지의 목록에서 관리 되는 포함 하 여 [CMFCToolBarImages 클래스](../../mfc/reference/cmfctoolbarimages-class.md) 개체입니다.  
...
### <a name="remarks"></a>주의  
 이 메서드는 원래 집합에 Outlook 표시줄 단추를 복원합니다. 이 방법은 `CMFCOutlookBarPane::RestoreOriginalstate`점을 제외 하 고 Outlook 표시줄 창에의 한 다시 그리기를 트리거하지 않습니다.  
...
##  <a name="settextcolor"></a>CMFCOutlookBarPane::SetTextColor  
 Outlook 표시줄 창에 정규 및 강조 표시 된 텍스트의 색을 설정 합니다.  
...
### <a name="remarks"></a>주의  
 투명 한 색은 투명 한 이미지를 표시 해야 합니다. 이미지에서이 색의 모든 항목은 대신 배경색으로 그려집니다.  배경 및 전경 이미지의 혼합이 없습니다.  
