...
### <a name="remarks"></a>설명  
 생성할는 `CProgressCtrl` 두 단계를 수행에서 하는 개체입니다. 먼저 생성자를 호출을 만드는 `CProgressCtrl` 개체를 호출 합니다 `Create`는 진행률 표시줄 컨트롤을 만듭니다.  
...
|[in] *n 간격*|움직이는 텍스트 애니메이션의 한 업데이트 간격 (밀리초)에 대 한 시간입니다.|  
