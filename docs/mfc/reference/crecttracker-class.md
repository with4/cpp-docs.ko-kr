---
title: "CRectTracker 클래스 | Microsoft 문서"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
항목을을 표시, 이동 하 고 다른 방식에서으로 크기가 조정 될 수 있습니다.  
...
|[CRectTracker::DrawTrackerRect](#drawtrackerrect)|테두리를 그릴 때 호출는 `CRectTracker` 개체입니다.|  
...
|[CRectTracker::OnChangedRect](#onchangedrect)|사각형의 크기가 조정 되거나 이동 된 경우 호출 됩니다.|  
...
## <a name="remarks"></a>주의  
 ...
 하지만 `CRectTracker` 클래스는 사용자가 그래픽 인터페이스를 사용 하 여 OLE 항목 상호 작용할 수 있도록 설계 되었습니다의 사용은 OLE 지원 응용 프로그램에 제한 되지 않습니다. 사용할 수 있습니다 사용자 인터페이스가 필요한 경우 언제 든 지 합니다.  
  
 `CRectTracker`테두리 단색 수 또는 점선입니다. 항목을 교차 무늬 테두리를 지정 또는 항목의 다양 한 상태를 나타내기 위해 빗살 무늬로 오버레이되 수 있습니다. 외부 또는 내부에&8; 개의 크기 조정 핸들을 배치할 수 있습니다 항목의 테두리입니다. (참조에 대 한 설명은 크기 조정 핸들 [GetHandleMask](#gethandlemask).) 마지막으로, 한 `CRectTracker` 항목의 크기를 조정 하는 동안 방향을 변경할 수 있습니다.  
  
 사용 하 여 `CRectTracker`, 생성 된 `CRectTracker` 개체를 초기화 되는 표시 상태를 지정 합니다. 이 인터페이스는 사용자 시각적 피드백을 제공할와 연결 된 OLE 항목의 현재 상태에 사용할 수 있습니다는 `CRectTracker` 개체입니다.  
  
 사용 하 여 대 한 자세한 내용은 `CRectTracker`, 문서를 참조 하십시오 [추적기](../../mfc/trackers.md)합니다.  
...
##  <a name="a-nameadjustrecta--crecttrackeradjustrect"></a><a name="adjustrect"></a>CRectTracker::AdjustRect  
 추적 사각형 크기 조정 핸들을 사용 하 여 크기를 조정할 때 프레임 워크에 의해 호출 됩니다.  
...
### <a name="remarks"></a>주의  
 이 함수의 기본 동작은 사각형의 방향을 변경할 경우에만 허용 `Track` 및 `TrackRubberBand` 허용 반전 된 호출 됩니다.  
  
 끌기 작업 중 추적 사각형의 조정 제어 하려면이 함수를 재정의 합니다. 으로 지정 된 좌표를 조정 하는 한 가지 방법은 `lpRect` 반환 하기 전에 합니다.  
...
- **CRectTracker::solidLine** 사각형 테두리에 대 한 실선을 사용 합니다.  
...
- **CRectTracker::hatchInside** Hatched 패턴 전체 사각형에 설명 합니다.  
  
### <a name="remarks"></a>주의  
 기본 생성자는 초기화는 `CRectTracker` 개체의 값으로 `lpSrcRect` 다른 크기 시스템 기본값으로 초기화 합니다. 매개 변수 없이 개체를 만든 경우는 `m_rect` 및 `m_nStyle` 데이터 멤버는 초기화 되지 않습니다.  
...
### <a name="remarks"></a>주의  
 추적기의 스타일은 드로잉을 수행 하는 방법을 결정 합니다. 에 대 한 생성자를 참조 하십시오. `CRectTracker` 사용할 수 있는 스타일에 대 한 자세한 내용은 합니다.  
...
### <a name="parameters"></a>매개 변수  
 `lpRect`  
 에 대 한 포인터는 `RECT` 그릴 사각형을 포함 하는 합니다.  
  
 `pWndClipTo`  
 클리핑 사각형에 사용 하 여 창에 대 한 포인터입니다.  
...
### <a name="remarks"></a>주의  
 크기 조정 핸들 양쪽 및 사각형의 모서리에 표시 되 고 모양과 사각형의 크기를 제어 하는 데 사용할 수 있습니다.  
  
 사각형에는 번호가 0-7 인 8 크기 조정 핸들에 있습니다. 각 크기 조정 핸들 나타내는 비트 마스크;에서 해당 비트의 값은 2 ^ *n*여기서 *n* 크기 조정 핸들 수입니다. 비트 0-3 시계 방향으로 이동 왼쪽 위에서부터 시작 하 고 모퉁이 크기 조정 핸들에 해당 합니다. 크기 조정 핸들 시계 방향으로 위쪽에서 시작을 비트 4-7 옆에 해당 합니다. 다음 그림에서는 사각형의 크기 조정 핸들 및 그에 해당의 크기를 조정 핸들 번호 및 값을 보여 줍니다.  
  
 ![크기 조정 핸들 번호](../../mfc/reference/media/vc35dp1.gif "vc35dp1")  
  
 기본 구현은 **GetHandleMask** 크기 조정 핸들을 표시 하는 비트 마스크를 반환 합니다. 단일 비트에 있으면 해당 크기 조정 핸들이 그려집니다.  
...
### <a name="parameters"></a>매개 변수  
 `lpTrueRect`  
 에 대 한 포인터는 `RECT` 의 좌표를 장치를 포함 하는 구조는 `CRectTracker` 개체입니다.  
  
### <a name="remarks"></a>주의  
 사각형의 크기는 바깥쪽 테두리에 있는 모든 크기 조정 핸들의 너비와 높이 포함 합니다. 를 반환 시 `lpTrueRect` 항상 장치 좌표에 정규화 된 사각형입니다.  
  
##  <a name="a-namehittesta--crecttrackerhittest"></a><a name="hittest"></a>CRectTracker::HitTest  
 사용자가 크기 조정 핸들을 잡고 있는지 여부를 확인 하려면이 함수를 호출 합니다.  
...
### <a name="return-value"></a>반환 값  
 열거 된 형식에 따라 반환 되는 값 **CRectTracker::TrackerHit** 있으며 다음 값 중 하나를 가질 수 있습니다.  
...
##  <a name="a-namemrecta--crecttrackermrect"></a><a name="m_rect"></a>M_rect  
...
### <a name="remarks"></a>주의  
 모두 기본값을 **cx** 및 **cy**, 테두리 너비에 대 한 기본 시스템 값에서 계산 됩니다. 이 데이터 멤버 에서만 사용 되는 `AdjustRect` 멤버 함수입니다.  
...
### <a name="remarks"></a>주의  
 때 `CRectTracker::Track` 또는 `CRectTracker::TrackRubberBand` 라고 허용 뒤집기와 x 축, y 축 또는 둘 다 반전 사각형입니다. 이 경우 `HitTest` 사각형에 대해 반전 또한 핸들을 반환 합니다. 이 피드백 사각형 데이터 구조를 수정 하는 부분이 사각형의 화면 위치에 따라 달라 지므로 그리기 커서 피드백에 대 한 적합 하지 않습니다.  
...
### <a name="remarks"></a>주의  
 ...
 사각형 크기를 조정한 후 작업을 수행 하려는 경우이 함수를 재정의 합니다.  
  
##  <a name="a-namesetcursora--crecttrackersetcursor"></a><a name="setcursor"></a>CRectTracker::SetCursor  
 위에 있는 동안 커서 모양을 변경 하려면이 함수를 호출 하는 `CRectTracker` 개체의 지역입니다.  
...
### <a name="parameters"></a>매개 변수  
 ...
 `nHitTest`  
 이전 적중된 테스트의 결과에서 `WM_SETCURSOR` 메시지입니다.  
  
### <a name="return-value"></a>반환 값  
 이전 결과 추적기 사각형; 통해 경우 0이 아닌 그렇지 않으면 0입니다.  
  
### <a name="remarks"></a>주의  
 함수 내부에서 처리 하는 창에서이 함수를 호출 하 여 `WM_SETCURSOR` 메시지 (일반적으로 `OnSetCursor`).  
...
### <a name="parameters"></a>매개 변수  
 ...
 `bAllowInvert`  
 경우 **TRUE**, x 축 또는 y 축을 따라 반전 하 고, 그렇지 않으면 사각형 수 **FALSE**합니다.  
  
 `pWndClipTo`  
 창 그리기 작업에 클리핑됩니다입니다. 경우 **NULL**, `pWnd` 클리핑 사각형으로 사용 됩니다.  
  
### <a name="return-value"></a>반환 값  
 ESC 키를 누르면 추적 프로세스는 중지, 추적기에 저장 되는 사각형 변경 되지 않으면 및 0이 반환 됩니다. 변경 된 커밋된 경우 마우스를 이동 하 고 마우스 왼쪽된 단추를 해제 하 여 새 위치 및/또는 크기는 기록 추적기의 사각형에 되며&0;이 아닌 값이 반환 됩니다.  
  
### <a name="remarks"></a>주의  
 이 일반적으로 호출에서 처리 하는 응용 프로그램의 함수 내부에서 `WM_LBUTTONDOWN` 메시지 (일반적으로 `OnLButtonDown`).  
  
 이 함수는 사용자가 왼쪽된 마우스 단추를, ESC 키를 누르거나 마우스 오른쪽 단추를 누를 때까지 마우스를 캡처합니다. 마우스 커서를 이동할 피드백을 호출 하 여 업데이트 됩니다 `DrawTrackerRect` 및 `OnChangedRect`합니다.  
  
 경우 `bAllowInvert` 는 **TRUE**, x 축 또는 y 축에서 추적 사각형을 반전 시킬 수 있습니다.  
...
### <a name="parameters"></a>매개 변수  
 ...
 `bAllowInvert`  
 경우 **TRUE 이면** x 축 또는 y 축을 따라 반전 하 고, 그렇지 않으면 사각형 수 **FALSE**합니다.  
  
### <a name="return-value"></a>반환 값  
 마우스를 이동 하 고 사각형이 비어 있습니다. 0이 아닌 그렇지 않으면 0입니다.  
  
### <a name="remarks"></a>주의  
 일반적으로 처리 하는 응용 프로그램의 함수 내에서 호출 되어는 `WM_LBUTTONDOWN` 메시지 (일반적으로 `OnLButtonDown`).  
  
 이 함수는 사용자가 왼쪽된 마우스 단추를, ESC 키를 누르거나 마우스 오른쪽 단추를 누를 때까지 마우스를 캡처합니다. 마우스 커서를 이동할 피드백을 호출 하 여 업데이트 됩니다 `DrawTrackerRect` 및 `OnChangedRect`합니다.  
  
 추적은 오른쪽 아래 파일에 대 한 핸들에서 고무 밴드 형식 선택을 사용 하 여 수행 됩니다. 반전 허용 되는 경우 사각형 중 하나를 왼쪽 또는 아래쪽 및 오른쪽으로 끌어서 조정할 수 있습니다.  
