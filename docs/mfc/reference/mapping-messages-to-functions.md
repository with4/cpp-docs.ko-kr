...
3.  메시지 처리기가 없으면, 같이 처리기의 제안 된 이름을 표시 하려면 속성 창의 오른쪽 열에 셀을 클릭 한 다음 \<추가 >*HandlerName*합니다. (예를 들어 WM_TIMER 메시지 처리기 제안 \<추가 >`OnTimer`).  
