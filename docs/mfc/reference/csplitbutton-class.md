...
|이름|설명|  
|----------|-----------------|  
|[CSplitButton::OnDropDown](#ondropdown)|현재는 분할 단추 컨트롤의 드롭다운 화살표를 클릭 하면 시스템에서 보내는 BCN_DROPDOWN 알림을 처리 합니다.|  
  
## <a name="remarks"></a>설명  
 `CSplitButton` 클래스에서 파생 된는 [CButton](../../mfc/reference/cbutton-class.md) 클래스입니다. 분할 단추 컨트롤에 스타일은 BS_SPLITBUTTON 단추 컨트롤이입니다. 드롭다운 화살표를 클릭 하면 사용자 지정 메뉴를 표시 합니다. 자세한 내용은 참조에서 BS_SPLITBUTTON 및 BS_DEFSPLITBUTTON 스타일 [단추 스타일](http://msdn.microsoft.com/library/windows/desktop/bb775951)합니다.  
...
|[in] *pMenu*|에 대 한 포인터는 [CMenu](../../mfc/reference/cmenu-class.md) 하위 메뉴를 지정 하는 개체입니다. `CSplitButton` 삭제 개체의 `CMenu` 개체와 해당 관련된 HMENU 때는 `CSplitButton` 범위를 벗어나면 합니다.|  
  
### <a name="remarks"></a>설명  
 사용 하 여는 [CSplitButton::Create](#create) 분할 단추 컨트롤을 만들고에 연결 하는 메서드는 `CSplitButton` 개체입니다.  
  
##  <a name="ondropdown"></a>  CSplitButton::OnDropDown  
 현재는 분할 단추 컨트롤의 드롭다운 화살표를 클릭 하면 시스템에서 보내는 BCN_DROPDOWN 알림을 처리 합니다.  
...
### <a name="remarks"></a>설명  
 시스템 BCN_DROPDOWN 알림을 전송 분할 단추 컨트롤에서 드롭다운 화살표를 클릭할 때는 메시지는 `OnDropDown` 메서드 핸들입니다. 그러나는 `CSplitButton` BCN_DROPDOWN 알림 분할 단추 컨트롤을 포함 하는 컨트롤에 개체를 전달 하지 않습니다. 따라서 포함 하는 컨트롤 알림에 대 한 응답으로 사용자 지정 작업을 지원할 수 없습니다.  
  
 포함 하는 컨트롤을 지원 하는 사용자 지정 동작을 구현 하려면 사용을 [CButton](../../mfc/reference/cbutton-class.md) 대신 BS_SPLITBUTTON 스타일과 개체는 `CSplitButton` 개체입니다. 그런 다음에서 BCN_DROPDOWN 알림에 대 한 처리기를 구현는 `CButton` 개체입니다. 자세한 내용은 참조 [단추 스타일](../../mfc/reference/styles-used-by-mfc.md#button-styles)합니다.  
  
 사용 하는 분할 단추 컨트롤 자체 지원 하 여 사용자 지정 동작을 구현 하려면 [메시지 리플렉션](../../mfc/tn062-message-reflection-for-windows-controls.md)합니다. 고유한 클래스를 파생 되는 `CSplitButton` 클래스 하 고 이름을, 예를 들어 CMySplitButton 합니다. 다음과 같은 메시지 맵을 BCN_DROPDOWN 알림 메시지를 처리 하도록 응용 프로그램에 추가 합니다.  
...
### <a name="example"></a>예  
 다음 코드 예제에서 첫 번째 문과 [CSplitButton::SetDropDownMenu](#setdropdownmenu) 메서드. IDR_MENU1 메뉴 모음 ID를 자동으로 이름이 지정 된 편집기의 메뉴는 Visual Studio 리소스와 만든 합니다. *nSubMenuId* 매개 변수를 0 인 메뉴 표시줄의 유일한 하위 메뉴를 가리킵니다.  
