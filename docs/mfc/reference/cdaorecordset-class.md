...
>  레코드를 편집 하는 경우 및를 호출 하지 않고 다른 레코드로 아래로 스크롤한 후 **업데이트**, 변경 내용이 경고 없이 손실 됩니다.  
 ...
 새 레코드의 위치를 레코드 집합의 유형에 따라 달라 집니다.  
 ...
 **삭제** 현재 레코드를 제거 하 고 액세스할 수 없게 합니다. 편집 하거나 삭제 한 레코드를 사용 하 여 수, 없지만 현재 유지 됩니다. 그러나 사용자가 다른 레코드로 이동 하면 만들 수 없습니다 삭제 된 레코드 현재 다시 합니다.  
...
### <a name="remarks"></a>주의  
 ...
 테이블 형식의 레코드 집합에서 레코드를 찾으려면 호출의 [Seek](#seek) 멤버 함수입니다.  
...
### <a name="remarks"></a>주의  
 ...
 레코드 집합에서 레코드가 없는 경우로, 현재 레코드가 없을 경우 1이 반환 됩니다. 현재 레코드가 삭제 되 면 AbsolutePosition 속성 값이 정의 되지 않은 및 MFC 참조 하는 경우 예외를 throw 합니다. 다이너셋 형식의 레코드 집합에 대 한 새 레코드 시퀀스의 끝에 추가 됩니다.  
...
### <a name="parameters"></a>매개 변수  
 ...
 `dwInfoOptions`  
 레코드 집합을 검색 하는 방법에 대 한 정보를 지정 하는 옵션입니다. 사용 가능한 옵션은 한 원인을 반환 하는 함수를 함께 여기 나열 됩니다. 최상의 성능을 위해 필요한 정보 수준의 검색 합니다.  
  
- `AFX_DAO_PRIMARY_INFO`(기본값) 이름, 유형, 크기, 속성  
  
- `AFX_DAO_SECONDARY_INFO`기본 정보, plus: 서 수 위치에 필요한 길이, 데이터 정렬 순서, 외부 이름, 원본 필드를 원본 테이블&0; 허용  
  
- `AFX_DAO_ALL_INFO`기본 및 보조 정보를 더한: Default Value, 유효성 검사 규칙 유효성 검사 텍스트  
...
### <a name="parameters"></a>매개 변수  
 ...
 `nIndex`  
 인덱스에 의해 조회에 대 한 레코드 집합의 필드 컬렉션의 필드는&0;부터 시작 하는 인덱스입니다.  
  
### <a name="return-value"></a>반환 값  
 두 버전의 `GetFieldValue` 값을 반환 하는 반환 된 [COleVariant](../../mfc/reference/colevariant-class.md) 필드의 값이 포함 된 개체입니다.  
  
### <a name="remarks"></a>주의  
 이름 또는 서 수 위치 필드를 찾을 수 있습니다.  
  
> [!NOTE]
>  것이 더 효율적이 멤버 함수는 버전 중 하나를 호출 하는 `COleVariant` 매개 변수를 반환 하는 버전을 호출 하는 대신 개체 참조는 `COleVariant` 개체입니다. 이 함수의 두 번째 버전은 이전 버전과 호환성을 위해 유지 됩니다.  
  
 사용 하 여 `GetFieldValue` 및 [SetFieldValue](#setfieldvalue) 실행된 시간 보다는 정적으로 사용 하 여 바인딩 열에 필드를 동적으로 바인딩할는 [DoFieldExchange](#dofieldexchange) 메커니즘입니다.  
  
 `GetFieldValue`고 `DoFieldExchange` 성능을 향상 시키기 위해 메커니즘을 결합할 수 있습니다. 예를 들어 사용 `GetFieldValue` 를, 필요할 경우에 해야 하는 값을 검색 하 여 인터페이스의 "추가 정보" 단추에 대 한 호출을 할당 합니다.  
...
### <a name="return-value"></a>반환 값  
 테이블 형식의 레코드 집합의 인덱스 수입니다.  
  
### <a name="remarks"></a>주의  
 `GetIndexCount`레코드 집합의 모든 인덱스를 통해 반복 하는 데 유용 합니다. 이 위해 사용 하 여 `GetIndexCount` 함께에서 [GetIndexInfo](#getindexinfo)합니다. 다이너셋 형식 또는 스냅숏 형식의 레코드 집합에서이 멤버 함수를 호출 하면 MFC 예외를 throw 합니다.  
...
### <a name="parameters"></a>매개 변수  
 `nIndex`  
 숫자 형식의 위치 하 여 조회에 대 한 테이블의 인덱스 컬렉션에서&0;부터 시작 하는 인덱스입니다.  
  
 `indexinfo`  
 에 대 한 참조는 [CDaoIndexInfo](../../mfc/reference/cdaoindexinfo-structure.md) 구조입니다.  
  
 `dwInfoOptions`  
 인덱스를 검색 하는 방법에 대 한 정보를 지정 하는 옵션입니다. 사용 가능한 옵션은 한 원인을 반환 하는 함수를 함께 여기 나열 됩니다. 최상의 성능을 위해 필요한 정보 수준의 검색 합니다.  
  
- `AFX_DAO_PRIMARY_INFO`(기본값) 이름, 필드 정보 필드  
  
- `AFX_DAO_SECONDARY_INFO`기본 정보, plus: 주, Unique, Clustered, 필요한 IgnoreNulls 외부  
...
### <a name="return-value"></a>반환 값  
 A `COleVariant` 추가 되거나 변경 된 레코드 나타내는 가장 최근에 책갈피를 포함 합니다.  
...
### <a name="remarks"></a>주의  
 ...
 레코드를 추가 하는 경우 레코드 집합의 끝에 나타납니다과 현재 레코드를 않습니다. 새 레코드를 현재 레코드로 호출 `GetLastModifiedBookmark` 다음 호출 `SetBookmark` 새로 추가 된 레코드 돌아갑니다.  
...
### <a name="return-value"></a>반환 값  
 잠금 유형을 최악 하면 0이 아니고 낙관적 레코드 잠금에 대 한 그렇지 않으면 0입니다.  
  
### <a name="remarks"></a>주의  
 호출 하는 즉시 잠긴 경우 비관적 잠금이 적용 됩니다, 편집 하는 레코드를 포함 하는 데이터 페이지는 [편집](#edit) 멤버 함수입니다. 페이지 잠금이 해제 호출 하는 경우는 [업데이트](#update) 또는 [닫기](#close) 멤버 함수 또는 이동 또는 찾기 작업 중 하나입니다.  
  
 와 레코드를 업데이트 하는 동안에 레코드를 포함 하는 데이터 페이지를 잠겨 때 낙관적 잠금이 적용 됩니다는 **업데이트** 멤버 함수입니다.  
...
### <a name="remarks"></a>주의  
 레코드 집합의 이름을 문자로 시작 해야 하며 최대 40 자까지 포함할 수 있습니다. 숫자를 포함할 수 및 밑줄 문자는 문장 부호 또는 공백을 포함할 수 없습니다.  
...
### <a name="return-value"></a>반환 값  
 레코드 집합에서 레코드의 백분율을 기반으로 레코드 집합 개체의 현재 레코드의 대략적인 위치를 나타내는 0에서 100 사이의 숫자입니다.  
  
### <a name="remarks"></a>주의  
 이동할 수 있습니다 마지막 레코드를 호출 하 여 [MoveLast](#movelast) 를 완료 했지만 모든 레코드 집합의 채우기 걸릴 수 있습니다 상당한 시간이 있습니다.  
  
 호출할 수 있습니다 `GetPercentPosition` recordset 개체의 모든 세 가지 형식에 대해 인덱스가 없는 테이블을 포함 합니다. 그러나 호출할 수 없습니다 `GetPercentPosition` 정방향 스크롤 스냅숏 또는 외부 데이터베이스에 대 한 통과 쿼리에서 연 레코드 집합입니다. 현재 레코드가 없는 또는 그 현재 레코드를 삭제 하는 경우는 `CDaoException` throw 됩니다.  
...
### <a name="remarks"></a>주의  
 `GetRecordCount`모든 레코드 액세스 되기 전까지 레코드 수가 다이너셋 형식 또는 스냅숏 형식의 레코드 집합에 포함 된을 나타내지 않습니다. 이 멤버 함수 호출을 완료 하려면 시간이 많이 걸릴 수 있습니다.  
  
 마지막 레코드에 액세스 되 면 반환 값은 레코드 집합에서 삭제 되지 않은 레코드의 총 수를 나타냅니다. 호출에 액세스 하는 마지막 레코드를 적용 하려면는 `MoveLast` 또는 `FindLast` 레코드 집합에 대 한 멤버 함수입니다. 또한 여 쿼리에서 반환 될 레코드의 대략적인 수를 확인 하려면 SQL Count를 사용할 수 있습니다.  
  
 다이너셋 형식의 레코드 집합의 반환 값의 레코드를 삭제 하는 응용 프로그램으로 `GetRecordCount` 감소 합니다. 그러나 다른 사용자가 삭제 된 레코드에서 반영 되지 않습니다 `GetRecordCount` 도달할 때까지 현재 레코드는 삭제 된 레코드입니다. 레코드 수에 영향을 주는 트랜잭션을 실행 하 고 트랜잭션을 롤백할 경우 `GetRecordCount` 나머지 레코드의 실제 수를 반영 하지 것입니다.  
  
 값 `GetRecordCount` 스냅숏 형식의 레코드 집합에서 영향을 받지 않는 기본 테이블의 변경 내용입니다.  
  
 값 `GetRecordCount` 테이블 형식에서 레코드 집합 테이블의 레코드의 대략적인 수를 반영 하 고는 영향을 즉시 테이블 레코드를 추가 하 고 삭제 합니다.  
  
 레코드가 없는 레코드 집합 값이 0 반환합니다. ODBC 데이터베이스 또는 연결 된 테이블을 작업할 때 `GetRecordCount` 1 – 항상 반환 합니다. 호출 된 **Requery** 레코드 집합에서 멤버 함수 값을 다시 설정 `GetRecordCount` 하면 쿼리가 다시 실행 된 것 처럼입니다.  
...
### <a name="return-value"></a>반환 값  
 A `CString` 하는 SQL 문을 포함 합니다.  
  
### <a name="remarks"></a>주의  
 일반적으로 SQL 든다고 **선택** 문입니다.  
  
 반환한 문자열 `GetSQL` 일반적으로 다른 모든 문자열에서 레코드 집합에 전달 했습니다는 `lpszSQL` 매개 변수를는 [열려](#open) 멤버 함수입니다. 레코드 집합에 전달 하는 내용에 따라 전체 SQL 문을 생성 하기 때문에 이것이 **열려**, 클래스 마법사를 사용 하 여 지정 된 내용 및 어떻게 지정에 [m_strFilter](#m_strfilter) 및 [m_strSort](#m_strsort) 데이터 멤버입니다.  
  
> [!NOTE]
>  이 멤버 함수를 호출한 후에 호출 **열려**합니다.  
...
### <a name="return-value"></a>반환 값  
 A `CString` 변경 이나 테이블에 추가 되는 레코드의 데이터 유효성을 검사 하는 값을 포함 하는 개체입니다.  
  
### <a name="remarks"></a>주의  
 이 규칙은 텍스트 기반 이므로 기본 테이블이 변경 될 때마다 적용 됩니다. 데이터를 사용할 수 없는 경우 MFC는 예외를 throw 합니다. 반환 된 오류 메시지를 지정 하는 경우 기본 필드 개체의 유효성 검사 텍스트 속성의 텍스트 또는 내부 필드 개체의 유효성 검사 규칙 속성에 지정 된 식의 텍스트입니다. 호출할 수 있습니다 [GetValidationText](#getvalidationtext) 오류 메시지의 텍스트를 얻을 수 있습니다.  
  
 예를 들어 월의 일을 해야 하는 레코드의 필드는 규칙이 있을 수 유효성 검사와 같은 "일 BETWEEN 1에서 31."  
...
### <a name="return-value"></a>반환 값  
 A `CString` 필드의 값은 기본 필드 개체의 유효성 검사 규칙을 만족 하지 않는 경우 표시 되는 메시지의 텍스트를 포함 하는 개체입니다.  
...
### <a name="remarks"></a>주의  
 호출할 수도 있습니다 `IsBOF` 와 함께 `IsEOF` 레코드 집합 모든 레코드를 포함 하거나 비어 있는지 확인 하려면. 호출한 후에 즉시 **열려**레코드 집합에 레코드가 없는 경우, `IsBOF`&0;이 아닌 값을 반환 합니다. 첫 번째 레코드는 현재 레코드 하나 이상의 레코드가 포함 된 레코드 집합을 열 때 및 `IsBOF` 0을 반환 합니다.  
  
 첫 번째 레코드는 현재 기록 된 경우 호출 하면 `MovePrev`, `IsBOF` 이후에&0;이 아닌 반환 합니다. 경우 `IsBOF`&0;이 아닌 값을 반환 하 고 호출 `MovePrev`, 예외가 throw 됩니다. 경우 `IsBOF`&0;이 아닌 값을 반환, 현재 레코드는 정의 되지 하 고 현재 레코드를 필요로 하는 모든 작업 예외가 발생 합니다.  
  
 특정 메서드의 효과 `IsBOF` 및 `IsEOF` 설정:  
  
-   호출 **열려** 내부적으로 사용 하면 첫 번째 레코드는 레코드 집합의 현재 레코드를 호출 하 여 **MoveFirst**합니다. 따라서 호출 **열려** 레코드 원인의 빈 집합에서 `IsBOF` 및 `IsEOF`&0;이 아닌 값을 반환 하 합니다. (실패 한 작업의 동작에 대 한 다음 표를 참조 **MoveFirst** 또는 `MoveLast` 를 호출 합니다.)  
  
-   성공적으로 레코드를 찾는 이동 작업의 모든 발생 모두 `IsBOF` 및 `IsEOF` 0을 반환할 수 있습니다.  
  
-   `AddNew` 호출을 차례로 **업데이트** 새 레코드를 삽입 하는 성공적으로 호출 하면 `IsBOF` 0, 하지만 경우에만 반환 하 `IsEOF` 0이 아닌 이미 있습니다. 상태 `IsEOF` 은 항상 변경 되지 않습니다. Microsoft Jet 데이터베이스 엔진에 정의 된 대로 빈 레코드 집합의 현재 레코드 포인터 이므로 파일의 끝에 새 레코드가 현재 레코드 뒤에 삽입 됩니다.  
...
|------|-------------------------|-----------------------------|------------|-----------------------------|  
|`IsBOF`=&0;이 아닌 경우<br /><br /> `IsEOF`=0|Allowed|예외|예외|Allowed|  
|`IsBOF`=0,<br /><br /> `IsEOF`0이 아닌 값 =|Allowed|Allowed|예외|예외|  
|둘 다&0;이 아닌|예외|예외|예외|예외|  
|모두 0|Allowed|Allowed|Allowed|Allowed|  
  
 허용 하는 이동 작업을 작업 레코드를 제대로 찾는 것은 아닙니다. 지정된 된 이동 작업을 수행 하려고는 허용 하 고 예외를 생성 하지 것입니다 의미일 뿐입니다. 값은 `IsBOF` 및 `IsEOF` 멤버 함수는 move 메서드 실행된의 결과로 변경 될 수 있습니다.  
  
 값에는 레코드를 찾지 못하면 이동 작업의 효과 `IsBOF` 및 `IsEOF` 설정은 다음 표에 표시 됩니다.  
...
> [!NOTE]
>  스냅숏 또는 테이블 형식의 레코드 집합의 레코드에 대 한 삭제 된 상태를 확인할 필요가 없습니다. 호출 하지 않아도 됩니다 스냅숏에서 레코드를 삭제할 수 없으므로, `IsDeleted`합니다. 테이블 형식의 레코드 집합에 대 한 삭제 된 레코드는 레코드 집합에서 실제로 제거 됩니다. 레코드가 다른 레코드 집합 또는 다른 사용자를 통해 삭제 된 후 해당 레코드를 다시 스크롤할 수 없습니다. 따라서는 호출할 필요가 없습니다 `IsDeleted`합니다.  
  
 다이너셋에서 레코드를 삭제 하는 경우 레코드 집합에서 제거 되 고 해당 레코드로 다시 스크롤할 수 없습니다. 그러나의 레코드에 다이너셋은 삭제 또는 다른 사용자가 동일한 테이블을 기반으로 하는 다른 레코드 집합에서 `IsDeleted` 돌아옵니다 **TRUE** 때 나중에 스크롤할 때 해당 레코드입니다.  
...
### <a name="remarks"></a>주의  
 호출할 수도 있습니다 `IsEOF` 레코드 집합 모든 레코드를 포함 하거나 비어 있는지 확인 하려면. 호출한 후에 즉시 **열려**레코드 집합에 레코드가 없는 경우, `IsEOF`&0;이 아닌 값을 반환 합니다. 첫 번째 레코드는 현재 레코드 하나 이상의 레코드가 포함 된 레코드 집합을 열 때 및 `IsEOF` 0을 반환 합니다.  
  
 마지막 레코드의 현재 레코드 인지를 호출할 때 `MoveNext`, `IsEOF` 이후에&0;이 아닌 반환 합니다. 경우 `IsEOF`&0;이 아닌 값을 반환 하 고 호출 `MoveNext`, 예외가 throw 됩니다. 경우 `IsEOF`&0;이 아닌 값을 반환, 현재 레코드는 정의 되지 하 고 현재 레코드를 필요로 하는 모든 작업 예외가 발생 합니다.  
  
 특정 메서드의 효과 `IsBOF` 및 `IsEOF` 설정:  
  
-   호출 **열려** 내부적으로 사용 하면 첫 번째 레코드는 레코드 집합의 현재 레코드를 호출 하 여 **MoveFirst**합니다. 따라서 호출 **열려** 레코드 원인의 빈 집합에서 `IsBOF` 및 `IsEOF`&0;이 아닌 값을 반환 하 합니다. (실패 한 작업의 동작에 대 한 다음 표를 참조 **MoveFirst** 를 호출 합니다.)  
...
### <a name="remarks"></a>주의  
 모든 더티 필드 데이터 멤버의 데이터를 전송할 레코드에 데이터 원본에 대해 호출 하 여 현재 레코드가 업데이트 될 때의 **업데이트** 의 멤버 함수 `CDaoRecordset` (에 대 한 호출을 다음 **편집** 또는 `AddNew`). 이러한 지식을 바탕으로 작업을 수행할 추가,와 같은 해제는 필드 데이터 멤버를 데이터 원본에 쓸 수는 열을 표시 합니다.  
...
### <a name="remarks"></a>주의  
 (데이터베이스 용어에서 "가치가 없는" 의미를 Null 아니며 동일 **NULL** c + +에서.) 필드 데이터 멤버를 Null로 플래그가 지정 되어 없는 값이 없는 현재 레코드의 열으로 해석 됩니다.  
...
### <a name="return-value"></a>반환 값  
 지정 된 필드 데이터 멤버를 만들 수 있습니다 null 인 경우 0이 아닌 그렇지 않으면 0입니다.  
...
### <a name="return-value"></a>반환 값  
 0이 아닌 값 레코드 집합 개체의 **열려** 또는 **Requery** 멤버 함수를 이전에 호출 하 고 레코드 집합 닫히지 않은; 그렇지 않으면 0입니다.  
...
### <a name="remarks"></a>주의  
 기본적으로 플래그 **TRUE**합니다. 이 데이터 멤버의 설정은 전체 이중 버퍼링 메커니즘을 제어합니다. 플래그를 설정 하는 경우 **TRUE**, DFX 메커니즘을 사용 하 여 필드 별로 캐싱을 해제할 수 있습니다. 플래그를 설정 하는 경우 **FALSE**를 호출 해야 `SetFieldDirty` 및 `SetFieldNull` 자신입니다.  
...
### <a name="remarks"></a>주의  
 레코드 집합 클래스에 대 한 생성자를 초기화 해야 `m_nFields` 정적으로 바인딩된 필드의 올바른 번호로 합니다. 레코드 집합 클래스 선언에 사용 하는 경우이 초기화를 작성 하는 classwizard 함께 사용 합니다. 쓸 수 있습니다도 수동으로.  
  
 이 번호를 사용 하 여 필드 데이터 멤버와 데이터 소스에서 현재 레코드의 해당 열 간의 상호 작용을 관리 하는 프레임 워크입니다.  
  
> [!NOTE]
>  이 숫자에 등록 하는 출력 열 수가 일치 해야 `DoFieldExchange` 를 호출한 후 `SetFieldType` 매개 변수와 함께 **CDaoFieldExchange::outputColumn**합니다.  
...
### <a name="remarks"></a>주의  
 레코드 집합 클래스에 매개 변수 데이터 멤버를 하는 경우에 클래스의 생성자를 초기화 해야 `m_nParams` 올바른 번호입니다. 값 `m_nParams` 기본값은 0입니다. 매개 변수 데이터 멤버를 추가 하는 경우-수동으로 수행 해야 하는-매개 변수 개수를 반영 하도록 클래스 생성자에서 초기화를 수동으로 추가 해야 (최소한의 수와 크기와 같아야 '의 자리 표시자 프로그램 **m_strFilter** 또는 `m_strSort` 문자열)입니다.  
  
 레코드 집합의 쿼리를 매개 변수화 하는 경우이 값을 사용 하는 프레임 워크입니다.  
  
> [!NOTE]
>  이 숫자에 등록 된 "params"의 수와 일치 해야 `DoFieldExchange` 를 호출한 후 `SetFieldType` 매개 변수와 함께 **CFieldExchange::param**합니다.  
...
### <a name="remarks"></a>주의  
 이 변수는 두 가지 방법으로 설정 합니다. 일반적으로 이미 열려 있는에 포인터를 전달 하면 `CDaoDatabase` recordset 개체를 생성할 때 개체입니다. 전달 하는 경우 **NULL** 대신 **CDaoRecordset** 만듭니다는 `CDaoDatabase` 수에 대 한 개체를 엽니다. 두 경우 모두 `CDaoRecordset` 이 변수에 포인터를 저장 합니다.  
  
 일반적으로 직접 원하지에 저장 된 포인터를 사용 하 여 **m_pDatabase**합니다. 그러나에 대 한 고유한 확장을 작성 하는 경우 `CDaoRecordset`, 포인터를 사용 하도록 할 수 있습니다. 예를 들어 할 수 있습니다 포인터 throw 되는 경우 사용자 고유의 `CDaoException`(s).  
...
### <a name="remarks"></a>주의  
 예약어는 포함 되지 않습니다 **여기서** 를 레코드 집합을 필터링 합니다. 이 데이터 멤버를 사용 하는 테이블 형식의 레코드 집합에 적용 되지 않습니다. 사용 **m_strFilter** 사용 하 여 레코드 집합을 열 때 아무 효과가 `CDaoQueryDef` 포인터입니다.  
  
 미국 날짜 형식 (월-일-연도)를 사용 하 여 Microsoft Jet 데이터베이스 엔진; (미국) 버전을 사용 하지 않는 경우에 날짜를 포함 하는 필드를 필터링 하는 경우 그렇지 않으면 예상 대로 데이터를 필터링 하지 않을 수 있습니다.  
...
### <a name="remarks"></a>주의  
 다이너셋 및 스냅숏 형식 recordset 개체를 정렬할 수 있습니다.  
  
 테이블 형식 recordset 개체를 정렬할 수 없습니다. 테이블 형식의 레코드 집합의 정렬 순서를 확인 하려면 호출 [SetCurrentIndex](#setcurrentindex)합니다.  
...
### <a name="parameters"></a>매개 변수  
 `lRows`  
 앞으로 또는 뒤로 이동 하는 레코드의 수입니다. 양수 값을 레코드 집합의 끝 방향으로 앞으로 이동 합니다. 음수 값 시작 부분을 향해 뒤로 이동합니다.  
  
### <a name="remarks"></a>주의  
 앞으로 또는 뒤로 이동할 수 있습니다. `Move( 1 )`에 해당 `MoveNext`, 및 `Move( -1 )` 같습니다 `MovePrev`합니다.  
  
> [!CAUTION]
>  호출 해도 **이동** 함수는 레코드 집합에 레코드가 없는 경우 예외를 throw 합니다. 일반적으로 둘 다 호출 `IsBOF` 및 `IsEOF` 레코드 집합에 레코드가 있는지 확인 하기 위해 이동 작업이 이전 합니다. 호출한 후 **열려** 또는 **Requery**, 호출 `IsBOF` 또는 `IsEOF`.  
  
> [!NOTE]
>  시작 또는 레코드 집합의 끝을 지나서 스크롤 하는 경우 ( `IsBOF` 또는 `IsEOF`&0;이 아닌 값을 반환), 호출을 **이동** throw 한 `CDaoException`합니다.  
  
> [!NOTE]
>  중 하나를 호출 하는 경우는 **이동** 작동 중에 현재 레코드는 업데이트 하거나 추가할 업데이트가 경고 없이 손실 됩니다.  
  
 호출 하는 경우 **이동** 정방향 스크롤 스냅숏에 `lRows` 책갈피는 허용 되지 않으므로 진행 하려면만 및 매개 변수는 양의 정수 여야 합니다.  
  
 첫 번째, 마지막을 하려면 다음 또는 이전 기록 레코드 집합에서 현재 레코드, 호출 된 **MoveFirst**, `MoveLast`, `MoveNext`, 또는 `MovePrev` 멤버 함수입니다.  
...
>  중 하나를 호출 하는 경우는 **이동** 작동 중에 현재 레코드는 업데이트 하거나 추가할 업데이트가 경고 없이 손실 됩니다.  
 ...
 사용 된 **이동** 조건을 적용 하지 않고 레코드 간을 이동 하는 함수입니다. 찾기 작업을 사용 하 여 다이너셋 형식 또는 특정 조건을 만족 하는 스냅숏 형식 recordset 개체에서 레코드를 찾을 수 있습니다. 레코드를 테이블 형식 recordset 개체에서를 찾으려면 호출 `Seek`합니다.  
  
 레코드 집합 테이블 형식의 레코드 집합을 참조 하는 경우 이동 테이블의 현재 인덱스를 따릅니다. 기본 DAO 개체의 인덱스 속성을 사용 하 여 현재 인덱스를 설정할 수 있습니다. 현재 인덱스를 설정 하지 않으면 반환 된 레코드의 순서가 정의 되지 않습니다.  
  
 호출 하는 경우 `MoveLast` SQL 쿼리 또는 쿼리 정의에 따라 recordset 개체에 쿼리가 완료 될 때까지 강제로 및 레코드 집합 개체를 완벽 하 게으로 채워집니다.  
  
 호출할 수 없습니다는 **MoveFirst** 또는 `MovePrev` 정방향 스크롤 스냅숏 사용 하 여 멤버 함수입니다.  
  
 특정 수의 레코드 앞 이나 뒤로 돌립니다 recordset 개체의 현재 위치 레코드를 이동 하려면 호출 **이동**합니다.  
...
### <a name="remarks"></a>주의  
 호출 하는 것이 좋습니다. `IsBOF` 이전 레코드로 이동 하기 전에 합니다. 에 대 한 호출 `MovePrev` 를 발생 시킵니다는 `CDaoException` 경우 `IsBOF`&0;이 아니고 첫 번째 레코드 앞 스크롤 이미 또는 레코드가 없는 레코드 집합에서 선택한 있는지를 나타내는 반환 합니다.  
...
- **dbOpenSnapshot** 양방향 스크롤이 스냅숏 형식의 레코드 집합입니다.  
...
-   하나 이상의 테이블 정의 및/또는 쿼리 정의 (쉼표로 구분 됨)의 이름입니다.  
  
-   SQL **선택** 문 (SQL 선택적으로 **여기서** 또는 **ORDERBY** 절).  
...
- **dbAppendOnly** 새 레코드 (다이너셋 형식의 레코드 집합에만 해당)만 추가할 수 있습니다. 이 옵션은 문자 그대로 레코드 추가 될 수 있습니다. MFC ODBC 데이터베이스 클래스 하는 옵션이 추가 전용 레코드를 검색 하 고 추가 허용 합니다.  
...
- **dbDenyRead** 다른 사용자 (테이블 형식의 레코드 집합에만 해당) 레코드를 볼 수 없습니다.  
  
- **dbReadOnly** 레코드를 볼 수 있습니다; 다른 사용자가 수정할 수 있습니다.  
  
- **dbInconsistent** 일관성 없는 업데이트 허용 (다이너셋 형식의 레코드 집합에만 해당).  
  
- **dbConsistent** 만 대 한 일관적인 업데이트 (다이너셋 형식의 레코드 집합에만 해당)를 허용 됩니다.  
  
> [!NOTE]
>  상수 **dbConsistent** 및 **dbInconsistent** 함께 사용할 수 없습니다. 하나를 사용 하면 또는 다른 하나만의 해당 인스턴스에서 **열려**합니다.  
  
 *pTableDef*  
 에 대 한 포인터는 [CDaoTableDef](../../mfc/reference/cdaotabledef-class.md) 개체입니다. 이 버전은 테이블 형식의 레코드 집합에 대해서만 유효 합니다. 이 옵션을 사용 하는 경우는 `CDaoDatabase` 포인터를 생성 하는 데는 `CDaoRecordset` 사용 되지 않습니다; 테이블 정의 프로시저가 있는 데이터베이스를 사용 하는 대신 합니다.  
  
 *pQueryDef*  
 에 대 한 포인터는 [CDaoQueryDef](../../mfc/reference/cdaoquerydef-class.md) 개체입니다. 이 버전은 다이너셋 형식 및 스냅숏 형식의 레코드 집합에 대해서만 유효 합니다. 이 옵션을 사용 하는 경우는 `CDaoDatabase` 포인터를 생성 하는 데는 `CDaoRecordset` 사용 되지 않습니다; 쿼리 정의 프로시저가 있는 데이터베이스를 사용 하는 대신 합니다.  
...
-   레코드 집합 개체를 생성할 때 전달에 대 한 포인터는 `CDaoDatabase` 열려 있는 개체입니다. 레코드 집합을 연는 `CDaoDatabase` 개체, 하지만 닫지 것입니다 recordset 개체를 닫으면 됩니다.  
  
-   레코드 집합 개체를 생성할 때 전달 된 **NULL** 포인터입니다. 레코드 집합 개체 호출 `GetDefaultDBName` Microsoft Access의 이름을 가져옵니다. MDB 파일을 열 수 있습니다. 레코드 집합을 연 다음는 `CDaoDatabase` 개체와 유지 되는 레코드 집합이 열려으로 엽니다. 호출 하는 경우 **닫기** 레코드 집합에는 `CDaoDatabase` 개체를 닫을 수도 있습니다.  
  
    > [!NOTE]
    >  레코드 집합을 연 경우는 `CDaoDatabase` 개체를 데이터 소스를 비독점적으로 액세스를 사용 하 여 합니다.  
  
 버전에 대 한 **열고** 를 사용 하는 `lpszSQL` 매개 변수를 레코드 집합 열기 되 면 여러 가지 방법 중 하나에서 레코드를 검색할 수 있습니다. DFX 함수를 첫 번째 옵션은 프로그램 `DoFieldExchange`합니다. 두 번째 옵션은 호출 하 여 동적 바인딩을 사용 하 여 `GetFieldValue` 멤버 함수입니다. 이러한 옵션은 개별적으로 또는 함께 구현할 수 있습니다. 결합 되 면 경우 전달 하는 SQL 문에 자신에 대 한 호출에서 **열려**합니다.  
  
 두 번째 버전을 사용 하는 경우 **열려** 에 전달 하는 `CDaoTableDef` 개체를 결과 열을 통해 바인딩할 수 사용할 수 있습니다 `DoFieldExchange` DFX 메커니즘 및을 통해 동적으로 바인딩 `GetFieldValue`합니다.  
  
> [!NOTE]
>  에서만 호출할 수 있습니다 **열려** 를 사용 하는 `CDaoTableDef` 테이블 형식의 레코드 집합에 대 한 개체입니다.  
  
 세 번째 버전의를 사용 하는 경우 **열려** 에 전달 하는 `CDaoQueryDef` 개체, 쿼리 실행 되 고, 결과 열을 통해 바인딩할 수 사용할 수 있습니다 `DoFieldExchange` DFX 메커니즘 및을 통해 동적으로 바인딩 `GetFieldValue`합니다.  
  
> [!NOTE]
>  에서만 호출할 수 있습니다 **열려** 를 사용 하는 `CDaoQueryDef` 다이너셋 형식 및 스냅숏 형식의 레코드 집합에 대 한 개체입니다.  
  
 첫 번째 버전에 대 한 **열려** 를 사용 하는 `lpszSQL` 매개 변수를 레코드는 다음 표에 표시 된 기준에 따라 선택된 합니다.  
  
|`lpszSQL` 매개 변수의 값|선택한 레코드에 의해 결정 됩니다.|예제|  
|--------------------------------------|----------------------------------------|-------------|  
|**NULL**|반환한 문자열 `GetDefaultSQL`합니다.||  
|하나 이상의 테이블 정의 및/또는 쿼리 정의 이름을 포함 하는 쉼표로 구분 된 목록입니다.|모든 열에 표시 되는 `DoFieldExchange`합니다.|`"Customer"`|  
|**선택** 열 목록 **FROM** 테이블 목록|지정 된 tabledef(s) 및/또는 querydef(s)에서 지정 된 열입니다.|`"SELECT CustId, CustName`<br /><br /> `FROM Customer"`|  
  
 전달 하는 일반적인 절차는 **NULL** 를 **열려**; 경우 **열려** 호출 `GetDefaultSQL`, 만들 때 클래스 마법사에서 생성 하는 재정의 가능한 멤버 함수는 `CDaoRecordset`-파생 클래스입니다. 이 값은 클래스 마법사에서 지정한 tabledef(s) 및/또는 쿼리 정의 이름이 제공 합니다. 대신에 있는 다른 정보를 지정할 수 있습니다는 `lpszSQL` 매개 변수입니다.  
  
 모든 항목을 전달 하면 **열려** 쿼리에 대 한 최종 SQL 문자열을 만듭니다 (문자열 SQL 있을 수 있습니다 **여기서** 및 **ORDERBY** 절에 추가 `lpszSQL` 전달 된 문자열) 다음 쿼리를 실행 합니다. 호출 하 여 생성 된 문자열을 검사할 수 있습니다 `GetSQL` 호출한 후 **열려**합니다.  
  
 레코드 집합 클래스의 필드 데이터 멤버는 선택한 데이터의 열에 바인딩됩니다. 레코드가 반환 되는 경우 첫 번째 레코드는 현재 레코드가 됩니다.  
  
 필터 또는 정렬 같은 레코드 집합에 대 한 옵션을 설정 하려면 설정 `m_strSort` 또는 **m_strFilter** recordset 개체를 생성 한 후 하지만 호출 하기 전에 **열려**합니다. 레코드 집합이 이미 열려 후 레코드 집합의 레코드를 새로 고치려면 하려는 경우, 호출 **Requery**합니다.  
  
 호출 하는 경우 **열려** 다이너셋 형식 또는 스냅숏 형식의 레코드 집합에서 나 사용할 수 없는 데이터 소스는 SQL 문 또는 연결된 된 테이블을 나타내는 테이블 정의를 나타내면 **dbOpenTable** 형식 인수가 있습니다; 이렇게 하면 MFC 예외를 throw 합니다. Tabledef 개체는 연결 된 테이블을 나타내는지 여부를 확인 하려면 만든는 [CDaoTableDef](../../mfc/reference/cdaotabledef-class.md) 개체와 호출의 [GetConnect](../../mfc/reference/cdaotabledef-class.md#getconnect) 멤버 함수입니다.  
  
 사용 하 여는 **dbSeeChanges** 편집 하거나 동일한 레코드를 삭제 하는 경우 다른 사용자 또는 컴퓨터에서 다른 응용 프로그램에서 변경한 내용을 트래핑 하지 않으려면 플래그를 지정 합니다. 예를 들어 두 명의 사용자가 동일한 레코드를 호출 하는 첫 번째 사용자 편집을 시작 하는 경우는 **업데이트** 멤버 함수가 성공적으로 실행 합니다. 때 **업데이트** 두 번째 사용자에 의해 호출 됩니다는 `CDaoException` throw 됩니다. 마찬가지로, 두 번째 사용자를 호출 하려고 하는 경우 **삭제** 하며 레코드를 삭제 하려면 이미 변경 된 첫 번째 사용자는 `CDaoException` 발생 합니다.  
  
 일반적으로이 얻으면 `CDaoException` 를 업데이트 하는 동안 코드 필드의 내용을 새로 고 해야 새로 수정된 된 값을 검색 합니다. 삭제 중에 예외가 발생 하면 코드 사용자와 데이터 최근에 변경 되었음을 나타내는 메시지를 새 레코드 데이터를 표시할 수 있습니다. 이 시점에서 코드는 레코드를 삭제 하는 사용자가 여전히는 확인 메시지를 요청할 수 있습니다.  
  
> [!TIP]
>  앞 으로만 이동 가능한 스크롤 옵션을 사용 하 여 ( **dbForwardOnly**) 응용 프로그램에서 레코드 집합을 통해 단일 패스로 때 성능을 개선 하려면 ODBC 데이터 원본에서 열입니다.  
...
### <a name="remarks"></a>주의  
 ...
 다이너셋 또는 스냅숏 중 하나에 대 한 호출 **Requery** 매개 변수 값을 사용 하 여 레코드 집합을 다시 작성 하려면 언제 든 지 합니다. 설정 하 여 새 필터 또는 정렬 설정 [m_strFilter](#m_strfilter) 및 [m_strSort](#m_strsort) 호출 하기 전에 **Requery**합니다. 호출 하기 전에 매개 변수 데이터 멤버를 새 값을 할당 하 여 새 매개 변수를 설정 **Requery**합니다.  
  
 레코드 집합을 다시 작성 하는 시도가 실패 하면 레코드 집합이 닫힙니다. 호출 하기 전에 **Requery**를 호출 하 여 레코드 집합 다시 쿼리되어 수 있는지 여부를 확인할 수 있습니다는 [CanRestart](#canrestart) 멤버 함수입니다. `CanRestart`확실 하지 않습니다 **Requery** 성공 합니다.  
...
>  호출 [Requery](#requery) DAO 책갈피를 변경 합니다.  
 ...
 호출할 수 없습니다 **Requery** 다이너셋 형식 또는 스냅숏 형식 recordset를 호출 하는 경우에 `CanRestart` 0을 반환 하거나 테이블 형식의 레코드 집합에 사용할 수 있습니다.  
  
 모두 `IsBOF` 및 `IsEOF` 호출한 후에&0;이 아닌 반환 **Requery**, 쿼리 반환 하지 않았습니다. 모든 레코드와 레코드 집합에 없는 데이터가 포함 됩니다.  
 ...
 *pKey2*  
 에 대 한 포인터는 `COleVariant` 있는 경우 인덱스에는 두 번째 필드에 해당 되는 값입니다. 기본적으로 **NULL**합니다.  
  
 *pKey3*  
 에 대 한 포인터는 `COleVariant` 있는 경우 인덱스에는 세 번째 필드에 해당 되는 값입니다. 기본적으로 **NULL**합니다.  
  
 *pKeyArray*  
 Variant의 배열에 대 한 포인터입니다. 배열 크기는 인덱스의 필드 수에 해당합니다.  
  
 *nKeys*  
 인덱스의 필드 수는 배열의 크기에 해당 하는 정수입니다.  
...
### <a name="remarks"></a>주의  
 ...
 `Seek`테이블 형식의 레코드를 검색 하는 고성능 인덱스를 설정 합니다. 호출 하 여 현재 인덱스를 설정 해야 `SetCurrentIndex` 호출 하기 전에 `Seek`합니다. 인덱스는 고유 하지 않은 키 필드 또는 필드를 식별 하는 경우 `Seek` 기준을 충족 하는 첫 번째 레코드를 찾습니다. 인덱스를 설정 하지 않으면 예외가 throw 됩니다.  
  
 유니코드 레코드 집합을 만들지 않는 경우, `COleVariant` 개체가 명시적으로 선언 해야 ANSI 합니다. 사용 하 여 이렇게는 [COleVariant::COleVariant](../../mfc/reference/colevariant-class.md#colevariant)**(** `lpszSrc` **,** `vtSrc` **)** 형태의 생성자와 `vtSrc` 로 설정 `VT_BSTRT` (ANSI) 또는 사용 하 여는 **COleVariant** 함수 [SetString](../../mfc/reference/colevariant-class.md#setstring)**(** `lpszSrc` **,** `vtSrc` **)** 와 `vtSrc` 로 설정 `VT_BSTRT`.  
  
 호출 하는 경우 `Seek`, 하나 이상의 키 값과 비교 연산자를 전달 하면 ("<",></",>\<=", "=", "> =", 또는 ">"). `Seek`지정 된 키 필드를 통해 검색 하 고 지정 된 조건에 맞는 첫 번째 레코드 `lpszComparison` 및 `pKey1`합니다. 발견 한 번 `Seek`&0;이 아닌 값을 반환 하 고 현재 해당 레코드를 만듭니다. 경우 `Seek` 일치 하는 항목을 찾지 못하면 `Seek` ,&0;을 반환 하 고 현재 레코드 정의 되지 않습니다. DAO를 직접 사용 하는 경우 NoMatch 속성이 명시적으로 확인 해야 합니다.  
  
 경우 `lpszComparison` 는 "=" "> =", 또는 ">", `Seek` 인덱스의 시작 부분에서 시작 합니다. 경우 `lpszComparison` 는 "<" or=""> </"> <=",> </=",> `Seek` 인덱스의 끝에서 시작 및 끝에서 중복 인덱스 항목이 없는 경우 뒤로 검색 합니다. 이 경우 `Seek` 항목 끝 인덱스에 중복 인덱스 항목에서 임의의 항목에서 시작 합니다.  
  
 있이 필요는 없습니다 사용할 때 현재 레코드 수 `Seek`합니다.  
  
 다이너셋 형식 또는 특정 조건을 충족 하는 스냅숏 형식의 레코드 집합에서 레코드를 찾으려면 찾기 작업을 사용 합니다. 뿐만 아니라 특정 조건에 맞는 모든 레코드를 포함 하려면 레코드 간을 이동 하려면 이동 작업을 사용 합니다.  
  
 호출할 수 없습니다 `Seek` 모든 연결된 된 테이블에 연결 된 테이블 다이너셋 형식 또는 스냅숏 형식의 레코드 집합으로 열 수 있어야 하기 때문에 입력 합니다. 그러나 호출 하는 경우 `CDaoDatabase::Open` 설치 가능한 ISAM 데이터베이스를 열려면 직접 호출할 수 있습니다 `Seek` 해당 데이터베이스의 테이블에 있는 성능 수도 있지만 느리게 합니다.  
...
### <a name="remarks"></a>주의  
 호출 `SetAbsolutePosition` 다이너셋 형식 또는 스냅숏 형식의 레코드 집합에서의 서 수 위치에 따라 특정 레코드로 현재 레코드 포인터 수 있습니다. 또한 호출 하 여 현재 레코드 번호를 확인할 수 있습니다 [GetAbsolutePosition](#getabsoluteposition)합니다.  
  
> [!NOTE]
>  이 멤버 함수는 다이너셋 형식 및 스냅숏 형식의 레코드 집합에 대해서만 유효 합니다.  
  
 기본 DAO 개체의 AbsolutePosition 속성 값은 0부터 시작 합니다. 0으로 설정 된 레코드 집합의 첫 번째 레코드를 가리킵니다. 채워진된 레코드 하면 예외를 throw 하는 MFC의 숫자 보다 큰 값을 설정 합니다. 호출 하 여 레코드 집합의 레코드의 수를 확인할 수는 `GetRecordCount` 멤버 함수입니다.  
  
 현재 레코드가 삭제 되 면 AbsolutePosition 속성 값이 정의 되지 않은 및 MFC 참조 하는 경우 예외를 throw 합니다. 새 레코드 시퀀스의 끝에 추가 됩니다.  
  
> [!NOTE]
>  이 속성으로 서로게이트 레코드 번호를 사용할 수 없습니다. 책갈피는 여전히 유지 하 고 지정된 된 위치를 반환 하는 권장된 방법은 이며 모든 유형의 지 원하는 책갈피 recordset 개체에서 현재 레코드를 배치 하는 유일한 방법입니다. 특히, 주어진된 레코드의 위치를 앞으로 레코드가 삭제 되 면 변경 합니다. 지정된 된 레코드 들이 있는 동일한 절대 위치 레코드를 다시 생성할 경우 다시 사용 하 여 SQL 문을 생성 되지 않는 한 레코드 집합 내에서 개별 레코드의 순서가 보장 되지 않기 때문은 보장 되지 못한 이기도 한 **ORDERBY** 절.  
...
### <a name="remarks"></a>주의  
 레코드 집합 개체를 만들거나 열 때 각 레코드에 이미 고유 책갈피가 합니다. 호출 하 여 현재 레코드에 대 한 책갈피를 검색할 수 있습니다 `GetBookmark` 값을 저장 하는 `COleVariant` 개체입니다. 호출 하 여 해당 레코드로 돌아갈 나중 `SetBookmark` 저장 된 책갈피 값을 사용 하 여 합니다.  
...
### <a name="parameters"></a>매개 변수  
 `lSize`  
 레코드의 수를 지정합니다. 일반적인 값은 100입니다. 캐시를 해제 하는 0으로 설정 합니다. 5-1200 개의 레코드 사이 설정 해야 합니다. 캐시는 상당한 양의 메모리를 사용할 수 있습니다.  
  
### <a name="remarks"></a>주의  
 캐시에서 데이터 다시 요청 되는 응용 프로그램을 실행 하는 동안 서버에서 가장 최근에 검색 된 데이터를 보유 하는 로컬 메모리 공간이 합니다. 데이터 캐싱 다이너셋 형식의 레코드 집합 개체를 통해 원격 서버에서 데이터를 검색 하는 응용 프로그램의 성능을 향상 시킵니다. 데이터가 요청 되 면 Microsoft Jet 데이터베이스 엔진 확인 요청된 된 데이터에 대 한 캐시 먼저 더 많은 시간이 소요 되는 서버의 검색 하는 대신 합니다. ODBC 데이터 소스에서 제공 되지 않는 데이터 캐시에 저장 되지 않습니다.  
  
 연결된 된 테이블 같은 ODBC 데이터 원본에는 로컬 캐시를 가질 수 있습니다. 캐시를 만들려면 호출 원격 데이터 원본에서 레코드 집합 개체를 열기는 `SetCacheSize` 및 `SetCacheStart` 멤버 함수 및 호출의 `FillCache` 멤버 함수 또는 이동 작업 중 하나를 사용 하 여 레코드를 하나씩 있습니다. `lSize` 의 매개 변수는 `SetCacheSize` 멤버 함수는 응용 프로그램이 한 번에 작업할 수 레코드의 수에 따라 될 수 있습니다. 예를 들어 화면에 표시할 데이터의 원본으로 레코드 집합을 사용 하는 경우 전달할 수 있습니다는 `SetCacheSize``lSize` 매개 변수를 한 번에 20 개의 레코드를 표시 하는 20으로 합니다.  
...
### <a name="remarks"></a>주의  
 에 대 한 모든 레코드의 책갈피 값을 사용할 수는 `varBookmark` 의 매개 변수는 `SetCacheStart` 멤버 함수입니다. 현재 레코드와 함께 캐시를 시작, 사용 하 여 해당 레코드에 대 한 책갈피를 설정 하려는 레코드를 만들고 [SetBookmark](#setbookmark), 책갈피 값에 대 한 매개 변수로 전달 하 고는 `SetCacheStart` 멤버 함수입니다.  
  
 Microsoft Jet 데이터베이스 엔진, 캐시에서 캐시 범위 내에서 레코드를 요청 하 고 서버에서 캐시 범위 외부의 레코드를 요청 키를 누릅니다.  
  
 캐시에서 검색 된 레코드에는 다른 사용자가 원본 데이터에 동시에 변경한 내용을 반영 하지 않습니다.  
  
 모든 캐시 된 데이터의 업데이트를 적용 하려면 전달는 `lSize` 의 매개 변수 `SetCacheSize` 0으로 호출 `SetCacheSize` 다시 캐시의 크기를 사용할 때는 원래 요청를 다음 호출에서 `FillCache` 멤버 함수입니다.  
...
### <a name="remarks"></a>주의  
 기본 테이블의 레코드는 특정 순서로 저장 되지 않습니다. 데이터베이스에서 반환 된 레코드의 순서를 변경 하는 인덱스를 설정 하지만 레코드가 저장 된 순서에 영향을 주지 않습니다. 지정된 된 인덱스를 이미 정의 되어 있어야 합니다. 존재 하지 않는 인덱스 개체를 사용 하려는 경우 또는 호출 하는 경우 인덱스를 설정 하지 않으면 [Seek](#seek), MFC 예외를 throw 합니다.  
  
 호출 하 여 테이블에 대 한 새 인덱스를 만들 수 있습니다 [CDaoTableDef::CreateIndex](../../mfc/reference/cdaotabledef-class.md#createindex) 를 호출 하 여 기본 테이블 정의의 인덱스 컬렉션에 새 인덱스를 추가 하 고 [CDaoTableDef::Append](../../mfc/reference/cdaotabledef-class.md#append), 다음 레코드 집합을 닫은 후입니다.  
  
 기본 테이블 정의 대해 정의 된 인덱스에 의해서만 테이블 형식의 레코드 집합에서 반환 된 레코드를 주문할 수 있습니다. 다른 순서로 레코드를 정렬 하려면 다이너셋 형식 또는 스냅숏 형식 recordset SQL을 사용 하 여 열 수 있습니다 **ORDERBY** 절에 저장 된 [CDaoRecordset::m_strSort](#m_strsort)합니다.  
...
### <a name="remarks"></a>주의  
 변경 되지 않고 다음과 같이 필드를 표시 하면 필드가 업데이트 되지 않습니다.  
  
 프레임 워크 표시가 변경 필드 데이터 멤버는 DAO 레코드 필드 교환 (DFX) 메커니즘으로 데이터 소스에서 레코드에 작성 되도록 합니다. 일반적으로 필드의 값을 변경 필드 설정 더티 자동으로 호출할 필요가 거의 `SetFieldDirty` 수 있지만 사용자가 직접 경우가 있는지 확인할 수 있는지 또는 열을 명시적으로 업데이트 어떤 값이 필드 데이터 멤버에 관계 없이 삽입 합니다. DFX 메커니즘의 사용을 손쉽게 **PSEUDONULL**합니다. 자세한 내용은 참조 [CDaoFieldExchange::m_nOperation](../../mfc/reference/cdaofieldexchange-class.md#m_noperation)합니다.  
  
 이중 버퍼링 메커니즘을 사용 하지 않는 경우 필드의 값을 변경한 후 설정 되지 않습니다 필드 커밋되지 않은 것으로 합니다. 이 경우 커밋되지 않은 것으로 필드를 명시적으로 설정 해야 합니다. 에 포함 된 플래그 [m_bCheckCacheForDirtyFields](#m_bcheckcachefordirtyfields) 이 자동 필드 검사를 제어 합니다.  
  
> [!NOTE]
>  이 멤버 함수를 호출한 후에 호출 [편집](#edit) 또는 [AddNew](#addnew)합니다.  
  
 사용 하 여 **NULL** 함수의 첫 번째 인수는 모든 함수를 적용할 **outputColumn** 필드 하지 **param** 필드에 `CDaoFieldExchange`합니다. 예를 들어, 호출  
  
 [!code-cpp[NVC_MFCDatabase #&6;](../../mfc/codesnippet/cpp/cdaorecordset-class_6.cpp)]  
  
 만 설정 됩니다 **outputColumn** 필드를 **NULL**; **param** 필드에 영향을 받지 않습니다.  
  
 작업에 **param**, 개별의 실제 주소를 제공 해야 **param** ,와 같은 작업 하려면:  
...
### <a name="remarks"></a>주의  
 ...
 레코드 집합에 새 레코드를 추가할 때 모든 필드 데이터 멤버는 처음 Null 값으로 설정 하 고 "더티" (변경)으로 플래그가 지정 합니다. 데이터 소스에서 레코드를 검색 하는 경우 열이 이미 값이 있는 또는 null입니다. Null 필드에 적합 하지 않은 경우는 [CDaoException](../../mfc/reference/cdaoexception-class.md) throw 됩니다.  
  
 현재 레코드의 필드 값, 호출 되지 않는 것으로 지정 하 고 싶을 경우 예를 들어, 이중 버퍼링 메커니즘을 사용 중인 경우 `SetFieldNull` 와 `bNull` 로 설정 **TRUE** Null로 플래그를 설정 합니다. 필드는 Null 이전에 표시 된 값을 지정 하려는 경우 새 값을 설정 하면 됩니다. Null 플래그를 제거할 필요가 없습니다 `SetFieldNull`합니다. 필드는 Null 일 수 있는지를 확인 하려면 호출 [IsFieldNullable](#isfieldnullable)합니다.  
  
 이중 버퍼링 메커니즘을 사용 하지 않는 경우 필드의 값을 변경한 후 설정 되지 않습니다 필드 더티 및 Null입니다. 특히 더티 및 Null이 아닌 필드를 설정 해야 합니다. 에 포함 된 플래그 [m_bCheckCacheForDirtyFields](#m_bcheckcachefordirtyfields) 이 자동 필드 검사를 제어 합니다.  
  
 DFX 메커니즘의 사용을 사용 **PSEUDONULL**합니다. 자세한 내용은 참조 [CDaoFieldExchange::m_nOperation](../../mfc/reference/cdaofieldexchange-class.md#m_noperation)합니다.  
  
> [!NOTE]
>  이 멤버 함수를 호출한 후에 호출 [편집](#edit) 또는 [AddNew](#addnew)합니다.  
  
 사용 하 여 **NULL** 함수의 첫 번째 인수는 함수를에 적용 됩니다 **outputColumn** 필드 하지 **param** 필드에 `CDaoFieldExchange`합니다. 예를 들어, 호출  
...
### <a name="parameters"></a>매개 변수  
 ...
 `nIndex`  
 레코드 집합의 필드 컬렉션 (0부터 시작)에 있는 필드의 서 수 위치를 나타내는 정수입니다.  
  
 `lpszValue`  
 필드의 내용의 값을 포함 하는 문자열에 대 한 포인터입니다.  
  
### <a name="remarks"></a>주의  
 사용 하 여 `SetFieldValue` 및 [GetFieldValue](#getfieldvalue) 실행된 시간 보다는 정적으로 사용 하 여 바인딩 열에 필드를 동적으로 바인딩할는 [DoFieldExchange](#dofieldexchange) 메커니즘입니다.  
  
 유니코드 레코드를 만들지 않는 경우 해야 하거나 사용 하 여 있습니다는 형태의 `SetFieldValue` 포함 하지 않는 한 `COleVariant` 매개 변수를 또는 `COleVariant` 개체가 명시적으로 선언 해야 ANSI 합니다. 사용 하 여 이렇게는 [COleVariant::COleVariant](../../mfc/reference/colevariant-class.md#colevariant)**(** `lpszSrc` **,** `vtSrc` **)** 형태의 생성자와 `vtSrc` 로 설정 `VT_BSTRT` (ANSI) 또는 사용 하 여는 **COleVariant** 함수 [SetString](../../mfc/reference/colevariant-class.md#setstring)**(** `lpszSrc` **,** `vtSrc` **)** 와 `vtSrc` 로 설정 `VT_BSTRT`.  
...
### <a name="parameters"></a>매개 변수  
 `nIndex`  
 인덱스&0;부터 시작 하 여 조회에 대 한 레코드 집합의 필드의 인덱스입니다.  
...
### <a name="remarks"></a>주의  
 호출 하는 즉시 잠겨 때 비관적 잠금을 페이지인 사실상 2k 편집 하는 레코드를 포함 하는 **편집** 멤버 함수입니다. 페이지 잠금이 해제 호출 하는 경우는 **업데이트** 또는 **닫기** 멤버 함수 또는 이동 또는 찾기 작업 중 하나입니다.  
  
 와 레코드를 업데이트 하는 동안에 레코드를 포함 하는 2, 000 페이지 잠긴 경우 낙관적 잠금이 적용 됩니다는 **업데이트** 멤버 함수입니다.  
  
 페이지를 잠근 경우 다른 사용자는 동일한 페이지에 레코드를 편집할 수 있습니다. 호출 하는 경우 `SetLockingMode` 하 고&0;이 아닌 값을 전달 하 고 다른 사용자가 이미 잠긴 페이지, 호출할 때 예외가 throw 됩니다 **편집**합니다. 다른 사용자가 잠긴된 페이지에서 데이터를 읽을 수 있습니다.  
  
 호출 하는 경우 `SetLockingMode` 값이&0; 인 이상 호출 **업데이트** 페이지를 다른 사용자에 의해 잠겨 하는 동안 예외가 발생 합니다. 레코드에 다른 사용자가 수행한 변경 내용을 보려면 (및 변경 내용이 손실)를 호출 하는 `SetBookmark` 현재 레코드의 책갈피 값을 가진 멤버 함수입니다.  
...
### <a name="remarks"></a>주의  
 매개 변수는 레코드 집합의 SQL 문자열의 일부로 설정 이미 해야 합니다. 매개 변수 이름 또는 컬렉션의 해당 인덱스 위치에 액세스할 수 있습니다.  
...
### <a name="remarks"></a>주의  
 다이너셋 형식 또는 스냅숏 형식의 레코드 집합으로 작업할 때는 먼저 채우는 레코드 집합을 이동 하 여 마지막 레코드를 호출 하기 전에 `SetPercentPosition`합니다. 호출 하는 경우 `SetPercentPosition` 레코드 집합을 완전히 채우는 하기 전에 이동량을 값으로 표시 된 대로 액세스 하는 레코드의 수를 기준으로 [GetRecordCount](#getrecordcount)합니다. 이동할 수 있습니다 마지막 레코드를 호출 하 여 `MoveLast`합니다.  
  
 호출 되 면 `SetPercentPosition`, 현재 값에 해당 하는 대략적인 위치에 있는 레코드 수입니다.  
  
> [!NOTE]
>  호출 `SetPercentPosition` 이동할 현재 레코드를 레코드 집합의 특정 레코드에 권장 되지 않습니다. 호출 된 [SetBookmark](#setbookmark) 멤버 함수를 대신 합니다.  
...
### <a name="remarks"></a>주의  
 이 호출을 완료 해야 합니다.는 `AddNew` 또는 **편집** 작업 합니다.  
  
 둘 다 `AddNew` 및 **편집** 데이터 원본에 저장 하기 위한 추가 또는 편집 된 데이터가 저장 되는 편집 버퍼를 준비 합니다. **업데이트** 데이터를 저장 합니다. 표시 또는 변경 된 것으로 검색 된 필드만 업데이트 됩니다.  
  
 데이터 소스에서 트랜잭션을 지원할 경우 만들 수 있습니다는 **업데이트** 호출 (및 해당 `AddNew` 또는 **편집** 호출) 트랜잭션의 일부입니다.  
  
> [!CAUTION]
>  호출 하는 경우 **업데이트** 호출 먼저 `AddNew` 또는 **편집**, **업데이트** throw 한 `CDaoException`합니다. 호출 하는 경우 `AddNew` 또는 **편집**를 호출 해야 **업데이트** 호출 하기 전에 [MoveNext](#movenext) 하거나 레코드 집합 또는 데이터 원본 연결을 닫습니다. 그렇지 않은 경우 변경 내용을 알림 없이 손실 됩니다.  
...
>  낙관적 잠금 항상 ODBC 및 설치 가능한 ISAM 등의 외부 데이터베이스 형식에 사용 됩니다.  
 ...
 관련된 정보에 대 한 "AddNew 메서드", "CancelUpdate 메서드", "Delete 메서드", "LastModified 속성", "Update 메서드" 및 "EditMode 속성" DAO 도움말의 항목을 참조 합니다.  
