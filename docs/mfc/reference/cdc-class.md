 ...
 *dwRop*  
 ...
 래스터 연산을 코드의 전체 목록은 참조 하십시오. [래스터 연산 코드에 대 한](http://msdn.microsoft.com/library/windows/desktop/dd162892) Windows sdk에서입니다.  
...
##  <a name="deletetempmap"></a>  CDC::DeleteTempMap  
 자동으로 호출 된 `CWinApp` 유휴 시간 처리기 `DeleteTempMap` 임시 삭제 `CDC` 가 만든 개체 `FromHandle`, 장치 컨텍스트 핸들을 삭제 하지 않습니다 ( `hDC`s)는 와연결된일시적으로`CDC` 개체입니다.  
...
### <a name="return-value"></a>반환 값  
 반환 된 마지막 값을 지정 된 [콜백 함수](callback-functions-used-by-mfc.md#enum_objects)합니다. 의미는 사용자 정의입니다.  
...
- [CDC::StartPage](#startpage)  
 ...
 또한 [CDC::GetDeviceCaps](#getdevicecaps) 다른 프린터 이스케이프를 교체 하는 Win32 인덱스를 지원 합니다. 참조 [GetDeviceCaps](http://msdn.microsoft.com/library/windows/desktop/dd144877) 자세한 내용은 Windows sdk입니다.  
  
 이 멤버 함수 GDI를 통해 직접 사용할 수 없는 특정 장치 기능에 액세스할 수 있습니다.  
  
 응용 프로그램 미리 정의 된 이스케이프 값을 사용 하는 경우 첫 번째 버전을 사용 합니다. 응용 프로그램 개인 이스케이프 값을 정의 하는 경우 두 번째 버전을 사용 합니다. 참조 [ExtEscape](http://msdn.microsoft.com/library/windows/desktop/dd162708) 두 번째 버전에 대 한 자세한 내용은 Windows sdk입니다.  
...
### <a name="remarks"></a>설명  
 ...
 경우 사각형의 **아래쪽** 좌표 보다 작거나 같음 **top**, if 또는 **오른쪽** 보다 작거나 같음 **왼쪽**, 사각형은 그리지 않습니다.  
...
### <a name="remarks"></a>설명  
 이 멤버 함수는 함수의 기능을 에뮬레이션 [GetCharacterPlacement](http://msdn.microsoft.com/library/windows/desktop/dd144860\(v=vs.85\).aspx)Windows SDK에 설명 된 대로 합니다.  
...
### <a name="remarks"></a>설명  
 ...
 경우는 `GetCharABCWidths` 음수 "A"를 검색 하는 멤버 함수 또는 해당 문자는 문자에 대 한 "C" 너비 underhangs 또는 돌출부 포함 됩니다.  
  
 ABC 폭 글꼴 디자인 단위로으로 변환 하려면 응용 프로그램 만들어야 글꼴 높이 (에 지정 된 대로 **lfHeight** 의 멤버는 [LOGFONT](http://msdn.microsoft.com/library/windows/desktop/dd145037) 구조)는 에저장된값과일치**ntmSizeEM** 의 멤버는 [NEWTEXTMETRIC](http://msdn.microsoft.com/library/windows/desktop/dd162741) 구조입니다. (값은 **ntmSizeEM** 멤버를 호출 하 여 검색할 수 있습니다는 [EnumFontFamilies](http://msdn.microsoft.com/library/windows/desktop/dd162619) Windows 함수입니다.)  
...
### <a name="remarks"></a>설명  
 이 멤버 함수는 함수의 기능을 에뮬레이션 [GetCharABCWidthsI](http://msdn.microsoft.com/library/windows/desktop/dd144859)Windows SDK에 설명 된 대로 합니다.  
...
### <a name="remarks"></a>설명  
 이 멤버 함수는 함수의 기능을 에뮬레이션 [GetCharWidthI](http://msdn.microsoft.com/library/windows/desktop/dd144864)Windows SDK에 설명 된 대로 합니다.  
...
### <a name="remarks"></a>설명  
 이 멤버 함수는 함수의 기능을 에뮬레이션 [GetDCBrushColor](http://msdn.microsoft.com/library/windows/desktop/dd144872)Windows SDK에 설명 된 대로 합니다.  
...
### <a name="remarks"></a>설명  
 이 멤버 함수는 Win32 함수 사용 하 여 [GetDCPenColor](http://msdn.microsoft.com/library/windows/desktop/dd144875)Windows SDK에 설명 된 대로 합니다.  
...
### <a name="remarks"></a>설명  
 이 멤버 함수는 함수의 기능을 에뮬레이션 [GetFontLanguageInfo](http://msdn.microsoft.com/library/windows/desktop/dd144886)Windows SDK에 설명 된 대로 합니다.  
...
### <a name="remarks"></a>설명  
 이 멤버 함수는 함수의 기능을 에뮬레이션 [GetTextExtentExPointI](http://msdn.microsoft.com/library/windows/desktop/dd144936)Windows SDK에 설명 된 대로 합니다.  
...
### <a name="remarks"></a>설명  
 이 멤버 함수는 함수의 기능을 에뮬레이션 [GetTextExtentPointI](http://msdn.microsoft.com/library/windows/desktop/dd144939)Windows SDK에 설명 된 대로 합니다.  
...
### <a name="remarks"></a>설명  
 자세한 내용은 참조 `GradientFill` Windows sdk에서입니다.  
 ...
 **PT_BEZIERTO** 3 집합 모두에 항상 발생 하는 형식입니다. 현재 위치 Bzier 스플라인에 대 한 시작 지점을 정의합니다. 처음 두 **PT_BEZIERTO** 요소가 컨트롤 포인트 및 세 번째 **PT_BEZIERTO** 포인트는 끝점입니다. 끝점에는 새 현재 위치 됩니다. 없는 경우 3 개의 연속 된 **PT_BEZIERTO** 점, 오류가 발생 합니다.  
     ...
     A **PT_LINETO** 또는 **PT_BEZIERTO** 유형 비트 연산자를 사용 하 여 다음 상수와 결합 되거나 해당 하는 점 도형과 그림에서 마지막 점 임을 나타내려면 닫혀 :  
...
### <a name="remarks"></a>설명  
 ...
 자세한 내용은 참조 [폴리라인](http://msdn.microsoft.com/library/windows/desktop/dd162815) Windows sdk에서입니다.  
...
### <a name="return-value"></a>반환 값  
 대체 되 고 개체에 대 한 포인터입니다. 이 개체에서 파생 된 클래스 중 하나에 대 한 포인터는 `CGdiObject`와 같은 `CPen`사용 되는 함수는 버전에 따라 합니다. 반환 값은 **NULL** 오류가 발생 하는 경우. 이 함수는 임시 개체에 대 한 포인터를 반환할 수 있습니다. 이 임시 개체가 하나의 Windows 메시지 처리 에서만 유효 합니다. 자세한 내용은 `CGdiObject::FromHandle`을 참조하세요.  
...
### <a name="remarks"></a>설명  
 이 메서드는 함수의 기능을 에뮬레이션 [SetDCBrushColor](http://msdn.microsoft.com/library/windows/desktop/dd162969)Windows SDK에 설명 된 대로 합니다.  
...
### <a name="remarks"></a>설명  
 이 멤버 함수는 Win32 함수 사용 하 여 [SetDCPenColor](http://msdn.microsoft.com/library/windows/desktop/dd162970)Windows SDK에 설명 된 대로 합니다.  
...
### <a name="remarks"></a>설명  
 일반적으로 호출 하면 **SetLayout** 창에 대 한 합니다. 설정 하 여 창에서 오른쪽에서 왼쪽 레이아웃을 제어 대신는 [확장 창 스타일](../../mfc/reference/styles-used-by-mfc.md#extended-window-styles) 같은 **WS_EX_RTLREADING**합니다. 장치 컨텍스트에 메타 파일, 프린터 등이 레이아웃을 상속 하지 않습니다. 호출 하 여 오른쪽에서 왼쪽 레이아웃에 대 한 장치 컨텍스트를 설정 하는 유일한 방법은 **SetLayout**합니다.  
...
- **R2_NOTMASKPEN** 픽셀의 역함수 값은는 **R2_MASKPEN** 색 (최종 픽셀 = 안 함 (AND 화면 픽셀 펜)).  
...
### <a name="return-value"></a>반환 값  
 ...
 Windows SDK에서 제공 된 값 중 하나일 수 있습니다.  
 ...
 **BLACKONWHITE** ( **STRETCH_ANDSCANS**) 및 **WHITEONBLACK** ( **STRETCH_ORSCANS**) 모드를 유지 하는 데 일반적으로 단색 비트맵의 전경 픽셀입니다. **COLORONCOLOR** ( **STRETCH_DELETESCANS**) 모드는 일반적으로 색 비트맵에서 색을 유지 하기 위해 사용 됩니다.  
...
### <a name="remarks"></a>설명  
 장치 컨텍스트 창 함께 뷰포트 GDI 매핑되는 방법을 논리적 좌표계로 포인트 실제 장치 점의 좌표 시스템을 정의 합니다. 즉, GDI 논리적 좌표로 장치 좌표로 변환 하는 방법을 정의 합니다.  
...
### <a name="remarks"></a>설명  
 디바이스 컨텍스트 뷰포트는 창 GDI 매핑되는 방법을 논리적 좌표계로 포인트 장치 좌표계의 요소를 정의 합니다.  
...
### <a name="remarks"></a>설명  
 ...
 색상표를 사용 하는 방법에 대 한 자세한 내용은 참조 [UpdateColors](http://msdn.microsoft.com/library/windows/desktop/dd145166) Windows sdk에서입니다.  
