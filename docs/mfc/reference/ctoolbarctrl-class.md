...
|[CToolBarCtrl::Create](#create)|에 연결 하 고 도구 모음 컨트롤을 만듭니다.는 `CToolBarCtrl` 개체입니다.|  
...
## <a name="remarks"></a>설명  
 ...
 `CToolBarCtrl`몇 가지 중요 한 내부 데이터 구조를 포함 하는 개체: 단추 이미지 비트맵의 목록 또는 이미지 목록, 단추 레이블 문자열의 목록 및 목록이 `TBBUTTON` 이미지로 연결 및/또는 위치 스타일을 포함 하는 문자열에 구조, 상태 이며 명령 단추의 ID입니다. 각각의 이러한 데이터 구조 요소에서 참조 하는 0부터 시작 인덱스입니다. 사용 하기 전에 `CToolBarCtrl` 개체를 이러한 데이터 구조를 설정 해야 합니다. 단추 레이블;에 대 한 문자열 목록에만 사용할 수 있습니다. 도구 모음에서 문자열을 검색할 수 없습니다.  
...
5.  아닌 소유자 창에서 도구 모음 단추에 대 한 도구 설명 하려는 경우는 `CFrameWnd`를 처리 해야는 **TTN_NEEDTEXT** 모음의 소유자 창에서 메시지에 설명 된 대로 [도구 팁 알림 처리](../../mfc/handling-tool-tip-notifications.md)합니다. 도구 모음의 부모 창에서 파생 된 경우 `CFrameWnd`, 때문에 사용자의 추가 작업 없이 도구 설명이 표시 되는 `CFrameWnd` 기본 처리기를 제공 합니다.  
...
### <a name="remarks"></a>주의  
 ...
 이 메서드는 전송 된 [TB_CHANGEBITMAP](http://msdn.microsoft.com/library/windows/desktop/bb787301) 에 설명 되어 있는 메시지는 [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]합니다.  
...
##  <a name="create"></a>CToolBarCtrl::Create  
 에 연결 하 고 도구 모음 컨트롤을 만듭니다.는 `CToolBarCtrl` 개체입니다.  
...
### <a name="remarks"></a>설명  
 생성 한 `CToolBarCtrl` 두 단계를 수행에서 합니다. 먼저 생성자를 호출 하 고 호출 **만들기**, 도구 모음 컨트롤을 만들고를 연결 하는 `CToolBarCtrl` 개체입니다. Toolbar 컨트롤에 다음 창 스타일을 적용 합니다.  
...
- **WS_DISABLED** 거의  
 ...
 필요에 따라 조합을 적용할 [공통 컨트롤 스타일](http://msdn.microsoft.com/library/windows/desktop/bb775498)에 설명 된 대로 [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]합니다.  
  
 도구 모음 스타일의 조합을 컨트롤이 나 자체 단추에 적용 됩니다. 스타일의 항목에 설명 된 [도구 모음 컨트롤과 단추 스타일](http://msdn.microsoft.com/library/windows/desktop/bb760439) 에 [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]합니다.  
...
### <a name="remarks"></a>주의  
 이 메서드는 전송 된 [TB_GETBUTTONTEXT](http://msdn.microsoft.com/library/windows/desktop/bb787325) 메시지 Windows SDK에 설명 되어 있습니다.  
...
### <a name="remarks"></a>주의  
 이 메서드는 전송 된 [TB_GETCOLORSCHEME](http://msdn.microsoft.com/library/windows/desktop/bb787327) 에 설명 되어 있는 메시지는 [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]합니다.  
...
### <a name="remarks"></a>주의  
 이 멤버 함수는 Win32 메시지의 동작을 구현 [TB_GETDISABLEDIMAGELIST](http://msdn.microsoft.com/library/windows/desktop/bb787329)에 설명 된 대로 [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]합니다. MFC 구현 `GetDisabledImageList` 사용 하 여 한 `CImageList` 도구 모음 컨트롤의 단추를 포함 하는 개체 이미지, 이미지 목록에 대 한 핸들을 대신 합니다.  
...
### <a name="return-value"></a>반환 값  
 A `DWORD` 도구 모음 컨트롤에 대 한 사용 중인 확장된 스타일을 나타내는입니다. 목록이 스타일에 대 한 참조 [확장 스타일 도구 모음](http://msdn.microsoft.com/library/windows/desktop/bb760430)에 [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]합니다.  
...
### <a name="remarks"></a>설명  
 이 함수는 상태가 설정 단추에 대 한 경계 사각형을 검색 하지 않습니다 `TBSTATE_HIDDEN`합니다.  
...
### <a name="remarks"></a>주의  
 기능을 에뮬레이션 하는이 멤버 함수는 [TB_GETMETRICS](http://msdn.microsoft.com/library/windows/desktop/bb787342) 메시지에 설명 된 대로 [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]합니다.  
...
### <a name="remarks"></a>설명  
 이 메서드는 전송 된 [TB_GETPADDING](http://msdn.microsoft.com/library/windows/desktop/bb787344) 에 설명 되어 있는 메시지는 [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]합니다.  
...
### <a name="remarks"></a>주의  
 이 메서드는 전송 된 [TB_GETPRESSEDIMAGELIST](http://msdn.microsoft.com/library/windows/desktop/bb787345) 에 설명 되어 있는 메시지는 [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]합니다.  
...
### <a name="return-value"></a>반환 값  
 ...
 반환 값이 음수 이면 지점 단추 내에 포함 되지 않습니다. 반환 값의 절대 값은 구분 기호 항목 또는 가장 가까운 nonseparator 항목의 인덱스.  
...
### <a name="parameters"></a>매개 변수  
 [in] `nID`  
 누르거나 해제 단추의 명령 식별자입니다.  
  
 [in] `bPress`  
 `true`지정 된 단추를 눌러야 합니다. `false` 지정 된 단추를 놓을 합니다. 기본값은 `true`입니다.  
...
### <a name="remarks"></a>설명  
 ...
 이 메서드는 전송 된 [TB_PRESSBUTTON](http://msdn.microsoft.com/library/windows/desktop/bb787389) 에 설명 되어 있는 메시지는 [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]합니다.  
  
##  <a name="replacebitmap"></a>CToolBarCtrl::ReplaceBitmap  
 현재 도구 모음 컨트롤에서 기존 비트맵을 새 비트맵을 바꿉니다.  
...
|매개 변수|설명|  
|---------------|-----------------|  
|[in] `pReplaceBitmap`|에 대 한 포인터는 [TBREPLACEBITMAP](http://msdn.microsoft.com/library/windows/desktop/bb760484) 교체 비트맵 및 새 비트맵을 설명 하는 구조입니다.|  
  
### <a name="return-value"></a>반환 값  
 이 메서드가 성공적으로 수행되면 `true`이고, 그렇지 않으면 `false`입니다.  
  
### <a name="remarks"></a>주의  
 이 메서드는 전송 된 [TB_REPLACEBITMAP](http://msdn.microsoft.com/library/windows/desktop/bb787391) 에 설명 되어 있는 메시지는 [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]합니다.  
  
### <a name="example"></a>예제  
 다음 코드 예제에서는 다른 비트맵으로 표준 도구 모음에 대 한 비트맵을 바꿉니다.  
...
### <a name="parameters"></a>매개 변수  
 `hKeyRoot`  
 레지스트리 또는 다음과 같은 미리 정의 된 예약 된 핸들 값 중 하나에서 현재 열려 있는 키를 식별합니다.  
...
- **HKEY_USERS**  
 ...
 `lpszSubKey`  
 값을 연결 된 하위 키의 이름을 포함 하는 null로 끝나는 문자열을 가리킵니다. 이 매개 변수는 null 또는 빈 문자열에 대 한 일 수 있습니다. 매개 변수가 **NULL**, 키로 식별 되는 값이 추가 되는 `hKeyRoot` 매개 변수입니다.  
  
 `lpszValueName`  
 검색할 값의 이름을 포함 하는 문자열을 가리킵니다. 없으면이 이름 가진 값 이미 키에서, 함수는 키에 추가 됩니다.  
  
##  <a name="savestate"></a>CToolBarCtrl::SaveState  
 매개 변수에 의해 지정 된 레지스트리에서 위치에 도구 모음 컨트롤의 상태를 저장 합니다.  
...
- **HKEY_USERS**  
 ...
 `lpszValueName`  
 설정할 값의 이름을 포함 하는 문자열을 가리킵니다. 없으면이 이름 가진 값 이미 키에서, 함수는 키에 추가 됩니다.  
...
### <a name="parameters"></a>매개 변수  
 [in] `fAnchor`  
 지정 하는 경우 앵커 강조 사용 하거나 사용 하지 않도록 설정 합니다. 이 값은 0이 아니고, 앵커 강조 표시 설정 됩니다. 이 값이 0 이면 앵커 강조 표시를 사용할 수 없습니다.  
  
### <a name="return-value"></a>반환 값  
 이전 앵커 설정입니다. 강조 표시를 설정한 경우이 값은 0이 아닌 값입니다. 강조 표시 된 사용할 수 없는 경우이 값은 0입니다.  
...
### <a name="parameters"></a>매개 변수  
 `size`  
 너비와 높이 픽셀 비트맵 이미지의 단위로 합니다.  
  
### <a name="return-value"></a>반환 값  
 성공하는 경우 0이 아니고, 그렇지 않으면 0입니다.  
  
### <a name="remarks"></a>주의  
 이 함수는 모든 비트맵을 도구 모음에 추가 하기 전에 호출 되어야 합니다. 응용 프로그램은 비트맵 크기를 명시적으로 설정 하지 않으면, 16 x 15 픽셀을 기본값입니다.  
...
### <a name="remarks"></a>주의  
 멤버 함수는 Win32 메시지의 동작을 구현 [TB_SETBUTTONINFO](http://msdn.microsoft.com/library/windows/desktop/bb787413)에 설명 된 대로 [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]합니다.  
...
### <a name="remarks"></a>주의  
 단추 크기 항상 포함 비트맵 크기 이상이 여야 합니다. 이 함수는 모든 비트맵을 도구 모음에 추가 하기 전에 호출 되어야 합니다. 응용 프로그램 단추 크기를 명시적으로 설정 하지 않으면를 기본값으로 사용 하 여 24 x 22 픽셀로 됩니다.  
...
### <a name="parameters"></a>매개 변수  
 `nSize`  
 바이트에서의 크기는 `TBBUTTON` 구조입니다.  
  
### <a name="remarks"></a>주의  
 추가 데이터를 저장 하려는 경우는 `TBBUTTON` 구조 수 하거나 파생에서 새 구조 `TBBUTTON`, 멤버를 추가 하면 필요에 따라 또는 포함 하는 새 구조를 만들는 `TBBUTTON` 첫 번째 멤버로 구조입니다. 도구 모음 컨트롤 새 구조체의 크기를 지시 하려면이 함수를 호출 합니다.  
  
 참조 [CToolBarCtrl::AddButtons](#addbuttons) 대 한 자세한 내용은 `TBBUTTON` 구조입니다.  
  
##  <a name="setbuttonwidth"></a>CToolBarCtrl::SetButtonWidth  
 도구 모음 컨트롤에서 단추 최소 및 최대 너비를 설정합니다.  
 ...
 *cxMax*  
 최대 단추 너비 (픽셀)에서입니다. 단추 텍스트 너무 넓은 경우 컨트롤과 줄임표 지점을 표시 합니다.  
...
### <a name="parameters"></a>매개 변수  
 `nIndex`  
 명령 ID를 설정할 단추의 0부터 시작 하는 인덱스입니다.  
  
 `nID`  
 선택한 단추를 설정 하려면 명령 ID입니다.  
...
|매개 변수|설명|  
|---------------|-----------------|  
|[in] `lpColorScheme`|에 대 한 포인터는 [COLORSCHEME](http://msdn.microsoft.com/library/windows/desktop/bb775502) 강조 색 및 도구 모음 컨트롤의 그림자 색을 설명 하는 구조입니다.|  
  
### <a name="remarks"></a>주의  
 경우이 메서드는 아무런 영향을 주지는 [!INCLUDE[windowsver](../../build/reference/includes/windowsver_md.md)] 시각적 테마 설정 됩니다.  
  
 이 메서드는 전송 된 [TB_SETCOLORSCHEME](http://msdn.microsoft.com/library/windows/desktop/bb787421) 에 설명 되어 있는 메시지는 [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]합니다.  
  
### <a name="example"></a>예제  
 다음 코드 예제에서는 현재 도구 모음 컨트롤에 대 한 색 구성표를 설정합니다. 코드 예제에서는 빨간색 각 도구 단추의 왼쪽 및 위쪽 가장자리와 오른쪽 아래 모서리 파란색입니다. 사용자가 단추를 누를 때 단추의 빨간색 가장자리 파란색으로 바뀌고 파란색 가장자리 빨간색으로 바뀝니다.  
...
##  <a name="setdisabledimagelist"></a>CToolBarCtrl::SetDisabledImageList  
 사용 하지 않도록 설정 하는 표시 단추에 도구 모음 컨트롤은 사용 된 이미지 목록을 설정 합니다.  
...
### <a name="parameters"></a>매개 변수  
 `pImageList`  
 에 대 한 포인터는 `CImageList` 단추 이미지 표시 사용 하지 않도록 설정 하려면 도구 모음 컨트롤에서 사용할 이미지를 포함 하는 개체입니다.  
  
### <a name="return-value"></a>반환 값  
 에 대 한 포인터는 [CImageList](../../mfc/reference/cimagelist-class.md) 단추 이미지 표시 사용 하지 않도록 설정 하려면 도구 모음 컨트롤에서 이전에 사용 된 개체입니다.  
  
### <a name="remarks"></a>주의  
 이 멤버 함수는 Win32 메시지의 동작을 구현 [TB_SETDISABLEDIMAGELIST](http://msdn.microsoft.com/library/windows/desktop/bb787423)에 설명 된 대로 [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]합니다. MFC 구현 `SetDisabledImageList` 사용 하 여 한 `CImageList` 도구 모음 컨트롤의 비활성화 된 단추를 포함 하는 개체 이미지, 이미지 목록에 대 한 핸들을 대신 합니다.  
  
##  <a name="setdrawtextflags"></a>CToolBarCtrl::SetDrawTextFlags  
 Win32 함수에는 플래그가 설정 [DrawText](http://msdn.microsoft.com/library/windows/desktop/dd162498), 설정 된 플래그에 맞는 형식의 지정된 된 사각형에는 텍스트를 그리는 데 사용 되는 합니다.  
...
### <a name="parameters"></a>매개 변수  
 `dwMask`  
 Win32 함수에 지정 된 DT_ 플래그 중 하나 이상을 조합 [DrawText](http://msdn.microsoft.com/library/windows/desktop/dd162498), 있는 비트를 나타내는 `dwDTFlags` 는 텍스트를 그릴 때 사용 됩니다.  
  
 `dwDTFlags`  
 Win32 함수에 지정 된 DT_ 플래그 중 하나 이상을 조합 `DrawText`, 단추 텍스트를 그릴 어떻게 함을 나타냅니다. 이 값이 전달 `DrawText` 단추 텍스트를 그릴 합니다.  
  
### <a name="return-value"></a>반환 값  
 A `DWORD` 플래그 그리기 이전 텍스트를 포함 하 합니다.  
  
### <a name="remarks"></a>주의  
 이 멤버 함수는 Win32 메시지의 동작을 구현 [TB_SETDRAWTEXTFLAGS](http://msdn.microsoft.com/library/windows/desktop/bb787425)에 설명 된 대로 [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]합니다. 이 멤버 함수는 Win32 함수에는 플래그를 설정 `DrawText`, 설정 된 플래그에 맞는 형식의 지정된 된 사각형에 텍스트를 그립니다입니다.  
...
### <a name="parameters"></a>매개 변수  
 `dwExStyle`  
 새로운 확장된 스타일을 지정 하는 값입니다. 이 매개 변수는 확장 스타일 도구 모음에서의 조합일 수 있습니다.  
  
### <a name="return-value"></a>반환 값  
 A `DWORD` 나타내는 이전 스타일을 확장 합니다. 목록이 스타일에 대 한 참조 [확장 스타일 도구 모음](http://msdn.microsoft.com/library/windows/desktop/bb760430)에 [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]합니다.  
...
##  <a name="sethotimagelist"></a>CToolBarCtrl::SetHotImageList  
 "핫" 단추를 표시 하려면 도구 모음 컨트롤은 사용 된 이미지 목록을 설정 합니다.  
...
### <a name="remarks"></a>주의  
 ...
 MFC 구현 `SetHotImageList` 사용 하 여 한 `CImageList` 도구 모음 컨트롤의 활성 단추를 포함 하는 개체 이미지, 이미지 목록에 대 한 핸들을 대신 합니다. 포인터가 그 위에 있을 때 활성 단추를 강조 표시 됩니다.  
...
### <a name="parameters"></a>매개 변수  
 *nHot*  
 핫 적용 될 항목의 0부터 시작 인덱스 번호를 지정 합니다. 이 값이-1 이면 항목이 없는 핫 됩니다.  
...
##  <a name="setimagelist"></a>CToolBarCtrl::SetImageList  
 기본 상태로에 있는 단추를 표시 하려면 도구 모음에서 사용할 이미지 목록을 설정 합니다.  
...
### <a name="parameters"></a>매개 변수  
 `pImageList`  
 에 대 한 포인터는 `CImageList` 해당 기본 상태인 단추 이미지를 표시 하려면 도구 모음 컨트롤에서 사용할 이미지를 포함 하는 개체입니다.  
  
### <a name="return-value"></a>반환 값  
 에 대 한 포인터는 [CImageList](../../mfc/reference/cimagelist-class.md) 개체의 기본 상태로 단추 이미지를 표시 하려면 도구 모음 컨트롤에서 이전에 사용한입니다.  
...
### <a name="remarks"></a>설명  
 ...
 MFC 구현 `SetImageList` 사용 하 여 한 `CImageList` 도구 모음 컨트롤의 단추를 포함 하는 개체 이미지, 이미지 목록에 대 한 핸들을 대신 합니다.  
...
### <a name="remarks"></a>주의  
 기능을 에뮬레이션 하는이 멤버 함수는 [TB_SETMETRICS](http://msdn.microsoft.com/library/windows/desktop/bb787446) 메시지에 설명 된 대로 [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]합니다.  
...
### <a name="parameters"></a>매개 변수  
 `pWnd`  
 에 대 한 포인터는 `CWnd` 또는 `CWnd`-파생 도구 모음 컨트롤에 대 한 새 소유자 창이 될 개체입니다.  
  
### <a name="remarks"></a>주의  
 도구 모음에서 알림을 수신 하는 창 소유자입니다.  
...
### <a name="return-value"></a>반환 값  
 A `DWORD` 낮은 인 word 이전 가로 여백 값과 해당 높은 단어 이전 세로 여백 값이 포함 됩니다. 안쪽 여백 값 (픽셀) 단위로 측정 됩니다.  
  
### <a name="remarks"></a>주의  
 이 메서드는 전송 된 [TB_SETPADDING](http://msdn.microsoft.com/library/windows/desktop/bb787448) 에 설명 되어 있는 메시지는 [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]합니다.  
  
### <a name="example"></a>예제  
 다음 코드 예제에서는 20 픽셀을 현재 도구 모음 컨트롤의 가로 및 세로 여백을 설정합니다.  
...
|[in] `pImageList`|에 대 한 포인터는 [CImageList](../../mfc/reference/cimagelist-class.md) 새 이미지 목록이 들어 있는입니다.|  
  
### <a name="return-value"></a>반환 값  
 에 대 한 포인터는 [CImageList](../../mfc/reference/cimagelist-class.md) 현재 컨트롤에 대 한 이전 이미지 목록이 포함 된 또는 `NULL` 이미지 목록이 없는 설정 된 경우.  
  
### <a name="remarks"></a>주의  
 이 메서드는 전송 된 [TB_SETPRESSEDIMAGELIST](http://msdn.microsoft.com/library/windows/desktop/bb787453) 에 설명 되어 있는 메시지는 [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]합니다.  
  
### <a name="example"></a>예제  
 다음 코드 예제에는 기본 이미지 목록으로 동일 하 게 누른된 이미지 목록을 설정 합니다.  
...
### <a name="parameters"></a>매개 변수  
 ...
 `bLarger`  
 도구 모음에서 요청 된 행 수를 크기 조정할 수 없습니다 경우 더 많은 행 이나 적은 수의 행을 사용할지 여부를 알려 줍니다.  
...
### <a name="remarks"></a>주의  
 도구 모음을 요청 된 수 또는 행 자체를 조정할 수 없습니다, 그 크기를 조정 합니다 자체를 다음 크거나 다음 더 작은 올바른 크기 값에 따라 `bLarger`합니다. 경우 `bLarger` 은 **TRUE**, 새 행 수가 요청 된 수보다 커야 합니다. 경우 `bLarger` 은 **FALSE**, 새 행 수가 요청 된 수보다 작아야 합니다.  
  
 지정된 된 수의 행 도구 모음에 대 한 유효한 이면 모든 행 번호가 동일한 단추 (아마도 마지막 행)은 제외 되도록 단추 정렬 될 수 있습니다. 예를 들어 네 개의 단추가 있는 도구 모음 수 하지 수의 경우 3 개의 행 마지막 두 행 짧아야 하므로. 3 개의 행을 크기를 조정 하려는 경우 얻을 4 개 행 `bLarger` 되었습니다 **TRUE** 경우 두 행 및 `bLarger` 되었습니다 **FALSE**합니다.  
  
 도구 모음에 구분 기호 있는 경우에 특정된 행 수가 올바른지 규칙은 더 복잡 합니다. 레이아웃 단추 그룹 (첫 번째 앞에 구분 기호가 단추) 및 그룹에서 마지막 단추 되지 분리 됩니다 여러 행에는 그룹 한 행에 맞지 않는 한 되도록 계산 됩니다.  
  
 그룹 한 행에 맞지 않을 경우 다음 그룹 큰 그룹 종료 된 행에는 것이 맞습니다. 경우에 다음 행에서 시작 됩니다. 이 규칙의 목적은 큰 그룹 더욱 분명 하 게 구분 되도록 합니다. 결과 세로 구분 기호는 행으로 계산 됩니다.  
  
 또한는 `SetRows` 멤버 함수는 가장 작은 도구 모음 크기가 레이아웃 선택한 항상 합니다. 와 도구 모음을 만드는 `TBSTYLE_WRAPABLE` 스타일 및 컨트롤의 크기가 조정 컨트롤의 너비를 지정한 위에 나와 있는 메서드를 적용 하기만 하면 됩니다.  
...
### <a name="remarks"></a>주의  
 이 함수는 둘 이상의 단추 상태를 설정 하려는 경우에 특히 유용 합니다. 방금 한 상태를 설정 하려면 다음 멤버 함수 중 하나를 사용: [EnableButton](#enablebutton), [CheckButton](#checkbutton), [HideButton](#hidebutton), [비활성](#indeterminate), 또는 [PressButton](#pressbutton)합니다.  
...
### <a name="parameters"></a>매개 변수  
 `pszSubAppName`  
 설정 하려면 도구 모음 비주얼 스타일을 포함 하는 유니코드 문자열에 대 한 포인터입니다.  
