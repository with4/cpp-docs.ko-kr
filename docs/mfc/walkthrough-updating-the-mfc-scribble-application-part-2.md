---
title: "연습: MFC 자유 곡선 응용 프로그램 업데이트(파트 2) | Microsoft Docs"
ms.custom: ""
ms.date: "12/03/2016"
...
manager: "ghogen"
---
# 연습: MFC 자유 곡선 응용 프로그램 업데이트(파트 2)
...
2.  Create a panel named `Window` that has a split button.  When a user clicks the split button, a shortcut menu displays three commands that are already defined in the Scribble application.  
    ...
    3.  단추를 클릭합니다.  Change **Caption** to `Windows`, **Keys** to `w`, **Large Image Index** to `1`, and **Split Mode** to `False`.  Then click the ellipsis \(**...**\) next to **Menu Items** to open the **Items Editor** dialog box.  
...
3.  모든 변경을 저장한 다음 응용 프로그램을 빌드하고 실행합니다.  The **View** and **Window** panels should be displayed.  Click the buttons to confirm that they function correctly.  
...
5.  모든 변경을 저장한 다음 응용 프로그램을 빌드하고 실행합니다.  A **Help** panel that contains two ribbon buttons should be displayed.  
  
    > [!IMPORTANT]
    >  When you click the **Help Topics** button, the Scribble application opens a compressed HTML \(.chm\) help file named *your\_project\_name*.chm.  Consequently, if your project is not named Scribble, you must rename the help file to your project name.  
...
6.  The new combo boxes do not correspond to any existing menu items.  Therefore, you must create a menu item for every pen option.  
    ...
    1.  In the **Resource View** window, open the IDR\_SCRIBBTYPE menu resource.  
...
8.  모든 변경을 저장한 다음 응용 프로그램을 빌드하고 실행합니다.  New buttons and combo boxes should be displayed.  Try using different pen widths to scribble.  
...
1.  Before you add the color button, create a menu item for it.  In the **Resource View** window, open the IDR\_SCRIBBTYPE menu resource.  Click the **Pen** menu item to open the pen menu.  Then click **Type Here** and type `&Color`.  Right\-click the text that you just typed to display the **Properties** window.  Change the ID to `ID_PEN_COLOR`.  
...
4.  모든 변경을 저장한 다음 응용 프로그램을 빌드하고 실행합니다.  The new color button should be displayed on the **Pen** panel.  However, it cannot be used because it does not yet have an event handler.  The next steps show how to add an event handler for the color button.  
...
11. 모든 변경을 저장한 다음 응용 프로그램을 빌드하고 실행합니다.  You should be able to press the color button and change the pen's color.  
...
## 참고 항목  
 ...
 [연습: MFC 자유 곡선 응용 프로그램 업데이트\(파트 1\)](../mfc/walkthrough-updating-the-mfc-scribble-application-part-1.md)