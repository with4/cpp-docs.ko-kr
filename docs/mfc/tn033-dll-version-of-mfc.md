...
사용 하 여 빌드 A.DLL를 빌드할 때 **/DA_IMPL** 사용 하 여 빌드 B.DLL를 빌드할 때 및 **/DB_IMPL**합니다. 각 DLL에 대 한 별도 기호를 사용해 CExampleB 내보내집니다이 고 B.DLL를 빌드할 때 CExampleA 가져옵니다. CExampleA는 A.DLL 빌드될 때 이며 B.DLL (또는 다른 클라이언트)가 사용 될 때 가져오게 됩니다.
