---
title: "파일 시스템 탐색 | Microsoft Docs"
ms.custom: ""
ms.date: "11/04/2016"
ms.reviewer: ""
ms.suite: ""
...
\<filesystem\> 헤더는 초안 FileSystem 기술 사양\([ISO\/IEC JTC 1\/SC 22\/WG 21 N4100](http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2014/n4100.pdf)\)을 구현하며 파일 시스템을 탐색하기 위한 플랫폼 독립적인 코드를 작성할 수 있게 해주는 형식과 함수를 포함합니다. 크로스 플랫폼이기 때문에 Windows 시스템과 관련이 없는 API를 포함합니다. 예를 들어 `is_fifo(const path&)`는 Windows에서 항상 `false`를 반환합니다. 헤더는 Visual Studio 2015 RTM 당시 C\+\+17 표준으로 투표되지 않은 초안 기술 사양을 기반으로 합니다. 해당 멤버는 `std::experimental::filesystem::v1` 네임스페이스에 있습니다.  
...
### 경로 생성 및 작성  
 XP 이후 Windows의 경로는 기본적으로 유니코드로 저장됩니다.[path](../standard-library/path-class-cpp-standard-template-library.md) 클래스에서 필요한 모든 문자열 변환을 자동으로 수행합니다. 와이드 및 좁은 문자 배열 둘 다의 인수와 UTF8 또는 UTF16 형식의 `std::string` 및 `std::wstring` 형식을 수락합니다. 또한 `path` 클래스는 경로 구분 기호를 자동으로 정규화합니다. 생성자 인수에 단일 슬래시를 디렉터리 구분 기호로 사용할 수 있습니다. 이렇게 하면 동일한 문자열을 사용하여 Windows 및 UNIX 환경에 둘 다에 경로를 저장할 수 있습니다.  
...
```  
 ...
 두 경로를 연결하기 위해 `std::string` 및 `std::wstring`의 `+` 및 `+=` 연산자와 유사한 오버로드된 `/` 및 `/=` 연산자를 사용할 수 있습니다. 구분 기호를 제공하지 않을 경우 `path` 개체가 편리하게 제공합니다.  
...
```  
 ...
 이 코드를 실행하려면 위의 전체 예제에 붙여넣고 main에서 호출하는 줄의 주석 처리를 제거합니다.  
  
### 경로와 문자열 형식 간 변환  
 `path` 개체를 `std::wstring` 또는 `std::string`으로 암시적으로 변환할 수 있습니다. 즉, 다음 예제와 같이 [wofstream::open](../Topic/basic_ofstream::open.md) 등의 함수에 경로를 전달할 수 있습니다.  
...
## 디렉터리 및 파일 반복  
 \<filesystem\> 헤더는 단일 디렉터리를 반복하는 [directory\_iterator](../standard-library/directory-iterator-class.md) 형식과 디렉터리 및 하위 디렉터리를 재귀적으로 반복하는 [recursive\_directory\_iterator](../standard-library/recursive-directory-iterator-class.md) 클래스를 제공합니다.`path` 개체를 전달하여 반복기를 생성하면 반복기가 경로의 첫 번째 directory\_entry를 가리킵니다. 기본 생성자를 호출하여 끝 반복기를 만듭니다.  
  
 디렉터리를 반복하는 경우 디렉터리, 파일, 기호화된 링크 및 소켓 파일을 포함하되 이에 제한되지 않고 발견할 수 있는 여러 종류의 항목이 있습니다.`directory_iterator`는 해당 항목을 [directory\_entry](../standard-library/directory-entry-class.md) 개체로 반환하며, 각 개체에 보고 있는 항목의 종류를 나타내는 [status\(\)](http://msdn.microsoft.com/ko-kr/a70a3c55-3a76-417f-abaf-862ff94b2056) 멤버가 있습니다. 이 값을 검사하여 특정 항목에 대해 수행할 작업을 결정할 수 있습니다. 다음 예제에서는 단일 디렉터리를 반복하고, 디렉터리 항목이 일반 파일인 경우 코드에서 해당 파일에 대한 일부 정보를 출력합니다. 항목이 디렉터리인 경우 이름만 표시됩니다.  
...
### 재귀적으로 디렉터리 트리 반복  
 다음 예제에서는 디렉터리 트리를 재귀적으로 반복하는 방법을 보여 줍니다. 이 함수를 실행하려면 이전 코드 예제의 DisplayFolderContents 함수 뒤에 붙여넣고 main에서 호출하는 줄의 주석 처리를 제거합니다.  
...
### 기호화된 링크 처리  
 Visual Studio 2015를 기준으로 Visual C\+\+에서 기호화된 링크 검색은 아직 지원되지 않습니다.