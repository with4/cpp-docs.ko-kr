...
### <a name="remarks"></a>설명  
 `difference_type`은 컨테이너의 반복기를 빼거나 더할 때 반환되는 형식입니다. `difference_type` 는 일반적으로 범위에 있는 요소의 수를 나타내는 데 사용 [*첫 번째*, *마지막*) 반복기 사이의 `first` 및 `last`, 가리키는 요소를 포함 `first` 가 가리키는 요소 점을 포함 하지 않고 까지의 범위를 요소 및 `last`합니다.  
