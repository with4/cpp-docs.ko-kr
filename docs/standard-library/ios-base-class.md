...
`_Prec` 를 표시 하려면 유효 자릿수 또는 고정된 표기법의 소수점 뒤의 자릿수입니다.
...
`_Sync` 모든 스트림을와 동기화 되는지 **stdio**합니다.
