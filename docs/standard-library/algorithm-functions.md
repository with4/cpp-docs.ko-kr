...
`first` 정방향 반복기 범위에서 첫 번째 요소의 위치를 주소 지정를 검색할 수 있습니다.

`last` 정방향 반복기 위치를 주소 지정 하나 있는 마지막 요소의 범위를 검색할 수 있습니다.

`comp` 검색 중인 범위 내에서 인접 요소의 값에 의해 충족 하는 조건을 제공 하는 이진 조건자입니다.

### <a name="return-value"></a>반환 값

서로 같거나(첫 번째 버전) 이진 조건자가 지정한 조건을 충족하는(두 번째 버전) 인접 쌍의 첫 번째 요소에 대한 정방향 반복기입니다(해당 요소 쌍이 있는 경우). 그렇지 않으면 `last`를 가리키는 반복기가 반환됩니다.
...
`first` 조건을 확인 하려면 시작 위치를 나타내는 입력된 반복기입니다. 반복기는 요소 범위의 시작 위치를 표시합니다.

`last` 조건을 확인 하는 요소 범위 끝을 나타내는 입력된 반복기입니다.

`comp` 테스트 하는 조건입니다. 확인 중인 요소가 충족할 조건을 정의하는 사용자 정의 조건자 함수 개체입니다. 조건자는 단일 인수를 받아서 `true` 또는 `false`를 반환합니다.

### <a name="return-value"></a>반환 값

지정된 범위의 각 요소에서 조건이 검색되는 경우 `true`, 조건이 한 번도 검색되지 않는 경우 `false`를 반환합니다.
...
`first` 요소를 조건에 대해 범위 검사를 시작 하는 위치를 나타내는 입력된 반복기입니다.

`last` 조건을 확인 하는 요소 범위 끝을 나타내는 입력된 반복기입니다.

`comp` 테스트 하는 조건입니다. 사용자 정의 조건자 함수 개체에 의해 제공됩니다. 조건자는 테스트 중인 요소가 충족해야 할 조건을 정의합니다. 조건자는 단일 인수를 받아서 `true` 또는 `false`를 반환합니다.

### <a name="return-value"></a>반환 값

표시된 범위에서 조건이 한 번이라도 검색되면 `true`, 조건이 전혀 검색되지 않으면 `false`를 반환합니다.
...
`value` 요소의 값에 일치 하는 데 필요한 값 또는 요소 값이 이진 조건자로 지정 된 조건을 충족 해야 하 합니다.

`comp` 하나의 요소가 다른 노드보다 작은지 의미를 정의 하는 사용자 정의 조건자 함수 개체입니다. 이진 조건자는 두 개의 인수를 사용하여 조건이 충족되면 `true`를 반환하고, 충족되지 않으면 `false`를 반환합니다.

### <a name="return-value"></a>반환 값

지정된 값과 동일하거나 등가인 범위에서 요소가 발견될 경우 `true`, 아닌 경우 `false`입니다.
...
`first` 소스 범위에서 첫 번째 요소 위치의 주소를 지정 하는 입력된 반복기입니다.

`last` 소스 범위에 있는 마지막 요소 하나 있는 위치를 주소 지정 하는 입력된 반복기입니다.
...
알고리즘은 첫 요소부터 시작하여 소스 요소를 복사하기 때문에 소스 범위의 `last` 위치가 대상 범위에 포함되지 않은 경우 대상 범위가 소스 범위와 겹칠 수 있습니다. **copy** 는 소스 범위와 대상 범위가 겹치지 않는 경우 오른쪽이 아니라 왼쪽으로 요소를 이동하는 데 사용될 수 있습니다. 여러 위치를 오른쪽으로 이동하려면 [copy_backward](../standard-library/algorithm-functions.md#copy_backward) 알고리즘을 사용합니다.
...
`first` 소스 범위에서 첫 번째 요소 위치의 주소를 지정 하는 양방향 반복기입니다.

`last` 소스 범위에 있는 마지막 요소 하나 있는 위치를 주소 지정 하는 양방향 반복기입니다.

`destEnd` 대상 범위에서 마지막 요소 하나 위치를 주소 지정 하는 양방향 반복기입니다.
...
왜냐하면 알고리즘은 지난 요소의 시작하는 주문의 소스 요소를 복사하고, 도착 범위는 도착 범위를 포함하지 않는 소스 범위의 `first` 위치에 제공되는 소스 범위에 중복할 수 있습니다. `copy_backward`는 소스와 대상 범위가 겹치지 않는 한 왼쪽이 아닌 오른쪽으로 요소를 이동하는 데 사용할 수 있습니다. 여러 위치를 왼쪽으로 이동하려면, [복사](../standard-library/algorithm-functions.md#copy) 알고리즘을 사용합니다.
...
`first` 조건을 확인 하는 범위의 시작을 나타내는 입력된 반복기입니다.

`last` 범위의 끝을 나타내는 입력된 반복기입니다.

`dest` 복사 된 요소에 대 한 대상을 나타내는 출력 반복기입니다.

`_Pred` 범위의 모든 요소에에서 테스트 되는 조건입니다. 이 조건은 사용자 정의 조건자 함수 개체에 의해 제공됩니다. 조건자는 단일 인수를 받아서 `true` 또는 `false`를 반환합니다.

### <a name="return-value"></a>반환 값

조건을 만족하는 각 요소에 대해 `dest`와 동일한 출력 반복기가 한 번씩 증가합니다. 즉, 반환 값 빼기 `dest`는 복사된 요소의 수와 같습니다.
...
`[0, last - first)` 범위의 각 `N`에 대해 위의 식을 한 번 평가하고, 가장 낮은 값부터 시작하여 `N`의 값을 최소값부터 엄격하게 증가시킵니다. 경우 `dest` 및 `first` 저장소, 영역을 지정 `dest` 범위 밖에 있어야 `[ first, last )`합니다.
...
`first` 요소를 복사할 원본 위치를 나타내는 입력된 반복기입니다.

`count` 부호 있는 또는 부호 없는 정수 지정 하는 형식 요소 수를 복사 합니다.
...
템플릿 함수는 계산 `*(dest + N) = *(first + N))` 마다 한 번씩 `N` 범위의 `[0, count)`, 엄격 하 게 증가의 값에 대 한 `N` 가장 낮은 값으로 시작 합니다. 그런 다음 `dest + N`를 반환합니다. 경우 `dest` 및 `first` 저장소, 영역을 지정 `dest` 범위 밖에 있어야 `[first, last)`합니다.
...
`first` 입력된 반복기 범위에서 첫 번째 요소의 위치를 주소 지정 트래버스할 수 있습니다.

`last` 입력된 반복기에서에서 하나 다음 위치의 마지막 요소 범위를 트래버스할 수입니다.

`val` 요소 개수를 셀의 값입니다.

### <a name="return-value"></a>반환 값

값이 `val`인 [  `first`, `last` ) 범위에 있는 요소의 수를 계산하는 **InputIterator**의 차이 유형입니다.
...
`first` 입력된 반복기 범위에서 첫 번째 요소의 위치를 주소 지정를 검색할 수 있습니다.

`last` 입력된 반복기 위치를 주소 지정 하나 있는 마지막 요소의 범위를 검색할 수 있습니다.

`_Pred` 개수를 셀 요소가 충족 되어야 하는 조건을 정의 하는 사용자 정의 조건자 함수 개체입니다. 조건자는 단일 인수를 받아서 **true** 또는 **false**를 반환합니다.
...
`First1` 입력된 반복기는 첫 번째 범위의 첫 번째 요소의 위치를 주소 지정을 테스트할 수 있습니다.

`Last1` 입력된 반복기는 첫 번째 범위에서 마지막 요소 하나 다음의 위치를 주소 지정을 테스트할 수 있습니다.

`First2` 입력된 반복기 두 번째 범위에서 첫 번째 요소의 위치를 주소 지정을 테스트할 수 있습니다.

`First2` 입력된 반복기는 두 번째 범위에서 마지막 요소 하나의 위치를 주소 지정을 테스트할 수 있습니다.

`Comp` 두 요소가 간주 되려면 충족 해야 하는 조건을 정의 하는 사용자 정의 조건자 함수 개체와 동일 합니다. 이진 조건자는 두 개의 인수를 사용하며 조건이 충족되면 **true** 를 반환하고, 충족되지 않으면 **false** 를 반환합니다.
...
두 범위의 길이가 같은 경우 알고리즘의 시간 복잡도는 범위에 포함된 요소 수에 비례합니다. 그렇지 않은 경우 함수에서 바로 `false`를 반환합니다.
...
`val` 순서가 지정 된 범위에 대 한 검색 되는 값입니다.

`comp` 사용자 정의 조건자 함수 개체는 하나의 요소가 다른 노드보다 작은지 의미 정의입니다.

### <a name="return-value"></a>반환 값

사용된 이진 조건자에 의해 정의된 의미(`comp` 또는 기본값, less-than)에서 모든 요소가 `val`과 같은, 검색된 범위 내에 포함된 하위 범위를 지정하는 정방향 반복기의 쌍입니다.

해당 범위의 요소가 `val`과 같지 않으면 반환된 정방향 반복기 쌍은 동일하며, 범위의 순서를 방해하지 않고 `val`을 삽입할 수 있는 지점을 지정합니다.
...
`first` 정방향 반복기 범위에서 첫 번째 요소의 위치를 주소 지정 트래버스할 수 있습니다.

`last` 정방향 반복기 위치를 주소 지정 하나 있는 마지막 요소 범위에서 이동 합니다.

`val` 범위의 요소에 할당할 값 [ `first`, `last`).
...
`First` 범위에서 첫 번째 요소 위치의 주소를 지정 하는 출력 반복기 값이 할당 되어야 `Val`합니다.

`Count` 부호 있는 또는 부호 없는 정수 지정 하는 형식 요소의 수 값을 할당할 수 있습니다.

`Val` 범위의 요소에 할당할 값 [ `First`, *첫 번째 + Count*).
...
`first` 입력된 반복기 범위에서 첫 번째 요소의 위치를 주소 지정 지정된 된 값을 검색할 수 있습니다.

`last` 입력된 반복기 범위에 있는 마지막 요소 하나 다음의 위치를 주소 지정 지정된 된 값을 검색할 수 있습니다.

`val` 검색할 값입니다.

### <a name="return-value"></a>반환 값

검색 중인 범위 내에서 지정된 값이 처음 나타나는 위치의 주소를 지정하는 입력 반복기입니다. 동일한 값의 요소가 없으면 `last`가 반환됩니다.
...
`First1` 정방향 반복기 범위에서 첫 번째 요소의 위치를 주소 지정를 검색할 수 있습니다.

`Last1` 정방향 반복기 위치를 주소 지정 하나 요소를 지난 요소 범위를 검색할 수 있습니다.

`First2` 정방향 반복기 범위에서 첫 번째 요소의 위치를 주소 지정을 검색 합니다.

`Last2` 정방향 반복기 위치를 주소 지정 하나 요소를 지난 요소 범위를 검색 합니다.
...
`last1` 정방향 반복기 위치를 주소 지정 하나 있는 마지막 요소의 범위를 검색할 수 있습니다.

`first2` 정방향 반복기 범위에서 첫 번째 요소의 위치를 주소 지정 일치 합니다.

`last2` 정방향 반복기 위치를 주소 지정 하나 있는 마지막 요소 범위에서 일치 시킬 수 있습니다.
...
`pred` 사용자 정의 조건자 함수 개체 또는 [람다 식을](../cpp/lambda-expressions-in-cpp.md) 검색 중인 요소가 충족 하는 조건을 정의 하는 합니다. 조건자는 인수 하나를 사용하며 `true`(충족) 또는 `false`(미충족)를 반환합니다. `pred`의 서명은 `bool pred(const T& arg);`여야 합니다. 여기서 `T`는 역참조 시 `InputIterator`를 암시적으로 변환할 수 있는 형식입니다. `const` 키워드는 함수 개체 또는 람다가 인수를 수정하지 않아야 함을 나타내기 위한 용도로만 표시되어 있습니다.

### <a name="return-value"></a>반환 값

조건자로 지정된 조건을 충족하는 범위 내 첫 번째 요소를 참조하는 입력 반복기입니다. 이 경우 조건자는 `true`를 반환합니다. 조건자를 충족하는 요소가 없으면 `last`를 반환합니다.
...
`pred` 사용자 정의 조건자 함수 개체 또는 [람다 식을](../cpp/lambda-expressions-in-cpp.md) 검색 중인 요소가 충족 하지 않는 조건을 정의 하는 합니다. 조건자는 인수 하나를 사용하며 `true`(충족) 또는 `false`(미충족)를 반환합니다. `pred`의 서명은 `bool pred(const T& arg);`여야 합니다. 여기서 `T`는 역참조 시 `InputIterator`를 암시적으로 변환할 수 있는 형식입니다. `const` 키워드는 함수 개체 또는 람다가 인수를 수정하지 않아야 함을 나타내기 위한 용도로만 표시되어 있습니다.

### <a name="return-value"></a>반환 값

조건자로 지정된 조건을 충족하지 않는 범위 내 첫 번째 요소를 참조하는 입력 반복기입니다. 이 경우 조건자는 `false`를 반환합니다. 모든 요소가 조건자를 충족하는 경우(조건자가 모든 요소에 대해 `true`를 반환함) `last`를 반환합니다.
...
`first` 작업을 수행할 범위에서 첫 번째 요소 위치의 주소를 지정 하는 입력된 반복기입니다.

`last` 에서 하나 다음 위치의 마지막 요소 범위를 주소 지정 하는 입력된 반복기에 연산을 수행 합니다.
...
`first` 에 값이 할당 범위에서 첫 번째 요소의 위치를 주소 지정 하는 정방향 반복기입니다.

`last` 에 값이 할당 범위에 있는 마지막 요소 하나 다음 위치의 주소를 지정 하는 정방향 반복기입니다.

`_Gen` 함수 개체를 인수 없이 호출 되는 각 범위의 요소에 할당할 값을 생성 하는 데 사용 되는.
...
`First` 에 값이 할당 범위에서 첫 번째 요소 위치의 주소를 지정 하는 출력 반복기입니다.

`Count` 부호 있는 또는 부호 없는 정수 지정 하는 형식 요소 수를 생성기 함수를 통해 값을 할당할 수 있습니다.

`Gen` 함수 개체를 인수 없이 호출 되는 각 범위의 요소에 할당할 값을 생성 하는 데 사용 되는.
...
`first1` 두 개의 정렬 된 소스 범위 중 첫 번째 문에서 첫 번째에 두 번째의 요소를 모두 포함 되어 있는지 여부에 대 한 테스트할 첫 번째 요소 위치의 주소를 지정 하는 입력된 반복기입니다.

`last1` 입력된 반복기 위치를 주소 지정 하나 마지막 요소의 첫 번째 범위에서 두 개의 정렬 된 소스 범위 중 첫 번째에 두 번째의 요소를 모두 포함 되어 있는지 여부에 대 한 테스트 해야 합니다.

`first2` 2의 1 초 동안 첫 번째 요소 위치의 주소를 지정 하는 입력된 반복기 연속 정렬 된 소스 범위 첫 번째에 두 번째의 요소를 모두 포함 되어 있는지 여부에 대 한 테스트 해야 합니다.

`last2` 테스트할 첫 번째에 두 번째의 요소를 모두 포함 되어 있는지 여부에 대 한 두 개의 연속 정렬 된 소스 범위 중 두 번째에서 마지막 요소 하나 다음 위치의 주소를 지정 하는 입력된 반복기입니다.

`comp` 하나의 요소가 다른 노드보다 작은지 의미를 정의 하는 사용자 정의 조건자 함수 개체입니다. 이진 조건자는 두 개의 인수를 사용하며 조건이 충족되면 **true** 를 반환하고, 충족되지 않으면 **false** 를 반환합니다.
...
소스 범위는 **merge**알고리즘에 의해 수정되지 않습니다
...
`first` 두 개의 연속의 첫 번째 범위에서 첫 번째 요소 위치의 주소를 지정 하는 양방향 반복기 결합 하 고 단일 범위로 정렬 범위를 정렬 합니다.

`middle` 두 개의 연속의 첫 번째 요소 위치의 주소를 지정 하는 양방향 반복기 결합 하 고 단일 범위로 정렬 범위를 정렬 합니다.

`last` 두 개의 연속 중 두 번째에서 마지막 요소 하나 다음 위치의 주소를 지정 하는 양방향 반복기 결합 하 고 단일 범위로 정렬 범위를 정렬 합니다.

`comp` 하나의 요소가 다른 보다 큰 의미를 정의 하는 사용자 정의 조건자 함수 개체입니다. 이진 조건자는 두 개의 인수를 가져와 첫 번째 요소가 두 번째 요소보다 작은 경우 **true** 를 반환하고, 그렇지 않은 경우 **false** 를 반환합니다.
...
`first` 힙에 대해 확인할 범위의 시작을 표시 하는 임의 액세스 반복기입니다.

`last` 범위의 끝을 나타내는 임의 액세스 반복기입니다.

`comp` 요소 순서를 테스트 하는 조건입니다. 이진 조건자는 단일 인수를 사용 하 고 반환 `true` 또는 `false`합니다.
...
`begin` 힙에 대해 확인할 범위의 첫 번째 요소를 지정 하는 임의 액세스 반복기입니다.

`end` 힙에 대해 확인할 범위의 끝을 지정 하는 임의 액세스 반복기입니다.

`compare` 엄격한/약한 정렬 조건을 지정 하는 이진 조건자는 힙을 정의 합니다. `compare`를 지정하지 않는 경우의 기본 조건자는 `std::less<>`입니다.

### <a name="return-value"></a>반환 값

지정된 범위가 힙을 형성하거나 요소를 하나 이하로 포함하는 경우 `end`를 반복합니다. 그렇지 않으면 첫 번째로 발견된 힙 조건을 충족하지 않는 요소에 대한 반복기를 반환합니다.

### <a name="remarks"></a>설명

마지막 반복기를 반환 하는 첫 번째 템플릿 함수 `next` 에 `[ begin , end ]` 여기서 `[ begin , next)` 함수 개체에 의해 정렬 힙인 `std::less<>`합니다. 경우 거리 `end - begin < 2`, 함수 반환 `end`합니다.
...
`first` 조건을 확인 하는 범위 시작 위치를 나타내는 입력된 반복기입니다.
...
지정된 범위에서 조건에 대해 `true`를 테스트하는 모든 요소가 `false`를 테스트하는 요소 앞에 있는 경우 true, 아닌 경우 `false`를 반환합니다.
...
`Last1` 범위의 마지막 요소 하나를 참조 하는 정방향 반복기입니다.

`First2` 비교에 사용 되는 두 번째 범위의 첫 번째 요소를 참조 하는 정방향 반복기입니다.

`Last2` 비교에 사용 되는 두 번째 범위의 마지막 요소 하나를 참조 하는 정방향 반복기입니다.

`Pred` 동등성을 테스트 하 고 반환 하는 조건자는 `bool`합니다.

### <a name="return-value"></a>반환 값

비교 연산자 조건자에 따라 동일하게 범위를 다시 정렬할 수 있으면 `true`이고, 그렇지 않으면 `false`입니다.

### <a name="remarks"></a>설명

최악의 경우 `is_permutation`은 정방형 복잡성을 갖습니다.

첫 번째 템플릿 함수에서는 `First2`에서 시작하는 범위에 [ `First1`, `Last1`)로 지정된 범위만큼 많은 요소가 있다고 가정합니다. 두 번째 범위에 더 많은 요소가 있는 경우 무시되고, 더 적은 요소가 있는 경우 정의되지 않은 동작이 발생합니다. 세 째 템플릿 함수(C++14 이상)에서는 이러한 가정을 하지 않습니다.  [ `First1`, `Last1`)로 지정된 범위의 각 요소 X에 대해 `First2` 또는 [ `First2, Last2).`에서 시작하는 범위에 있는 것만큼 많은 Y 요소가 X == Y인 동일한 범위에 있는 경우에만 둘 다 `true`를 반환합니다. 여기에서 `operator==`는 해당 피연산자 간에 pairwise 비교를 수행해야 합니다.
...
`first` 검사할 때 범위를 시작 하는 위치를 지정 하는 정방향 반복기입니다.

`last` 범위의 끝을 표시 하는 정방향 반복기입니다.

`comp` 두 요소 사이의 순서를 결정 하려면 테스트 하는 조건입니다. 조건자는 단일 인수를 받아서 `true` 또는 `false`를 반환합니다. `operator<`와 동일한 작업을 수행합니다.

### <a name="remarks"></a>설명

첫 번째 템플릿 함수를 반환 [is_sorted_until](http://msdn.microsoft.com/en-us/bbad99d0-deaa-4fe6-ae58-eb5b3e4dded0)`( first, last ) == last`합니다. `operator<` 순서 비교를 수행 하는 함수입니다.

두 번째 템플릿 함수를 반환 `is_sorted_until( first, last , comp ) == last`합니다. `comp` 조건자 함수는 순서 비교를 수행합니다.
...
`first` 검사할 때 범위를 시작 하는 위치를 나타내는 정방향 반복기입니다.

`last` 범위의 끝을 표시 하는 정방향 반복기입니다.

`comp` 두 요소 사이의 순서를 결정 하려면 테스트 하는 조건입니다. 조건자는 단일 인수를 받아서 `true` 또는 `false`를 반환합니다.

### <a name="return-value"></a>반환 값

정렬된 순서에서 마지막 요소에 대해 설정된 `ForwardIterator`를 반환합니다. 정렬된 시퀀스는 `first`부터 시작합니다.

### <a name="remarks"></a>설명

`[` `first , next)`가 `operator<`에 의해 정렬된 시퀀스가 되도록 첫 번째 템플릿 함수는 `[` `first ,` `last ]`에서 마지막 반복기 `next`를 반환합니다. `distance()` `< 2`인 경우 함수는 `last`를 반환합니다.
...
`left` 값을 교환 하는 정방향 반복기 중 하나입니다.

`right` 정방향 반복기의 두 번째는 값을 교환할 수 있습니다.
...
`first1` 입력된 반복기는 첫 번째 범위의 첫 번째 요소의 위치를 주소 지정을 비교할 수 있습니다.

`last1` 입력된 반복기는 첫 번째 범위에서 마지막 요소 하나 다음의 위치를 주소 지정 비교할 수 있습니다.

`first2` 입력된 반복기 두 번째 범위에서 첫 번째 요소의 위치를 주소 지정을 비교할 수 있습니다.

`last2` 입력된 반복기 두 번째 범위에서 마지막 요소 하나 다음의 위치를 주소 지정을 비교할 수 있습니다.
...
`value` 인 첫 번째 위치 또는 가능한 첫 번째 위치는 검색 되는 값에 대 한 정렬 된 범위에 있습니다.
...
`first` 힙으로 변환할 범위에서 첫 번째 요소 위치의 주소를 지정 하는 임의 액세스 반복기입니다.

`last` 힙으로 변환할 수 범위에 있는 마지막 요소 하나 다음 위치의 주소를 지정 하는 임의 액세스 반복기입니다.
...
`left` 비교 되는 두 개체의 첫 번째입니다.

`right` 비교 되는 두 개체의 두 번째입니다.
...
`first` 정방향 반복기 범위에서 첫 번째 요소의 위치를 주소 지정 가장 큰 요소를 검색할 수 있습니다.

`last` 정방향 반복기 범위에 있는 마지막 요소 하나 다음의 위치를 주소 지정 가장 큰 요소를 검색할 수 있습니다.
...
`last1` 에서 하나 다음 위치의 마지막 요소는 첫 번째 결합 하 고 정렬 된 단일 범위로 두 개의 정렬 된 소스 범위 중 주소를 지정 하는 입력된 반복기입니다.

`first2` 결합 하 고 정렬 된 단일 범위로 두 개의 연속 정렬 된 소스 범위 중 두 번째에 첫 번째 요소 위치의 주소를 지정 하는 입력된 반복기입니다.

`last2` 결합 하 고 정렬 된 단일 범위로 두 개의 연속 정렬 된 소스 범위 중 두 번째에서 마지막 요소 하나 다음 위치의 주소를 지정 하는 입력된 반복기입니다.

`result` 두 소스 범위는 단일 정렬 범위로 결합를 대상 범위에서 첫 번째 요소 위치의 주소를 지정 하는 출력 반복기입니다.
...
`_IList` 비교 될 멤버를 포함 하는 initializer_list입니다.
...
`first` 정방향 반복기 범위에서 첫 번째 요소의 위치를 주소 지정 가장 작은 요소에 대 한 검색입니다.

`last` 정방향 반복기 범위에 있는 마지막 요소 하나 다음의 위치를 주소 지정 가장 작은 요소에 대 한 검색입니다.
...
`first` 범위의 시작을 나타내는 정방향 반복기입니다.

`last` 범위의 끝을 표시 하는 정방향 반복기입니다.

`comp` 요소 순서 하는 데 사용 하는 선택적 테스트 합니다.
...
첫 번째 템플릿 함수를 반환 `pair<const Type&, const Type&>( right , left )` 경우 `right` 는 보다 작은 `left`합니다. 그 외의 경우 `pair<const Type&, const Type&>( left , right )`를 반환합니다.

두 번째 멤버 함수는 `comp` 조건자와 비교할 때 첫 번째 요소가 더 작고 두 번째 요소가 더 큰 쌍을 반환합니다.

나머지 템플릿 함수는 `left` 및 `right` 매개 변수를 `_IList`로 바꾸는 것을 제외하고 동일하게 동작합니다.
...
`Last2` 입력된 반복기는 두 번째 범위에서 마지막 요소 하나의 위치를 주소 지정을 테스트할 수 있습니다.

`Comp` 각 범위에 있는 현재 요소를 비교 하 고 동일한 지 여부를 확인 하는 사용자 정의 조건자 함수 개체입니다. 조건이 충족되면 **true**를 반환하고, 충족되지 않으면 **false**를 반환합니다.
...
`first` 이동할 요소 범위의 시작 위치를 나타내는 입력된 반복기입니다.

`last` 이동할 요소 범위의 끝을 나타내는 입력된 반복기입니다.

`dest` 출력 반복기 이동된 된 요소를 포함 하는 것입니다.

### <a name="remarks"></a>설명

템플릿 함수는 계산 `*(dest + N) = move(*(first + N))` 마다 한 번씩 `N` 범위의 `[0, last - first)`, 엄격 하 게 증가의 값에 대 한 `N` 가장 낮은 값으로 시작 합니다. 그런 다음 `dest + N`를 반환합니다. 경우 `dest` 및 `first` 저장소, 영역을 지정 `dest` 범위 밖에 있어야 `[first, last)`합니다.
...
`first` 요소를 이동 범위의 시작을 표시 하는 반복기입니다.

`last` 요소를 이동 범위의 끝을 표시 하는 반복기입니다. 이 요소를 이동하지 않습니다.

`destEnd` 대상 범위에서 마지막 요소 하나 위치를 주소 지정 하는 양방향 반복기입니다.

### <a name="remarks"></a>설명

템플릿 함수는 계산 `*(destEnd - N - 1) = move(*(last - N - 1))` 마다 한 번씩 `N` 범위의 `[0, last - first)`, 엄격 하 게 증가의 값에 대 한 `N` 가장 낮은 값으로 시작 합니다. 그런 다음 `destEnd - (last - first)`를 반환합니다. 경우 `destEnd` 및 `first` 저장소, 영역을 지정 `destEnd` 범위 밖에 있어야 `[first, last)`합니다.
...
`first` 양방향 반복기 범위에서 첫 번째 요소의 위치를 가리키는 순열 수입니다.

`last` 수 있는 양방향 반복기 위치를 가리키는 하나 있는 마지막 요소 범위에 배치 되어야 합니다.

`comp` 순서에 따라 연속적인 요소에 충족 될 비교 조건을 정의 하는 사용자 정의 조건자 함수 개체입니다. 이진 조건자는 두 개의 인수를 사용하며 조건이 충족되면 **true** 를 반환하고, 충족되지 않으면 **false** 를 반환합니다.
...
`first` 임의 액세스 반복기 범위에서 첫 번째 요소의 위치를 주소 지정 분할할 수입니다.

*_Nth* 요소의 위치를 주소 지정 하는 임의 액세스 반복기 파티션의 경계에 올바르게 지정 합니다.

`last` 임의 액세스 반복기 범위에 있는 마지막 요소 하나 다음의 위치를 주소 지정 분할할 수입니다.
...
`first` 임의 액세스 반복기 범위에서 첫 번째 요소 위치의 주소를 지정 수입니다.

`sortEnd` 임의 액세스 반복기는 하위 범위에서 마지막 요소 하나 다음 위치의 주소를 지정 수입니다.

`last` 임의 액세스 반복기 위치를 주소 지정 하나 있는 마지막 요소 범위에서 부분적으로 정렬 합니다.
...
둘 중 어느 요소도 다른 것보다 작지 않은 경우 두 요소는 등가이지만, 반드시 같은 것은 아닙니다. **sort** 알고리즘은 안정적이지 않으며 등가 요소의 상대적인 순서가 유지될 것임을 보증하지 않습니다. `stable_sort` 알고리즘은 원래의 순서를 유지합니다.
...
`last1` 에서 하나 다음 위치의 마지막 요소 소스 범위의 주소를 지정 하는 입력된 반복기입니다.

`first2` 정렬된 된 대상 범위에서 첫 번째 요소 위치의 주소를 지정 하는 임의 액세스 반복기입니다.

`last2` 주소에서 하나 다음 위치의 마지막 요소의 정렬된 된 대상 범위를 지정 하는 임의 액세스 반복기입니다.

`comp` 두 요소가 간주 되려면 충족 해야 하는 조건을 정의 하는 사용자 정의 조건자 함수 개체와 동일 합니다. 이진 조건자는 두 개의 인수를 사용하여 조건이 충족되면 `true`를 반환하고, 충족되지 않으면 `false`를 반환합니다.
...
`first` 수 있는 양방향 반복기 범위에서 첫 번째 요소의 위치를 주소 지정 분할할 수 있습니다.

`last` 수 있는 양방향 반복기 범위에 있는 마지막 요소 하나 다음의 위치를 주소 지정 분할할 수 있습니다.

`comp` 분류 해야 하는 요소는 충족 해야 하는 조건을 정의 하는 사용자 정의 조건자 함수 개체입니다. 조건자는 단일 인수를 받아서 **true** 또는 **false**를 반환합니다.
...
`first` 조건에 대해 확인할 범위의 시작 부분을 나타내는 입력된 반복기입니다.

`last` 범위의 끝을 나타내는 입력된 반복기입니다.

`dest1` 조건에 대해 true를 반환 하는 요소를 복사 하는 데 사용 하는 출력 반복기를 사용 하 여 테스트 `_Pred`합니다.

`dest2` 조건에 대해 false를 반환 하는 요소를 복사 하는 데 사용 하는 출력 반복기를 사용 하 여 테스트 `_Pred`합니다.
...
`first` A `ForwardIterator` 조건을 확인 하는 범위의 시작을 나타내는입니다.

`last` A `ForwardIterator` 범위의 끝을 나타내는입니다.
...
`first` 힙에 있는 첫 번째 요소 위치의 주소를 지정 하는 임의 액세스 반복기입니다.

`last` 하나 다음 위치의 마지막 요소 힙에서 주소를 지정 하는 임의 액세스 반복기입니다.
...
`comp` 순서에 따라 연속적인 요소에 충족 될 비교 조건을 정의 하는 사용자 정의 조건자 함수 개체입니다. 이진 조건자는 두 개의 인수를 사용하여 조건이 충족되면 `true`를 반환하고, 충족되지 않으면 `false`를 반환합니다.

### <a name="return-value"></a>반환 값

사전식으로 이전 순열이 존재하여 범위의 원래 순서를 교체한 경우 `true`, 아닌 경우 `false`입니다. 이 경우 순서 지정은 사전식으로 가장 큰 순열로 변환됩니다.
...
Std::random_shuffle() 함수는 사용 되지 않는 교체 [std:: shuffle](../standard-library/algorithm-functions.md#shuffle)합니다. 코드 예제 및 자세한 정보에 대 한 참조 [ \<임의 >](../standard-library/random.md) 과 Stackoverflow 게시물 [이유 std:: random_shuffle 메서드가 사용 되지 않습니다 C + + 14에서?](http://go.microsoft.com/fwlink/p/?linkid=397954)합니다.
...
`first` 요소가 제거 되는 범위에서 첫 번째 요소 위치의 주소를 지정 하는 정방향 반복기입니다.

`last` 요소가 제거 되 고 있는 범위에 있는 마지막 요소 하나 다음 위치의 주소를 지정 하는 정방향 반복기입니다.
...
`first` 요소가 제거 되는 범위에서 첫 번째 요소 위치의 주소를 지정 하는 입력된 반복기입니다.

`last` 요소가 제거 되 고 있는 범위에 있는 마지막 요소 하나 다음 위치의 주소를 지정 하는 입력된 반복기입니다.

`result` 요소가 제거 되는 대상 범위에서 첫 번째 요소 위치의 주소를 지정 하는 출력 반복기입니다.
...
`_Pred` 충족 되어야 하는 단항 조건자는 대체 되어야 하는 요소의 값입니다.
...
`first` 요소가 제거 되는 범위에서 첫 번째 요소의 위치를 가리키는 정방향 반복기입니다.

`last` 요소가 제거 되 고 있는 범위에 있는 마지막 요소 하나 다음의 위치를 가리키는 정방향 반복기입니다.
...
`first` 요소가 대체 되 고 있는 범위에서 첫 번째 요소의 위치를 가리키는 정방향 반복기입니다.

`last` 요소가 대체 되 고 있는 범위에 있는 마지막 요소 하나 다음의 위치를 가리키는 정방향 반복기입니다.

`_OldVal` 대체 되는 요소의의 이전 값입니다.
...
`first` 요소가 대체 되 고 있는 범위에서 첫 번째 요소의 위치를 가리키는 입력된 반복기입니다.

`last` 에서 하나 다음 위치의 마지막 요소를 요소가 대체 되는 범위를 가리키는 입력된 반복기입니다.

`result` 변경된 된 요소 시퀀스를 복사할 대상 범위의 첫 번째 요소를 가리키는 출력 반복기입니다.
...
`result` 요소가 복사 되 고 대상 범위에서 첫 번째 요소의 위치를 가리키는 출력 반복기입니다.

`_Pred` 충족 되어야 하는 단항 조건자는 대체 되어야 하는 요소의 값입니다.

`val` 이전 값이 조건자를 충족 하는 요소에 할당 되 고 새 값입니다.
...
`last` 요소가 대체 되 고 있는 범위에 있는 마지막 요소 하나 다음의 위치를 가리키는 반복기입니다.
...
`replace_if` 알고리즘은 **replace** 알고리즘의 일반화로서, 지정된 상수 값과 동일하지 않고 조건자를 지정하도록 허용합니다.
...
`first` 요소가 순열 되는 범위에서 첫 번째 요소의 위치를 가리키는 양방향 반복기입니다.

`last` 에서 하나 다음 위치의 마지막 요소는 요소가 순열 되는 범위를 가리키는 양방향 반복기입니다.
...
`first` 요소가 순열 되는 소스 범위에서 첫 번째 요소의 위치를 가리키는 양방향 반복기입니다.

`last` 에서 하나 다음 위치의 마지막 요소는 요소가 순열 되는 소스 범위를 가리키는 양방향 반복기입니다.
...
`first` 정방향 반복기 범위에서 첫 번째 요소의 위치를 주소 지정 회전 해야 합니다.

`middle` 요소가 범위의 첫 번째 부분에 있는와 교환 해야 하는 범위의 두 번째 부분에서 첫 번째 요소의 위치 하는 범위 내 경계를 정의 하는 정방향 반복기입니다.

`Last` 정방향 반복기 위치를 주소 지정 하나 있는 마지막 요소 범위를 회전할 수 있습니다.
...
_ `Last` 순환할 하나 다음 위치의 마지막 요소 범위에서 주소 지정 하는 정방향 반복기입니다.

`result` 대상 범위에서 첫 번째 요소 위치의 주소를 지정 하는 출력 반복기입니다.
...
`count` 검색 중인 하위 시퀀스의 크기입니다.

`val` 검색 되는 시퀀스에 있는 요소의 값입니다.
...
`first1` 통합 하 고 정렬 된 두 소스 범위의 차이 나타내는 단일 범위로 두 개의 정렬 된 소스 범위 중 첫 번째에서 첫 번째 요소 위치의 주소를 지정 하는 입력된 반복기입니다.

`last1` 입력된 반복기 위치를 주소 지정 하나 마지막 요소의 첫 번째 범위에서 두 개의 정렬 된 소스 범위 중 통합 하 고 두 소스 범위의 차이 나타내는 단일 범위로 정렬 합니다.

`first2` 두 개의 연속의 1 초 동안 첫 번째 요소 위치의 주소를 지정 하는 입력된 반복기 통합 하 고 두 소스 범위의 차이 나타내는 단일 범위로 정렬 된 소스 범위를 정렬 합니다.

`last2` 지난 하나 다음의 위치를 주소 지정 하는 입력된 반복기는 두 개의 연속 중 두 번째에서 마지막 요소 정렬 된 소스 범위 통합 하 고 두 소스 범위의 차이 나타내는 단일 범위로 정렬 합니다.

`result` 두 소스 범위의 두 소스 범위의 차이 나타내는 정렬 된 단일 범위로 통합 하는를 대상 범위에서 첫 번째 요소 위치의 주소를 지정 하는 출력 반복기입니다.
...
`first1` 통합 하 고 정렬 된 두 소스 범위의 교집합을 나타내는 단일 범위로 두 개의 정렬 된 소스 범위 중 첫 번째에서 첫 번째 요소 위치의 주소를 지정 하는 입력된 반복기입니다.

`last1` 입력된 반복기 위치를 주소 지정 하나 마지막 요소의 첫 번째 범위에서 두 개의 정렬 된 소스 범위 중 통합 하 고 두 소스 범위의 교집합을 나타내는 단일 범위로 정렬 합니다.

`first2` 두 개의 연속의 1 초 동안 첫 번째 요소 위치의 주소를 지정 하는 입력된 반복기 통합 하 고 두 소스 범위의 교집합을 나타내는 단일 범위로 정렬 된 소스 범위를 정렬 합니다.

`last2` 지난 하나 다음의 위치를 주소 지정 하는 입력된 반복기는 두 개의 연속 중 두 번째에서 마지막 요소 정렬 된 소스 범위 통합 하 고 두 소스 범위의 교집합을 나타내는 단일 범위로 정렬 합니다.

**_** *결과* 여기서 두 소스 범위는 대상 범위에서 첫 번째 요소 위치의 주소를 지정 하는 출력 반복기는 두 개의 소스의 교집합을 나타내는 정렬 된 단일 범위로 통합 하는 범위입니다.
...
`first1` 통합 하 고 정렬 된 두 소스 범위의 대칭 차를 나타내는 단일 범위로 두 개의 정렬 된 소스 범위 중 첫 번째에서 첫 번째 요소 위치의 주소를 지정 하는 입력된 반복기입니다.

`last1` 입력된 반복기 위치를 주소 지정 하나 마지막 요소의 첫 번째 범위에서 두 개의 정렬 된 소스 범위 중 통합 하 고 두 소스 범위의 대칭 차를 나타내는 단일 범위로 정렬 합니다.

`first2` 두 개의 연속의 1 초 동안 첫 번째 요소 위치의 주소를 지정 하는 입력된 반복기 통합 하 고 두 소스 범위의 대칭 차를 나타내는 단일 범위로 정렬 된 소스 범위를 정렬 합니다.

`last2` 지난 하나 다음의 위치를 주소 지정 하는 입력된 반복기는 두 개의 연속 중 두 번째에서 마지막 요소 정렬 된 소스 범위 통합 하 고 두 소스 범위의 대칭 차를 나타내는 단일 범위로 정렬 합니다.

**_** *결과* 여기서 두 소스 범위는 대상 범위에서 첫 번째 요소 위치의 주소를 지정 하는 출력 반복기 두 대칭 차를 나타내는 정렬 된 단일 범위로 통합 하는 원본 범위입니다.
...
`first1` 통합 하 고 정렬 된 두 소스 범위의 합집합을 나타내는 단일 범위로 두 개의 정렬 된 소스 범위 중 첫 번째에서 첫 번째 요소 위치의 주소를 지정 하는 입력된 반복기입니다.

`last1` 입력된 반복기 위치를 주소 지정 하나 마지막 요소의 첫 번째 범위에서 두 개의 정렬 된 소스 범위 중 통합 하 고 두 소스 범위의 합집합을 나타내는 단일 범위로 정렬 합니다.

`first2` 두 개의 연속의 1 초 동안 첫 번째 요소 위치의 주소를 지정 하는 입력된 반복기 통합 하 고 두 소스 범위의 합집합을 나타내는 단일 범위로 정렬 된 소스 범위를 정렬 합니다.

`last2` 지난 하나 다음의 위치를 주소 지정 하는 입력된 반복기는 두 개의 연속 중 두 번째에서 마지막 요소 정렬 된 소스 범위 통합 하 고 두 소스 범위의 합집합을 나타내는 단일 범위로 정렬 합니다.

**_** *결과* 두 소스 범위의 합집합을 나타내는 정렬 된 단일 범위로 통합 하는 여기서 두 소스 범위는 대상 범위에서 첫 번째 요소 위치의 주소를 지정 하는 출력 반복기입니다.
...
`first` 순서가 섞인 (포함)를 범위에서 첫 번째 요소에 반복기입니다. 
          `RandomAccessIterator` 및 `ValueSwappable`의 요구 사항을 충족해야 합니다.

`last` 순서가 섞인, 배타적 범위에서 마지막 요소에는 반복기입니다. 
          `RandomAccessIterator` 및 `ValueSwappable`의 요구 사항을 충족해야 합니다.

`gen` 난수 생성기를는 `shuffle()` 함수는 작업을 사용 합니다. `UniformRandomNumberGenerator`의 요구 사항을 충족해야 합니다.
...
`last` 임의 액세스 반복기 범위에 있는 마지막 요소 하나 다음 위치의 주소를 지정 수입니다.

`comp` 순서에 따라 연속적인 요소에 충족 될 비교 조건을 정의 하는 사용자 정의 조건자 함수 개체입니다. 이 이진 조건자는 두 개의 인수를 받아서, 두 인수가 순서대로 되어 있는 경우 `true`, 아닌 경우 `false`를 반환합니다. 이 비교 함수는 시퀀스의 요소 쌍에 대해 엄밀히 약한 순서를 적용해야 합니다. 자세한 내용은 [알고리즘](../standard-library/algorithms.md)을 참조하세요.
...
`first` 대상 힙 첫 번째 요소의 위치를 주소 지정 하는 임의 액세스 반복기입니다.

`last` 대상 힙에 있는 마지막 요소 하나 다음 위치의 주소를 지정 하는 임의 액세스 반복기입니다.
...
`_Pred` 분류 해야 하는 요소는 충족 해야 하는 조건을 정의 하는 사용자 정의 조건자 함수 개체입니다. 조건자는 단일 인수를 받아서 **true** 또는 **false**를 반환합니다.
...
`first` 양방향 반복기 범위에서 첫 번째 요소 위치의 주소를 지정 수입니다.

`last` 양방향 반복기 범위에 있는 마지막 요소 하나 다음 위치의 주소를 지정 수입니다.
...
`left` 첫 번째 재정의 첫 번째 개체를 해당 내용을 교환 합니다. 두 번째 재정의에서는 내용이 있는 첫 번째 개체 배열이 교환됩니다.

`right` 에 대 한 첫 번째 재정의 두 번째 개체를 해당 내용을 교환 합니다. 두 번째 재정의에서는 내용이 있는 두 번째 개체 배열이 교환됩니다.
...
`first1` 요소가를 교환할 수 인 첫 번째 범위의 첫 번째 위치를 가리키는 정방향 반복기입니다.

`last1` 지난 다음 요소는 첫 번째 범위의 요소가를 교환할 수의 최종 위치를 가리키는 정방향 반복기입니다.

`first2` 교환할 요소가 두 번째 범위의 첫 번째 위치를 가리키는 정방향 반복기입니다.
...
`first1` 작업을 수행할 첫 번째 소스 범위에서 첫 번째 요소 위치의 주소를 지정 하는 입력된 반복기입니다.

`last1` 첫 번째 소스 범위에 있는 마지막 요소 하나 다음 위치의 주소를 지정 하는 입력된 반복기에 연산을 수행 합니다.

`first2` 작업을 수행할 두 번째 소스 범위에서 첫 번째 요소 위치의 주소를 지정 하는 입력된 반복기입니다.

`result` 대상 범위에서 첫 번째 요소 위치의 주소를 지정 하는 출력 반복기입니다.

`_Func` 첫 번째 소스 범위의 각 요소에 적용 되는 알고리즘의 첫 번째 버전에 사용 되는 개체 또는 사용자 정의 (UD) 이항 함수 개체 쌍 단위 정방향 순서로 적용 되는 알고리즘의 두 번째 버전에서 사용 되는 사용자 정의 단항 함수 두 개의 소스 범위에 있습니다.
...
`first` 검색 하 여 중복 제거에 대 한 범위에서 첫 번째 요소 위치의 주소를 지정 하는 정방향 반복기입니다.

`last` 에서 하나 다음 위치의 마지막 요소를 검색 하 여 중복 제거 범위를 주소 지정 하는 정방향 반복기입니다.
...
`first` 정방향 반복기 소스 범위의 첫 번째 요소의 위치를 주소 지정을 복사할 수 있습니다.

`last` 정방향 반복기 소스 범위에 있는 마지막 요소 하나 다음의 위치를 주소 지정을 복사할 수 있습니다.

`result` 연속 중복 항목이 사용 하 여 복사본을 수신 하는 대상 범위에서 첫 번째 요소 위치의 주소를 지정 하는 출력 반복기를 제거 합니다.
...
`first` 검색할 범위에서 첫 번째 요소의 위치입니다.

`last` 에서 하나 다음 위치의 마지막 요소 범위를 검색할 수 있습니다.

`value` 반환 된 반복기가 주소를 지정 하는 요소의 값을 초과 하는 정렬된 범위 값입니다.
