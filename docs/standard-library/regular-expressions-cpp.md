...
이 항목에서는 다양한 정규식 엔진의 문법에 대해 설명합니다.  
  
## 항목 내용  
  
-   [정규식 문법](#regexgrammar)  
  
-   [문법 요약](#grammarsummary)  
  
-   [의미 정보](#semanticdetails)  
  
-   [일치 및 검색](#matchingandsearching)  
...
-   줄 바꿈을 제외하고 대상 시퀀스에서 문자와 일치하는 *와일드카드 문자*입니다.  
  
-   `expr` 식으로 정의된 집합에도 있는 대상 시퀀스의 문자 또는 데이터 정렬 요소와 일치하는 "\[`expr`\]" 형식, 또는 `expr` 식으로 정의된 집합에 없는 대상 시퀀스의 문자 또는 데이터 정렬 요소와 일치하는 "\[^`expr`\]" 형식의 *대괄호 식*입니다.  
    ...
    -   "\[\=`elt`\=\]" 형식의 *동급 클래스*.  `elt`와 동급의 데이터 정렬 요소를 `expr`로 정의된 집합에 추가합니다.  
  
    -   "\[.`elt`.\]" 형식의 *데이터 정렬 기호*.  `expr`로 정의된 집합에 데이터 정렬 요소 `elt`를 추가합니다.  
  
-   *앵커*.  '^' 앵커는 대상 시퀀스의 시작과 일치합니다. '$' 앵커는 대상 시퀀스의 끝과 일치합니다.  
  
 구분 기호 간 패턴이 일치하는 대상 시퀀스에서 문자 시퀀스와 일치하는 "\(*하위 식*\)" 형식, 또는 `BRE` 및 `grep`의 "\\\(*하위 식*\\\)"의 *캡처 그룹*입니다.  
  
-   대상 시퀀스에서 `k` 문자와 일치하는 "\\`k`" 형식의 *ID 이스케이프*입니다.  
...
-   "\(a\)"는 대상 시퀀스 "a"와 일치하고 캡처 그룹 1을 하위 시퀀스 "a"에 연결하지만, 대상 시퀀스 "B", "b", "c"와 일치하지 않습니다.  
 ...
 `ECMAScript`, `BRE`, `grep`에서 요소는 "\\`dd`" 형식의 *역참조*도 될 수 있습니다. 여기서, `dd`는 N번째 *캡처 그룹*과 일치하는 문자 시퀀스와 동일한 대상 시퀀스 내 문자 시퀀스와 일치하는 십진수 값 N을 나타냅니다.  예를 들어, "\(a\)\\1"는 대상 시퀀스 "aa"와 일치합니다. 그 이유는 첫 번째이자 유일한 캡처 그룹이 초기 시퀀스인 "a"와 일치하고 \\1이 마지막 시퀀스인 "a"와 일치하기 때문입니다.  
...
-   "\(?\= *하위 식*\) 형식의 *긍정 어셜션*.  구분 기호 간 패턴이 일치하는 대상 시퀀스 내 문자 시퀀스와 일치하지만 대상 시퀀스에서 일치 위치는 변경되지 않습니다.  
  
-   "\(?\! *하위 식*\)" 형식의  *부정 어설션*.  구분 기호 간 패턴이 일치하지 않는 대상 시퀀스 내 모든 문자 시퀀스와 일치하며 대상 시퀀스에서 일치 위치를 변경하지 않습니다.  
...
-   "\\\\", "\\a", "\\b", "\\f", "\\n", "\\r", "\\t", "\\v" 형식의 제한된 *파일 형식 이스케이프*.  대상 시퀀스에서 각각 백슬래시, 경고, 백스페이스, 폼 피드, 줄 바꿈, 캐리지 리턴, 가로 탭, 수직 탭과 일치합니다.  
  
-   "\\`ooo`" 형식의 *8진수 이스케이프 시퀀스*.  대상 시퀀스에서 표현이 한 자리, 두 자리 또는 세 자리 8진수 `ooo`로 표현된 값인 문자와 일치합니다.  
  
### 반복  
 *긍정 어설션*, *부정 어설션* 또는 *앵커* 이외의 모든 요소 다음에 반복 횟수가 올 수 있습니다.  가장 일반적인 유형의 반복 횟수는 "{`min`,`max`}", 또는 `BRE` 및 `grep`에서 "\\{`min`,`max`\\}"의 형태입니다.  이러한 형태의 반복 횟수 앞에 오는 요소는 해당 요소와 일치하는 시퀀스가 연속적으로 `min`개 이상, `max`개 이하일 경우 일치합니다.  예를 들어, "a{2,3}"은 대상 시퀀스 "aa" 및 대상 시퀀스 "aaa"와 일치하지만 대상 시퀀스 "a" 또는 대상 시퀀스 "aaaa"와는 일치하지 않습니다.  
  
 또한 반복 횟수는 다음 형식 중 하나를 사용할 수 있습니다.  
  
-   "{`min`}", 또는 `BRE` 및 `grep`의 "\\{`min`\\}".  "{`min`,`min`}"와 같습니다.  
  
-   "{`min`,}", 또는 `BRE` 및 `grep`의 "\\{`min`,\\}".  "{`min`,unbounded}"와 같습니다.  
...
-   "a\*"는 대상 시퀀스 "", 대상 시퀀스 "a", 대상 시퀀스 "aa" 등과 일치합니다.  
 ...
 `BRE` 및 `grep`를 제외한 모든 문법의 경우 반복 횟수는 다음 중 하나의 형식이 될 수도 있습니다.  
...
-   "a?"는 대상 시퀀스 "", 대상 시퀀스 "a"와 일치하지만 대상 시퀀스 "aa"와는 일치하지 않습니다.  
...
### 교체  
 `BRE` 및 `grep`를 제외한 모든 정규식 문법에서는 연결된 정규식 다음에 문자 '&#124;' 및 다른 연결된 정규식이 올 수 있습니다.  이러한 방식으로 연결된 정규식을 무한히 결합할 수 있습니다.  결과 식은 하나 이상의 연결된 정규식과 일치하는 대상 시퀀스와 일치합니다.  
  
 둘 이상의 연결된 정규식이 대상 시퀀스와 일치할 경우 `ECMAScript`는 일치 항목으로 시퀀스와 일치하는 연결된 정규식 중 첫 번째 정규식을 선택하고\(*첫 번째 일치*\), 다른 정규식 문법은 *가장 긴 일치*를 달성하는 정규식을 선택합니다.  예를 들어, "ab&#124;cd"는 대상 시퀀스 "ab" 및 대상 시퀀스 "cd"와 일치하지만 대상 시퀀스 "abd" 또는 대상 시퀀스 "acd"와는 일치하지 않습니다.  
  
 `grep` 및 `egrep`에서 줄 바꿈 문자\('\\n'\)를 사용하여 교체를 구분할 수 있습니다.  
  
### 하위 식  
 `BRE` 및 `grep`에서 하위 식은 연결입니다.  다른 정규식 문법에서 하위 식은 교체입니다.  
  
##  <a name="grammarsummary"></a> 문법 요약  
 다음 표에 다양한 정규식 문법에서 사용할 수 있는 기능이 요약되어 있습니다.  
  
|요소|BRE|ERE|ECMA|grep|egrep|awk|  
|--------|---------|---------|----------|----------|-----------|---------|  
|'&#124;'를 사용하는 교체||\+|\+||\+|\+|  
|'\\n'을 사용하는 교체||||\+|\+||  
|앵커|\+|\+|\+|\+|\+|\+|  
...
### 역참조  
 역참조는 백슬래시와 10진수 값 N으로 구성됩니다.  역참조는 N번째 *캡처 그룹*의 내용과 일치합니다.  N 값은 역참조 앞의 캡처 그룹 수보다 많을 수 없습니다.  `BRE` 및 `grep`에서 N 값은 백슬래시 다음의 10진수에 의해 결정됩니다.  `ECMAScript`에서 N 값은 백슬래시 바로 다음의 모든 10진수에 의해 결정됩니다.  따라서 `BRE` 및 `grep`에서 정규식의 캡처 그룹이 9개를 초과하는 경우에도 N 값은 절대 9를 초과하지 않습니다.  `ECMAScript`에서 N 값은 제한이 없습니다.  
...
-   "\(b\(\(\(\(\(\(\(\(\(a\)\)\)\)\)\)\)\)\)\)\\10"은 `BRE`와 `ECMAScript`에서 의미가 다릅니다.  `BRE`에서 역참조는 "\\1"입니다.  역참조는 첫 번째 캡처 그룹의 내용\(즉, "\(b"로 시작하고 마지막 "\)"로 끝나며 역참조 앞에 오는 내용\)과 일치하고, 마지막 '0'은 일반 문자 '0'과 일치합니다.  `ECMAScript`에서 역참조는 "\\10"입니다.  10번째 캡처 그룹, 즉 가장 안쪽의 캡처 그룹과 일치합니다.  
...
### 대괄호 식  
 ...
 문자 집합은 *개별 문자*, *문자 범위*, *문자 클래스*, *동급 클래스*, *데이터 정렬 기호*를 임의로 결합하여 정의할 수 있습니다.  
...
### 문자 클래스  
 대괄호 식의 문자 클래스는 명명된 클래스의 모든 문자를 대괄호 식으로 정의된 문자 집합에 추가합니다.  문자 클래스를 만들려면 "\[:" 클래스 이름 ":\]"를 사용합니다.  내부적으로 문자 클래스의 이름은 `id = traits.lookup_classname`을 호출하여 인식됩니다.  `traits.isctype(ch, id)`에서 true를 반환할 경우 문자 `ch`는 해당 클래스에 속합니다.  기본 `regex_traits` 템플릿은 다음 표의 클래스 이름을 지원합니다.  
...
### 데이터 정렬 기호  
 대괄호 식의 데이터 정렬 기호는 대괄호 식으로 정의된 집합에 *데이터 정렬 요소*를 추가합니다.  데이터 정렬 기호를 만들려면 "\[." 데이터 정렬 요소 ".\]"를 사용합니다.  
...
### 동급 클래스  
 대괄호 식의 동급 클래스는 동급 클래스 정의의 데이터 정렬 요소와 동일한 모든 문자와 *데이터 정렬 요소*를 대괄호 식으로 정의된 집합에 추가합니다.  동급 클래스를 만들려면 "\[\=" 데이터 정렬 요소 "\=\]"를 사용합니다.  `traits.transform_primary(elt1.begin(), elt1.end()) == traits.transform_primary(elt2.begin(), elt2.end())`일 경우, 내부적으로 두 개의 데이터 정렬 요소 `elt1` 및 `elt2`는 동일합니다.  
...
### ID 이스케이프  
 ID 이스케이프는 백슬래시와 단일 문자의 순서로 구성되어 있습니다.  해당 문자와 일치합니다.  문자에 특별한 의미가 있을 경우 필요합니다. ID 이스케이프를 사용할 경우 특별한 의미는 제거됩니다.  예를 들면 다음과 같습니다.  
  
-   "a\*"는 대상 시퀀스 "aaa"와 일치하지만 대상 시퀀스 "a\*"와는 일치하지 않습니다.  
  
-   "a\\\*"는 대상 시퀀스 "aaa"와 일치하지 않지만 대상 시퀀스 "a\*"와는 일치합니다.  
...
|`ERE`, `egre`|{ '\(', '\)', '{', '.', '\[', '\\', '\*', '^', '$', '\+', '?', '&#124;' }|  
|`awk`|`ERE` 더하기 { '"', '\/' }|  
|`ECMAScript`|식별자에 포함될 수 있는 문자를 제외한 모든 문자.  일반적으로 문자, 숫자, '$', '\_' 및 유니코드 이스케이프 시퀀스가 포함됩니다.  자세한 내용은 ECMAScript 언어 사양을 참조하십시오.|  
...
### 비캡처 그룹  
 비캡처 그룹은 정규식 문법에서 내용을 단일 단위로 표시하지만 대상 텍스트에 레이블을 지정하지 않습니다.  예를 들어, "\(a\)\(?:b\)\*\(c\)는 대상 텍스트 "abbc"와 일치하고 캡처 그룹 1을 하위 시퀀스 "a"와 연결하며 캡처 그룹 2를 하위 시퀀스 "c"와 연결합니다.  
  
### non\-greedy 반복  
 non\-greedy 반복은 패턴과 일치하는 대상 시퀀스의 가장 짧은 하위 시퀀스를 사용합니다.  greedy 반복은 가장 긴 항목을 사용합니다.  예를 들어, "\(a\+\)\(a\*b\)"는 대상 시퀀스 "aaab"와 일치합니다.  non\-greedy 반복을 사용할 경우 캡처 그룹 1을 대상 시퀀스의 시작 부분에 있는 시퀀스 "a"와 연결하고 캡처 그룹 2를 대상 시퀀스의 끝에 있는 시퀀스 "aab"와 연결합니다.  greedy 일치를 사용할 경우 캡처 그룹 1을 하위 시퀀스 "aaa"와 연결하고 캡처 그룹 2를 하위 시퀀스 "b"와 연결합니다.  
...
-   ^  $  \\  .  \*  \+  ?  \(  \)  \[  \]  {  }  &#124;  
 ...
 `BRE` 및 `grep`에서 다음 문자는 특별한 의미가 있습니다.  
  
-   .   \[   \\  
  
 또한 `BRE` 및 `grep`에서 다음 문자를 특정 컨텍스트에서 사용할 경우 특별한 의미가 있습니다.  
  
-   '\*'는 정규식에서 첫 번째 문자일 경우 또는 정규식에서 최초 '^' 다음의 첫 번째 문자를 제외한 모든 경우 또는 캡처 그룹의 첫 문자일 경우 또는 캡처 그룹에서 최초 '^' 다음의 첫 번째 문자일 경우 특별한 의미가 있습니다.  
  
-   '^'는 정규식의 첫 번째 문자일 경우 특별한 의미가 있습니다.  
  
-   '$'는 정규식의 마지막 문자일 경우 특별한 의미가 있습니다.  
  
 `ERE`, `egrep` 및 `awk`에서 다음 문자는 특별한 의미가 있습니다.  
  
-   .   \[   \\   \(   \*   \+   ?   {   &#124;  
  
 또한 `ERE`, `egrep` 및 `awk`에서 다음 문자를 특정 컨텍스트에서 사용할 경우 특별한 의미가 있습니다.  
...
-   '$'는 정규식의 마지막 문자일 경우 특별한 의미가 있습니다.  
 ...
 대상 시퀀스에서 동일한 문자와 일치하는 일반 문자.  기본적으로 두 문자가 동일한 값으로 표현될 경우 일치가 성공함을 의미합니다.  대\/소문자를 구분하는 일치에서 `traits.translate_nocase(ch0) == traits.translate_nocase(ch1)`일 경우 두 문자 `ch0` 및 `ch1`이 일치합니다.  로캘을 구분하는 일치에서 `traits.translate(ch0) == traits.translate(ch1)`일 경우 두 문자 `ch0` 및 `ch1`이 일치합니다.  
...
-   "\(aa\)\(a\*\)"는 대상 시퀀스 "aaaa"와 일치하고 캡처 그룹 1을 대상 시퀀스의 시작 부분의 시퀀스 "aa"와 연결하고 캡처 그룹 2를 대상 시퀀스의 끝의 시퀀스 "aa"와 연결합니다.  
...
|"$n"|"\\n"|`n` 위치에서 캡처 그룹과 일치하는 문자 시퀀스, 여기서 `n`은 0~9 사이의 숫자\(`[match[n].first, match[n].second)`|  
||"\\\\n"|"\\n"|  
|"$nn"||`nn` 위치에서 캡처 그룹과 일치하는 문자 시퀀스, 여기서 `nn`은 10~99 사이의 숫자\(`[match[nn].first, match[nn].second)`|  
