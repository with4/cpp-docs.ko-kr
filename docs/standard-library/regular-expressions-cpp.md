...
C + + 표준 라이브러리는 여러 정규식 문법 지원합니다. 이 항목에서는 정규식을 사용 하는 경우 사용할 수 있는 문법 변형을 설명 합니다.  
...
-   `egrep`:이 `extended`, alternatios 구분 하는 줄 바꿈 문자를 수행할 수도 있습니다.
...
0 개 또는 그 이상의 플래그 정규식 엔진 동작을 지정 하는 문법을와 함께 사용할 수 있습니다. 플래그 지정 `ECMAScript` 문법 라고 가정 합니다.
