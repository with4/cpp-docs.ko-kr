...
>  `SafeInt` 클래스는 모든 종류의 정수를 허용하지만, 부호 없는 형식이 보다 효율적으로 수행됩니다.
 ...
 오류 정책을 사용자 지정하는 방법은 두 가지가 있습니다. 첫 번째는 `SafeInt`를 만들 때 매개변수 `E`를 설정하는 것입니다. `SafeInt` 개체 하나의 오류 처리 정책 변경 시 이 방법을 사용합니다. 다른 방법은 `SafeInt` 라이브러리를 포함하기 전에 `_SAFEINT_DEFAULT_ERROR_POLICY`를 사용자 정의 오류 처리 클래스로 지정하는 것입니다. 코드에 사용된 모든 `SafeInt` 클래스 인스턴스의 기본 오류 처리 정책을 변경하려면 이 방법을 사용하십시오.
