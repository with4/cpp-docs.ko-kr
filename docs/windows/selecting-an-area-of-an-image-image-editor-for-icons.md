...
> [!NOTE]
>  참조는 **사각형 선택** 및 **부정형** 도구에 나온 [이미지 편집기 도구 모음](../windows/toolbar-image-editor-for-icons.md) 는 에있는각단추와연결된도구설명을보거나**이미지 편집기** 도구 모음입니다.  
  
 선택 영역에서 사용자 지정 브러시를 만들 수도 있습니다. 자세한 내용은 참조 [사용자 지정 브러시 만들기](../windows/creating-a-custom-brush-image-editor-for-icons.md)합니다.  
  
### <a name="to-select-an-area-of-an-image"></a>이미지의 영역을 선택  
  
1.  에 **이미지 편집기** 도구 모음 (또는 **이미지** 메뉴 **도구** 명령)을 원하는 선택 도구를 클릭 합니다.  
  
2.  이미지 영역 선택 하려는으로의 한쪽에 삽입 포인터를 이동 합니다. 십자 모양 커서를 이미지 위로 가져갈 때 표시 됩니다.  
  
3.  선택 영역의 반대 모서리에 커서를 끕니다. 사각형을 픽셀 선택 될 보여 줍니다. 사각형을 포함 하는 사각형 내에서 모든 픽셀 선택 영역에 포함 됩니다.  
  
4.  마우스 단추를 놓습니다. 선택 영역 테두리 선택된 영역을 포함합니다.  
  
### <a name="to-select-an-entire-image"></a>전체 이미지를 선택 하려면  
  
1.  현재 선택 영역 외부의 이미지를 클릭 합니다. 선택 영역 테두리 포커스를 변경 하 고 다시 한 번에 전체 이미지를 포함 합니다.  
