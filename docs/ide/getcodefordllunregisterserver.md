...
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "GetCodeForDllUnregisterServer 메서드"
...
## <a name="return-value"></a>반환 값  
 서버 등록 취소에 대 한 코드를 포함 하는 문자열입니다.  
  
## <a name="remarks"></a>설명  
 서버 등록 취소에 대 한 적절 한 코드를 검색 하려면이 멤버 함수를 호출 합니다.  
...
|3|경우 (hRes! S_OK =)|  
|4|\treturn hRes;|  
|5|if (!입니다. COleObjectFactory::UpdateRegistryAll(FALSE))|  
|6|\treturn ResultFromScode(SELFREG_E_CLASS);|  
|7|S_OK를 반환 합니다.|  
  
 반환 되는 줄은 각각에 대 한 `GetCodeForDllUnregisterServer` 선행 탭 추가 (`\t`) 및 후행 "CR-LF" (캐리지 리턴-줄 바꿈) 문자 쌍 (`\r\n`).  
