---
title: "유니버설 Windows 플랫폼으로 포팅(C++) | Microsoft Docs"
ms.custom: ""
ms.date: "12/16/2016"
...
manager: "ghogen"
---
# 유니버설 Windows 플랫폼으로 포팅(C++)
[!INCLUDE[vs2017banner](../assembler/inline/includes/vs2017banner.md)]

이 항목에서는 Windows 10 앱 플랫폼인 유니버설 Windows 플랫폼에 기존 C\+\+ 코드를 이식하는 방법에 대한 정보를 찾을 수 있습니다.*유니버설*이라는 용어는 코드가 데스크톱, 휴대폰, 태블릿 및 앞으로 제공될 장치\(Windows 10 실행\)를 비롯하여 Windows 10을 실행하는 모든 장치에서 실행될 수 있음을 의미합니다. Windows 8.1을 사용할 경우 공유 프로젝트라는 특수한 프로젝트 시스템 기능을 사용하여 Windows 8.1 및 Windows Phone 8.1 모두를 대상으로 하는 앱을 만들었을 것입니다. 유니버설 Windows 앱은 이 메커니즘을 사용하지 않으며 대신, Windows 10이 실행되는 모든 장치에서 잘 작동 하는 단일 프로젝트 및 단일 XAML을 사용합니다. XAML의 동적 레이아웃 기능을 사용하여 앱 UI를 다른 디스플레이 크기에 맞출 수 있습니다.  
  
 Windows 개발자 센터 설명서에는 유니버설 Windows 플랫폼으로 Windows 8.1 앱을 포팅하기 위한 가이드가 포함되어 있습니다.[Windows 런타임 8에서 UWP로 이동](http://msdn.microsoft.com/library/windows/apps/dn954974.aspx)을 참조하세요. 이 가이드는 C\# 코드에 주로 초점을 맞추고 있지만 대부분의 지침이 C\+\+에도 적용됩니다. 다음 절차에는 보다 자세한 정보가 포함되어 있습니다.  
...
##  <a name="BK_81StoreApp"></a> Windows 8.1 스토어 앱을 UWP에 이식  
 Windows 8.1 스토어 앱을 사용하는 경우 이러한 작업을 위해 UWP 및 Windows 10을 실행하는 모든 장치에서 다음 절차를 사용할 수 있습니다.  컴파일러 및 라이브러리 변경으로 인해 발생하는 모든 문제를 제거하려면 먼저 [!INCLUDE[vs_dev14](../ide/includes/vs_dev14_md.md)]를 Windows 8.1 프로젝트로 빌드하는 것이 좋습니다. 작업을 완료한 후 이 작업을 Windows 10 UWP 프로젝트로 변환하는 두 가지 방법이 있습니다. 가장 쉬운 방법은\(다음 절차에서 설명\) 유니버설 Windows 프로젝트를 만들고 기존 코드를 복사하는 것입니다. Windows 8.1 데스크톱 및 Windows 8.1 Phone용 유니버설 프로젝트를 사용한 경우 프로젝트는 XAML의 두 개의 다른 레이아웃으로 시작되지만 디스플레이 크기에 맞게 조정되는 단일 동적 레이아웃으로 끝납니다. Windows 10으로 프로젝트를 변환하는 두 번째 방법은 프로젝트 파일 및 Package.appxmanifest 파일을 편집하는 것입니다. 이 메서드는 [UWP\(유니버설 Windows 플랫폼\)로 앱 마이그레이션](../misc/migrate-apps-to-the-universal-windows-platform-uwp.md)에서 설명합니다.  
  
#### Windows 8.1 스토어 앱을 UWP에 이식하려면  
  
1.  [!INCLUDE[vs_dev14](../ide/includes/vs_dev14_md.md)]에서 아직 Windows 8.1 앱 프로젝트를 열지 않은 경우 열고 지침에 따라 프로젝트 파일을 업그레이드합니다.  
  
     Visual Studio 설치 프로그램에서 Windows 8.1 도구를 설치해야 합니다. 이러한 도구를 설치하지 않은 경우 프로그램 및 기능 창에서 Visual Studio 설치를 시작하고 [!INCLUDE[vs_dev14](../ide/includes/vs_dev14_md.md)]를 선택한 후 설치 창에서 **수정**을 선택합니다. Windows 8.1 도구를 찾은 후 선택되어 있는지 확인하고 확인을 선택합니다.  
  
2.  프로젝트 속성 창을 열고 C\+\+, 일반에서 플랫폼 도구 집합을 [!INCLUDE[vs_dev14](../ide/includes/vs_dev14_md.md)]용 빌드 도구인 v140으로 설정합니다.  
  
3.  프로젝트를 Windows 8.1 프로젝트로 빌드하고 빌드 오류를 해결합니다. 이 단계에서 발생하는 모든 오류는 빌드 도구 및 라이브러리의 주요 변경 내용 때문일 수 있습니다. 코드에 영향을 줄 수 있는 변경 사항에 대한 자세한 내용은 [Visual C\+\+ 2015의 주요 변경 내용](../porting/visual-cpp-change-history-2003-20151.md)을 참조하세요.  
...
#### Windows 8.1 런타임 구성 요소를 UWP에 포팅하려면  
  
1.  [!INCLUDE[vs_dev14](../ide/includes/vs_dev14_md.md)]의 **새 프로젝트** 대화 상자에서 **Windows 유니버설** 노드를 찾습니다. 이 노드가 표시되지 않으면 먼저 [Windows 10용 도구](http://go.microsoft.com/fwlink/p/?LinkID=617903)를 설치합니다.**Windows 런타임 구성 요소** 템플릿을 선택하고 구성 요소에 이름을 지정한 다음 **확인** 단추를 선택합니다. 구성 요소 이름은 네임스페이스 이름으로 사용되므로 이전 프로젝트의 네임스페이스와 동일한 이름을 사용하려고 할 수 있습니다. 그러려면 이전과 다른 폴더에 프로젝트를 만들어야 합니다. 다른 이름을 선택하면 생성된 코드 파일에서 네임스페이스 이름을 업데이트할 수 있습니다.  
  
2.  프로젝트를 닫습니다.  
  
3.  모든 코드 파일\(.cpp, .h, .xaml 등\)을 Windows 8.1 구성 요소에서 새로 만든 프로젝트에 복사합니다. Package.appxmanifest 파일은 복사하지 마세요.  
...
```Output  
'platform.winmd' 어셈블리를 찾을 수 없습니다. /AI를 사용하거나 LIBPATH 환경 변수를 설정하여 어셈블리 검색 경로를 지정하세요.  
```  
...
## 참고 항목  
 ...
 [UWP\(유니버설 Windows 플랫폼\)용 앱 개발](../Topic/Develop%20apps%20for%20the%20Universal%20Windows%20Platform%20\(UWP\).md)